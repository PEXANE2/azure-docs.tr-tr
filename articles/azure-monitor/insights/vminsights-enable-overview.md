...
Yönetim Paketi *Microsoft. ıntelligencepacks. ApplicationDependencyMonitor*olarak adlandırılır. Bu,%Programfiles%\Microsoft Monitoring Fılest\sistem sağlığı hizmeti State\Management Packs \ klasörüne yazılır. Yönetim Paketi kullanan veri kaynağı % Program files%\Microsoft izleme Agent\Agent\Health hizmet State\Resources,\<AutoGeneratedID > \ Microsoft.EnterpriseManagement.Advisor.ApplicationDependencyMonitorDataSource.dll.
