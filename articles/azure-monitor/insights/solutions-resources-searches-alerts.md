...
|:---|:---|:---|
| Savedaramalarındaki aramalar | 2017-03-15-Önizleme | Olay &#124; EventLevelName = = "Error"  |
...
[Log Analytics kayıtlı arama](../../azure-monitor/log-query/log-query-overview.md) kaynaklarının türü vardır `Microsoft.OperationalInsights/workspaces/savedSearches` ve aşağıdaki yapıya sahiptir. Bu kod parçacığını kopyalayıp çözüm dosyanıza yapıştırarak parametre adlarını değiştirebilmeniz için ortak değişkenleri ve parametreleri içerir.
...
Kayıtlı aramada, her zamanlamaya göre ayrı bir uyarı kuralını temsil eden bir veya daha fazla zamanlama bulunabilir. Zamanlama, aramanın ne sıklıkla çalıştırılacağını ve verilerin alınma zaman aralığını tanımlar. Zamanlama kaynaklarının türü vardır `Microsoft.OperationalInsights/workspaces/savedSearches/schedules/` ve aşağıdaki yapıya sahiptir. Bu kod parçacığını kopyalayıp çözüm dosyanıza yapıştırarak parametre adlarını değiştirebilmeniz için ortak değişkenleri ve parametreleri içerir.
...
**Tür** özelliği tarafından belirtilen iki tür eylem kaynağı vardır. Bir zamanlama, uyarı kuralının ayrıntılarını ve bir uyarı oluşturulduğunda hangi eylemlerin alındığını tanımlayan bir **Uyarı** eylemi gerektirir. Eylem kaynaklarının türü vardır `Microsoft.OperationalInsights/workspaces/savedSearches/schedules/actions`.
...
| Öğe adı | Gerekli | Açıklama |
|:--|:--|:--|
| `TriggerCondition` | Yes | Eşiğin, şu değerlerden alınan toplam ihlal sayısı veya ardışık ihlal olup olmadığını belirtir:<br><br>**Ardışık<br>toplam** |
| `Operator` | Yes | Aşağıdaki değerlerden karşılaştırma için işleç:<br><br>**gt = büyüktür<br>lt = küçüktür** |
