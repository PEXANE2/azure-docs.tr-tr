...
Daha önce bir yere JVM bağımsız değişkeni ekleyin `-javaagent:path/to/applicationinsights-agent-3.0.1.jar` `-jar` , örneğin:
...
*Exec* formunu kullanıyorsanız, parametresini parametre `"-javaagent:path/to/applicationinsights-agent-3.0.1.jar"` listesine parametresinden önce bir yerde ekleyin `"-jar"` , örneğin:

```
ENTRYPOINT ["java", "-javaagent:path/to/applicationinsights-agent-3.0.1.jar", "-jar", "<myapp.jar>"]
```

*Kabuk* formunu kullanıyorsanız, daha önce bir yere JVM bağımsız değişkeni ekleyin `-javaagent:path/to/applicationinsights-agent-3.0.1.jar` `-jar` , örneğin:
...
Dosya `<tomcat>/bin/setenv.sh` zaten varsa, bu dosyayı değiştirip `-javaagent:path/to/applicationinsights-agent-3.0.1.jar` öğesine ekleyin `CATALINA_OPTS` .
...
Dosya `<tomcat>/bin/setenv.bat` zaten varsa, bu dosyayı değiştirip `-javaagent:path/to/applicationinsights-agent-3.0.1.jar` öğesine ekleyin `CATALINA_OPTS` .

### <a name="running-tomcat-as-a-windows-service"></a>Windows hizmeti olarak Tomcat çalıştırma

Dosyayı bulun `<tomcat>/bin/tomcat8w.exe` .  Bu çalıştırılabiliri çalıştırın ve `-javaagent:path/to/applicationinsights-agent-3.0.1.jar` `Java Options` sekmesinde altına ekleyin `Java` .
...
`-javaagent:path/to/applicationinsights-agent-3.0.1.jar` `JAVA_OPTS` Dosyadaki `JBOSS_HOME/bin/standalone.conf` (Linux) veya (Windows) var olan ortam değişkenine ekleyin `JBOSS_HOME/bin/standalone.conf.bat` :
...
`-javaagent:path/to/applicationinsights-agent-3.0.1.jar`' De var olan öğesine ekleyin `jvm-options` `JBOSS_HOME/domain/configuration/host.xml` :
...
`-javaagent:path/to/applicationinsights-agent-3.0.1.jar`' De var olan öğesine ekleyin `jvm-options` `glassfish/domains/domain1/config/domain.xml` :
