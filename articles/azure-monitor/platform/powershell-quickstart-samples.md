...
`Get-AzLog`diğer birçok parametreyi destekler. Daha fazla `Get-AzLog` bilgi için başvuruya bakın.
...
Bir uyarı kuralı oluşturmak `Add-AlertRule` , güncelleştirmek veya devre dışı bırakmak için cmdlet 'ini kullanabilirsiniz.

Sırasıyla ve `New-AzAlertRuleEmail` `New-AzAlertRuleWebhook`kullanarak e-posta ve Web kancası özellikleri oluşturabilirsiniz. Uyarı kuralı cmdlet 'inde, bu özellikleri uyarı kuralının **Eylemler** özelliğine eylem olarak atayın.
...
| Oluşturulan uyarının MetricName |\Fiziksel disk (_Total) \Disk yazma/sn. Tam ölçüm `Get-MetricDefinitions` adlarını alma hakkında cmdlet 'e bakın |
...
Belirli bir kaynak için `Get-AzMetricDefinition` tüm ölçümlerin listesini görüntülemek için cmdlet 'ini kullanabilirsiniz.
...
İçin `Get-AzMetricDefinition` kullanılabilir seçeneklerin tam listesi [Get-MetricDefinitions](https://msdn.microsoft.com/library/mt282458.aspx)adresinde bulunabilir.

## <a name="create-and-manage-activity-log-alerts"></a>Etkinlik günlüğü uyarıları oluşturma ve yönetme
Bir etkinlik günlüğü Uyarısı `Set-AzActivityLogAlert` ayarlamak için cmdlet 'ini kullanabilirsiniz. Bir etkinlik günlüğü uyarısı, koşullarınızı öncelikle koşulların bir sözlüğü olarak tanımlamanızı ve sonra bu koşulları kullanan bir uyarı oluşturmanızı gerektirir.
...
Ek Web kancası özellikleri isteğe bağlıdır. Kullanarak `Get-AzActivityLogAlert`bir etkinlik günlüğü uyarısının içeriğini geri alabilirsiniz.
...
Otomatik ölçeklendirme geçmişini almak `Get-AzAutoScaleHistory` için cmdlet 'ini kullanabilirsiniz.
...
Otomatik ölçeklendirme ayarı hakkında `Get-Autoscalesetting` daha fazla bilgi almak için cmdlet 'ini kullanabilirsiniz.
...
Bir otomatik ölçeklendirme ayarını `Remove-Autoscalesetting` silmek için cmdlet 'ini kullanabilirsiniz.
