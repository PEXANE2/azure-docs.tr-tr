...
1. Aracı yükleme dosyalarını ayıklamak için, yükseltilmiş bir komut isteminden `MMASetup-<platform>.exe /c` çalıştırın ve dosyaları ayıklama yolunu sorar.  Alternatif olarak, `MMASetup-<platform>.exe /c /t:<Full Path>`bağımsız değişkenleri geçirerek de yolu belirtebilirsiniz.  
