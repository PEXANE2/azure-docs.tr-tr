...
Yerel veri merkezinizdeki veya Azure Izleyici ile diğer bulut ortamınızdaki sanal makineleri veya fiziksel bilgisayarları izlemek ve yönetmek için, Log Analytics aracısını (Ayrıca Microsoft Monitoring Agent (MMA) olarak da bilinir) dağıtmanız ve onu bir veya daha fazla Log Analytics çalışma alanına rapor verecek şekilde yapılandırmanız gerekir. Aracı Ayrıca Azure Otomasyonu için karma Runbook Worker rolünü destekler.  
