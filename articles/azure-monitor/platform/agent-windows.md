...
Tamamlandığında, **Denetim Masası**'nda **Microsoft Monitoring Agent** gösterilir. To confirm it is reporting to Log Analytics, review [Verify agent connectivity to Log Analytics](#verify-agent-connectivity-to-log-analytics). 
...
Bilgisayarın **Denetim Masası** sayfasında **Microsoft Monitoring Agent**'ı bulun.  Select it and on the **Azure Log Analytics** tab, the agent should display a message stating: **The Microsoft Monitoring Agent has successfully connected to the Microsoft Operations Management Suite service.**<br><br> ![MMA'nın Log Analytics'e bağlantı durumu](media/agent-windows/log-analytics-mma-laworkspace-status.png)

You can also perform a simple log query in the Azure portal.  

1. Azure portalında **Tüm hizmetler**’e tıklayın. In the list of resources, type **Azure Monitor**. Yazmaya başladığınızda liste, girişinize göre filtrelenir. Select **Azure Monitor**.  
