...
Azure Monitor ile yerel veri merkezinizdeki veya diğer bulut ortamınızdaki sanal makineleri veya fiziksel bilgisayarları izlemek ve yönetmek için Log Analytics aracısını (Microsoft İzleme Aracısı (MMA) olarak da adlandırılır) dağıtmanız ve bir veya daha fazla Log Analytics çalışma alanlarına rapor verin. Aracı, Azure Otomasyonu için Karma Runbook Worker rolünü de destekler.  
