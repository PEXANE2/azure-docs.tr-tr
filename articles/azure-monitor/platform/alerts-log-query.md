...
Uyarı sorguları, sorunu belirten [Log Analytics günlük verilerini sorgulamadan](alerts-log.md#create-a-log-alert-rule-with-the-azure-portal) başlar. Neleri keşfedebileceğinizi anlamak için [Uyarı sorgu örnekleri konusunu](../log-query/saved-queries.md) kullanabilirsiniz. [Kendi sorgunuzu yazmaya de başlamanızı](../log-query/get-started-portal.md)sağlayabilirsiniz. 
...
Çapraz kaynak sorguları, [cross-resource queries](../log-query/cross-workspace-query.md) `union` sorgu kapsamını belirli kaynaklarla sınırlayan bir türü kullandığından, çapraz kaynak sorgularını kullanan günlük uyarı kuralları bu değişiklikten etkilenmez. Aşağıdaki örnek geçerli günlük uyarı sorgusu olacaktır:
