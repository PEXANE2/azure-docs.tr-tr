...
Başka bir çalışma alanından veya klasik uygulamadan veri almak için [çalışma alanı](../logs/workspace-expression.md) veya [uygulama](../logs/app-expression.md) komutunu kullanırsanız, zaman seçici farklı davranabilir. Kapsam bir Log Analytics çalışma alanım ve **uygulama** kullanıyorsanız ya da kapsam klasik bir Application Insights uygulaması ise ve **çalışma alanı** kullanıyorsanız, Log Analytics filtrede kullanılan sütunun saat filtresini belirlemesi gerektiğini anlayamayabilir.
...
- [Azure portal Log Analytics kullanma hakkında öğreticiyi](../logs/log-analytics-tutorial.md)adım adım inceleyin.
- [Sorgu yazma hakkında öğreticiyi](../logs/get-started-queries.md)adım adım inceleyin.