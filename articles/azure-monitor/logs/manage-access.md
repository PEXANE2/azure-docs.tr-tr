...
Azure Izleyici, [günlük](../logs/data-platform-logs.md) verilerini bir Log Analytics çalışma alanında depolar. Çalışma alanı, veri ve yapılandırma bilgilerini içeren bir kapsayıcıdır. Günlük verilerine erişimi yönetmek için, çalışma alanınız ile ilgili çeşitli yönetim görevlerini gerçekleştirirsiniz.
...
* Azure rol tabanlı erişim denetimi (Azure RBAC) kullanarak belirli kaynaklardan günlük verilerine erişmesi gereken kullanıcılar ( [kaynak bağlamı](../logs/design-logs-deployment.md#access-mode) olarak da bilinir)
* Azure RBAC kullanarak çalışma alanındaki belirli bir tablodaki günlük verilerine erişmesi gereken kullanıcılar.

Azure RBAC ve erişim stratejileri etrafında günlük kavramlarını anlamak için, [Azure Izleyici günlükleri dağıtımınızı tasarlama](../logs/design-logs-deployment.md) konusunu okuyun

## <a name="configure-access-control-mode"></a>Erişim denetimi modunu yapılandırma

Bir çalışma alanında yapılandırılan [erişim denetimi modunu](../logs/design-logs-deployment.md) Azure portal veya Azure PowerShell ile görüntüleyebilirsiniz.  Bu ayarı, aşağıdaki desteklenen yöntemlerden birini kullanarak değiştirebilirsiniz:
...
`/read`izin genellikle, yerleşik okuyucu ve katkıda bulunan rolleri gibi _\* /Read veya_ Permissions içeren bir rolden verilir _\*_ . [](../../role-based-access-control/built-in-roles.md#reader) [](../../role-based-access-control/built-in-roles.md#contributor) Belirli eylemler veya adanmış yerleşik roller içeren özel roller bu izni içermeyebilir.

Farklı tablolar için farklı erişim denetimi oluşturmak isterseniz, aşağıdaki [tablo başına erişim denetimi tanımlama](#table-level-azure-rbac) bölümüne bakın.
...
## <a name="table-level-azure-rbac"></a>Tablo düzeyinde Azure RBAC

**Tablo düzeyi Azure RBAC** , diğer izinlerin yanı sıra bir Log Analytics çalışma alanındaki verilere daha ayrıntılı denetim tanımlamanızı sağlar. Bu denetim, yalnızca belirli bir kullanıcı kümesi için erişilebilen belirli veri türlerini tanımlamanızı sağlar.

Çalışma alanındaki belirli [tablolara](../logs/data-platform-logs.md) erişim sağlamak için [Azure özel rolleriyle](../../role-based-access-control/custom-roles.md) tablo erişim denetimi uygulayabilirsiniz. Bu roller, kullanıcının [erişim modundan](../logs/design-logs-deployment.md#access-mode)bağımsız olarak, çalışma alanı bağlamı veya kaynak bağlamı [erişim denetimi modlarıyla](../logs/design-logs-deployment.md#access-control-mode) çalışma alanlarına uygulanır.
...
```
Özel günlüklere erişimi yönetmeye yönelik alternatif bir yaklaşım, bunları bir Azure kaynağına atamak ve kaynak bağlamı paradigması kullanarak erişimi yönetmeleri. Bu yöntemi kullanmak için, [http veri toplayıcı API 'si](../logs/data-collector-api.md)aracılığıyla Log Analytics veriler istendiğinde [x-MS-azureresourceıd](../logs/data-collector-api.md#request-headers) üst bilgisinde belirterek kaynak kimliğini dahil etmeniz gerekir. Kaynak KIMLIĞI geçerli olmalı ve erişim kuralları uygulanmış olmalıdır. Günlükler alındıktan sonra, burada açıklandığı gibi kaynağa okuma erişimi olan kullanıcılar tarafından erişilebilir.
