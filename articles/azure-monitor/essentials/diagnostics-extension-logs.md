...
Tanılama uzantısını yükleme ve yapılandırma hakkında ayrıntılı bilgi için bkz. [Windows Azure tanılama uzantısı 'nı (WAD) yükleme ve yapılandırma](../agents/diagnostics-extension-windows-install.md) veya [Linux Tanılama uzantısı 'nı kullanma](../../virtual-machines/extensions/diagnostics-linux.md) . Bu, depolama hesabını belirtmenizi ve Azure Izleyici günlüklerine iletmek istediğiniz verilerin toplanmasını yapılandırmanızı sağlar.
...
* Desteklenen Azure hizmetleri için [Azure hizmetleri için günlükleri ve ölçümleri toplayın](../platform/resource-logs.md#send-to-log-analytics-workspace) .
