...
ms.locfileid: "71937569"
---
# <a name="support-matrix-for-vmware-assessment-and-migration"></a>VMware değerlendirmesi ve geçişi için destek matrisi
...
**Aygıtların** | **Bağlanma**
--- | ---
Elektrikli | TCP bağlantı noktası 3389 üzerindeki gelen bağlantılar, gereci Uzak Masaüstü bağlantılarına izin vermek için.<br/><br/> 44368 bağlantı noktası ile gereç yönetimi uygulamasına uzaktan erişim için bağlantı noktası ' deki gelen bağlantılar: ```https://<appliance-ip-or-name>:44368``` <br/><br/>Azure geçişi 'ne bulma ve performans meta verileri göndermek için 443, 5671 ve 5672 numaralı bağlantı noktası üzerinden giden bağlantılar.
...
**İzinler** | **Bilgileri**
--- | ---
Veri deposu. | Anlık görüntü oluşturma ve silme sorunlarını gidermek için VM günlük dosyalarına göz atmaya izin verin.
...
VirtualMachine. sağlama. AllowReadOnlyDiskAccess | VDDK kullanarak diski okumak için bir VM 'de disk açmaya izin verin.
VirtualMachine. sağlama. AllowVirtualMachineDownload  | , Günlükleri indirmek ve hata oluşursa sorunlarını gidermek için bir VM ile ilişkili dosyalardaki okuma işlemlerine izin verir.
...
Windows Server rolleri | Bu rolleri etkinleştirmeyin: <br> -Active Directory Domain Services <br>-Internet Information Services <br> -Hyper-V
...
https: \//dev. MySQL. com/Get/downloads/Mysqlınstaller/MySQL-installer-Community-5.7.20.0. msi | MySQL indirme işleminin tamamlanabilmesi için
...
**Şifrelenmiş diskler/birimler** | Şifrelenmiş disklere/birimlere sahip VM 'Ler geçiş için desteklenmez.
**Paylaşılan disk kümesi** | Desteklenmez.
**Bağımsız diskler** | Desteklenen.
**Geçiş diskleri** | Desteklenen.
**ALACAĞıNı** | VM 'Lere birim olarak bağlanmış NFS birimleri çoğaltılmaz.
Iscsı hedefleri | Iscsı hedefleri olan VM 'Ler aracısız geçiş için desteklenmez.
**Çok yollu GÇ** | Desteklenmez.
...
**Bileşeninde** | **Requirements** | **Bilgileri**
--- | --- | ---
...
VM adı | 1 ile 63 karakter arasında.<br/> Harfler, rakamlar ve kısa çizgilerden kısıtlıdır.<br/><br/> Makine adı bir harf veya sayıyla başlamalı ve bitmelidir. |  Site Recovery makine özelliklerindeki değeri güncelleştirin.
Geçişten sonra Bağlan-Windows | Geçişten sonra Windows çalıştıran Azure VM 'lerine bağlanmak için:<br/> -Geçiş öncesinde, şirket içi VM 'de RDP 'yi mümkün. **Genel** PROFIL için TCP ve UDP kurallarının eklendiğinden ve tüm profiller Için **Windows Güvenlik Duvarı** > **izin verilen uygulamalarda**RDP 'ye izin verildiğinden emin olun.<br/> Siteden siteye VPN erişimi için, RDP 'yi etkinleştirin ve **Windows Güvenlik Duvarı**'nda RDP 'ye izin verin @no__t **etki alanı ve özel** ağlar için**izin verilen uygulamalar ve Özellikler** . Ayrıca, işletim sisteminin SAN ilkesinin **OnlineAll**olarak ayarlandığından emin olun. [Daha fazla bilgi edinin](prepare-for-migration.md). |
