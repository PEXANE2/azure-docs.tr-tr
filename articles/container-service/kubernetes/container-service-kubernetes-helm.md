...
* [Yükleyip yapılandırın `kubectl` ](../container-service-connect.md) Yerel bir bilgisayarda
...
`kubectl get svc`Kümede çalışan tüm hizmetleri görüntülemek için yazarsanız, giriş denetleyicisine BIR IP adresinin atandığını görürsünüz. (Atama devam ederken, görürsünüz `<pending>` . Tamamlanmak üzere birkaç dakika sürer.) 
...
Komutunu olarak kısaltılabilir `helm ls` .
...
İstemciye bağlanmak için, dağıtımınızın adıyla değiştirerek aşağıdaki komutu yazın `v1-mariadb` :
