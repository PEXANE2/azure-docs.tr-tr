...
2. Adı `hello-nginx`verin. [ `nginx` Docker 'dan kapsayıcıyı](https://hub.docker.com/_/nginx/) kullanın ve bu Web hizmetinin üç çoğaltmasını dağıtın.
...
Kubernetes Kullanıcı arabirimi, Kapsayıcılarınızı çalıştırmanın yanı sıra bir dış `Service` oluşturmuş ve bu sayede trafiği kümenizdeki kapsayıcılara getirecek bir yük dengeleyici sağlar.
...
`spec.replicas` Alanı olacak `2`şekilde düzenleyin ve **Güncelleştir**' e tıklayın.
