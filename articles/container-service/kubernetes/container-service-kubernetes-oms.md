...
Alternatif olarak, [Azure Cloud Shell](https://docs.microsoft.com/azure/cloud-shell/overview) `az` Azure CLI ve `kubectl` araçları 'nın zaten yüklü olduğu Azure Cloud Shell kullanabilirsiniz.
...
Log Analytics aracısının hizmet ile iletişim kurmasını sağlamak için, bir çalışma alanı KIMLIĞI ve bir çalışma alanı anahtarıyla yapılandırılması gerekir. Çalışma alanı KIMLIĞINI ve anahtarını almak için ' de bir hesap oluşturmanız gerekir <https://mms.microsoft.com> .
...
İşte [DaemonSet YAML dosyası](https://github.com/Microsoft/OMS-docker/tree/master/Kubernetes). Bu dosyayı adlı bir dosyaya kaydedin `oms-daemonset.yaml` ve için yer tutucu değerlerini, `WSID` `KEY` çalışma alanınızın kimliği ve anahtarınızla değiştirin.
