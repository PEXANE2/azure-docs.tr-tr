...
`host`Çıktıda değerini (şuna benzer BIR IP adresi `10.32.0.x` ) ve değerini bulun `ports` .


Şimdi kümenin yönetim FQDN 'sine bir SSH terminal bağlantısı (tünel dışı bağlantı değil) yapın. Bağlandıktan sonra, ve değerlerinin doğru değerlerini değiştirerek aşağıdaki isteği yapın `host` `ports` :
