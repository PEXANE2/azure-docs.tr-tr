...
3. Burada `wsid` (Log Analytics çalışma alanı kimliği) ve `wskey` (çalışma alanı kimliği için birincil anahtar) girmeniz istenir. Her ikisini de almak için, `wsid` ' de `wskey` bir hesap oluşturmanız gerekir <https://mms.microsoft.com> .
   Hesap oluşturmak için adımları izleyin. Hesabı oluşturmayı tamamladıktan sonra, `wsid` `wskey` aşağıda gösterildiği gibi **Ayarlar**, **bağlı kaynaklar**ve **Linux sunucuları**' na tıklayarak ve ' yi edinmeniz gerekir.
...
Azure portal için oturum açın <https://portal.microsoft.com/> . **Market**'e gidin, **izleme + yönetim** ' i seçin ve **Tümünü gör**' e tıklayın. Ardından `containers` arama yazın. Arama sonuçlarında "kapsayıcılar" ifadesini görürsünüz. **Kapsayıcılar** ' ı seçin ve **Oluştur**' a tıklayın.
...
Gerçek düğüm sayısının Log Analytics Aracısı ' nı yüklemiş olmanız veya daha fazla VM ekleyerek sanal makine ölçek kümesini ölçeklendirmeniz gerekiyorsa, hizmeti ölçeklendirerek bunu yapabilirsiniz `msoms` .
