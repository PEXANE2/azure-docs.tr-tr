...
**Başlangıç tarihi, 1 Mayıs 2018 tarihinden önce olan doğrudan kayıtlar**: Doğrudan Kurumsal Azure (EA) Müşterileri, Azure Hizmetleri için Yıllık Faturalama dönemine tabi olur (Market hizmetleri hariç). Faturalama döneminiz, Yıl Dönümü tarihinize göre belirlenir. Yıl dönümü tarihi, anlaşmanızın yürürlüğe gireceği tarihtir. İlk Azure Hizmet Fazla Kullanımı faturanızı almak için Parasal Taahhüt ("MC") eşiğinizi %150 oranında aşmanız gerekir.  Toplam hizmet tüketiminiz, MC eşiğinizi %150 oranında aştığında otomatik olarak Yıl Dönümü tarihinize göre Üç Aylık Faturalama dönemine geçersiniz.  MC eşiğinizi %150 oranında aşmazsanız kayıt, Yıllık Faturalama dönemi olarak kalır ve Fazla Kullanım faturası, Taahhüt yılının sonunda oluşturulur.

**Başlangıç tarihi, 1 Mayıs 2018 tarihinden sonra olan doğrudan kayıtlar**: Müşterinin Azure Tüketimi ve Ücretleri İçin Ayrı Ücretlendirilen Faturaları, Aylık Faturalama dönemini kullanır.  Azure Parasal Taahhüdü kapsamında olmayan ücretler, Fazla Kullanımı ödemesi olarak ödenir.  

**Kayıt tarihi 1 Mayıs 2018 tarihinden önce olan dolaylı kayıtlar**: Dolaylı Kurumsal Azure (EA) Müşterileri, Üç Aylık Faturalama dönemini kullanır.  Kanal İş Ortağı (CP), Müşteriyi doğrudan faturalandıracaktır.  

**Başlangıç tarihi, 1 Mayıs 2018 tarihinden sonra olan dolaylı kayıtlar**: Başlangıç tarihi 1 Mayıs 2018 veya sonrası olan Dolaylı Anlaşmalar için Aylık faturalama yapılacaktır.  
...
Faturadaki her bir öğe için toplam ücretleri (Genişletilmiş Tutar), ücretleri karşılamak için kullanılan taahhüt miktarını (Taahhüt Kullanımı) ve taahhüdü aşan ücretlerin tutarını (Net Tutar) görürsünüz.  Geçerli vergiler yalnızca taahhüdü artan net tutar üzerinden hesaplanır.
...
Kaydınıza uygulanan tüm krediler veya düzeltmeler [https://ea.azure.com](https://ea.azure.com) adresinin **Raporlar** bölümünde görünür.
...
1. Bu satırda kaydınıza uygulanan kredileri ve kısa bir açıklamasını görürsünüz. Örneğin: Hizmet Düzeyi Sözleşmesi Kredisi.
...
![Fiyat listesi bilgilerini gösteren örnek](./media/billing-ea-portal-enrollment-invoices/ea-create-view-price-sheet-information.png)
...
![Eski fiyat listesinin indirileceği yeri gösteren örnek](./media/billing-ea-portal-enrollment-invoices/create-ea-view-price-sheet-download-historical-price-list.png)
...
Excel, tüm alanları "Genel" metin olarak içeri aktarır ve bunun şu matematik standardında ayrılmış bir sayı olduğunu kabul eder: "1,000.00".  Bu nedenle binlik ayırıcı olarak nokta (.), ondalık ayırıcı olarak da virgül (,) kullanılan bir Avrupa para birimi için "1.000,00" değeri hatalı bir şekilde görüntülenecektir. Bu durum, bölge ve dil ayarınıza göre değişebilir.
...
Kullanım Ayrıntısı Raporu ölçüm adları, ölçüm türleri ve tüketilen miktarlar hakkında bilgiler dahil olmak üzere bir kayıt tarafından tüketilen hizmetlerin ve miktarların aylık özetini sunar.
...
- **Aylık Ücret:** Kullanıma/erişime göre aylık olarak faturalandırılır.
...
### <a name="enterprise-msdn-devtest"></a>Kurumsal MSDN Geliştirme ve Test
...
### <a name="enterprise-program-usage-calculation-guidelines"></a>Kurumsal program kullanımı hesaplama yönergeleri
...
Örneğin gerçek SQL Server kullanım saatinin İndirilen Kullanım Raporunda 694,533404 olduğunu düşünelim. Bu birimler önce 6,94533404 birim 100 İşlem Saatine dönüştürülür ve ardından 6,9453 şeklinde yuvarlanır ve Enterprise Portal'da bu şekilde görüntülenir.
...
Yeni ölçüm, 27-28 Ocak 2016 sonrasında geçerli olacaktır. Bu geçiş döneminde dağıtımınıza ait .csv raporunda iki satır göreceksiniz: Bunlardan biri A1 ölçümüne göre hesaplanan kullanım, diğeri de dağıtımınızın boyutuna göre yeni ayrılmış ölçüme göre hesaplanan kullanımdır.
...
Müşteriler, doğrudan Azure aboneliklerini Azure EA Portal kayıtlarıyla ilişkilendirerek EA aboneliğine dönüştürebilir ancak bu işlem sonucunda alt market abonelikleri otomatik olarak dönüştürülmez.
...
- **Kullanıma dayalı:** Ürünlerin kullanımı, işlem birimi üzerinden ölçülür.  Örneğin sanal makineler saat, Bing API aramaları ise arama sayısı üzerinden ücretlendirilir.
- **Kullanıma dayalı olmayan:** Kullanımdan bağımsız olan tek seferlik veya yinelenen aylık ücretlerdir.
...
Belirli Azure Market teklifleri, parasal taahhüt fonlarını kullanabilir. Bu programa katılan ürünlerin tam listesi için [Azure parasal taahhüdü](https://azure.microsoft.com/updates/azure-marketplace-third-party-reseller-services-now-use-azure-monetary-commitment) sayfasına bakın.
...
- **Genel amaçlı**: "Microsoft Azure Enterprise" için genişletilmiş Power BI Pro içerik paketi deneme sürümü teklifi ("Teklif"), Teklifin süresi boyunca mevcut nitelikli kullanıcıların kullanımına açıktır ve bu kullanıcılara belirli bir Power BI içerik paketinin kullanımı aracılığıyla Microsoft Azure tüketimleriyle ilgili belirli içgörülere erişim imkanı sunacaktır.
- **Uygunluk**: Kurumsal Anlaşma (EA) kapsamındaki müşteriler, kuruluşlarının Microsoft Azure faturalama, hizmet, hizmet ve/veya maliyet yönetimi alanlarıyla ilgili bir görevde olmaları durumunda Teklife katılımda bulunabilir.
- **İstisnalar**:
  ...
  - Dönem: Teklif 1 Haziran 2017'de başlayacak ve 31 Mayıs 2018'de sona erecektir.  Kabul, 12 aylık dönem içinde herhangi bir zamanda gerçekleşebilir ancak teklif, kullanıcıların Teklifi kabul etme zamanına bakılmaksızın 31 Mayıs 2018 tarihinde tüm kullanıcılar için sonlandırılacaktır.

### <a name="to-access-the-microsoft-azure-consumption-insights-content-pack"></a>Microsoft Azure Consumption Insights içerik paketine erişmek için:
...
**Doğrudan kayıt** Hesap sahibi veya bölümü yöneticisiyseniz lütfen EA yöneticinizle iletişime geçerek fiyatlandırma özelliğini etkinleştirmesini isteyin:
...
**Birim Başına MOCP Kaynağı:** Yukarı akış kullanım sistemi, her hizmet için kullanımları farklı birimlerde yayar. (Önceden ayarlanmıştır) **Birim Başına Tüketim:** EA Ölçü Birimidir. (Önceden ayarlanmıştır) **Fiyat:** Azure EA Portal'dan alınan birim fiyat.
**Toplam Maliyet:** Azure EA Portal'dan alınan ayrıntılı kullanım raporu veya taahhüt kullanımı + fazla kullanım ile bulunan genişletilmiş maliyettir.
...
- Faturanızı ve ücretlerinizi anlama konusunda bilgi için bkz. [Azure Kurumsal Anlaşma faturanızı anlama](billing-understand-your-bill-ea.md).
