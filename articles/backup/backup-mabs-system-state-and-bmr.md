...
|SQL Server/Exchange<br /><br />Azure Backup Sunucusu uygulama yedeklemesi<br /><br />BMR/sistem durumu yedeklemesi|Kayıp veya hasarlı işletim sistemi|N|E|E|
...
1. **Disk ayırmayı İncele** sayfasında, koruma grubu için kullanılabilir depolama havuzu disk alanını gözden geçirin.
    ...
    * İş yükleri için açılan menüden tercih edilen depolamayı seçin. Düzenlemeleriniz, **kullanılabilir disk depolama** bölmesinde **toplam depolama** ve **boş depolama** değerlerini değiştirir. Yetersiz sağlanan alan, yedekleme sunucusunun kesintisiz yedeklemeler sağlamak için birime eklemenizi önereceği depolama miktarıdır.
...
1. **Çevrimiçi yedekleme zamanlamasını belirtin** sayfasında, Azure 'a artımlı olarak ne sıklıkta yedekleme yapılacağını seçin. 
    ...
    Yedeklemeleri her gün, hafta, ay ve yılda çalışacak şekilde zamanlayabilirsiniz. Ayrıca, yedeklemelerin çalıştırılacağı saati ve tarihi de seçebilirsiniz. Yedeklemeler günde en fazla iki kez bulunabilir. Her yedekleme işlemi çalıştırıldığında, yedekleme sunucusu diskinde depolanan yedekleme verilerinin kopyasından Azure 'da bir veri kurtarma noktası oluşturulur.
...
BMR 'yi veya sistem durumunu bir ağ konumuna kurtarabilirsiniz. BMR 'yi yedeklediniz, sisteminizi başlatmak ve ağa bağlamak için Windows kurtarma ortamı 'nı (WinRE) kullanın. Ardından, ağ konumundan kurtarmak için Windows Server Yedekleme kullanın. Sistem durumunu yedeklediniz, ağ konumundan kurtarmak için Windows Server Yedekleme kullanmanız yeterlidir.
...
1. Kullanılabilir kurtarma noktaları, takvim üzerinde kalın yazı tipiyle belirtilir. Kullanmak istediğiniz kurtarma noktasının tarihini ve saatini seçin.
...
1. **Bir sistem görüntüsü yedeklemesi seçin** sayfasında, **Gelişmiş** >  > **sistem görüntüsü Seç** ' i seçin ve **ağ üzerinde bir sistem görüntüsü arayın**. Bir uyarı görünürse **Evet**' i seçin. Paylaşma yoluna gidin, kimlik bilgilerini girin ve kurtarma noktasını seçin. Sistem, bu kurtarma noktasında kullanılabilir olan belirli yedekleri tarar. Kullanmak istediğiniz kurtarma noktasını seçin.
...
1. Kurtarmayı başlatmak istediğinizi onaylayın. İşlemi komut Istemi penceresinde görebilirsiniz. Geri yükleme günlüğü oluşturulur. 
