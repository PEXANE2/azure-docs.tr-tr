...
### <a name="get-list-of-storage-accounts-with-file-shares-that-can-be-backed-up-with-recovery-services-vault"></a>Kurtarma Hizmetleri kasasıyla yedeklenebilir dosya paylaşımları olan depolama hesapları listesini alın

"Önbelleğe alma" işleminin yapıldığını doğrulamak için, abonelikteki tüm depolama hesaplarını, kurtarma hizmetleri kasasıyla yedeklenebilir dosya paylaşımları ile listeleyin. Ardından, yanıttaki istenen depolama hesabını bulun. Bu işlem, [korunabilir kapsayıcı al](/rest/api/backup/protectablecontainers/list) işlemi kullanılarak yapılır.
