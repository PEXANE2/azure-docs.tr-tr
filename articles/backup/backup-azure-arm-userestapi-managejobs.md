...
Yedeklemenin tetiklenmesi gibi bir işlem, her zaman bir iş kimliği döndürür. Örneğin: bir [tetikleyici yedekleme REST API işleminin](backup-azure-arm-userestapi-backupazurevms.md#example-responses-for-on-demand-backup) son yanıtı aşağıdaki gibidir:
...
Azure VM yedekleme işi "JobId" alanı tarafından tanımlanır ve basit bir *Get* isteği kullanılarak [burada](/rest/api/backup/jobdetails/) belirtildiği gibi izlenebilir.
...
, `{jobName}` Yukarıda bahsedilen "JobId" dır. Yanıt, işin geçerli durumunu gösteren "durum" alanı ile her zaman 200 Tamam olur. "Completed" veya "Completedwithuyarılar" olduktan sonra, ' Extendeınfo ' bölümü iş hakkında daha fazla ayrıntı gösterir.
