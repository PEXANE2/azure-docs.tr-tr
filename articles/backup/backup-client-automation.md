...
İlke nesnesini kaydetme, [set-OBPolicy](https://docs.microsoft.com/powershell/module/msonlinebackup/set-obpolicy?view=winserver2012-ps) cmdlet 'i kullanılarak yapılır. Bu ayrıca onay ister. Onayı atlamak için cmdlet 'iyle birlikte `-Confirm:$false` bayrağını kullanın.
...
Şimdi, `Get-OBRecoverableItem` cmdlet 'inin çıktısından seçili `$Item` [Başlat-OBRecovery](https://docs.microsoft.com/powershell/module/msonlinebackup/start-obrecovery?view=winserver2012-ps) komutunu kullanarak geri yükleme işlemini tetikleyin:
