...
1. Şablondan yeni bir cihaz eklemek için **+** ' ı seçin. **Yeni cihaz oluştur** sayfasında **Oluştur**' u seçin.
...
1. `login:` istemine bakmak için **ENTER** tuşuna basın. Oturum açmak için Kullanıcı adınızı ve parolanızı girin.
...
1. IoT Edge config. YAML dosyasını açmak için `nano` düzenleyicisini kullanın:

    ```bash
    sudo nano /etc/iotedge/config.yaml
    ```

1. `# Manual provisioning configuration`görene kadar aşağı kaydırın. Aşağıdaki kod parçacığında gösterildiği gibi sonraki üç satırı Açıklama:
...
1. `# DPS symmetric key provisioning configuration`görene kadar aşağı kaydırın. Aşağıdaki kod parçacığında gösterildiği gibi sonraki sekiz satırın açıklamasını kaldırın:
...
1. Daha önce bir örneği yaptığınız için `{scope_id}` **kimlik kapsamıyla** değiştirin.

1. `{registration_id}`, daha önce bir notunuz yaptığınız **CIHAZ kimliğiyle** değiştirin.

1. `{symmetric_key}` değerini, daha önce bir nota yaptığınız **birincil anahtarla** değiştirin.

1. Değişiklikleri Kaydet (**CTRL-O**) ve çıkış (**CTRL-X**) `nano` Düzenleyicisi.
