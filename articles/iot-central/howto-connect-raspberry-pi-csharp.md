...
1. **Program.cs** açın ve içeriğini aşağıdaki kodla değiştirin. `{your Device ID}`, Ve '`{your Device Primary Key}` yi daha önce bir nottan yaptığınız değerlerle güncelleştirin: `{your Scope ID}`
...
| Location        | Location     | location   | {Enlem: float, Long: float, alt?: float} |
