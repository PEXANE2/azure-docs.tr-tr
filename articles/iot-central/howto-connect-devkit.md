...
Azure IoT Central uygulamanızda, **Mxyonga** cihaz şablonundan gerçek bir cihaz ekleyin ve cihaz bağlantısı ayrıntılarını bir yere göz önünde alın: **Kapsam kimliği, CIHAZ kimliği ve birincil anahtar**:
...
1. Device Explorer 'den **gerçek bir cihaz** ekleyin, gerçek bir cihaz eklemek Için **+ Yeni > Real** ' ı seçin.
    ...
    * Bir **Cihaz adı**girin veya önerilen adı kullanın
    ...
    ![Cihaz Ekle](media/howto-connect-devkit/add-device.png)
...
1. Web tarayıcınızı açın ve adresine [http://192.168.0.1/start](http://192.168.0.1/start)gidin. Aşağıdaki Web sayfası görüntülenir:
...
Önceki komut, kaynak kodu adlı `iot-central-firmware`bir klasöre indirir.

> [!NOTE]
> **Git** , geliştirme ortamınızda yüklü değilse, adresinden [https://git-scm.com/download](https://git-scm.com/download)indirebilirsiniz.

## <a name="review-the-code"></a>Kodu gözden geçirin

`MXCHIP/mxchip_advanced` Klasördeki klasörü`iot-central-firmware` açmak için Visual Studio Code kullanın:

![Visual Studio Code](media/howto-connect-devkit/vscodeview.png)

Telemetriyi Azure IoT Central uygulamasına nasıl gönderildiğini görmek için, şu `src` klasördeki **telemetri. cpp** dosyasını açın:

- İşlevi `TelemetryController::buildTelemetryPayload` , cihazdaki sensörlerden alınan verileri kullanarak JSON telemetri yükünü oluşturur.

- İşlevi `TelemetryController::sendTelemetryPayload` , JSON `sendTelemetry` yükünü Azure IoT Central uygulamanızın kullandığı IoT Hub göndermek için **AzureIOTClient. cpp** ' de çağırır.

Özellik değerlerinin Azure IoT Central uygulamasına nasıl raporlanacağı görmek için, şu `src` klasördeki **telemetri. cpp** dosyasını açın:

- İşlevi `TelemetryController::loop` , bildirilen **konum** özelliğini yaklaşık olarak her 30 saniyede bir gönderir. `sendReportedProperty` **AzureIOTClient. cpp** kaynak dosyasındaki işlevini kullanır.

- İşlev `TelemetryController::loop` , cihaz hızlandırivmesi bir çift dokunma algıladığında, **dienumber** bildirilen özelliğini gönderir. `sendReportedProperty` **AzureIOTClient. cpp** kaynak dosyasındaki işlevini kullanır.
...
| Name          | Display name   | OLAĞAN | DIKKATLI | OLMA TEHLIKESI | 
| ------------- | -------------- | ------ | ------- | ------ | 
...
| Text            | Üretilmiş     | Üreten Turedın   | Yok       |

### <a name="commands"></a>Komutlar

| Display name | Alan adı | Dönüş türü | Giriş alanı görünen adı | Giriş alanı adı | Giriş alanı türü |
| ------------ | ---------- | ----------- | ------------------------ | ---------------- | ---------------- |
| Girdilerinizi         | echo       | text        | görüntülenecek değer         | displayedValue   | text             |
| Sayıma    | sayıma  | numarası      | Sayım               | Sayaçdan        | numarası           |
