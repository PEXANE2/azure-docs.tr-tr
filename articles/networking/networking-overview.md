...
Bu bölümde, Azure kaynakları arasında bağlantı sağlayan hizmetler, şirket içi ağdan Azure kaynaklarına bağlantı ve Azure - Virtual Network (VNet), Virtual WAN, ExpressRoute, VPN Ağ Geçidi, Sanal ağ NAT Ağ Geçidi, Azure DNS, Azure Bakış hizmeti ve Azure Bastion'da daldan şubeye bağlantı sağlar.
...
|[Azure Bastion](#bastion)|TLS üzerinden doğrudan Azure portalında sanal makinelerinize güvenli ve sorunsuz RDP/SSH bağlantısını yapılandırın. Azure Bastion üzerinden bağlandığınızda, sanal makinelerinizin genel bir IP adresine ihtiyacı yoktur|<p>[Azure Bastion konağı oluşturma](../bastion/bastion-create-host-portal.md)</p><p>[SSH kullanarak Linux VM'ye bağlanma](../bastion/bastion-connect-vm-ssh.md)</p><p>[RDP kullanarak Windows VM'ye bağlanma](../bastion/bastion-connect-vm-rdp.md)</p>|
...
Azure Bastion hizmeti, sanal ağınızda sağladığınız tam platform tarafından yönetilen yeni bir PaaS hizmetidir. TLS üzerinden doğrudan Azure portalında sanal makinelerinize güvenli ve sorunsuz RDP/SSH bağlantısı sağlar. Azure Bastion aracılığıyla bağlandığınızda, sanal makinelerinizin bir genel IP adresi olması gerekmez. Daha fazla bilgi için [bkz: Azure Kalesi nedir?](../bastion/bastion-overview.md)
...
Bu bölümde, Azure'daki ağ kaynaklarınızın korunmasına yardımcı olan ağ hizmetleri açıklanır - Azure'daki bu ağ hizmetlerinin herhangi birini veya birleşimini kullanarak uygulamalarınızı koruyun - Özel Bağlantı, DDoS koruması, Güvenlik Duvarı, Ağ Güvenlik Grupları, Web Uygulaması Güvenlik Duvarı ve Sanal Ağ Bitiş Noktaları.
...
|[ExpressRoute Monitör](#expressroutemonitor)|Ağ performansının, kullanılabilirliğinin ve kullanımının gerçek zamanlı olarak izlenmesini sağlar, ağ topolojisinin otomatik olarak keşfedilmesine yardımcı olur, daha hızlı hata yalıtımı sağlar, geçici ağ sorunlarını algılar, geçmiş ağ performansı özelliklerini analiz eder, çoklu aboneliği destekler|<p>[ExpressRoute için Ağ Performansı İzleyicisi’ni Yapılandırma](../expressroute/how-to-npm.md)</p><p>[ExpressRoute izleme, ölçümler ve uyarılar](../expressroute/expressroute-monitoring-metrics-alerts.md)</p>|
