...
* Hedef Azure bölgesinde (`westus`, `eastus`veya `westeurope`) Service Fabric kafesi (Önizleme) kullanılabilirliği
...
3. Mevcut özellik değerlerinin hedef bölgede kullanmak istediklerinizle aynı olduğundan emin olmak için, bu şablonu gözden geçirin (ve gerekirse düzenleyin). Yeni `location` (Azure bölgesi) yeniden dağıtım sırasında sağlayacaksınız.
