...
Azure, Azure Güvenlik Merkezi aracılığıyla genel güvenlik çözümünüz kapsamında IoT güvenliğinizi izlemek ve yönetmek için entegre bir görünüm sağlar. Bir uygulama geliştiricisiyseniz, IoT uygulama güvenliğinizi yönetmek için IoT Hub görünümünü kullanabilirsiniz.
...
Evet. Azure Güvenlik Merkezi IoT, Azure IoT bağlantısına ve altyapısına dayanır.

## <a name="do-i-have-to-install-an-agent"></a>Bir temsilci yüklemem gerekiyor mu?

IoT aygıtlarınızda aracı yüklemesi, Microsoft Azure Güvenlik Merkezi'ni etkinleştirmek için zorunlu değildir. Seçiminize göre farklı güvenlik izleme ve yönetim yetenekleri düzeyleri kazanarak aşağıdaki üç seçenek arasından seçim yapabilirsiniz:
...
Azure Güvenlik Merkezi IoT aracısı için aygıt yapılandırması, davranış ve erişim (yapılandırmayı tarayarak), işleme & bağlantısı için aygıt düzeyinde tehdit kapsamı sağlar. IoT güvenlik aracısı için Azure Güvenlik Merkezi, işle ilgili verileri veya etkinliği tarar.

## <a name="where-can-i-get-the-azure-security-center-for-iot-security-agent"></a>IoT güvenlik aracısı için Azure Güvenlik Merkezi'ni nereden edinebilirim?
...
Aracı veri oluşturma aygıt, uygulama, bağlantı türü ve müşteri aracısı yapılandırmatarafından yönlendirilir. Aygıtlar ve IoT çözümleri arasındaki yüksek değişkenlik nedeniyle, aracıyı önce bir laboratuvara veya test ayarında, oluşturulan veri miktarını ölçerken ihtiyaçlarınıza uygun özel yapılandırmayı gözlemlemek, öğrenmek ve ayarlamak için dağıtmanızı öneririz. Hizmeti başladıktan sonra, Azure Güvenlik Merkezi IoT aracısı yapılandırma ve özelleştirme işleminde size yardımcı olacak aracı iş ortasını optimize etmek için operasyonel öneriler sağlar.

## <a name="how-can-i-control-my-billing"></a>Faturamı nasıl kontrol edebilirim?

Azure Güvenlik Merkezi ioT için yapılandırılabilir aracı taramaları, veri arabellekleri ve aracı tarafından oluşturulan veri miktarını artıran veya azaltan özel uyarılar oluşturma olanağı sağlar.
...
1. Aracı [türünü cihazınızın belirlenmiş işletim sistemi platformuna uygun](how-to-deploy-agent.md) olup yok
...
Güvenlik aracısı, her aygıt yeniden başlatılırken otomatik olarak yeniden çalışacak şekilde tasarlanmıştır.

## <a name="can-the-agent-affect-the-performance-of-the-device-or-other-installed-software"></a>Aracı aygıtın veya diğer yüklü yazılımın performansını etkileyebilir mi?

Aracı, makine kaynaklarını başka bir uygulama/işlem olarak tüketir ve normal aygıt etkinliğini kesintiye uğratmamalıdır. Aracının çalıştığı aygıttaki kaynak tüketimi, kurulumu ve yapılandırmasıyla birleştiğinde. Bir üretim ortamında dağıtmaya çalışmadan önce aracı yapılandırmanızı, diğer IoT uygulamalarınız ve işlevlerinizle birlikte çalışabilirliğin yanı sıra, dahili bir ortamda test etmenizi öneririz.

## <a name="im-making-some-maintenance-on-the-device-can-i-turn-off-the-agent"></a>Cihazda biraz bakım yapıyorum. Ajanı kapatabilir miyim?
