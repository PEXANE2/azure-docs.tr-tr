...
1. Yerel makinenizde, [Windows makalesindeki bir Azure dosya paylaşma kullan](../storage/files/storage-how-to-use-files-windows.md) konusunun [Windows 'daki kalıcı Azure dosya paylaşımından kimlik bilgilerini](../storage/files/storage-how-to-use-files-windows.md#persisting-azure-file-share-credentials-in-windows) kullanarak betiği kullanın. 
2. Sonra, dosya `net use` paylaşımından makinenize bağlamak için komutunu kullanın. Örnek komut şu şekildedir: komutunu çalıştırmadan önce Azure depolama adınızı ve dosya paylaşımının adını belirtin. 
...
3. [Azure PowerShell 'ı yükler](/powershell/azure/install-az-ps).
...
4. Sanal makine içinde **dosya gezginini**başlatın ve dosya paylaşımınızı bulmak IÇIN **Bu bilgisayarı** seçin.
    ...
    > [!NOTE]
    > Herhangi bir nedenle, sanal makinenizde veya yerel makinenizde dosya paylaşımınızı bulamıyorsanız, `net use` komutunu çalıştırarak onu yeniden bağlayabilirsiniz. Komutu, `net use` Azure Portal **Dosya paylaşımınızın** **bağlanma** sihirbazında bulabilirsiniz.
