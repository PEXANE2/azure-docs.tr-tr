...
1. Laboratuvara aşağıdaki örneğe benzer yeni `ServiceRunner` bir kaynak eklemek IÇIN bir put https yöntemi gerçekleştirin. Hizmet Çalıştırıcısı kaynağı, DevTest Labs 'de yönetilen kimlikleri yönetmek ve denetlemek için bir ara sunucu kaynağıdır. Hizmet Çalıştırıcısı adı geçerli bir ad olabilir, ancak yönetilen kimlik kaynağının adını kullanmanızı öneririz. 
