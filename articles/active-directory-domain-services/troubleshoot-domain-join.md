...
1. Azure AD DS yönetilen etki alanının etki alanı adını kullanarak etki alanına ping yapmayı deneyin (örneğin,) `ping aaddscontoso.com`.
    * Ping yanıtı başarısız olursa, Azure AD DS yönetilen etki alanınız için portaldaki Genel Bakış sayfasında görüntülenecek etki alanının IP adreslerini ping yapmayı deneyin `ping 10.0.0.4`.
