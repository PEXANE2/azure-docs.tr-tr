...
1. **Özel dağıtım** sayfasında, bir WINDOWS Server VM 'sini oluşturmak ve Azure AD DS yönetilen etki alanına katmak için aşağıdaki bilgileri girin:
    ...
    | Etki alanı Kullanıcı adı           | Azure AD DS yönetilen etki alanındaki, VM 'yi yönetilen etki alanına (gibi) katmak için kullanılması gereken kullanıcı hesabı `contosoadmin@aaddscontoso.com`. Bu hesap, Azure AD DS yönetilen etki alanının bir parçası olmalıdır. |
...
1. **Özel dağıtım** sayfasında, VM 'yi Azure AD DS yönetilen etki alanına katmak için aşağıdaki bilgileri girin:
    ...
    | Etki alanına katılması Kullanıcı adı     | Azure AD DS yönetilen etki alanındaki, VM 'yi yönetilen etki alanına (gibi) katmak için kullanılması gereken kullanıcı hesabı `contosoadmin@aaddscontoso.com`. Bu hesap, Azure AD DS yönetilen etki alanının bir parçası olmalıdır. |
