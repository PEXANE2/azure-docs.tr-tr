...
VM 'nin yönetilen etki alanına sanal makineye katılması için bazı ek paketlere ihtiyacı vardır. Bu paketleri yüklemek ve yapılandırmak için, kullanarak etki alanına ekleme araçlarını güncelleştirin ve yapılandırın `yum` . RHEL 7. x ve RHEL 6. x arasında bazı farklılıklar vardır. bu nedenle, bu makalenin kalan bölümlerinde yer aldığı sürümünüz için uygun komutları kullanın.
...
Gerekli paketler VM 'ye yüklendikten sonra VM 'yi yönetilen etki alanına katın. Yine de, RHEL geçmiş sürümünüz için uygun adımları kullanın.
...
1. İlk olarak, komutunu kullanarak etki alanına katılarak `adcli join` , bu komut makinenin kimliğini doğrulamak için anahtar sekmesini de oluşturur. Yönetilen etki alanının bir parçası olan bir kullanıcı hesabı kullanın.

    ```console
    sudo adcli join aaddscontoso.com -U contosoadmin
    ```

1. Şimdi öğesini yapılandırıp `/ect/krb5.conf` `/etc/sssd/sssd.conf` Active Directory etki alanını kullanacak şekilde dosyaları oluşturun `aaddscontoso.com` .
   ...
   Dosyayı oluşturun `/etc/sssd/sssd.conf` :
...
1. `/etc/sssd/sssd.conf`İzinlerin 600 olduğundan ve kök kullanıcıya ait olduğundan emin olun:
...
1. `authconfig`VM 'YI ad Linux tümleştirmesi hakkında bildirmek için kullanın:
...
RHEL VM 'de *AAD DC yöneticileri* grubunun üyelerine yönetici ayrıcalıkları vermek için, */etc/sudoers*'a bir giriş eklersiniz. Eklendikten sonra, *AAD DC yöneticileri* grubunun üyeleri `sudo` RHEL VM üzerinde komutunu kullanabilir.
