...
Yukarıdaki örnekte, yeni bir `content-security-policy` üst bilgi eklenir, `cache-control` sunucu varsayılan değerini değiştirir ve `x-dns-prefectch-control` üstbilgi kaldırılır.
...
- _routes.js_ ' de tanımlanan üstbilgiler yalnızca statik içerik için geçerlidir. İşlevin kodundaki bir API uç noktasının yanıt üstbilgilerini özelleştirebilirsiniz.
