...
1. Bir [Azure Cloud Shell](https://shell.azure.com) açın ve örnek uygulama deposunu yerel makinenize kopyalayın.  Burada, önce uygulamayı kopyalamadan önce adlı `source-code` geçici bir dizin oluşturacağız.
...
5. Aynı kalıbı izleyerek, `account-service` ve `auth-service` uygulamalarını oluşturup jar dosyalarını dağıtın.
