...
Aşağıdaki tabloda, *function. JSON* dosyasında ayarladığınız bağlama yapılandırma özellikleri açıklanmaktadır. C# sınıf kitaplıkları için, bu *function. JSON* özelliklerine karşılık gelen bir öznitelik özelliği yok.

|Özellik  |Açıklama  |
|---------|---------|
| **türüyle** |Olarak `http`ayarlanmalıdır. |
| **Görünüm** | Olarak `out`ayarlanmalıdır. |
| **ada** | Yanıt için işlev kodunda kullanılan değişken adı veya `$return` dönüş değerini kullanmak. |

## <a name="usage"></a>Kullanım

HTTP yanıtı göndermek için, dil standardı yanıt düzenlerini kullanın. C# veya C# komut dosyasında, işlev dönüş türü `IActionResult` veya `Task<IActionResult>`' nu yapın. C# ' de, dönüş değeri özniteliği gerekli değildir.
...
|---------|---------|---------| 
| customHeaders|yok|HTTP yanıtında özel üstbilgiler ayarlamanıza olanak sağlar. Önceki örnek, içerik türü `X-Content-Type-Options` algılaması olmaması için üst bilgiyi yanıta ekler. |
|dynamicThrottlesEnabled|değeri<sup>\*</sup>|Bu ayar etkinleştirildiğinde, istek işleme işlem hattının benzer bir şekilde `connections/threads/processes/memory/cpu/etc` sistem performans sayaçlarını denetlemesini sağlar ve bu sayaçlardan herhangi biri yerleşik yüksek eşikten (%80%) olursa, sayaçlar normal düzeylere dönene kadar istekler `429 "Too Busy"` yanıt vererek reddedilir.<br/><sup>\*</sup>Bir tüketim planında varsayılan değer `true`. Adanmış bir planda varsayılan değer `false`.|
|HSTS|etkin değil|`isEnabled` , Olarak `true`ayarlandığında, [ `HstsOptions` sınıfında](/dotnet/api/microsoft.aspnetcore.httpspolicy.hstsoptions?view=aspnetcore-3.0)tanımlandığı şekilde [.NET Core 'un http katı taşıma güvenliği (HSTS) davranışı](/aspnet/core/security/enforcing-ssl?view=aspnetcore-3.0&tabs=visual-studio#hsts) zorlanır. Yukarıdaki örnek ayrıca [`maxAge`](/dotnet/api/microsoft.aspnetcore.httpspolicy.hstsoptions.maxage?view=aspnetcore-3.0#Microsoft_AspNetCore_HttpsPolicy_HstsOptions_MaxAge) özelliği 10 gün olarak ayarlar. Desteklenen özellikleri `hsts` şunlardır: <table><tr><th>Özellik</th><th>Açıklama</th></tr><tr><td>Excludedkonakları</td><td>HSTS üstbilgisinin eklendiği ana bilgisayar adlarının dize dizisi.</td></tr><tr><td>includeSubDomains</td><td>Strict-Transport-Security üstbilgisinin ıncludealt etki alanı parametresinin etkinleştirilip etkinleştirilmeyeceğini gösteren Boolean değer.</td></tr><tr><td>maxAge</td><td>Strict-Transport-Security üstbilgisinin Max-Age parametresini tanımlayan dize.</td></tr><tr><td>preload</td><td>Strict-Transport-Security üstbilgisinin preload parametresinin etkin olup olmadığını gösteren Boolean.</td></tr></table>|
