...
> * Azure Işlevleri uygulaması oluşturma
...
4. **Temel bilgiler** sekmesinde, görüntünün altındaki tabloda belirtilen VM ayarlarını kullanın:
    ...
    | [**Geli**](https://azure.microsoft.com/regions/) | ABD Orta Kuzey ABD | Size yakın veya erişilecek işlevlere yakın bir bölge seçin. |
    | **Ortak gelen bağlantı noktaları** | Yok | İnternet 'ten sanal makineye gelen bağlantı olmadığından emin olmak için **hiçbiri** ' ni seçin. SANAL makineye uzaktan erişim, Azure savunma hizmeti aracılığıyla yapılandırılır. |

5. Yeni bir sanal ağ yapılandırmak için **ağ** sekmesini seçin ve **Yeni oluştur** ' u seçin.

    >[!div class="mx-imgBorder"]
    >yeni VM için yeni bir sanal ağ oluşturmak ![](./media/functions-create-private-site-access/create-vm-networking.png)

6. **Sanal ağ oluştur**' da, görüntünün altındaki tabloda bulunan ayarları kullanın:

    >[!div class="mx-imgBorder"]
    >yeni VM için yeni bir sanal ağ oluşturmak ![](./media/functions-create-private-site-access/create-vm-vnet-1.png)
    ...
    | **Adres aralığı** (alt ağ) | 10.10.1.0/24 | Alt ağ boyutu, alt ağa kaç arabirim eklenebileceğini tanımlar. Bu alt ağ, VM tarafından kullanılır. Bir `/24` alt ağı 254 ana bilgisayar adresi sağlar. |
...
3. Yeni bir Azure savunma kaynağı oluşturma işlemini başlatmak için **Oluştur** ' u seçin. Henüz bir `AzureBastionSubnet` alt ağı olmadığı için **sanal ağ** bölümünde bir hata iletisi görürsünüz. Alt ağ aşağıdaki adımlarda oluşturulur. Görüntünün altındaki tabloda bulunan ayarları kullanın:

    >[!div class="mx-imgBorder"]
    >![Azure savunma](./media/functions-create-private-site-access/create-bastion-basics-1.png) oluşturma işlemi başlatıldı

    | Ayar      | Önerilen değer  | Açıklama      |
    | ------------ | ---------------- | ---------------- |
    | **Ad** | tasyon | Yeni savunma kaynağının adı |
    ...
    | **Alt ağ** | AzureBastionSubnet | Sanal ağınızdaki, yeni savunma ana bilgisayar kaynağının dağıtılacağı alt ağ. Ad değeri `AzureBastionSubnet`kullanarak bir alt ağ oluşturmanız gerekir. Bu değer, Azure 'un savunma kaynaklarını hangi alt ağa dağıtacağınızı bilmesini sağlar. En az `/27` veya daha büyük bir alt ağ kullanmanız gerekir (`/27`, `/26`, vb.). |
...
5. Alt ağ `AzureBastionSubnet` ad olmalıdır ve alt ağ ön eki en az `/27`olmalıdır.  Alt ağı oluşturmak için **Tamam ' ı** seçin.
...
6. Savunma **Oluştur** sayfasında, kullanılabilir alt ağlar listesinden yeni oluşturulan `AzureBastionSubnet` seçin.

    >[!div class="mx-imgBorder"]
    >belirli bir alt ağla Azure savunma ana bilgisayarı oluşturma ![](./media/functions-create-private-site-access/create-bastion-basics-2.png)
...
## <a name="create-an-azure-functions-app"></a>Azure Işlevleri uygulaması oluşturma
...
5. **Erişim kısıtlamaları** sayfası artık yeni bir kısıtlama olduğunu gösterir. **Uç nokta durumunun** `Enabled`ile `Provisioning` arasında `Disabled` değiştirilmesi birkaç saniye sürebilir.
...
1. Önceden oluşturulmuş işlev uygulamasına geri dönün.  **Genel bakış** bölümünde URL 'yi kopyalayın.
    ...
    >[!div class="mx-imgBorder"]
    >![Işlevi uygulama URL 'sini alın](./media/functions-create-private-site-access/access-function-overview.png)
...
3. İşlevin dağıtıldığını doğrulayın.
    ...
    >[!div class="mx-imgBorder"]
    >işlev listesinde dağıtılan ![işlevi](./media/functions-create-private-site-access/verify-deployed-function.png)
...
1. İşleve erişimi test etmek için, işlev URL 'sini kopyalamanız gerekir. Dağıtılan işlevi seçin ve ardından **</> işlev URL 'Sini al**' ı seçin. Ardından, URL 'YI panonuza kopyalamak için **Kopyala** düğmesine tıklayın.
    ...
    >[!div class="mx-imgBorder"]
    >![işlev URL 'sini kopyalayın](./media/functions-create-private-site-access/get-function-url.png)
...
>[!div class="mx-imgBorder"]
>Azure ![Azure 'a erişim aracılığıyla erişin](./media/functions-create-private-site-access/access-function-via-bastion-final.png)
