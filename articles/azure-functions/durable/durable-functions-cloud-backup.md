...
* `E2_BackupSiteContent`: Yedeklenecek dosyaların listesini almak için `E2_GetFileList` çağıran bir [Orchestrator işlevi](durable-functions-bindings.md#orchestration-trigger) , ardından her dosyayı yedeklemek için `E2_CopyFileToBlob` çağırır.
...
* `E2_CopyFileToBlob`: Azure Blob depolamaya tek bir dosya yedekleyen bir etkinlik işlevi.
...
`await Task.WhenAll(tasks);` satırına dikkat edin. `E2_CopyFileToBlob` işlevine yapılan tüm çağrılar *beklenmez* , bu da paralel çalışmasına izin verir. Bu görev dizisini `Task.WhenAll`ilettiğimiz zaman, *tüm kopyalama işlemleri tamamlanana kadar*tamamlanmamış bir görevi geri alırız. .NET 'teki görev paralel kitaplığı (TPL) hakkında bilginiz varsa, bu sizin için yeni değildir. Fark, bu görevlerin eşzamanlı olarak birden çok sanal makinede çalışabilmesidir ve Dayanıklı İşlevler uzantısı uçtan uca yürütmenin geri dönüşüm işleminin işlenmesine dayanıklı olmasını sağlar.

`Task.WhenAll`bekledikten sonra, tüm işlev çağrılarının tamamlandığını ve bize doğru değer döndürmediğini biliyoruz. `E2_CopyFileToBlob` yapılan her bir çağrı, karşıya yüklenen bayt sayısını döndürür, bu nedenle toplam bayt sayısını hesaplamak, bu dönüş değerlerinin tümünü birlikte eklemenin bir önemi olur.
...
`yield context.df.Task.all(tasks);` satırına dikkat edin. `E2_CopyFileToBlob` işlevine yapılan tüm çağrılar, bunların paralel olarak çalıştırılmasına izin veren bir *şekilde yapılmadı.* Bu görev dizisini `context.df.Task.all`ilettiğimiz zaman, *tüm kopyalama işlemleri tamamlanana kadar*tamamlanmamış bir görevi geri alırız. JavaScript 'te [`Promise.all`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise/all) hakkında bilgi sahibiyseniz, bu sizin için yeni değildir. Fark, bu görevlerin eşzamanlı olarak birden çok sanal makinede çalışabilmesidir ve Dayanıklı İşlevler uzantısı uçtan uca yürütmenin geri dönüşüm işleminin işlenmesine dayanıklı olmasını sağlar.
...
`E2_GetFileList` için *function. JSON* dosyası aşağıdaki gibi görünür:
...
İşlev, dizin yapısını özyinelemeli olarak okumak için `readdirp` modülünü (sürüm 2. x) kullanır.
...
İşlevi, Azure Işlevleri bağlamalarının (yani [`Binder` parametresinin](../functions-dotnet-class-library.md#binding-at-runtime)kullanımı) bazı gelişmiş özelliklerini kullanır, ancak bu izlenecek yol için bu ayrıntılar konusunda endişelenmenize gerek yoktur.
...
> [!NOTE]
> Bu, g/ç işlemlerini `activityTrigger` işleve taşımaya yönelik kusursuz bir örnektir. İş yalnızca birçok farklı makineye dağıtılmayabilir, ancak ilerlemeyi gösterme avantajlarının avantajlarından de yararlanabilirsiniz. Ana bilgisayar işlemi herhangi bir nedenle sonlandırılırsa, hangi karşıya yüklemelerinin zaten tamamlandığını bilirsiniz.
