...
1. İşlev bir [Oluşturucu işlevidir.](/scripting/javascript/advanced/iterators-and-generators-javascript).
...
JavaScript Orchestration işlevinin aksine, etkinlik işlevinin özel bir kurulum yapması gerekmez. Orchestrator işlevi tarafından kendisine geçirilen giriş, `context.bindings` `activityTrigger` Bu örnekte, bağlamanın adı altındaki nesne üzerinde bulunur `context.bindings.name` . Bağlama adı, içe aktarılmış işlevin parametresi olarak ayarlanabilir ve doğrudan erişilebilen, örnek kodun yaptığı şeydir.
