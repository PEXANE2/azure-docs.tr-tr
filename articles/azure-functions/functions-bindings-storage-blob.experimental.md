...
Kullanarak `context.bindings.<name from function.json>`blob verilerine erişin.
...
`@BlobInput` Özniteliği, işlevi tetikleyen bloba erişmenizi sağlar. Özniteliği ile bir bayt dizisi kullanırsanız, olarak `dataType` `binary`ayarlayın. Ayrıntılar için [giriş örneğine](#input---example) bakın.
...
`@BlobOutput` Özniteliği, işlevi tetikleyen bloba erişmenizi sağlar. Özniteliği ile bir bayt dizisi kullanırsanız, olarak `dataType` `binary`ayarlayın. Ayrıntılar için [Çıkış örneğine](#output---example) bakın.
