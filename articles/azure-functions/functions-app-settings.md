...
`true` means disable the default landing page that is shown for the root URL of a function app. `false` varsayılan değerdir.
...
`true` means use Release mode when compiling .NET code; `false` means use Debug mode. `true` varsayılan değerdir.
...
|-|-|-|
|AZURE_FUNCTION_PROXY_DISABLE_LOCAL_CALL|doğru|Calls with a backend url pointing to a function in the local Function App will no longer be sent directly to the function, and will instead be directed back to the HTTP front end for the Function App|
|AZURE_FUNCTION_PROXY_DISABLE_LOCAL_CALL|yanlış|Varsayılan değer budur. Calls with a  backend url pointing to a function in the local Function App will be forwarded directly to that Function|
...
|Anahtar|Değer|Açıklama|
|-|-|-|
|AZURE_FUNCTION_PROXY_BACKEND_URL_DECODE_SLASHES|doğru|Route parameters with encoded slashes will have them decoded. `example.com/api%2ftest` will become `example.com/api/test`|
|AZURE_FUNCTION_PROXY_BACKEND_URL_DECODE_SLASHES|yanlış|This is the default behavior. All route parameters will be passed along unchanged|
