...
Copy the URL of your `HttpTrigger` function from this output and paste it into your browser's address bar. `?name=<yourname>` sorgu dizesini bu URL’ye ekleyip isteği yürütün. The following screenshot shows the response to the GET request that the local function returns to the browser:
...
Run the following command. Replace `<APP_NAME>` with a unique function app name. Replace `<STORAGE_NAME>` with a storage account name. `<APP_NAME>` aynı zamanda işlev uygulamasının varsayılan DNS etki alanıdır. Bu ad Azure'daki tüm uygulamalar arasında benzersiz olmalıdır.
