...
- `DisplayControl` türü `VerificationControl`.
- `DisplayClaims`
  ...
  - **Doğrulama kodu** -kod gönderildikten sonra kullanıcının sağladığı doğrulama kodu talebi. Bu talebin gerekli olarak ayarlanması ve `ControlClaimType` `VerificationCode`olarak ayarlanması gerekir.
...
- Aşağıdaki adlarla iki `Action`s:
