...
* İlkelerinizi Web veya mobil uygulamanızdan bağımsız olarak test etmek için **Şimdi Çalıştır** ve `https://jwt.ms` kullanın. Bu web sitesi, bağlı olan taraf uygulaması gibi davranır. Azure AD B2C ilkeniz tarafından oluşturulan JSON Web belirtecinin (JWT) içeriğini görüntüler.

    Belirteç incelemesi için `https://jwt.ms` yeniden yönlendirebilmek üzere bir test uygulaması oluşturmak için:
