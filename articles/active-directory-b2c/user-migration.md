...
![UsersData. json dosyasının iki kullanıcının JSON bloklarını gösteren bölümü](./media/user-migration/pre-migration-data-file.png)
...
![UserMigration. exe komutunun çıkışını gösteren komut Istemi penceresi](./media/user-migration/pre-migration-demo.png)
...
- Bir kullanıcıyı oturum açma e-posta adresi ile almak için örnek uygulamayı kullanın:
   ...
   1. Kullanıcının bilgilerini görmek için, bir JSON düzenleyicisinde USERPROFILE. json dosyasını açın.
      ...
      ![Visual Studio Code düzenleyicisinde açık olan UserProfile. JSON dosyası](./media/user-migration/pre-migration-get-by-email2.png)
...
Parola sıfırlama ilkenizin bağlantısını almak için aşağıdaki adımları izleyin. Bu yordamda daha önce bir parola sıfırlama [özel ilkesi](custom-policy-get-started.md)oluşturmuş olduğunuz varsayılmaktadır.
...
1. **Şimdi Çalıştır uç noktası** metin kutusunda gösterilen URL 'yi kopyalayın ve sonra kullanıcılarınıza gönderin.
    ...
    ![Şimdi Çalıştır uç noktası vurgulanmış şekilde parola sıfırlama ilkesi sayfası](./media/user-migration/pre-migration-policy-uri.png)
...
1. Geçirilmiş kullanıcılardan birinin kimlik bilgilerini girin ve **oturum aç**' ı seçin. REST API aşağıdaki hata iletisini oluşturması gerekir:
    ...
    ![Parolayı değiştirme hata iletisini gösteren oturum açma sayfası](./media/user-migration/pre-migration-error-message.png)
...
1. **Kaydet**’i seçin
    ...
    ![Azure portal 'de tanılama günlükleri yapılandırma sayfası](./media/user-migration/pre-migration-diagnostic-logs.png)
