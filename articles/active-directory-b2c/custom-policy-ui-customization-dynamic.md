...
Bu makalede, özel ilkeler kullanılarak Azure AD B2C Kullanıcı arabiriminin *Dinamik içerikle* nasıl özelleştirileceği konusuna odaklanılmaktadır. Başlamak için bkz. [özel Ilkede UI özelleştirmesi](custom-policy-ui-customization.md).

>[!NOTE]
>Azure AD B2C makalesi, [Kullanıcı arabirimi özelleştirmesini özel bir Ilkede yapılandırma](custom-policy-ui-customization.md), aşağıdaki temelleri anlatmaktadır:
...
[Özel bir ilke için Kullanıcı arabirimi özelleştirmesini yapılandırma](custom-policy-ui-customization.md) makalesinde HTML5 dosyalarını Azure Blob depolamaya yüklersiniz. Bu HTML5 dosyaları statiktir ve her istek için aynı HTML içeriğini işler.
...
3. Uygulamayı adlandırın (örneğin, *contoso. AADB2C. UI*) ve ardından **Tamam**' ı seçin.
    ...
    ![Yeni Visual Studio projesi oluştur](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-create-project1.png)

4. **Web uygulaması** şablonunu seçin.

5. Kimlik doğrulamasını **kimlik doğrulaması olmadan**ayarlayın.

    ![Web uygulaması şablonu seçin](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-create-project2.png)
...
1. Görünümler/giriş klasörüne sağ tıklayın ve ardından > **Yeni öğe** **ekleyin** .
    ...
    ![Visual Studio 'da yeni öğe Ekle menü öğesi](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-add-view1.png)
...
5. **Add (Ekle)** seçeneğini belirleyin.
    ...
    ![MVC görünümü sayfası vurgulanmış şekilde Visual Studio 'da yeni öğe Ekle iletişim kutusu](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-add-view2.png)
...
10. Dosyanın içeriğini kopyalayın ve düzen tanımının altına yapıştırın. Kodunuz şöyle görünmelidir:
    ...
    ![HTML5 eklendikten sonra birleştirilmiş. cshtml dosyası](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-edit-view1.png)
...
![Özel background_background_image src değeri olan img öğesi](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-add-static-background.png)
...
1. **Controllers\homecontroller.cs**dosyasını açın ve aşağıdaki yöntemi ekleyin:
    ...
    ![Denetleyiciyi, görünümü işleyecek şekilde değiştirin](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-controller-view.png)
...
1. **Çözüm Gezgini**, **contoso. AADB2C. UI** projesine sağ tıklayın ve ardından **Yayımla**' yı seçin.
    ...
    ![Microsoft Azure App Service yayımlayın](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-publish1.png)

2. **Microsoft Azure App Service** kutucuğunu seçip **Yayımla**' yı seçin.

    ![Yeni Microsoft Azure App Service oluştur](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-publish2.png)
...
4. Azure kaynaklarını oluşturmaya başlamak için **Oluştur**’u seçin.
    ...
    ![App Service özellikleri sağlama](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-publish3.png)
...
1. [Azure Portal](https://portal.azure.com/), **uygulama hizmetleri**' nı seçin ve ardından API uygulamanızın adını seçin.
    ...
    ![Azure portal API uygulamasını seçin](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-CORS1.png)

2. **Ayarlar** bölümünde, **API** bölümü altında **CORS**' yi seçin.

    ![CORS menü öğesi Azure portal App Service menüsünde vurgulanır](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-CORS2.png)
...
4. **Kaydet**’i seçin.
    ...
    ![Izin verilen çıkış noktaları ile birlikte yıldız vurgulanmış CORS ayarları sayfası](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-CORS3.png)
...
6. `LoadUri` değerini _~/Tenant/default/Unified_ ile _https://< app_name >. azurewebsites. net/Home/Unified_olarak değiştirin.
    ...
    ![LoadUri öğesi vurgulanmış örnek XML kod parçacığı](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-content-definition.png)
...
2. Karşıya yüklediğiniz bağlı olan taraf (RP) özel ilkesini **B2C_1A_signup_signin**açın ve **Şimdi Çalıştır**' ı seçin.
    ...
    ![Kaydolma veya oturum açma ilkeniz](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-demo1.png)
...
2. KIMLIĞI `background_background_image``<img>` öğeyi bulun ve `src` değerini `@ViewData["background"]`ile değiştirin.
    ...
    ![src değeri vurgulanmış img öğesi ](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-add-dynamic-background.png)
...
5. URI 'ye _Kampanya NID_ sorgu dizesi parametresini ekleyin. Örneğin, aşağıdaki görüntüde gösterildiği gibi `&campaignId=hawaii`ekleyin:
    ...
    ![Kampanyakimliği sorgu dizesi parametresi vurgulanmış URI](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-campaignId-param.png)

6. Hawaii arka plan resmini göstermek için **ENTER** ' u seçin.

    ![Hawaii resmi özel arka planı ile oturum açma sayfası](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-demo2.png)

7. Değeri *Tokyo*olarak değiştirin ve **ENTER**' u seçin.
    Tarayıcı, Tokyo arka planını görüntüler.

    ![Tokyo görüntüsü özel arka planı ile kaydolma oturum açma sayfası](./media/custom-policy-ui-customization-dynamic/aadb2c-ief-ui-customization-demo3.png)
