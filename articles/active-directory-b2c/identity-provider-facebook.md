...
Azure Active Directory B2C (Azure AD B2C) ' de bir Facebook hesabını [kimlik sağlayıcısı](authorization-code-flow.md) olarak kullanmak için kiracınızda onu temsil eden bir uygulama oluşturmanız gerekir. Henüz bir Facebook hesabınız yoksa [https://www.facebook.com/](https://www.facebook.com/)oturum açabilirsiniz.
...
1. **Temel** > **Ayarlar** ' ı seçin.
1. Bir **Kategori**seçin, örneğin `Business and Pages`. Bu değer Facebook için gereklidir, ancak Azure AD B2C için kullanılmaz.
1. Sayfanın alt kısmındaki **Platform Ekle**' yi ve ardından **Web sitesi**' ni seçin.
1. **Site URL 'si**' nde `your-tenant-name`, kiracınızın adıyla değiştirme `https://your-tenant-name.b2clogin.com/` girin. **Gizlilik ILKESI URL**'si IÇIN bir URL girin, örneğin `http://www.contoso.com`. İlke URL 'SI, uygulamanız için gizlilik bilgilerini sağlamak üzere tuttuğunuz bir sayfasıdır.
...
1. **Geçerli OAuth yeniden yönlendirme URI 'lerinde**`https://your-tenant-name.b2clogin.com/your-tenant-name.onmicrosoft.com/oauth2/authresp`girin. `your-tenant-name`, kiracınızın adıyla değiştirin. Sayfanın alt kısmındaki **Değişiklikleri Kaydet** ' i seçin.
