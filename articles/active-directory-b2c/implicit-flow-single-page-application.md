...
Bu istekte istemci, `scope` parametresindeki kullanıcıdan ve Kullanıcı akışının çalışması için gereken izinleri gösterir. İsteğin nasıl çalıştığına ilişkin bir fikir almak için, isteği bir tarayıcıya yapıştırmayı ve çalıştırmayı deneyin. Azure AD B2C `{tenant}` kiracınızın adıyla değiştirin. Daha `90c0fe63-bcf2-44d5-8fb7-b8bbc0b29dc6` önce kiracınıza kaydettiğiniz UYGULAMANıN uygulama kimliğiyle değiştirin. Kiracınızda oluşturduğunuz bir ilkenin adıyla değiştirin `{policy}` (örneğin `b2c_1_sign_in`,).
...
| response_type | Yes | OpenID `id_token` Connect oturum açma için içermelidir. Yanıt türünü `token`de içerebilir. Kullanıyorsanız `token`, uygulamanız yetkilendirme uç noktası için ikinci bir istek yapmadan yetkilendirme uç noktasından hemen bir erişim belirteci alabilir.  `token` Yanıt türünü kullanırsanız, `scope` parametresi belirtecinin hangi kaynağa verilmeyeceğini belirten bir kapsam içermelidir. |
...
' I kullanan `response_mode=fragment` başarılı bir yanıt `response_type=id_token+token` , okunabilirliği için satır sonları ile aşağıdaki gibi görünür:
...
* KIMLIK belirtecinin `iat` sona `exp` ermediğinden emin olmak için ve taleplerini doğrulayın.
...
| response_type |Gerekli |OpenID `id_token` Connect oturum açma için içermelidir.  Yanıt türünü `token`de içerebilir. Burada kullanıyorsanız `token` , uygulamanız yetkilendirme uç noktası için ikinci bir istek yapmadan yetkilendirme uç noktasından hemen bir erişim belirteci alabilir. `token` Yanıt türünü kullanırsanız, `scope` parametresi belirtecinin hangi kaynağa verilmeyeceğini belirten bir kapsam içermelidir. |
...
| login_hint |Gerekli |Gizli bir iframe içindeki belirteçleri yenilemek ve almak için, kullanıcının belirli bir zamanda sahip olabileceği birden çok oturumu ayırt etmek için bu ipucuna kullanıcının Kullanıcı adını ekleyin. `preferred_username` Talebi kullanarak daha önceki bir oturum açma işleminden Kullanıcı adını ayıklayabilirsiniz ( `preferred_username` talebi almak için `profile` kapsam gereklidir). |
| domain_hint |Gerekli |`consumers` veya `organizations` olabilir.  Gizli bir IFRAME içindeki belirteçleri yenilemek ve almak için, istekteki `domain_hint` değeri ekleyin.  Hangi değerin `tid` kullanılacağını öğrenmek için daha önceki bir oturum açma kimlik belirtecinden talebi ayıklayın ( `profile` `tid` talebi almak için kapsam gereklidir). `tid` Talep değeri ise `9188040d-6c67-4c5b-b112-36a304b66dad`, kullanın `domain_hint=consumers`.  Aksi takdirde, `domain_hint=organizations`kullanın. |
...
GitHub 'daki Bu örnek, [msal. js][github-msal-js] ile oluşturulmuş basit bir Web uygulamasında ve açılır stil kimlik doğrulamasını kullanarak Azure AD B2C başlamanıza yardımcı olmaya yöneliktir.
