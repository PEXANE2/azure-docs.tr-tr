...
**Inputclaim** Öğesı, SAML AuthN isteğinin **konusu** içinde bir **NameID** göndermek için kullanılır. Bunu başarmak için, aşağıda gösterildiği gibi bir **Partnerclaimtype** kümesine sahip bir giriş talebi ekleyin `subject` .
...
| SamlAssertionDecryption |No | X509 sertifikası (RSA anahtar kümesi). SAML kimlik sağlayıcısı, SAML yanıtının onayını şifrelemek için sertifikanın ortak kısmını kullanır. Azure AD B2C, onay şifresini çözmek için sertifikanın özel kısmını kullanır. |
...
## <a name="saml-entityid-customization"></a>SAML EntityId özelleştirmesi

Farklı EntityId değerlerine bağımlı birden çok SAML uygulamanız varsa, `issueruri` bağlı olan taraf dosyanızdaki değeri geçersiz kılabilirsiniz. Bunu yapmak için, temel dosyadaki "Saml2AssertionIssuer" KIMLIĞIYLE teknik profili kopyalayın ve değeri geçersiz kılın `issueruri` .
...
- [SAML aracılığıyla Salesforce hesaplarını kullanarak oturum açın](identity-provider-salesforce-saml.md)