...
## <a name="associate"></a>4. hub VPN Gateway için özel IP adreslerini alın
...
* **Vpnsiteconnections**: Bu bölüm aşağıdaki ayarlarla ilgili bilgiler sağlar:
    ...
    * Hub 'a bağlı sanal ağların adres alanı.<br>Örnek:
           ...
           "ConnectedSubnets":["10.51.231.0/24"]
            ```
    * Sanal hub 'ın VPN ağ geçidinin IP adresleri. Her VPN Ağ Geçidi bağlantısı etkin-etkin yapılandırmada iki tünelde bulunduğundan, bu dosyada her iki IP adresinin de listelendiğini görürsünüz. Bu örnekte, her site için `Instance0` ve `Instance1` görürsünüz ve bunlar genel IP adresleri yerine özel IP adresleridir.<br>Örnek:
           ``` 
...
1. Sanal WAN 'a gidin.
1. **Genel bakış** sayfasında, haritadaki her bir nokta bir hub 'ı temsil eder. Hub 'ın sistem durumu özetini görüntülemek için herhangi bir noktanın üzerine gelin.
...
1. WAN 'nize gidin.
1. **Destek + sorun giderme** bölümünde, **sistem durumu** ' nu seçin ve kaynağınızı görüntüleyin.
...
Bu kaynaklara artık ihtiyacınız kalmadığında, [Remove-AzResourceGroup](/powershell/module/az.resources/remove-azresourcegroup) komutunu kullanarak kaynak grubunu ve içerdiği tüm kaynakları kaldırabilirsiniz. Aşağıdaki PowerShell komutunu çalıştırın ve `myResourceGroup` yerine kaynak grubunuzun adını koyun:
