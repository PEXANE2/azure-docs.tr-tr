...
Azure sanal veri merkezi (VDC) hub ve-ışınsal bağlantısı modeli, Azure ağ 'ın basit ve ölçeklenebilir bulut ağları oluşturmak için varsayılan geçişli yönlendirme davranışından yararlanmak üzere binlerce müşterimiz tarafından benimsenmiştir. Azure sanal WAN derlemeleri bu kavramlarda değil, yalnızca şirket içi konumlar ve Azure arasında değil, genel bağlantı Topolojilerine izin veren yeni yetenekler sunar, ancak müşterilerin Microsoft ağ ölçeğinden yararlanarak mevcut küresel ağlar.
...
## <a name="architecture"></a>Azure sanal WAN mimarisi
...
![tek bölgeli topoloji](./media/migrate-from-hub-spoke-topology/figure1.png)
...
![Şekil 2 ' de sanal WAN hub 'larını dağıtma](./media/migrate-from-hub-spoke-topology/figure2.png)
 **: VDC hub 'ı-sanal WAN geçişine bağlı**
...
![uzak siteleri sanal WAN 'a bağlama](./media/migrate-from-hub-spoke-topology/figure3.png)
**Şekil 3: sanal WAN geçişine VDC hub ve-bağlı bileşen**
...
Şekil 4 aracılığıyla ![test hibrit bağlantısı **: VDC hub ve-sanal WAN geçişine bağlı**
...
**Şekil 5: VDC hub ve-sanal WAN geçişine bağlı bileşen**
...
![eski hub, paylaşılan hizmetlere bağlı olur](./media/migrate-from-hub-spoke-topology/figure6.png)
**Şekil 6: sanal WAN hub 'ına ve-sanal WAN geçişine bağlı**

Sanal WAN hub 'ı yönetilen bir varlık olduğundan ve sanal makineler gibi özel kaynakların dağıtımına izin vermediğinden, paylaşılan hizmetler bloğu artık bağlı bir sanal ağ olarak bulunmaktadır ve Azure Application Gateway veya internet girişi gibi işlevleri barındırır veya Ağ sanallaştırılmış gereç. Paylaşılan hizmetler ortamı ve arka uç sanal makineleri arasındaki trafik artık sanal WAN ile yönetilen hub 'ı geçişli.
...
Sanal WAN 'ı tam olarak kullanmak için şirket içi bağlantıyı Iyileştirmek ![](./media/migrate-from-hub-spoke-topology/figure7.png)
**Şekil 7: VDC hub 'ı ve sanal WAN geçişine bağlı bileşen**
