...
ms.locfileid: "83006267"
---
# <a name="sd-wan-connectivity-architecture-with-azure-virtual-wan"></a>SD-Azure sanal WAN ile WAN bağlantısı mimarisi

Azure sanal WAN, tek bir işlemsel arabirimle birlikte birçok bulut bağlantısı ve güvenlik hizmetini birlikte getiren bir ağ hizmetidir. Bu hizmetler dalı (siteden siteye VPN aracılığıyla), uzak kullanıcının (Noktadan siteye VPN), özel (ExpressRoute) bağlantısının yanı sıra VNET 'ler, VPN ve ExpressRoute bağlantısı, yönlendirme, Azure Güvenlik Duvarı ve özel bağlantı için şifreleme için de bulut geçişli bağlantı içerir.
...
Sanal WAN ile kullanıcılar Azure yol seçimini alabilir ve bu, CPE 'ten sanal WAN VPN ağ geçitleri arasında birden çok ISS bağlantısı arasında ilke tabanlı yol seçimidir. Sanal WAN, birden çok bağlantı (yol) için aynı SD-WAN dalından, her bağlantının SD-WAN CPE 'ın farklı genel IP arabiriminde sonlandırılmasını sağlar. SD-WAN satıcıları, bu yollara özgü trafik ilkelerine bağlı olarak en iyi Azure yolunu seçmek için bu özellikten yararlanabilir.
