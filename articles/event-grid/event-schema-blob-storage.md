...
* `dataVersion` anahtarı bir `2`değerine ayarlanır.

* `data.api` anahtarı dize `CreateFile` veya `FlushWithClose`olarak ayarlanır.

* `contentOffset` anahtarı veri kümesine dahil edilir.
...
* `data.api` anahtarı dize `DeleteFile`olarak ayarlanır.

* `url` anahtarı `dfs.core.windows.net`yolunu içerir.
...
| özyinelemeli | string | tüm alt dizinlerde işlemi gerçekleştirmek için `True`; Aksi takdirde `False`. <br>Yalnızca hiyerarşik bir ad alanına sahip BLOB depolama hesaplarında tetiklenen olaylar için görüntülenir. |
