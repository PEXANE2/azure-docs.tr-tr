...
Bu makalede, Azure Iletişim Hizmetleri olayları için özellikler ve şema sağlanmaktadır. Olay şemalarına giriş için bkz. [Azure Event Grid olay şeması](event-schema.md).
...
| Microsoft. Communication. ChatMessageReceived *                | Kullanıcının üyesi olduğu bir sohbet iş parçacığında kullanıcı için bir ileti alındığında yayımlandı.        |
...
* Tetiklenecek bu olaylara yönelik "ileti gönder" API çağrılarında "gönderen adı" nı sağladığınızdan emin olun.
...
Bir olay tetiklendiğinde, Event Grid hizmeti bu olayla ilgili verileri, uç noktalara abone olacak şekilde gönderir. Bu bölüm, verilerin her olay için nasıl görüneceğine ilişkin bir örnek içerir.
