...
---
# <a name="configure-private-endpoints-for-azure-event-grid-topics-or-domains-preview"></a>Azure Event Grid konuları veya etki alanları için özel uç noktaları yapılandırma (Önizleme)
...
> [!IMPORTANT]
> Özel uç noktalar özelliği, yalnızca Premium katmandaki konular ve etki alanları için kullanılabilir. Temel katmandan Premium katmana yükseltmek için bkz. [fiyatlandırma katmanını güncelleştirme](update-tier.md) makalesi. 
...
- `private-connection-resource-id`için, **konunun** veya **etkı alanının**kaynak kimliğini belirtin. Yukarıdaki örnek, türü: konusunu kullanır.
- `group-ids`için `topic` veya `domain`belirtin. Yukarıdaki örnekte `topic` kullanılır. 
...
- Sanal ağ
- Sanal ağdaki alt ağ
- Azure Event Grid konu (Premium katman)
