...
Azure sanal makinenizin NIC 'lerinin RSS 'yi desteklediğini görmek için aşağıdaki gibi komutu `Get-SmbClientNetworkInterface` çalıştırın ve `RSS Capable`alanı denetleyin: 
...
Aşağıdaki `Get-SmbClientNetworkInterace` çıkışının gösterdiği gibi, sanal makinenin iki ağ arabirimi vardır--15 ve 12.  Aşağıda gösterildiği gibi, komut `Get-SmbMultichannelConnection`altında gösterildiği gibi, iki RSS özellikli NIC olmasına karşın, SMB paylaşımıyla bağlantılı olarak yalnızca 12. arabirim kullanılır; 15. arabirim kullanımda değil.
...
İstemcide çok kanallı SMB devre dışı bırakıldığında, FIO ve 40-GiB çalışma kümesi kullanılarak saf 8-KiB okuma ve yazma testleri gerçekleştirildi.  SMB paylaşımının her bir test arasında, `1`,`4`,`8`,`16`, `set-SmbClientConfiguration -ConnectionCountPerRSSNetworkInterface <count>`RSS ağ arabirimi ayarları başına SMB istemci bağlantı sayısı artımlarıyla ayrılmıştı. Testler, g/ç yoğunluklu iş yükleri için `4` varsayılan ayarının yeterli olduğunu gösterir; `8` ve `16` artırma etkisi yoktur. 

Komut, `1` `8` `4` ve `16`arasında `netstat -na | findstr 445` artışlarla ek bağlantı kurulmasını sağlar.  Her test sırasında her bir `Per Processor Network Activity Cycles` sınama sırasında SMB için dört CPU çekirdeği tam olarak kullanıldı (Bu makaleye dahil değildir.)
...
## <a name="is-advanced-networking-recommended"></a>Gelişmiş ağ önerilir mi?

En yüksek performans için mümkün olduğunda gelişmiş ağ yapılandırmanız önerilir. Aşağıdaki noktaları göz önünde bulundurun:  

* Azure portal, bu özelliği destekleyen sanal makineler için varsayılan olarak gelişmiş ağa izin vermez.  Ancak, anormal ve benzer yapılandırma araçları gibi diğer dağıtım yöntemleri de olmayabilir.  Gelişmiş ağın etkinleştirilmemesi makinenin performansını hobble sağlayabilir.  
* Bir sanal makinenin ağ arabiriminde gelişmiş ağ etkinleştirilmemişse, örnek türü veya boyutu için destek olmaması gerekir, daha büyük örnek türleriyle devre dışı kalır. Bu durumlarda el ile müdahale etmeniz gerekir.
