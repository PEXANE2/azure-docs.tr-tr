...
description: Öğretici Azure portal ve SQL Server Management Studio kullanarak SQL Analytics için küresel bir Azure blobundan yeni York Taxicab verilerini yükler.
...
ms.locfileid: "78190405"
---
# <a name="tutorial-load-the-new-york-taxicab-dataset"></a>Öğretici: New York Taxicab veri kümesini yükleme

Bu öğretici, genel bir Azure Blob depolama hesabından yeni York Taxicab verilerini yüklemek için PolyBase 'i kullanır. Öğreticide aşağıdaki işlemler için [Azure Portal](https://portal.azure.com) ve [SQL Server Management Studio](/sql/ssms/download-sql-server-management-studio-ssms) (SSMS) kullanılır: 
...
Bir SQL havuzu tanımlanmış bir [işlem kaynakları] bellek-eşzamanlılık-limits.md) kümesiyle oluşturulur. Veritabanı bir [Azure kaynak grubu](../azure-resource-manager/management/overview.md) ve bir [Azure SQL mantıksal sunucusu](../sql-database/sql-database-features.md) içinde oluşturulur. 
...
3. Formu aşağıdaki bilgilerle doldurun: 
   ...
   | ------------------ | --------------------- | ------------------------------------------------------------ |
   | *Ad**            | mySampleDataWarehouse | Geçerli veritabanı adları için bkz. [Veritabanı Tanımlayıcıları](/sql/relational-databases/databases/database-identifiers). |
...
4. Yeni veritabanınız için yeni bir sunucu oluşturmak ve yapılandırmak üzere **sunucu** ' yı seçin. **Yeni sunucu formu**’nu aşağıdaki bilgilerle doldurun: 
...
8. **Uygula**’yı seçin.
9. Sağlama dikey penceresinde boş veritabanı için bir **harmanlama** seçin. Bu öğreticide varsayılan değeri kullanın. Harmanlamalar hakkında daha fazla bilgi için bkz. [Harmanlamalar](/sql/t-sql/statements/collations)

11. Formu tamamladığınıza göre, veritabanını sağlamak için **Oluştur** ' u seçin. Sağlama birkaç dakika sürer. 

12. Araç çubuğunda, dağıtım sürecini izlemek için **Bildirimler** ' i seçin.
...
Sunucu düzeyinde, dış uygulamaların ve araçların sunucuya ya da sunucu üzerindeki herhangi bir veritabanına bağlanmasını engelleyen bir güvenlik duvarı. Bağlantıyı etkinleştirmek için, belirli IP adresleri için bağlantıyı etkinleştiren güvenlik duvarı kuralları ekleyebilirsiniz.  İstemcinizin IP adresine yönelik bir [sunucu düzeyi güvenlik duvarı kuralı](../sql-database/sql-database-firewall-configure.md) oluşturmak için bu adımları izleyin. 
...
4. Geçerli IP adresinizi yeni bir güvenlik duvarı kuralına eklemek için araç çubuğunda **istemci IP 'Si Ekle** ' yi seçin. Güvenlik duvarı kuralı, 1433 numaralı bağlantı noktasını tek bir IP adresi veya bir IP adresi aralığı için açabilir.
...
2. Verilerinizi yüklenirken görüntüleyin. Birkaç GB veri yüklüyorsunuz ve yüksek performanslı kümelenmiş columnstore dizinlerine sıkıştırıyorsunuz. Yüklemenin durumunu göstermek için, dinamik yönetim görünümleri (DMV’ler) kullanan aşağıdaki sorguyu çalıştırın. 
...
1. Depolama hesabınız altında **Access Control (IAM)** bölümüne gidin ve **rol ataması Ekle**' yi seçin. SQL veritabanı sunucunuza **Depolama Blobu veri katılımcısı** RBAC rolü atayın.
...
1. **Azure depolama hesabına PolyBase bağlantısı:**
   ...
   1. Yönetilen Hizmet Kimliği veritabanı kapsamlı kimlik bilgilerini belirten dış veri kaynağını oluşturun.
...
Azure SYNAPSE Analytics için sanal ağ hizmet uç noktalarını ayarlamak isterseniz aşağıdaki [belgelere](https://docs.microsoft.com/azure/sql-database/sql-database-vnet-service-endpoint-rule-overview) bakın. 
...
> [Mevcut bir veritabanını Azure SYNAPSE Analytics 'e geçirmeye yönelik tasarım kararları](sql-data-warehouse-overview-migrate.md)
