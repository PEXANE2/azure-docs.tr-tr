...
1. Blob kapsayıcısındaki Blobları listelemek için `CloudBlobContainer.ListBlobs` yöntemini kullanın. `CloudBlobContainer.ListBlobs` `IListBlobItem` Yöntemi `CloudBlockBlob`, `CloudPageBlob`, veya `CloudBlobDirectory` nesnesine tür bir nesne döndürür. Aşağıdaki kod parçacığı bir blob kapsayıcısındaki tüm Blobları numaralandırır. Her blob, türüne göre uygun nesneye yayın yapılır. Adı (veya **Cloudblobdirectory**söz konusu olduğunda URI) bir listeye eklenir.
