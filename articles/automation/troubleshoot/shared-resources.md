...
Bu makalede, Azure Otomasyonu 'nda [paylaşılan kaynakları](../automation-intro.md#shared-resources) kullanırken çalıştırabileceğiniz sorunlara yönelik çözümler ele alınmaktadır.
...
Azure Otomasyonu modüllerinizi içeri aktarırken veya güncelleştirdiğinizde bir modül Içeri aktarma durumunda takılmış olur.

#### <a name="cause"></a>Nedeni

PowerShell modüllerini içeri aktarma karmaşık bir çok adımlı işlem olduğundan, bir modül doğru şekilde içeri aktarılmayabilir ve geçici bir durumda kalabilir. İçeri aktarma işlemi hakkında daha fazla bilgi edinmek için bkz. [PowerShell modülünü Içeri aktarma](/powershell/scripting/developer/module/importing-a-powershell-module#the-importing-process).

#### <a name="resolution"></a>Çözüm

Bu sorunu çözmek için, [Remove-AzAutomationModule](https://docs.microsoft.com/powershell/module/Az.Automation/Remove-AzAutomationModule?view=azps-3.7.0) cmdlet 'Ini kullanarak içeri aktarma durumunda kalmış olan modülü kaldırmanız gerekir. Daha sonra modülün içeri aktarılmasını yeniden deneyebilirsiniz.
...
Azurermmodules, 0 ile başlayan sayısal bir ada sahip bir kaynak grubunda bulunan bir Otomasyon hesabındaki Azurerd modüllerini güncelleştirmede bilinen bir sorun vardır.
...
Modül içeri aktarılamıyor veya içeri aktarmaz, ancak hiçbir cmdlet ayıklanamaz.
...
* [New-AzAutomationModule](https://docs.microsoft.com/powershell/module/Az.Automation/New-AzAutomationModule?view=azps-3.7.0) cmdlet 'i modülünü karşıya yüklemek için kullanılıyor ve tam depolama yolu belirtmediniz veya genel olarak ERIŞILEBILEN bir URL kullanarak modülü yüklemediniz.

#### <a name="resolution"></a>Çözüm

Sorunu onarmak için bu çözümlerden herhangi birini kullanın.
...
**Update-AzureModule. ps1**kullanılırken aynı anda kaç modülün güncelleştirildiğini belirleme için varsayılan ayar 10 ' dur. Aynı anda çok fazla modül güncelleştirilirken güncelleştirme işlemi hatalara açıktır.

#### <a name="resolution"></a>Çözüm

Tüm Azurerd veya az modüllerin aynı Otomasyon hesabında gerekli olduğu yaygın değildir. Yalnızca ihtiyacınız olan belirli modülleri içeri aktarmanız önerilir.
...
Sorun bir kilit nedeniyle gerçekleşirse, kilidin kaldırılabildiğini doğrulayın. Ardından Azure portal kilitli olan kaynağa gidin, kilidi sağ tıklatın ve **Sil**' e tıklayın.
