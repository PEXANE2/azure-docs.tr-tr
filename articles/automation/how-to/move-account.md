...
Azure Otomasyonu hesabı, taşıyacağınız kaynaklardan biridir. Bu makalede, Otomasyon hesaplarını başka bir kaynağa veya aboneliğe taşımayı öğreneceksiniz. Otomasyon hesabınızı taşımaya yönelik üst düzey adımlar şunlardır:
...
2. Her bir çözümü bulun ve kaynakları Sil sayfasında **Sil** ' e tıklayın.
...
### <a name="remove-alert-rules-for-the-startstop-vms-during-off-hours-solution"></a>Çalışma saatleri dışında VM 'Leri Başlat/Durdur çözümü için uyarı kurallarını kaldır

Çalışma saatleri dışında VM 'Leri Başlat/Durdur çözümü için, çözüm tarafından oluşturulan uyarı kurallarını da kaldırmanız gerekir.
...
3. Kuralları tek seferde seçin ve **Sil** ' e tıklayarak bunları kaldırın.

    ![Kurallar sayfası seçili kuralların silinme onayını istiyor](../media/move-account/delete-rules.png)

    > [!NOTE]
    > Kurallar sayfasında herhangi bir uyarı kuralı görmüyorsanız, devre dışı bırakılmış olabilecek uyarıları göstermek için **durum** alanını devre dışı olarak değiştirin.

4. Uyarı kuralları kaldırıldığında, çalışma saatleri dışında VM 'Leri Başlat/Durdur çözüm bildirimleri sırasında oluşturulan eylem grubunu kaldırmanız gerekir. Azure Portal, **izleme** > **uyarıları** > **işlem gruplarını yönet**' i seçin.
...
## <a name="recreate-run-as-accounts"></a>Farklı Çalıştır hesaplarını yeniden oluştur
...
2. Özellikler sayfasındaki **Sil** düğmesini kullanarak farklı çalıştır hesaplarını tek seferde silin. 
...
1. Değişiklik İzleme ve envanter çözümünü açmak için Otomasyon hesabınızda Değişiklik İzleme ve envanter ' ı seçin. Üzerinde taşınan Log Analytics çalışma alanını seçin ve **Etkinleştir**' i seçin.
...
3. Çözümlerinizle eklendi olan makineler, mevcut Log Analytics çalışma alanını bağladığınızda görülebilir. Çalışma saatleri dışında VM 'Leri Başlat/Durdur çözümünü açmak için çözümü yeniden dağıtmanız gerekir. **İlgili kaynaklar**' ın altında, **VM 'leri** > Başlat/Durdur ' u seçin ve dağıtımı başlatmak için**çözüm** > **oluşturma** ' yı etkinleştirin.
...
|Değişiklik izleme ve stok|Makinelerinizden güncel envanter verilerini gördiğinizi doğrulayın.|[Değişiklik izleme sorunlarını giderme](../troubleshoot/change-tracking.md)|
...
|Paylaşılan kaynaklar|[Kimlik bilgileri](../shared-resources/credentials.md), [değişkenler](../shared-resources/variables.md)ve benzeri tüm paylaşılan kaynaklarınızı gördiğinizi doğrulayın.|
