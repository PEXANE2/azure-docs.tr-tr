---
title: Azure Otomasyonu'nda grafik çalışma kitabı oluşturma
description: Azure Otomasyonu'nda basit bir grafik çalışma kitabının nasıl oluşturulup test edilebildiğini ve yayımlandırılabildiğini gösteren öğretici.
...
---
# <a name="tutorial-create-a-graphical-runbook"></a>Öğretici: Grafik çalışma kitabı oluşturma

Bu öğretici, Azure Automation’da bir [grafik runbook uygulaması](../automation-runbook-types.md#graphical-runbooks) oluşturulmasını adım adım göstermektedir. Azure portalındaki grafik düzenleyiciyi kullanarak grafiksel ve grafiksel PowerShell İş Akışı çalışma kitapları oluşturabilir ve edinebilirsiniz. 

Bu öğreticide şunların nasıl yapıldığını öğreneceksiniz:

> [!div class="checklist"]
> * Basit bir grafik çalışma kitabı oluşturma
> * Çalışma kitabını test edin ve yayımlayın
> * Runbook işinin durumunu çalıştırma ve izleme
> * Runbook'u, runbook parametreleri ve koşullu bağlantılarla bir Azure sanal makinesi başlatmak için güncelleştirin
...
5. Bu durumda, bir [grafik runbook](../automation-graphical-authoring-intro.md)oluşturmak için gidiyoruz. **Runbook türü**için **Grafik'i** seçin.<br> ![Yeni runbook](../media/automation-tutorial-runbook-graphical/create-new-runbook.png)<br>
...
1. Kitaplık denetiminde, arama alanında tıklatın `write-output`ve yazın. Arama sonuçları aşağıdaki resimde gösterilir. <br> ![Microsoft.PowerShell.Utility](../media/automation-tutorial-runbook-graphical/search-powershell-cmdlet-writeoutput.png)

2. Listenin aşağısına kaydırın. **Yazma-Çıktı'yı** sağ tıklatın ve **tuvale ekle'yi**seçin. Alternatif olarak, cmdlet adının yanındaki elipsleri (...) tıklatıp **tuvale ekle'yi**seçebilirsiniz.

3. Tuvalde **Write-Output** etkinliğine tıklayın. Bu eylem, etkinliği yapılandırmanızı sağlayan Yapılandırma denetim sayfasını açar.

4. **Etiket** alanı varsayılan olarak cmdlet adına değişir, ancak daha kolay bir şeyle değiştirebilirsiniz. `Write Hello World to output`Değiştirin.
...
6. Parametreyi `InputObject` seçin. Bu, çıktı akışına gönderilecek metni belirtmek için kullandığınız parametredir.

7. **Veri kaynağı** açılır menüsü, parametre değerini doldurmak için kullanabileceğiniz kaynaklar sağlar. Bu menüde **PowerShell ifadesini**seçin. 
...
8. **İfade** alanında, tuvale dönmek için Iki kez Tamam yazın `Hello World` ve sonra **Tamam'ı** tıklatın.
...
3. **MyFirstRunbook-Graphical**için sayfayı görüntülemek için sağa geri kaydırın.
...
5. Oluşturulan runbook işi için bir İş bölmesi açılır. **İş durumu** alanının **Tamamlandı'yı**gösterdiğini doğrulayın.

6. Görüntülenen görmek `Hello World` için Çıktı sayfasını açmak için **Çıktı'yı** tıklatın.
...
9. MyFirstRunbook-Graphical sayfasına dönmek için Akışlar bölmesini ve İş bölmesini kapatın.

10. Runbook'un tüm işlerini görüntülemek **için, Kaynaklar**altında **İşler'i** seçin. İşler sayfası, runbook'unuzun oluşturduğu tüm işleri listeler. İşi yalnızca bir kez çalıştırdığınızdan, yalnızca bir iş listelenmiş olarak görmeniz gerekir.
...
4. Yeni değişken sayfasında, sağlanan alanlarda aşağıdaki ayarları yapın.
...
2. Artık girişe `Write Hello World to output` ihtiyacın yok. Elipsleri tıklatın ve **Sil'i**seçin.

3. Kitaplık denetiminde, **VARLıKLAR'ı,** ardından **Bağlantıları**genişletin. Tuvale Ekle'yi seçerek `AzureRunAsConnection` **tuvale**ekleyin.

4. Yeniden `AzureRunAsConnection` adlandırmak `Get Run As Connection`için.

5. Kitaplık denetiminde, `Connect-AzAccount` arama alanına yazın.

6. Tuvale ekle. `Connect-AzAccount`

7. Şeklin `Get Run As Connection` alt kısmında bir daire belirene kadar yukarı doğru yuvarlanın. Daireyi tıklatın ve bir `Connect-AzAccount` bağlantı oluşturmak için oku sürükleyin. Runbook ile `Get Run As Connection` başlar ve `Connect-AzAccount`sonra çalışır.<br> ![Etkinlikler arasında bağlantı oluşturma](../media/automation-tutorial-runbook-graphical/runbook-link-auth-activities.png)

8. Tuval üzerinde, `Connect-AzAccount`seçin. Yapılandırma denetim bölmesinde, **Etiket** alanına **Azure'a Giriş** yazın.

9. **Parametreler'i**tıklatın ve Etkinlik Parametre Yapılandırması sayfası görüntülenir.

10. Cmdlet birden `Connect-AzAccount` çok parametre kümesine sahiptir ve parametre değerleri sağlamadan önce bir tane seçmeniz gerekir. **Parametre Kümesi'ni** tıklatın ve ardından **ServicePrincipalCertificateWithSubscriptionId'i**seçin.

11. Bu parametre kümesinin parametreleri Etkinlik Parametresi Yapılandırma sayfasında görüntülenir. **APPLICATIONID**’ye tıklayın.<br> ![Azure hesap parametreleri ekleme](../media/automation-tutorial-runbook-graphical/Add-AzureRmAccount-params.png)

12. Parametre Değeri sayfasında aşağıdaki ayarları yapın ve ardından **Tamam'ı**tıklatın.
...
13. **CERTIFICATETHUMBPRINT'i**ve Parametre Değeri sayfasında aşağıdaki ayarları yapın ve ardından **Tamam'ı**tıklatın.

    * **Veri kaynağı** -- **Etkinlik çıktısını**seçin.
    * Veri kaynak listesi -- **Otomasyon Bağlantısı Al'ı**seçin.
    * **Alan yolu** `CertificateThumbprint`-- yazın.

14. **SERVICEPRINCIPAL'ı**tıklatın ve Parametre Değeri sayfasında **Veri kaynak** alanı için **ConstantValue'ı** seçin; **True**seçeneğini tıklatın ; ve sonra **Tamam'ı**tıklatın.

15. **TENANTID'yi**tıklatın ve Parametre Değeri sayfasında aşağıdaki ayarları yapın. Bittiğinde, Iki kez **Tamam'ı** tıklatın.
...
17. Tuvale ekle. `Set-AzContext`

18. Tuvalüzerinde seçin. `Set-AzContext` Yapılandırma denetim bölmesine, `Specify Subscription Id` **Etiket** alanına girin.

19. **Parametreler'i** tıklatın ve Etkinlik Parametre Yapılandırmasayfası görüntülenir.

20. Cmdlet birden `Set-AzContext` çok parametre kümesine sahiptir ve parametre değerleri sağlamadan önce bir tane seçmeniz gerekir. **Parametre Kümesi'ni** tıklatın ve ardından **SubscriptionId'i**seçin.

21. Bu parametre kümesinin parametreleri Etkinlik Parametresi Yapılandırma sayfasında görüntülenir. **SubscriptionID'yi**tıklatın.

22. Parametre Değeri sayfasında, **Veri kaynak** alanı için **Değişken Varlık'ı** seçin ve kaynak listeden **AzureSubscriptionId'i** seçin. Bittiğinde, Iki kez **Tamam'ı** tıklatın.

23. Şeklin `Login to Azure` alt kısmında bir daire belirene kadar yukarı doğru yuvarlanın. Daireyi tıklatın ve oku `Specify Subscription Id`' ya sürükleyin. Runbook bu noktada aşağıdaki gibi görünmelidir.
...
3. Şeklin `Specify Subscription Id` alt kısmında bir daire belirene kadar yukarı doğru yuvarlanın. Daireyi tıklatın ve oku `Start-AzVM`' ya sürükleyin.

4. `Start-AzVM` öğesini seçin. Etkinlik kümelerini görüntülemek için **Parametreler'i** ve ardından **Parametre Kümesi'ni** tıklatın.

5. Parametre kümesi için **ResourceGroupNameParameterSetName'i** seçin. Kaynak **Grubu Adı** ve **Adı** alanları, gerekli parametreler olduğunu belirtmek için yanlarında ünlem işaretleri içerir. Her iki alanın da dize değerleri beklediğini unutmayın.

6. **Ad**’ı seçin. **Veri kaynak** alanı için **PowerShell ifadesini** seçin. Bu runbook'u başlatmak için kullandığınız VM için, çift tırnak la çevrili makine adını yazın. **Tamam**'a tıklayın.

7. **ResourceGroupName**’i seçin. **Veri kaynak** alanı için **PowerShell** değerini kullanın ve çift tırnak la çevrili kaynak grubunun adını yazın. **Tamam**'a tıklayın.

8. Çalışma kitabını test etmek için **Test bölmesini** tıklatın.

9. Teste başlamak için **Başlat'ı** tıklatın. Tamamlandıktan sonra, VM'nin başladığından emin olun. Runbook bu noktada aşağıdaki gibi görünmelidir.
...
2. **Giriş ve çıktıyı** seçin ve runbook Giriş Parametre bölmesini açmak için **giriş ekleyin.**

3. Sağlanan alanlarda aşağıdaki ayarları yapın ve sonra **Tamam'ı**tıklatın.
   * **Ad** -- `VMName`belirtin.
   * **Yazın** -- dize ayarını koruyun.
   * **Zorunlu** -- değeri **Evet**olarak değiştirin.

4. İkinci bir zorunlu giriş parametresi adlı `ResourceGroupName` oluşturun ve sonra Giriş ve Çıktı bölmesini kapatmak için **Tamam'ı** tıklatın.<br> ![Runbook Giriş Parametreleri](../media/automation-tutorial-runbook-graphical/start-azurermvm-params-outputs.png)

5. `Start-AzVM` Etkinliği seçin ve ardından **Parametreler'i**tıklatın.

6. **Runbook** girişi için Ad için **Veri kaynak** alanını **değiştirin.** Sonra **VMName**seçin.

7. **ResourceGroupName** için **Veri kaynak** alanını **Runbook girişine** değiştirin ve ardından **ResourceGroupName'i**seçin.<br> ![Başlangıç-AzVM Parametreleri](../media/automation-tutorial-runbook-graphical/start-azurermvm-params-runbookinput.png)
...
12. Runbook'u başlatmak için **Başlat**’a tıklayın. Başlatacağınız VM için `VMName` ve `ResourceGroupName` için değerleri yazın.
...
2. Arasındaki `Specify Subscription Id` bağlantıyı kaldırın `Start-AzVM` ve üzerine tıklayarak ve sonra **Sil**tuşuna basarak.

3. Kitaplık denetiminde, `Get-Az` arama alanına yazın.

4. Tuvale ekle. `Get-AzVM`

5. Cmdlet kümelerini görüntülemek için Parametre Kümesi'ni seçin `Get-AzVM` ve ardından **Parametre Kümesi'ni** tıklatın. 

6. **GetVirtualMachineInResourceGroupNameParamSet** parametre kümesini seçin. **Kaynak Grubu Adı** ve **Ad** alanlarının yanında, gerekli parametreleri belirttiklerini belirten ünlem işaretleri vardır. Her iki alanın da dize değerleri beklediğini unutmayın.

7. **Ad**için **Veri kaynağı** altında, **Runbook girişi**seçin, sonra **VMName**. **Tamam**'a tıklayın.

8. **ResourceGroupName**için **Veri kaynağı** altında **Runbook girişi**seçin, sonra **ResourceGroupName**. **Tamam**'a tıklayın.

9. **Durum**için **Veri kaynağı** altında, **Sabit değeri**seçin, sonra **True**. **Tamam**'a tıklayın.

10. 'den `Specify Subscription Id` ' `Get-AzVM`e bir bağlantı oluşturun

11. Kitaplık denetiminde **Runbook Denetimi'ni** genişletin ve tuvale **Kod** ekleyin.  

12. 'den `Get-AzVM` ' `Code`e bir bağlantı oluşturun  

13. Yapılandırma `Code` bölmesinde etiketi tıklatın ve Durumu **Al'ı**değiştirin.

14. Seçin `Code` ve Kod Düzenleyicisi sayfası görüntülenir.  

15. Aşağıdaki kod parçacıklarını editör sayfasına yapıştırın.
...
16. 'den `Get Status` ' `Start-AzVM`e bir bağlantı oluşturun

    ![Kod Modülü ile Runbook](../media/automation-tutorial-runbook-graphical/runbook-startvm-get-status.png)  

17. Bağlantıyı seçin ve Yapılandırma bölmesinde Uygula **koşulunu** **Evet**olarak değiştirin. Bağlantının kesik çizgili bir çizgi ye dönüştüğünü ve hedef etkinliğin yalnızca koşul doğru giderici olduğunda çalıştığını belirteceğini unutmayın.  

18. **Koşul ifadesi**için `$ActivityOutput['Get Status'] -eq "Stopped"`, yazın. `Start-AzVM`şimdi yalnızca VM durdurulursa çalışır.

19. Kitaplık denetiminde, **Cmdlet'leri** ve ardından **Microsoft.PowerShell.Utility**’yi genişletin.

20. Tuvale iki kez ekleyin. `Write-Output`

21. İlk `Write-Output` denetim için **Parametreler'i** tıklatın ve **Başlatılan VM'yi Bildirmek**için **Etiket** değerini değiştirin.

22. **InputObject**için, **PowerShell ifadesi** **için Veri kaynağı** `$VMName successfully started.`değiştirin ve ifade yazın.

23. İkinci `Write-Output` denetimde Parametreler'i **tıklatın** ve **VM Start Failed'ü bildirmek**için **Etiket** değerini değiştirin.

24. **InputObject**için, **PowerShell ifadesi** **için Veri kaynağı** `$VMName could not start`değiştirin ve ifade yazın.

25. ve. `Start-AzVM` için `Notify VM Started` bağlantılar oluşturun `Notify VM Start Failed`

26. Bağlantıseçin `Notify VM Started` ve **uygula koşulunu** doğru olarak değiştirin.

27. Durum **ifadesi**için `$ActivityOutput['Start-AzVM'].IsSuccessStatusCode -eq $true`, yazın. Bu `Write-Output` denetim artık yalnızca VM başarıyla başlarsa çalışır.

28. Bağlantıseçin `Notify VM Start Failed` ve **uygula koşulunu** doğru olarak değiştirin.

29. Durum **ifadesi** alanı için `$ActivityOutput['Start-AzVM'].IsSuccessStatusCode -ne $true`. Bu `Write-Output` denetim şimdi yalnızca VM başarıyla başlatılmazsa çalışır. Runbook'unuz aşağıdaki resim gibi görünmelidir.
...
31. VM durdurularak runbook'u çalıştırın ve makinenin başlaması gerekir.
