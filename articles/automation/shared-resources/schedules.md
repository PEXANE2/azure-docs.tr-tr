...
4. Zamanlamanın bir kez mı yoksa bir **kez** veya **yinelenen**seçerek yeniden gerçekleşen bir zamanlamaya göre mi çalışacağını seçin. Bir **kez**seçer, bir başlangıç saati belirtin ve ardından **Oluştur**' a tıklayın. **Yinelenen**seçeneğini belirlerseniz, bir başlangıç saati belirtin. **Her**zaman yineleme için, runbook 'un saat, gün, hafta veya aya göre yinelenmesini istediğiniz sıklığı seçin.
...
Aşağıdaki örnek, bir yılda 1:00PM olmak üzere her gün çalışan bir yinelenen zamanlamanın nasıl oluşturulacağını gösterir.
...
Aşağıdaki örnek, ayın 1., 15. ve son gününde çalışan bir yinelenen zamanlamanın nasıl oluşturulacağını gösterir.
...
4. Runbook 'un parametreleri varsa, **çalışma ayarlarını değiştir (varsayılan: Azure)** seçeneğini belirleyebilirsiniz ve parametreler bölmesi sunulur. Burada parametre bilgilerini girebilirsiniz.
...
## <a name="scheduling-runbooks-to-run-more-frequently"></a>Runbook 'ları daha sık çalışacak şekilde zamanlama
...
Aşağıdaki örnek, Azure Resource Manager cmdlet 'i kullanarak bir runbook için zamanlamanın nasıl devre dışı bırakılacağını gösterir.
...
## <a name="removing-a-schedule"></a>Zamanlamayı kaldırma

Zamanlamalarınızı kaldırmaya hazırsanız, Azure portal ya da `Remove-AzureRmAutomationSchedule` cmdlet 'ini kullanabilirsiniz. Önceki bölümde açıklandığı gibi, yalnızca devre dışı bırakılmış bir zamanlamayı kaldıracağınızdan emin olarak unutmayın.
...
[Remove-AzAutomationSchedule](https://docs.microsoft.com/powershell/module/Az.Automation/Remove-AzAutomationSchedule?view=azps-3.7.0) cmdlet 'ini kullanarak var olan bir zamanlamayı silebilirsiniz. 
