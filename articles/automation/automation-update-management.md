...
Aşağıdaki tabloda, güncelleştirme değerlendirmeleri için desteklenen işletim sistemlerinin bir listesi gösterilmektedir. Düzeltme eki uygulama karma Runbook Worker gerektirir. Karma Runbook Worker gereksinimleri hakkında bilgi için bkz. [WINDOWS HRW](automation-windows-hrw-install.md#installing-the-windows-hybrid-runbook-worker) ve [Linux HRW](automation-linux-hrw-install.md#installing-a-linux-hybrid-runbook-worker)için Yükleme Kılavuzu.
...
Aşağıdaki bilgiler işletim sistemine özgü istemci gereksinimlerini açıklamaktadır.  Ayrıca, daha fazla rehberlik için [ağ planlamasını](#ports) gözden geçirmeniz gerekir.

#### <a name="windows"></a>Windows

Windows aracılarının bir WSUS sunucusuyla iletişim kuracak şekilde yapılandırılması veya Microsoft Update erişimi olması gerekir.
...
| Operations Manager yönetim grubu |Yes |Çözüm, bağlı bir yönetim grubundaki aracılardan sistem güncelleştirmeleri hakkında bilgi toplar.<br/>Operations Manager aracısından Azure Izleyici günlüklerine doğrudan bağlantı gerekli değildir. Veriler, yönetim grubundan Log Analytics çalışma alanına iletilir. |
...
## <a name="view-update-assessments"></a>Güncelleştirme değerlendirmelerini görüntüle
