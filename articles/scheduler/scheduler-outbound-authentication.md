...
* Bir Zamanlayıcı işine kimlik doğrulaması eklemek için, işi oluştururken veya güncelleştirdiğinizde `authentication` JavaScript Nesne Gösterimi (JSON) alt öğesini `request` öğesine ekleyin. 

  Yanıtlar, `authentication` nesnesinde bir PUT, PATCH veya POST isteği aracılığıyla Zamanlayıcı hizmetine geçirilen gizli dizileri hiçbir şekilde döndürmez. 
...
* Bir Zamanlayıcı işinden kimlik doğrulamasını kaldırmak için, iş üzerinde açıkça bir PUT veya PATCH isteği çalıştırın ve `authentication` nesnesini null olarak ayarlayın. Yanıt herhangi bir kimlik doğrulama özelliği içermemelidir.
...
| **type** | Yes | Kimlik doğrulama türü. SSL istemci sertifikaları için değer `ClientCertificate`. |
| **Türk** | Yes | PFX dosyasının Base64 ile kodlanmış içeriği |
| **parolayı** | Yes | PFX dosyasına erişim parolası |
||| 
...
| **type** | Yes | Kimlik doğrulama türü. Temel kimlik doğrulaması için değer `Basic`. | 
| **nitelen** | Yes | Kimlik doğrulama için Kullanıcı adı | 
| **parolayı** | Yes | Kimlik doğrulaması için parola |
|||| 
...
| **Kiracı** | Yes | Azure AD kiracısı için kiracı tanımlayıcısı. Azure AD kiracısının kiracı tanımlayıcısını bulmak için Azure PowerShell `Get-AzureAccount` çalıştırın. |
| **grubu** | Yes | Bu değer `https://management.core.windows.net/`olarak ayarlanır. | 
...
| **grubu** | Bu değer `https://management.core.windows.net/`olarak ayarlanır. |
