...
2. **Temel bilgiler** sekmesinde, aşağıdaki ayrıntıları girin:
    ...
    |Görüntü türü|Kapsayıcı resminiz kimlik bilgileri gerektirmeyen bir kapsayıcı kayıt defterinde depolanıyorsa, öğesini seçin `Public` . Kapsayıcı yansımanıza erişim için kimlik bilgileri gerekiyorsa, öğesini seçin `Private` . Kapsayıcı görüntüsünün [container repositories and images](../../cognitive-services-container-support.md#container-repositories-and-images) mi yoksa `Public` `Private` ("genel önizleme") olup olmadığı hakkında ayrıntılar için kapsayıcı depoları ve resimlere bakın. |
