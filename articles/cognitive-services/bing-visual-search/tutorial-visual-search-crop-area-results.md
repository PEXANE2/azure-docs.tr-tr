...
ms.locfileid: "74689342"
---
# <a name="tutorial-crop-an-image-with-the-bing-visual-search-sdk-for-c"></a>Öğretici: bir görüntüyü Bing Görsel Arama SDK 'Sı ile kırpınC#
...
Bu görüntü, kırpma alanından bir `ImageInfo` nesnesi oluşturarak ve `ImageInfo` nesnesini bir `VisualSearchRequest`yüklemeye göre kırpılır. `ImageInfo` nesnesi görüntünün URL 'sini de içerir:
...
`VisualSearchRequest` değişkeni, görüntünün kırpma alanı ve URL 'SI hakkındaki bilgileri içerir. `VisualSearchMethodAsync()` yöntemi sonuçları alır:
...
Bing Görsel Arama sonuçlar `ImageTag` nesnelerdir. Her etiket bir `ImageAction` nesneleri listesi içerir. Her `ImageAction`, eylem türüne bağlı değerlerin listesi olan bir `Data` alanı içerir.
...
## <a name="get-urls-for-pagesincluding-actiontype-images"></a>`PagesIncluding` `ActionType` görüntüleri için URL 'Ler al

Gerçek görüntü URL’lerini almak için, bir `ActionType` türünü `ImageModuleAction` olarak okuyan bir tür dönüştürme gerekir. Bu tür ise değerler listesine sahip bir `Data` öğesi içerir. Her değer, bir görüntünün URL’sidir. Aşağıdaki `PagesIncluding` eylem türünü `ImageModuleAction` ve değerleri okur:
