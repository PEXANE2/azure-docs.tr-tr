...
Aşağıdakiler, isteğinizde belirtilmesi gereken üst bilgilerdir. `Content-Type` `Ocp-Apim-Subscription-Key` Ve üst bilgileri yalnızca gerekli olan başlıklardır, `User-Agent`ancak,, ve `X-MSEdge-ClientID` `X-MSEdge-ClientIP` `X-Search-Location`de dahil etmelisiniz.
...
Web ve atısyon bilgilerine `enableEntityData` bağlantılar da dahil olmak üzere `true` , karşıya yüklediğiniz görüntüde ana varlıkla ilgili ayrıntılı bilgi için başlıktaki özniteliğini isteğe bağlı olarak ayarlayabilirsiniz. Bu alan varsayılan `false` olarak olur.
...
Görüntü, tam olarak tanınmış/popüler kişi, yer veya bir şey gibi tanınan bir varlık içeriyorsa, etiketlerden biri bir varlık öngörüsi içerebilir. `mainEntity` Ve `data` alanları yalnızca `enableEntityData` `Content-Type` başlıktaki özniteliği olarak `true`ayarlanmışsa kullanılabilir.
