...
2. Aşağıda belirtilen kodu yeni `.js` bir dosyaya ekleyin.
3. `accessKey` Değeri Azure 'daki metin analizi kaynağınızın bir abonelik anahtarıyla değiştirin.
4. `uri` içindeki konumu (şu anda `westus`) kaydolduğunuz bölge olacak şekilde değiştirin.
5. Programı IDE veya komut satırınızdan çalıştırın, örneğin `npm start` veya. `node detect.js`
...
3. `accessKey` Değeri Azure 'daki metin analizi kaynağınızın bir abonelik anahtarıyla değiştirin.
4. `uri` içindeki konumu (şu anda `westus`) kaydolduğunuz bölge olacak şekilde değiştirin.
5. Programı IDE veya komut satırınızdan çalıştırın, örneğin `npm start` veya. `node sentiment.js`
...
3. `accessKey` Değeri Azure 'daki metin analizi kaynağınızın bir abonelik anahtarıyla değiştirin.
4. `uri` içindeki konumu (şu anda `westus`) kaydolduğunuz bölge olacak şekilde değiştirin.
5. Programı IDE veya komut satırınızdan çalıştırın, örneğin `npm start` veya. `node key-phrases.js`
...
3. `accessKey` Değeri Azure 'daki metin analizi kaynağınızın bir abonelik anahtarıyla değiştirin.
4. `uri` içindeki konumu (şu anda `westus`) kaydolduğunuz bölge olacak şekilde değiştirin.
5. Programı IDE veya komut satırınızdan çalıştırın, örneğin `npm start` veya. `node entities.js`
