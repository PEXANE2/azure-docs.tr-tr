...
Sınıfı içinde adlı `TranslateTextRequest()`bir zaman uyumsuz işlev oluşturun. `Program` Bu işlev dört bağımsız değişken alır `subscriptionKey`: `host`, `route`, ve `inputText`.
...
İçinde `Main`,, ve `host` `subscriptionKey` bildirirsiniz`route`. Buna ek olarak, kullanıcıdan ile `Console.Readline()` giriş ve değer atamasını de istiyorsunuz. `textToTranslate`
