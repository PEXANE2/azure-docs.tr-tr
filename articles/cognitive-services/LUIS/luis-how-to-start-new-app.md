...
* Bir veya `.lu` `.json` daha fazla amaç, utde ve varlık IÇEREN bir dosyadan bir Luo uygulamasını içeri aktarın.
...
1. Uygulamanızın kültür ' i seçin ve **bitti**' yi seçin. Açıklama ve tahmin kaynağı bu noktada isteğe bağlıdır. Daha sonra portalın **Yönet** bölümünde istediğiniz zaman ayarlayabilirsiniz.
    ...
    Uygulama oluşturulduktan sonra, Lua portalı, sizin için önceden `None` oluşturulmuş **Amaç listesini gösterir** . Artık boş bir uygulamanız var. 
...
* Veya kullanarak `.lu` dosyadan içeri aktar`.json`
* Uygulamayı ( [LUI](https://github.com/microsoft/botbuilder-tools/tree/master/packages/Ludown)için), `.json`, `.zip` veya ( [lusıs kapsayıcısı](luis-container-howto.md)için) olarak `.lu` dışa aktar
* Uç nokta utslerini gözden geçirmek için kapsayıcı uç nokta günlüklerini içeri aktarma
* Çevrimdışı analiz için uç nokta günlüklerini `.csv`, bir olarak dışarı aktar
* Uygulamayı Sil
