...
> [!TIP]
> Bir web tarayıcısını hedefliyorsanız ve etiketi `<script>` kullanıyorsanız; `sdk` önek gerekli değildir. Önek, `sdk` bizim veya `import` `require` modülümüzü adlandırmak için kullandığımız bir takma addır.
