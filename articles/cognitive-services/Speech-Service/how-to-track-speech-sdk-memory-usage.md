...
Bir uyarı eşiğini etkinleştirmek için, `SpeechConfig` nesnesi üzerinde belirtilmelidir. Bu nesne, yeni bir tanıyıcı oluşturulduğunda denetlenir. Aşağıdaki örneklerde `config`adlı bir `SpeechConfig` örneği oluşturduğunuzu varsayalım:
...
Bir hata eşiğini etkinleştirmek için, `SpeechConfig` nesnesi üzerinde belirtilmesi gerekir. Bu nesne, yeni bir tanıyıcı oluşturulduğunda denetlenir. Aşağıdaki örneklerde `config`adlı bir `SpeechConfig` örneği oluşturduğunuzu varsayalım:
...
> [!TIP]
> Bu özellik için varsayılan değer, `size_t` veri türü için platforma özgü en büyük değerdir. Tipik bir tanıma, 7 ila 10 iç nesne arasında tüketir.
