...
1. Arama hizmetiniz için ölçüm ve kaynak günlüğünü etkinleştirme:
    ...
    1. Sol sütundaki Izleme bölümü altında **Tanılama ayarları** ' nı seçin.
        ...
        ![Azure Bilişsel Arama hizmetinin Izleme bölümünde tanılama ayarlarının nasıl seçileceğini gösteren ekran görüntüsü.](media/search-monitor-logs-powerbi/diagnostic-settings.png)
    ...
    1. **Arşivi bir depolama hesabına**çekin, depolama hesabı bilgilerinizi sağlayın ve **Operationlogs** ve **allölçümlerini** denetleyin
        ...
        ![Tanılama Ayarları sayfasında ölçümler ve kaynak günlüğü için nasıl seçim yapılacağını gösteren ekran görüntüsü.](media/search-monitor-logs-powerbi/add-diagnostic-setting.png)
    1. **Kaydet**’i seçin
...
1. Uygulamayı yükledikten sonra Power BI uygulama listenizden uygulamayı seçin.
    ...
    ![Uygulama listesinden seçilecek Azure Bilişsel Arama uygulamasını gösteren ekran görüntüsü.](media/search-monitor-logs-powerbi/azure-search-app-tile.png)

1. Verilerinizi bağlamak için **Bağlan** ' ı seçin

    ![Azure Bilişsel Arama uygulamasında verilerinize nasıl bağlanagösterileceğini gösteren ekran görüntüsü.](media/search-monitor-logs-powerbi/get-started-with-your-new-app.png)
...
1. Günlüklerinizi ve ölçümlerinizi içeren depolama hesabının adını girin. Varsayılan olarak, uygulama son 10 güne kadar veri arayacaktır, ancak bu değer **günler** parametresiyle değiştirilebilir.
    ...
    ![Depolama hesabı adının nasıl ekleneceğini ve Azure Bilişsel Arama Bağlan sayfasında Sorgulanacak gün sayısını gösteren ekran görüntüsü.](media/search-monitor-logs-powerbi/connect-to-storage-account.png)
...
1. Kimlik doğrulama yöntemi olarak **anahtar** ' ı seçin ve depolama hesabı anahtarınızı sağlayın. Gizlilik düzeyi olarak **özel** ' i seçin. Oturum Aç'a tıklayın ve yükleme işlemine başlayın.
    ...
    ![Azure Bilişsel Arama 'e Bağlan sayfasında kimlik doğrulama yönteminin, hesap anahtarının ve gizlilik düzeyinin nasıl girişi gösteren ekran görüntüsü.](media/search-monitor-logs-powerbi/connect-to-storage-account-step-two.png)
...
1. Verilerin yenilenmesini bekleyin. Bu işlem, sahip olduğunuz veri miktarına bağlı olarak biraz zaman alabilir. Verilerin yine de aşağıdaki göstergeye göre yenilenip yenilenmediğini görebilirsiniz.
    ...
    ![Veri yenileme sayfasındaki bilgilerin nasıl okunacağını gösteren ekran görüntüsü.](media/search-monitor-logs-powerbi/workspace-view-refreshing.png)

1. Veri yenileme işlemi tamamlandıktan sonra raporu görüntülemek için **Azure bilişsel arama raporu** ' nu seçin.

    ![Veri yenileme sayfasında Azure Bilişsel Arama raporunun nasıl seçileceğini gösteren ekran görüntüsü.](media/search-monitor-logs-powerbi/workspace-view-select-report.png)

1. Verilerle birlikte açılacak şekilde raporu açtıktan sonra sayfayı yenilediğinizden emin olun.

    ![Azure Bilişsel Arama Power BI raporunun ekran görüntüsü.](media/search-monitor-logs-powerbi/powerbi-search.png)
...
1. Power BI uygulamalarınıza giderek Azure Bilişsel Arama uygulamanızı bulun ve **uygulamayı Düzenle** düğmesini seçerek çalışma alanını görüntüleyin.
    ...
    ![Azure Bilişsel Arama uygulaması için uygulamayı Düzenle düğmesinin nasıl seçileceğini gösteren ekran görüntüsü.](media/search-monitor-logs-powerbi/azure-search-app-tile-edit.png)

1. Veri kümesi seçeneklerindeki **ayarları** seçin.

    ![Azure Congnsisel arama veri kümesi seçeneklerinden ayarların nasıl seçileceğini gösteren ekran görüntüsü.](media/search-monitor-logs-powerbi/workspace-view-select-settings.png)
...
1. Çalışma alanına geri gidin ve veri kümesi seçeneklerinde **Şimdi Yenile** ' yi seçin.
    ...
    ![Azure Congnsisel arama veri kümesi seçeneklerinde şimdi yenilemeyi nasıl seçileceğini gösteren ekran görüntüsü.](media/search-monitor-logs-powerbi/workspace-view-select-refresh-now.png)
