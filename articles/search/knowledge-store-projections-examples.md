...
Bu beceri 'ı temel olarak null `knowledgeStore` ile kullanarak, ilk örneğimiz `knowledgeStore` nesneyi doldurduk ve diğer senaryolarda kullandığımız tablo veri yapılarını oluşturan projeksiyonlar ile yapılandırılır. 
...
Bu örnekte, özel şekil meta verileri ve tanımlanan varlıkları ve anahtar tümceleri birleştirir. Nesne çağrılır `pbiShape` ve altında `/document`üst öğe olur. 
...
Örnekte, `pbiShape` birleştirilmiş şekil (veya zenginleştirme düğümü). Projeksiyon tanımında `pbiShape` , şeklin ```keyPhrases``` parçalarını göndermenizi sağlayan ek tablolara dilimlenebilir. ```Entities``` Power BI, bu, birden çok varlık olarak yararlı olur ve keyPhrases her belgeyle ilişkilendirilir ve sınıflandırılan veriler olarak varlıkları ve keyPhrases 'yi görebiliyorsanız daha fazla öngörü elde edersiniz.

Alt tabloda aynı ada sahip bir sütun oluşturmak için üst tablodaki öğesini kullanarak ```generatedKeyName``` , Dilimleme, üst ve alt tablolar arasında örtülü olarak bir ilişki oluşturur. 

### <a name="naming-relationships"></a>Adlandırma ilişkileri

Ve ```generatedKeyName``` ```referenceKeyName``` özellikleri, verileri tablolar arasında veya yansıtma türleri arasında ilişkilendirmek için kullanılır. Alt tablodaki/projeksiyondaki her satır, üst öğeye işaret eden bir özelliğe sahiptir. Alt öğe içindeki sütunun veya özelliğin adı, üst öğeden oluşur ```referenceKeyName``` . ```referenceKeyName``` Sağlanmazsa, hizmet bunu üst öğeden varsayılan olarak ```generatedKeyName``` yapar. 

Power BI tablolardaki ilişkileri saptamak için bu oluşturulan anahtarlara bağımlıdır. Alt tablodaki sütunun farklı olarak adlandırılmış olması gerekiyorsa, üst tablodaki ```referenceKeyName``` özelliğini ayarlayın. Bir örnek, pbiDocument tablosundaki ```generatedKeyName``` as ID değerini ve belgeikimliği ```referenceKeyName``` olarak olarak ayarlamak için bir örnektir. Bu, Belgetıd olarak adlandırılan belge kimliğini içeren pbiEntities ve pbiKeyPhrases tablolarındaki sütuna neden olur.
...
Bir nesne projeksiyonu tanımlamak için, projeksiyondaki ```objects``` diziyi kullanacağız. Her yeteneği kullanarak yeni bir şekil oluşturabilir veya nesne projeksiyonu için satır içi şekillendirme kullanabilirsiniz. Tablolar örneği bir şekil ve dilimleme oluşturma yaklaşımını gösterirken, bu örnek satır içi şekillendirme kullanımını gösterir. 

Satır içi şekillendirme, projeksiyondaki girişlerin tanımında yeni bir şekil oluşturma olanağıdır. Satır içi şekillendirme, `pbiShape`beceriye göre (bizim örneğimizde) aynı olan anonim bir nesne oluşturur. Yeniden kullanmayı planlamadığınız bir şekil tanımlıyorsanız satır içi şekillendirme yararlı olur.
...
Bir dosya projeksiyonu oluşturmak için, bu `files` diziyi İzdüşüm nesnesinde kullanırız. Bu örnek, belgeden ayıklanan tüm görüntüleri, belgesinden adlı `samplefile`bir kapsayıcıya projeler.
...
Bu projeksiyonlar için ihtiyacımız olan şekilleri almak için, adlı `crossProjection`şekillendirilmiş bir nesne oluşturan yetenek başına yeni bir mil ekleyerek başlayın. 
...
Bu örnek ayrıca projeksiyonun başka bir özelliğini de vurgular. Aynı projeksiyon nesnesi içinde birden çok tür projeksiyonları tanımlayarak, farklı türlerde (tablolar, nesneler, dosyalar) ifade edilen bir ilişki vardır ve bu belgede, bir belge için tablo satırıyla başlayıp, nesne projeksiyonun içindeki görüntüler için tüm OCR metinlerini bulabilirsiniz. 
...
+ Dize enzenginleştirmelerinin geçerli bir JSON 'a şekillendirmiyor. Dizeler, anahtar tümcecikleriyle `merged_content` zenginleştirilmiş olduğunda, zenginleştirme özelliği, zenginleştirme ağacı `merged_content` içinde bir alt öğe olarak temsil edilir. Varsayılan Gösterim doğru biçimlendirilmiş JSON değil. Yansıtma zamanında, bir ad ve değer ile geçerli bir JSON nesnesine zenginleştirme dönüştürdiğinizden emin olun.
