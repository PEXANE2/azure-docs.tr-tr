...
Türündeki `Edm.Int64` bir alanı ile `NaN`karşılaştırma gibi izin verilmeyen karşılaştırmalar için, Azure Bilişsel Arama REST API "http 400: Hatalı istek" hatası döndürür.
...
Karşılaştırma işleçlerini kullanırken, Azure Bilişsel Arama tüm koleksiyon olmayan alanların potansiyel olarak `null`olabileceğini unutmamak önemlidir. Aşağıdaki tabloda her iki tarafın de bir karşılaştırma ifadesi için olası tüm sonuçlar gösterilmektedir `null`:
...
Dizininizdeki tür `Edm.Double` alanları varsa ve bu alanlara `NaN` değerleri yüklerseniz, filtre yazarken bu hesaba hesap eklemeniz gerekir. Azure Bilişsel Arama, değerleri işlemek `NaN` için IEEE 754 standardını uygular ve bu tür değerlerle karşılaştırmalar, aşağıdaki tabloda gösterildiği gibi belirgin olmayan sonuçlar üretir.
