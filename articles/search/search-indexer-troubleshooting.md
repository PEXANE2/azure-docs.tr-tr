...
Azure Storage, CosmosDB ve Azure SQL yapılandırılabilir bir güvenlik duvarı sağlar. Güvenlik Duvarı etkinleştirildiğinde belirli bir hata iletisi yoktur. Genellikle, güvenlik duvarı hataları geneldir ve `The remote server returned an error: (403) Forbidden` veya `Credentials provided in the connection string are invalid or have expired`gibi görünür.
...
* Alternatif olarak, kaynağınızın güvenlik duvarı kurallarında (IP adresi aralığı kısıtlaması), arama hizmetinizin IP adresine ve `AzureCognitiveSearch` [hizmeti ETIKETININ](https://docs.microsoft.com/azure/virtual-network/service-tags-overview#available-service-tags) IP adresi aralığına erişime izin verebilirsiniz.
...
Azure işlevleri ( [özel bir Web API 'si](cognitive-search-custom-skill-web-api.md)olarak kullanılabilir), [IP adresi kısıtlamalarını](https://docs.microsoft.com/azure/azure-functions/ip-addresses#ip-address-restrictions)da destekler. Yapılandırılacak IP adresleri listesi, arama hizmetinizin IP adresi ve `AzureCognitiveSearch` hizmeti etiketinin IP adresi aralığıdır.
