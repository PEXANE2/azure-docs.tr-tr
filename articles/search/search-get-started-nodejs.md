...
+ Örnek bir dizin yapısı ve eşleşen belgeler bu makalede ya da depodan verilmiştir. [](https://github.com/Azure-Samples/azure-search-javascript-samples/quickstart/)
...
## <a name="1---define-and-create-index"></a>1-Dizin tanımlama ve oluşturma 
...
Azure Search, belgeler, sorgulardan dizin oluşturma ve çıkışlara yönelik giriş olan veri yapılarıdır. Belge girişleri bir veritabanındaki satırlar, blob depolamada Bloblar veya bu örnekte olduğu gibi, diskteki JSON belgeleri olabilir. [Oteller. JSON](https://github.com/Azure-Samples/azure-search-javascript-samples/quickstart/blob/master/hotels.json) dosyasını indirebilir ya da aşağıdaki içerikle kendi **oteller. JSON** dosyanızı oluşturabilirsiniz:
...
## <a name="2---a-class-for-azure-search"></a>2-Azure Search için bir sınıf 
...
Aşağıdaki yöntemleri sınıf gövdesinin içine ekleyin:
...
Tanıtım amacıyla, HTTP isteği başarılı değilse yalnızca bir özel durum oluşturacağız. Gerçek bir uygulamada büyük olasılıkla, `response` arama hizmeti isteğinden içindeki http durum kodunu günlüğe kaydetme ve tanılar. 
...
Son olarak, Azure Search diziniyle çalışan yöntemleri ekleyin. Bu yöntemlerin hepsi aynı yapıya sahiptir:

* İsteğin oluşturulacağı uç noktayı alın.
* İsteği uygun uç nokta, HTTP fiili, API anahtarı ve gövde ile oluşturun. `queryAsync()`sorgu anahtarını kullanır, aksi takdirde yönetici anahtarı kullanılır.
...
## <a name="3---create-a-program"></a>3-program oluşturma

Nesne odaklı bir sınıf, olası yeniden kullanılabilir **AzureSearchClient. js** modülü için iyi bir seçimdir, ancak **index. js**adlı bir dosyaya koyacağımız ana program için gerekli değildir. 
...
* **Oteller. JSON** dosyasındaki veriler.
...
Şimdi bazı temel sorguları ekleyin: 
...
İlk sorgu tüm verileri (`*`) ve döndürülen kayıt sayısı sayısını döndürür. İkincisi yalnızca **hotels_quickstart_index. JSON** içinde "aranabilir" olarak tanımlanan ve `Rating` alanı 4 ' ten büyük bir değer içeren "geçmiş" sözcüğünü içeren belgeleri döndürür. [Azure Search bir sorgu oluşturma](https://docs.microsoft.com/azure/search/search-query-overview)hakkında daha fazla bilgi edinin. 

[ **NConf** paketi](https://github.com/indexzero/nconf) , yapılandırma verilerini ortam değişkenleri veya komut satırı gibi çeşitli biçimlerde belirtmenize olanak tanır. **Azure_search_config. JSON** dosyasını okumak ve dosyanın içeriğini bir sözlük olarak döndürmek için **NConf** 'yi temel bir biçimde kullanacağız. **NConf**'nin `get(key)` işlevini kullanarak, ["Azure Search hizmet bilgilerini yapılandırma"](#configure) adımının atlanmadığımızda hızlı bir denetim yapabiliriz. Son olarak, yapılandırmayı geri döndürüyoruz:
...
İşlevi bir `AzureSearchClient` nesnesi alır ve `queries` dizideki her değere `AzureSearchClient.queryAsync` yöntemi uygular. `doQueries()` Yalnızca tüm sorgular `Promise.all()` çözümlendikten sonra çözümlenen bir `Promise` Single döndürmek için işlevini kullanır. Sorgu sonucunu daha okunaklı olacak şekilde biçimlendirir.`JSON.stringify(body, null, 4)`
...
* **Oteller. JSON** ' dan yüklediğiniz oteller hakkındaki belgeleri ekleyin
* Yazdığınız `doQueriesAsync()` yöntemi kullanarak Azure Search dizinini sorgulama
...
Program tarafından gerçekleştirilen eylemleri açıklayan, bazı sorguların sonuçlarıyla biten bir ileti dizisi görmeniz gerekir. İsteklerle daha fazla ayrıntı görmek isterseniz, **AzureSearchClient. js**' nin [20-26](https://github.com/Azure-Samples/azure-search-javascript-samples/quickstart/blob/master/AzureSearchClient.js#LL20-LL26) satırlarına açıklama ekleyebilirsiniz. 
