...
3. NFS istemcisi yüklendikten sonra, Data Box cihazınızdaki NFS paylaşımını bağlamak için aşağıdaki komutu kullanın:
    ...
    Aşağıdaki örnekte, NFS aracılığıyla Data Box Heavy bir paylaşıma nasıl bağlanabileceği gösterilmektedir. Data Box Heavy IP `10.161.23.130`, paylaşımın `Mystoracct_Blob` ubuntuVM üzerine bağlanır, bağlama noktası `/home/databoxheavyubuntuhost/databoxheavy`.
...
- Büyük/küçük harfe duyarlı dizin ve dosya adlarını NFS paylaşımından Data Box Heavy: 
    ...
    Örneğin, `SampleFile.txt` ve `Samplefile.Txt`kopyalama yapıyorsanız, bu durum cihaza kopyalanırken adı saklanır, ancak ikinci dosya aynı dosyada olduğu gibi ilk dosyanın üzerine yazılır.
