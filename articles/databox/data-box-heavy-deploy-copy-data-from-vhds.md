...
Data Box Heavy, belirtilen kaynak gruplarına göre, düğüm başına her bir ilişkili kaynak grubu için bir paylaşma oluşturur. Örneğin, sipariş yerleştirilirken `mydbmdrg1` ve `mydbmdrg2` oluşturulduysa aşağıdaki paylaşımlar oluşturulur:
...
3. NFS istemcisi yüklendikten sonra, cihazınıza NFS paylaşımından bağlamak için aşağıdaki komutu kullanın:
    ...
    Aşağıdaki örnek, NFS aracılığıyla bir Data Box veya Data Box Heavy paylaşıma nasıl bağlanılacağını gösterir. Data Box veya Data Box Heavy cihaz IP 'si `169.254.250.200`, Share `mydbmdrg1_MDisk` ubuntuVM 'ye bağlanır, bağlama noktası `/home/databoxubuntuhost/databox`.
...
- [SMB aracılığıyla veri kopyalama](data-box-heavy-deploy-copy-data.md#copy-data-to-data-box-heavy)
