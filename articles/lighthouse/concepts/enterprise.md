...
Kuruluştaki çapraz Kiracı Yönetimi için, Azure açık belgeleri belgelerindeki hizmet sağlayıcılarına yapılan başvurular, bir kurum içindeki yönetim kiracısına uygulanabilir (yani, diğer kiracılardaki kaynakları Azure tarafından yetkilendirilen kaynak yönetimi aracılığıyla yönetecek kullanıcıları dahil edecek şekilde). Benzer şekilde, müşterilere yapılan başvurular, yönetim kiracısındaki kullanıcılar aracılığıyla yönetilecek kaynak temsilcisi olan kiracılar için de kullanılabilir.
