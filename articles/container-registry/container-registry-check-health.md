...
Aşağıdaki örneklerde `az acr check-health` komutunu çalıştırmanın farklı yolları gösterilmektedir.
...
Varsayılan olarak, komut her bir hata bulduğunda yanıt vermez. Ayrıca, hatalar bulunsa bile, tüm sistem durumu denetimleri için çıkış sağlamak üzere komutunu çalıştırabilirsiniz. Aşağıdaki örneklerde gösterildiği gibi `--ignore-errors` parametresini ekleyin:
