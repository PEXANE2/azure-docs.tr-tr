...
Tetiklenen bir Web kancası, Web kancasını yapılandırdığınızda belirlediğiniz URL uç noktasına bir HTTP `POST` isteği oluşturur.

### <a name="http-headers"></a>HTTP üstbilgileri

Web kancası istekleri için bir `Content-Type` özel üst bilgisi belirtmdıysanız, Web kancası istekleri `application/json` `Content-Type` içerir.
...
|`timestamp`|Tarih Saat|Web kancası olayının tetiklendiği zaman.|
