...
* Oluşturulan bir parolayla birlikte bir **belirteç** , kullanıcının kayıt defteriyle kimlik doğrulamasını sağlar. Belirteç parolası için bir sona erme tarihi ayarlayabilir veya bir belirteci dilediğiniz zaman devre dışı bırakabilirsiniz.  
  ...
  |`content/write`     |  Verileri depoya yaz     | Yapıtı göndermek için `content/read` ile kullanma |
...
* Bir **kapsam eşlemesi** , belirteç için uyguladığınız depo izinlerini gruplandırır ve diğer belirteçlere yeniden uygulanabilir. Her belirteç tek bir kapsam eşlemesiyle ilişkilendirilir. 
  ...
  Azure Container Registry Ayrıca, tüm depolarda sabit izinlerle uygulayabileceğiniz, sistem tarafından tanımlanan çeşitli kapsam haritaları sağlar.
...
* Azure **CLI** -belirteçleri oluşturmak ve yönetmek IÇIN Azure CLI komutları Azure CLI sürüm 2.0.76 veya sonraki sürümlerinde kullanılabilir. Sürümü bulmak için `az --version` komutunu çalıştırın. Yükleme veya yükseltme yapmanız gerekiyorsa bkz. [Azure CLI'yı yükleme](/cli/azure/install-azure-cli).
...
Aşağıdaki örnek, kayıt defteri *myregistry* 'de `samples/hello-world` deposu için aşağıdaki izinlerle bir belirteç oluşturur: `content/write` ve `content/read`. Varsayılan olarak, komut varsayılan belirteç durumunu `enabled`olarak ayarlar, ancak durum ' u dilediğiniz zaman `disabled` olarak güncelleştirebilirsiniz.
...
Çıktı, iki oluşturulan parola dahil olmak üzere belirteçle ilgili ayrıntıları gösterir. Daha sonra kimlik doğrulaması için kullanmak üzere parolaları güvenli bir yere kaydetmeniz önerilir. Parolalar yeniden alınamaz, ancak yeni olanlar oluşturulabilir.
...
Kapsam eşlemesi oluşturmak için [az ACR Scope-Map Create][az-acr-scope-map-create] komutunu kullanın. Aşağıdaki komut, daha önce kullanılan `samples/hello-world` deposunda aynı izinlere sahip bir kapsam haritası oluşturur. 
...
Bir belirteç oluşturmak için [az ACR Token Create][az-acr-token-create] komutunu çalıştırın, bu, *Kapsamım* kapsam eşlemesini belirterek. Önceki örnekte olduğu gibi, komut varsayılan belirteç durumunu `enabled`olarak ayarlar.
...
Belirteçleri ve kapsam eşlemelerini oluşturmak için Azure portal kullanabilirsiniz. `az acr token create` CLı komutunda olduğu gibi, bir veya daha fazla depo ve ilişkili eylemleri belirterek bir belirteç oluşturduğunuzda mevcut bir kapsam haritasını uygulayabilir veya bir kapsam eşlemesi oluşturabilirsiniz. Depoların kayıt defterinde henüz olması gerekmez. 

Aşağıdaki örnek bir belirteç oluşturur ve `samples/hello-world` deposunda aşağıdaki izinlerle bir kapsam haritası oluşturur: `content/write` ve `content/read`.
...
1. Kapsam eşlemesini yapılandırma:
    ...
    1. **Depolar**' ın altında `samples/hello-world`girin ve **izinler**altında `content/read` ve `content/write`' yı seçin. Ardından **+ Ekle**' yi seçin.  
    Portal 'da kapsam eşlemesi oluşturma ![](media/container-registry-repository-scoped-permissions/portal-scope-map-add.png)
...
Belirteç oluşturduktan sonra bir parola oluşturun. Kayıt defteri ile kimlik doğrulaması yapmak için belirtecin etkinleştirilmesi ve geçerli bir parolaya sahip olması gerekir.

Bir veya iki parola oluşturabilir ve her biri için bir sona erme tarihi ayarlayabilirsiniz. 

1. Portalda kapsayıcı Kayıt defterinize gidin.
1. **Hizmetler**altında **belirteçler (Önizleme)** öğesini seçin ve bir belirteç seçin.
1. Belirteç ayrıntılarında **Parola1** veya **paSsWorD2**' yi seçin ve Oluştur simgesini seçin.
1. Parola ekranında, isteğe bağlı olarak parola için bir sona erme tarihi ayarlayın ve **Oluştur**' u seçin.
...
Bir kullanıcı veya hizmet, hedef kayıt defteri ile kimlik doğrulaması yapmak için bir belirteç kullandığında, belirteç adını Kullanıcı adı ve oluşturduğu parolalardan biri olarak sağlar. Kimlik doğrulama yöntemi, belirteç ile ilişkili eyleme veya eylemlere bağlıdır.

|Eylem  |Kimlik doğrulaması nasıl yapılır?  |
  |---------|---------|
  |`content/delete`    | Azure CLı 'de `az acr repository delete` |
  |`content/read`     |  `docker login`<br/><br/>Azure CLı 'de `az acr login`  |
  |`content/write`     |  `docker login`<br/><br/>Azure CLı 'de `az acr login`     |
  |`metadata/read`    | `az acr repository show`<br/><br/>`az acr repository show-tags`<br/><br/>Azure CLı 'de `az acr repository show-manifests`   |
  |`metadata/write`     |  `az acr repository untag`<br/><br/>Azure CLı 'de `az acr repository update` |

## <a name="examples-use-token"></a>Örnekler: belirteci kullan

Aşağıdaki örneklerde, bir depoda yaygın işlemleri gerçekleştirmek için bu makalenin önceki kısımlarında oluşturulan belirteç kullanılmaktadır: gönderme ve çekme görüntüleri, görüntüleri silme ve havuz etiketlerini listeleme. Belirteç, başlangıçta `samples/hello-world` deposunda anında iletme izinleri (`content/write` ve `content/read` eylemleri) ile ayarlanmıştır.

### <a name="pull-and-tag-test-images"></a>Sınama görüntülerini çekme ve etiketleme

Aşağıdaki örneklerde, Docker Hub 'ından `hello-world` ve `alpine` görüntülerini çekin ve bunları kayıt defteriniz ve deponuzun etiketleyerek etiketleyin.
...
Kayıt defteri ile kimlik doğrulaması yapmak için `docker login` çalıştırın, Kullanıcı adı olarak belirteç adını sağlayın ve parolalarından birini sağlayın. Belirtecin `Enabled` durumu olmalıdır.
...
Başarılı oturum açma işleminden sonra, etiketli görüntüleri kayıt defterine göndermeye çalışır. Belirtecin `samples/hello-world` deposuna görüntü gönderme izinleri olduğundan, şu gönderim başarılı olur:

```bash
docker push myregistry.azurecr.io/samples/hello-world:v1
```

Belirtecin `samples/alpine` deposu için izinleri yoktur, bu nedenle aşağıdaki gönderme denemesi `requested access to the resource is denied`benzer bir hata vererek başarısız olur:

```bash
docker push myregistry.azurecr.io/samples/alpine:v1
```

### <a name="change-pushpull-permissions"></a>Gönderme/çekme izinlerini değiştir
...
Örneğin, `samples/alpine` deposunda `content/write` ve `content/read` eylemlerle `MyToken-scope-map` güncelleştirin ve `content/write` deposundaki `samples/hello-world` eylemini kaldırın.  
...
1. **Depolar**' ın altında `samples/alpine`girin ve **izinler**altında `content/read` ve `content/write`' yı seçin. Ardından **+ Ekle**' yi seçin.
1. **Depolar**altında `samples/hello-world` öğesini seçin ve **izinler**altında `content/write`seçimini kaldırın. Daha sonra **Kaydet**’e tıklayın.
...
Kapsam eşlemesi `samples/hello-world` deposunda yalnızca `content/read` iznine sahip olduğundan, şimdi `samples/hello-world` depoya anında iletme denemesi başarısız olur:
 
```bash
docker push myregistry.azurecr.io/samples/hello-world:v1
```

Kapsam eşlemesi her iki depoda da `content/read` izinleri sağladığından, her iki depoya da görüntü çekme başarılı oldu.
...
`alpine` deposuna `content/delete` eylemini ekleyerek kapsam eşlemesini güncelleştirin. Bu eylem depodaki görüntülerin silinmesine veya tüm deponun silinmesine izin verir.
...
`samples/alpine` deposunu silmek için aşağıdaki [az ACR Repository Delete][az-acr-repository-delete] komutunu kullanın. Görüntüleri veya depoları silmek için, belirteç `docker login`aracılığıyla kimlik doğrulaması yapmaz. Bunun yerine, belirtecin adını ve parolasını komuta geçirin. Aşağıdaki örnek, makalesinde daha önce oluşturulan ortam değişkenlerini kullanır:
...
`hello-world` deposuna `metadata/read` eylemini ekleyerek kapsam eşlemesini güncelleştirin. Bu eylem, depodaki bildirim ve etiket verilerinin okunmasına izin verir.
...
`samples/hello-world` deposundaki meta verileri okumak için [az ACR Repository Show-bildirimleri][az-acr-repository-show-manifests] veya [az ACR Repository Show-Tags][az-acr-repository-show-tags] komutunu çalıştırın. 

Meta verileri okumak için, belirteç `docker login`aracılığıyla kimlik doğrulaması yapmaz. Bunun yerine, belirtecin adını ve parolasını her iki komuta de geçirin. Aşağıdaki örnek, makalesinde daha önce oluşturulan ortam değişkenlerini kullanır:
...
Çıktıda, tanımladığınız kapsam haritaları ve belirteçleri yapılandırmak için kullanabileceğiniz sistem tanımlı çeşitli kapsam haritaları gösterilmektedir:
...
Aşağıdaki örnek, Parola1 for the *Mytoken* token için 30 günlük süre sonu ile yeni bir değer oluşturur. Parolayı `TOKEN_PWD`ortam değişkeninde depolar. Bu örnek bash kabuğu için biçimlendirilir.
...
Azure CLı kullanarak, `status` `disabled`ayarlamak için [az ACR Token Update][az-acr-token-update] komutunu çalıştırın:
