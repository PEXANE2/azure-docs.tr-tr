...
1. Bu hızlı başlangıçta çalıştırdığınız iki konsol uygulaması C# kullanılarak yazılmıştır. Geliştirme makinenize [.NET Core SDK sürüm 2.1.403 veya üstünü](https://www.microsoft.com/net/download) yükler. .NET Core SDK yüklüyse, geliştirme makinenizde geçerli C# sürümünü doğrulayın. Komutunu `dotnet --version` bir komut isteminde çalıştırın.

1. [Örnek C# projesini](https://github.com/Azure-Samples/digital-twins-samples-csharp/archive/master.zip)indirin. Digital-Twins-Samples-CSharp-Master. zip arşivini ayıklayın.
...
1. Sağlama adımı birkaç dakika sürebilir. Ayrıca, dijital TWINS örneğiniz içinde bir IoT Hub sağlar. IoT Hub durum =`Running`gösterdiği sürece döngüsü geçer.

    [![Örnek durum = çalışıyor olarak sağlama](media/quickstart-view-occupancy-dotnet/azure-digital-twins-quickstart-provision-sample.png)](media/quickstart-view-occupancy-dotnet/azure-digital-twins-quickstart-provision-sample.png#lightbox)

1. Yürütmenin sonunda cihazın cihaz simülatör örneğinde kullanılmak üzere `ConnectionString` cihazını kopyalayın. Yalnızca bu görüntüde özetlenen dizeyi kopyalayın.
...
1. `dotnet restore` öğesini çalıştırın.
1. **Deviceconnectionstring** öğesini öncekiyle `ConnectionString`güncelleştirmek için [appSettings. JSON](https://github.com/Azure-Samples/digital-twins-samples-csharp/blob/master/device-connectivity/appsettings.json) ' i düzenleyin. Güncelleştirilen dosyayı kaydedin.
1. Algılayıcı `dotnet run` verileri göndermeye başlamak için ' i çalıştırın. Aşağıdaki görüntüde gösterildiği gibi Azure dijital TWINS 'e gönderilir.
...
1. Sonraki adım eylemiyle sonuçları yan yana görüntüleyebilmeniz için bu simülatör çalışmasına izin verin. Bu pencere, dijital TWINS 'e gönderilen sanal algılayıcı verilerini gösterir. Sonraki adım, yeni uçak ile kullanılabilir odaları bulmak için gerçek zamanlı olarak sorgular.
    ...
    >[!TIP]
    > Bu adımı çalıştırdığınızda aşağıdaki hata iletisi görüntülenirse doğru `DeviceConnectionString` şekilde kopyalandığından emin olun:`EXIT: Unexpected error: The input is not a valid Base-64 string ...`
...
Bu hızlı başlangıçta ne olduğunu ve hangi API 'Lerin çağrıldığını anlamak için, içinde [Visual Studio Code](https://code.visualstudio.com/Download) `digital-twins-samples-csharp`bulunan kod çalışma alanı projesiyle Visual Studio Code açın. Aşağıdaki komutu kullanın:
