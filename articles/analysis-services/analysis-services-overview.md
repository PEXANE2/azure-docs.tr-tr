...
**Video** Sunucu oluşturmayı hızlandırmak için Azure Otomasyonu 'nu nasıl kullanabileceğiniz hakkında daha fazla bilgi edinmek için [otomatik dağıtımı](https://channel9.msdn.com/series/Azure-Analysis-Services/AzureAnalysisServicesAutomation) inceleyin.
...
Uyumluluk düzeyi Analysis Services altyapıdaki yayına özgü davranışları ifade eder. Azure Analysis Services, 1200 ve daha yüksek uyumluluk düzeylerinde tablo modellerini destekler. Daha fazla bilgi için bkz. [Analysis Services tablolu modeller Için uyumluluk düzeyi](analysis-services-compat-level.md).
...
Azure Analysis Services, Azure ölçümleriyle tümleşiktir ve sunucularınızın performans ve durumunu izlemenize yardımcı olan çok sayıda kaynağa özgü ölçüm sağlar. Daha fazla bilgi için bkz. [Sunucu ölçümlerini izleme](analysis-services-monitor.md). Ölçümleri [Azure kaynak tanılama günlükleri](../azure-monitor/platform/diagnostic-logs-overview.md) ile kaydedin. Azure [depolama](https://azure.microsoft.com/services/storage/)'ya günlükleri izleyip gönderin, [Azure Event Hubs](https://azure.microsoft.com/services/event-hubs/)'a aktarın ve Azure 'da bir hizmet olan Azure [izleyici günlüklerine](https://azure.microsoft.com/services/log-analytics/)dışarı aktarın. [](https://www.microsoft.com/cloud-platform/operations-management-suite) Daha fazla bilgi için bkz. [Tanılama günlüğüne kaydetmeyi ayarlama](analysis-services-logging.md).
...
Azure Analysis Services tablolu modeller, SQL Server Analysis Services tablolu modellerle çok aynı olduğundan, SQL Server Analysis Services ' de paylaşılan kavramsal, yordamsal, geliştirici ve başvuru makaleleriyle ilgili kapsamlı bir kitaplık vardır. [ Belgeler](https://docs.microsoft.com/analysis-services/analysis-services-overview). SQL Server Analysis Services belgelerindeki makaleler, bunların Azure Analysis Services için de geçerli olup olmadığını başlığın altındaki bir APPLIES TO başlığı ile gösterir.
