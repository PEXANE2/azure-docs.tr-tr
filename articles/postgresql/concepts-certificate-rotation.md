...
SSL/TLS kullanan ve kök sertifikanın, PostgreSQL için Azure veritabanı 'na bağlanmak üzere kök sertifikayı güncelleştirmesi gerektiğini doğrulayan tüm uygulamalar. Şu anda SSL/TLS kullanmıyorsanız (sslmode = Disable) veya sunucu sertifikasını doğrulamadıysanız (sslmode = izin ver, sslmode = tercih edilen veya sslmode = gerektir), uygulamanızın kullanılabilirliği üzerinde hiçbir etkisi olmaz. İstemci uygulamanızın önceden tanımlanmış güvenilir sertifika yetkilisi ( [CA) Ile](concepts-ssl-connection-security.md#enforcing-tls-connections)SSL modunu kullanmaya çalıştığını doğrulayabilirsiniz.
...
Kök sertifikayı 30 Kasım 2020 tarihinden önce güncelleştirmediğinizi, SSL/TLS aracılığıyla bağlanan ve kök sertifika için doğrulama yapan uygulamalarınız PostgreSQL veritabanı sunucusuyla iletişim kuramayacaktır ve uygulama PostgreSQL veritabanı sunucunuza bağlantı sorunlarıyla karşılaşacaktır.
...
PostgreSQL için Azure veritabanı tarafından kullanılan bu sertifikalar, güvenilen sertifika yetkilileri (CA) tarafından sağlanır. Bu nedenle, PostgreSQL için Azure veritabanı 'nda bu sertifikaların desteklenmesi, CA tarafından bu sertifikaların desteğine bağlıdır. Ancak, bu örnekte olduğu gibi, önceden tanımlanmış bu sertifikalarda, en erken düzeltilmelidir.
