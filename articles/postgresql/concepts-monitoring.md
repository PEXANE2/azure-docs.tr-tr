---
title: Monitör ve tune - PostgreSQL için Azure Veritabanı - Tek Sunucu
description: Bu makalede, PostgreSQL - Single Server için Azure Veritabanı'ndaki izleme ve ayar özellikleri açıklanmaktadır.
author: rachel-msft
ms.author: raagyema
...
---
# <a name="monitor-and-tune-azure-database-for-postgresql---single-server"></a>PostgreSQL için Azure Veritabanını izleyin ve ayarlayın - Tek Sunucu
Sunucularınız hakkındaki verileri izlemek, iş yükünüz için sorun gidermenize ve en iyi duruma getirmenize yardımcı olur. PostgreSQL için Azure Veritabanı, sunucunuzun davranışı hakkında bilgi sağlamak için çeşitli izleme seçenekleri sağlar.

## <a name="metrics"></a>Ölçümler
PostgreSQL için Azure Veritabanı, PostgreSQL sunucusunu destekleyen kaynakların davranışları hakkında bilgi veren çeşitli ölçümler sağlar. Her metrik bir dakikalık frekansta yayılır ve 30 güne kadar geçmişi vardır. Ölçümlerdeki uyarıları yapılandırabilirsiniz. Adım adım kılavuzluk için [uyarıları nasıl ayarlayabilirsiniz.](howto-alert-on-metric.md) Diğer görevler arasında otomatik eylemler ayarlama, gelişmiş analitik gerçekleştirme ve geçmişi arşivleme yer alır. Daha fazla bilgi için [Azure Ölçümlerine Genel Bakış'a](../monitoring-and-diagnostics/monitoring-overview-metrics.md)bakın.
...
|pg_replica_log_delay_in_bytes|Çoğaltmalar Arasında Max Lag|Bayt|Ana ve en geri kalan çoğaltma arasındaki bayt gecikmesi. Bu metrik yalnızca ana sunucuda kullanılabilir.|
|pg_replica_log_delay_in_seconds|Çoğaltma Gecikmesi|Saniye|Son yeniden oynan işlemden bu yana geçen süre. Bu metrik yalnızca yineleme sunucuları için kullanılabilir.|

## <a name="server-logs"></a>Sunucu günlükleri
Sunucunuzda günlüğe kaydetmeyi etkinleştirebilirsiniz. Bu günlükler, [Azure Monitor günlüklerinde,](../azure-monitor/log-query/log-query-overview.md)Etkinlik Hub'larında ve Depolama Hesabındaazure Tanılama Günlükleri'nde de kullanılabilir. Günlüğe kaydetme hakkında daha fazla bilgi edinmek için [sunucu günlükleri](concepts-server-logs.md) sayfasını ziyaret edin.

## <a name="query-store"></a>Sorgu Deposu
[Sorgu Deposu,](concepts-query-store.md) sorgu çalışma zamanı istatistikleri ve bekleme olayları da dahil olmak üzere zaman içinde sorgu performansını izler. Özellik, query_store şeması altında **azure_sys** adlı bir sistem veritabanında sorgu çalışma zamanı performans bilgilerini devam ettir. Çeşitli yapılandırma topuzları aracılığıyla verilerin toplanmasını ve depolanmasını denetleyebilirsiniz.

## <a name="query-performance-insight"></a>Sorgu Performansı İçgörüleri
[Sorgu Performans Öngörüsü,](concepts-query-performance-insight.md) Azure portalından erişilebilen görselleştirmeler sağlamak için Sorgu Mağazası ile birlikte çalışır. Bu grafikler, performansı etkileyen anahtar sorguları belirlemenize olanak tanır. Sorgu Performans Öngörüsi'ne PostgreSQL sunucunun portal sayfası için Azure Veritabanınızın **Destek + sorun giderme** bölümünden erişilebilir.
...
- Bir metrikte uyarı oluşturma konusunda rehberlik için [uyarılar nasıl ayarlayacağınız](howto-alert-on-metric.md) üzerinde görün.
...
- [Sunucunuzu izlemek için en iyi uygulamalar](https://azure.microsoft.com/blog/best-practices-for-alerting-on-metrics-with-azure-database-for-postgresql-monitoring/)hakkında blogumuzu okuyun.
