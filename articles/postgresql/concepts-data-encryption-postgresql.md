...
description: PostgreSQL için Azure veritabanı tek sunuculu veri şifrelemesi müşteri tarafından yönetilen anahtarla, bekleyen veri koruması için Kendi Anahtarını Getir (BYOK) sağlar ve kuruluşların anahtar ve veri yönetiminde görevlerin ayrılmasını sağlar.
author: kummanish
ms.author: manishku
...
PostgreSQL için Azure veritabanı tek sunucu, Key Vault depolanan müşterinin yönetilen anahtarıyla şifrelendikten sonra, sunucunun yeni oluşturulan kopyası (yerel veya coğrafi geri yükleme işlemi ya da okuma çoğaltmaları aracılığıyla) aynı zamanda aynı şekilde şifrelenir Müşterinin yönetilen anahtarı. Ancak, bu kullanıcılar, şifreleme için yeni müşterinin yönetilen anahtarını yansıtacak şekilde değiştirilebilir. Müşteri tarafından yönetilen anahtar değiştirildiğinde, sunucunun eski yedekleri en son anahtarı kullanmaya başlar.
...
[Azure Portal kullanarak, PostgreSQL Için Azure veritabanınız için müşteri tarafından yönetilen anahtarla veri şifrelemeyi ayarlamayı](howto-data-encryption-portal.md)öğrenin.
