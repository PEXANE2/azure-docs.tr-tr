---
title: 'Hızlı başlangıç: PostgreSQL için Azure veritabanı esnek sunucusu ile Java ve JDBC kullanma'
description: Bu hızlı başlangıçta, bir PostgreSQL için Azure veritabanı esnek sunucusu ile Java ve JDBC kullanmayı öğreneceksiniz.
author: mksuni
ms.author: sumuth
...
ms.locfileid: "98606156"
---
# <a name="quickstart-use-java-and-jdbc-with-azure-database-for-postgresql-flexible-server"></a>Hızlı başlangıç: PostgreSQL için Azure veritabanı esnek sunucusu ile Java ve JDBC kullanma

Bu konuda, [PostgreSQL Için Azure veritabanı esnek sunucusu](./index.yml)'nda bilgileri depolamak ve almak üzere Java ve [JDBC](https://en.wikipedia.org/wiki/Java_Database_Connectivity) kullanan bir örnek uygulama oluşturma gösterilmektedir.
...
> [!NOTE]
> Azure portal kullanarak PostgreSQL [Için Azure veritabanı oluşturma sunucusu](./quickstart-create-server-portal.md)oluşturma hakkında daha ayrıntılı bilgi edinebilirsiniz.
...
Daha önce oluşturduğunuz PostgreSQL sunucusunun boş bir veritabanı **Postgres** 'si vardır. Bu hızlı başlangıç için aşağıdaki adımları izleyerek adlı yeni bir veritabanı oluşturalım `demo` :

1. Azure PostgreSQL sunucusuna bağlanmak için yerel [psql](https://www.postgresql.org/docs/current/static/app-psql.html) örneğini yükleme. 
2. PostgreSQL sunucunuzda **Postgres** veritabanına bağlanmak için aşağıdaki psql komutunu çalıştırın
...
3. Aşağıdaki komutu yazarak istemde **demo** adlı yeni bir veritabanı oluşturun:
