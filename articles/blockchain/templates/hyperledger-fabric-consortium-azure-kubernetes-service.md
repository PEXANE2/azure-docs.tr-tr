...
Sorgu işlev adı ve `<queryFunction>` bağımsız değişkenlerin virgülle ayrılmış listesini sırasıyla `<queryFuncArgs>` geçirin. Bir kez `fabcar` chaincode 'u başvuru olarak alarak, World durum kümesindeki tüm otomobillerin `"queryAllCars"` ve `<queryArgs>' to `"" "olarak sorgulamak için `<queryFunction>`.
