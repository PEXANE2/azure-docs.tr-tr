...
1. U-SQL veritabanı projesi için UıDO/UDAG/UDF içeren C# projesine başvurular ekleyin.
   ...
   ![Visual Studio için Data Lake araçları-başvuru Ekle](./media/data-lake-analytics-data-lake-tools-develop-usql-database/data-lake-tools-add-project-reference.png)
...
1. **Veritabanı kaynağı** otomatik olarak doldurulur ve projenin derleme çıkış klasöründeki. ustodbpack paketine işaret eder.

1. Veritabanı oluşturmak için **veritabanı adında** bir ad girin. Hedef Azure Data Lake Analytics hesabında aynı ada sahip bir veritabanı zaten varsa, veritabanı projesinde tanımlanan tüm nesneler veritabanını yeniden oluşturmadan oluşturulur.

1. U-SQL veritabanını dağıtmak için **Gönder**' i seçin. Tüm kaynaklar (derlemeler ve ek dosyalar) karşıya yüklenir ve tüm DDL deyimlerini içeren bir U-SQL işi gönderilir.
...
1. **Veritabanı kaynağını** U-SQL veritabanı dağıtım paketi (. ustodbpack dosyası) yoluna ayarlayın.

1. Veritabanı oluşturmak için **veritabanı adını** girin. Hedef Azure Data Lake Analytics hesabında zaten aynı ada sahip bir veritabanı varsa, veritabanı projesinde tanımlanan tüm nesneler veritabanını yeniden oluşturmadan oluşturulur.
