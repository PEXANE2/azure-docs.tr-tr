...
* U- `REFERENCE ASSEMBLY` SQL betiği için R uzantılarını etkinleştirmek üzere ifadesini kullanın.
* Giriş verilerini `REDUCE` bir anahtar üzerinde bölümlemek için işlemi kullanın.
* U-SQL için R uzantıları, Reducer atanan her bir köşede R kodu`Extension.R.Reducer`çalıştıran yerleşik bir Reducer () içerir. 
* U-SQL ve R arasında veri `inputFromUSQL` geçirmek `outputToUSQL` için çağrılan ve sırasıyla adanmış adlandırılmış veri çerçevelerinin kullanımı. giriş ve çıkış veri çerçevesi tanımlayıcı adları sabittir (yani, kullanıcılar bu giriş ve çıkış veri çerçevesi tanımlayıcılarının önceden tanımlı adlarını değiştiremezler).
...
* U-SQL ' den gelen dize ve sayısal sütunlar, R DataFrame ve U-SQL [desteklenen `double`türler:, `string`, `bool`, `integer`, `byte`] arasında olduğu gibi dönüştürülür.
* `Factor` Veri türü U-SQL ' de desteklenmez.
* `byte[]`Base64 kodlamalı `string`olarak serileştirilmelidir.
* U-SQL dizeleri r-SQL create R Input veri çerçevesini bir kez veya Reducer parametresini `stringsAsFactors: true`ayarlayarak r kodundaki faktörlere dönüştürülebilir.
...
İlk olarak, bir R özel modülü oluşturun ve ZIP yapın ve ardından daraltılmış R özel modül dosyasını ADL deponuza yükleyin. Bu örnekte, 5. zip magittr_1, kullandığımız ADLA hesabı için varsayılan ADLS hesabının köküne yükleyeceğiz. Modülü ADL deposuna yükledikten sonra, U-SQL betiğinizdeki kullanılabilir hale getirmek için DAĞıTıM kaynağı kullan ' ı kullanın ve yükleme çağrısı `install.packages` yapın.
