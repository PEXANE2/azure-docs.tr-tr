...
1. `Get-NetAdapterRss` PowerShell komutuyla bir ağ bağdaştırıcısı için RSS 'nin etkinleştirilip etkinleştirilmediğini inceleyin. `Get-NetAdapterRss`döndürülen aşağıdaki örnek çıktıda RSS etkin değildir.
...
3. `Get-NetAdapterRss` komutunu tekrar girerek, sanal makinede RSS 'nin etkinleştirildiğini doğrulayın. Başarılı olursa aşağıdaki örnek çıktı döndürülür:
