...
Bu makalede, çift yığın alt ağına sahip bir çift yığın sanal ağı içeren bir çift yığın (IPv4 + IPv6) Standart Load Balancer uygulamasının nasıl dağıtılacağı gösterilmektedir, Çift (IPv4 + IPv6) ön uç yapılandırmalarına sahip bir Standart Load Balancer, ile sanal makineler Çift IP yapılandırmasına, çift ağ güvenlik grubu kuralına ve çift genel IP 'ye sahip NIC 'Ler.
...
Azure sanal ağ için IPv6 özelliğini kullanmak üzere aboneliğinizi aşağıdaki gibi Azure PowerShell kullanarak yapılandırmanız gerekir:
...
Çift yığın Sanal ağınızı oluşturabilmeniz için [az Group Create](/cli/azure/group)ile bir kaynak grubu oluşturmanız gerekir. Aşağıdaki örnek *eastus* konumunda *Myrgdualstack* adlı bir kaynak grubu oluşturur:
