...
description: Test için ağı hedeflemek ve performansı etkileyebilecek diğer kaynakların kullanımını en aza indirmek için NTTTCP kullanın.
...
En son sürümü indirin: https://github.com/microsoft/ntttcp/releases/download/v5.35/NTttcp.exe

Veya üst düzey GitHub sayfasını görüntüleyin: <https://github.com/microsoft/ntttcp>\
...
CentOS-GCC ve git 'i yükler:
``` bash
  yum install gcc -y  
  yum install git -y
```
Ubuntu-Build-Essential ve git 'i yükler:
``` bash
 apt-get -y install build-essential  
 apt-get -y install git
```
SUSE-git-Core, GCC ve Make 'i yükler:
``` bash
