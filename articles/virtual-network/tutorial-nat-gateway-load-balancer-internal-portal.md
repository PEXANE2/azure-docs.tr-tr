---
title: 'Öğretici: NAT ağ geçidini iç yük dengeleyici ile tümleştirme-Azure portal'
titleSuffix: Virtual Network NAT
description: Bu öğreticide, Azure portal kullanarak bir sanal ağ NAT ağ geçidini iç yük dengeleyiciye nasıl tümleştirileceğini öğrenin.
author: asudbring
ms.author: allensu
...
ms.locfileid: "104670867"
---
# <a name="tutorial-integrate-a-nat-gateway-with-an-internal-load-balancer-using-the-azure-portal"></a>Öğretici: Azure portal kullanarak bir NAT ağ geçidini iç yük dengeleyiciye tümleştirme

Bu öğreticide, bir NAT ağ geçidini iç yük dengeleyiciye tümleştirmeyi öğreneceksiniz.
...
SNAT, başka bir genel yük dengeleyici, ağ yönlendirmesi veya bir sanal makinede tanımlanmış genel IP aracılığıyla iç arka uç havuzu için etkinleştirilir.

NAT ağ geçidi tümleştirmesi, arka uç havuzundaki bir sanal makinede tanımlanmış bir genel yük dengeleyici, ağ yönlendirmesi veya genel IP dağıtımı gereksinimini ortadan kaldırır.
...
3. **Sanal ağ oluştur**' da, **temel** bilgiler sekmesinde bu bilgileri girin veya seçin:
    ...
    | Kaynak Grubu   | **Yeni oluştur**’u seçin. **Tutorintlbnat-RG** girin. </br> **Tamam**’ı seçin. |
    | **Örnek ayrıntıları** |                                                                 |
    | Name             | **Myvnet** girin                                    |
...
4. **Yük dengeleyici oluştur** sayfasında girin veya aşağıdaki bilgileri seçin: 
    ...
    | Kaynak grubu         | **Tutorintlbnat-RG** öğesini seçin.|
    | **Örnek ayrıntıları** |    |
    ...
    | Kullanılabilirlik alanı | Esnek yük dengeleyici oluşturmak için **bölge yedekli** ' ı seçin. </br> Bir ZGen yük dengeleyici oluşturmak için, 1, 2 veya 3 ' ten belirli bir bölge seçin |
...
3. **Arka uç Havuzu Ekle** sayfasında, ad için, arka uç havuzunuzun adı olarak **mybackendpool** girin ve ardından **Ekle**' yi seçin.
...
2. **Sanal makine oluştur**' da **temel bilgiler** sekmesinde değerleri yazın veya seçin:
    ...
    | **Proje ayrıntıları** |  |
    | Abonelik | Azure aboneliğinizi seçin |
    | Kaynak Grubu | **Tutorintlbnat-RG** seçin |
    | **Örnek ayrıntıları** |  |
...
2. Genel IP adresini bir yere getirin:
    ...
    :::image type="content" source="./media/tutorial-nat-gateway-load-balancer-internal-portal/find-public-ip.png" alt-text="NAT ağ geçidinin genel IP adresini bulma ekran görüntüsü." border="true":::

3. Sol taraftaki menüden **tüm hizmetler** ' i seçin, **tüm kaynaklar**' ı seçin ve ardından kaynaklar listesinden, **Tutorintlbnat-RG** kaynak grubunda bulunan **myVM1** ' yi seçin.
...
9. Görünen IP adresinin, önceki adımda not ettiğiniz NAT ağ geçidi adresiyle eşleştiğini doğrulayın:
    ...
    :::image type="content" source="./media/tutorial-nat-gateway-load-balancer-internal-portal/my-ip.png" alt-text="Dış giden IP 'yi gösteren Internet Explorer 'ın ekran görüntüsü." border="true":::
...
2. **Tutorintlbnat-RG** kaynak grubunu seçin.

3. **Kaynak grubunu sil**'i seçin.

4. **Tutorintlbnat-RG** girin ve **Sil**' i seçin.
