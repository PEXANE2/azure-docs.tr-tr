...
description: Bu konuda, veritabanı işlemlerinin toplu işleme, Azure SQL veritabanınızın ve Azure SQL yönetilen örnek uygulamalarınızın hızını ve ölçeklenebilirliğini büyük ölçüde geliştirmekte olan kanıt sağlar. Bu toplu işlem teknikleri herhangi bir SQL veritabanı için çalışsa da, makalenin konusu Azure 'da yer alır.
...
Bu makalede çeşitli toplu işlem stratejilerini ve senaryolarını incelemek istiyoruz. Bu stratejiler SQL Server kullanan şirket içi uygulamalar için de önemli olsa da, Azure SQL veritabanı ve Azure SQL yönetilen örneği için toplu işlem kullanımını vurgulamanın çeşitli nedenleri vardır:

* Azure SQL veritabanı 'na ve Azure SQL yönetilen örneğine erişirken, özellikle de aynı Microsoft Azure veri merkezi dışından Azure SQL veritabanı veya Azure SQL yönetilen örneği 'ne erişirken büyük olasılıkla daha fazla ağ gecikmesi vardır.
* Azure SQL veritabanı ve Azure SQL yönetilen örneği 'nin çok kiracılı özellikleri, veri erişim katmanının verimliliğinin veritabanının genel ölçeklenebilirliğiyle ilişkili olduğu anlamına gelir. Önceden tanımlanmış kotaların fazla olması halinde, Azure SQL veritabanı ve Azure SQL yönetilen örneği, performansı azaltabilir veya azaltma özel durumlarıyla yanıt verebilir. Toplu işlem gibi verimlilik, bu sınırlara ulaşmadan önce daha fazla çalışma yapabilmeniz için bir işlem sağlar.
...
Azure SQL veritabanı veya Azure SQL yönetilen örneği kullanmanın avantajlarından biri, veritabanını barındıran sunucuları yönetmeniz gerekmez. Ancak, bu yönetilen altyapı ayrıca veritabanı iyileştirmeleri hakkında farklı düşünmeniz gereken anlamına gelir. Artık veritabanı donanımını veya ağ altyapısını geliştirmek için göz atabilirsiniz. Microsoft Azure bu ortamları denetler. Denetleyebilmeniz gereken ana alan, uygulamanızın Azure SQL veritabanı ve Azure SQL yönetilen örneği ile nasıl etkileşime gidir. Toplu işleme, bu iyileştirmelerin biridir.

Bu makalenin ilk bölümü, Azure SQL veritabanı veya Azure SQL yönetilen örneği kullanan .NET uygulamalarına yönelik çeşitli toplu işlem tekniklerini inceler. Son iki bölüm toplu işlem kılavuzlarını ve senaryolarını kapsar.
...
ADO.NET içindeki işlemler hakkında daha fazla bilgi için bkz. [ADO.net Içindeki yerel işlemler](/dotnet/framework/data/adonet/local-transactions).
...
Tablo değerli parametreler hakkında daha fazla bilgi için bkz. [tablo değerli parametreler](/sql/relational-databases/tables/use-table-valued-parameters-database-engine).
...
ADO.NET ' de toplu kopyalama hakkında daha fazla bilgi için bkz. [toplu kopyalama işlemleri](/dotnet/framework/data/adonet/sql/bulk-copy-operations-in-sql-server).
...
Aşağıdaki bölümlerde, Azure SQL veritabanı ve Azure SQL yönetilen örnek uygulamalarında toplu işlem kullanımı için daha fazla rehberlik sağlanmaktadır.
...
Bu örnekte, yerel olarak tanımlanan @IdentityLink tablo, yeni yerleştirilen satırlardan gerçek OrderID değerlerini depolar. Bu sıra tanımlayıcıları, @orders ve tablo değerli parametrelerde geçici OrderID değerlerinden farklıdır @details . Bu nedenle, tablo, @IdentityLink tablodaki ordervalues değerlerini, @orders PurchaseOrder tablosundaki yeni satırların gerçek OrderID değerlerine bağlar. Bu adımdan sonra tablo, @IdentityLink yabancı anahtar kısıtlamasını karşılayan gerçek OrderID ile sipariş ayrıntılarının eklenmesi işlemini kolaylaştırabilir.
...
Sonra, bir saklı yordam oluşturun veya Update ve INSERT işlemini gerçekleştirmek için MERGE ifadesini kullanan kodu yazın. Aşağıdaki örnek, EmployeeTableType türünde tablo değerli bir parametre üzerinde MERGE ifadesini kullanır @employees . @employeesTablonun içeriği burada gösterilmez.
...
* Azure SQL veritabanı ve Azure SQL yönetilen örnek uygulamalarının performansını ve ölçeklenebilirliğini artırmak için arabelleğe alma ve toplu işleme ' yı kullanın.
...
* Toplu işlem boyut yönergeleri:
  ...
  * Azure SQL veritabanı veya Azure SQL yönetilen örneğinin bunu reddetmediğini doğrulamak için en büyük toplu iş boyutunu test edin.
...
Bu makalede, toplu işleme ile ilgili veritabanı tasarımı ve kodlama teknikleri, uygulama performansınızı ve ölçeklenebilirliğini nasıl iyileştirebileceğinizi yoğunlaşmaktadır. Ancak bu, genel stratejinizde yalnızca bir faktördür. Performansı ve ölçeklenebilirliği artırmanın daha fazla yolu için bkz. elastik havuz için [veritabanı performans Kılavuzu](database/performance-guidance.md) ve [Fiyat ve performans konuları](database/elastic-pool-overview.md).
