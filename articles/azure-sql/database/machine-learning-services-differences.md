...
- R sürümü 3.4.4 ' dir.
- Aracılığıyla yapılandırmaya gerek yoktur `external scripts enabled` `sp_configure` .
...
Azure SQL veritabanı 'nda RODBC kullanıyorsanız, bunun oturum içinde mi yoksa dışında mı oluşturulduğuna bakılmaksızın geçici bir tabloya yazamıyoruz `sp_execute_external_script` . Geçici çözüm olarak, [Rxodbcdata](https://docs.microsoft.com/machine-learning-server/r-reference/revoscaler/rxodbcdata) ve [Rxdatastep](https://docs.microsoft.com/machine-learning-server/r-reference/revoscaler/rxdatastep) (üzerine yazma = false ve Append = "Rows") kullanılarak sorgu öncesinde oluşturulan genel geçici bir tabloya yazılır `sp_execute_external_script` .
...
- Bkz. genel bakış, [Azure SQL veritabanı Machine Learning Services R (Önizleme)](machine-learning-services-overview.md).
