...
1. Tüm R betiğini [sp_execute_external_script](https://docs.microsoft.com/sql/relational-databases/system-stored-procedures/sp-execute-external-script-transact-sql) saklı yordamına geçirin.
   ...
   <br>**herhangi BIR dış BETIĞI yürütmeye verin** *\<username\>* .
...
| @input_data_1 | veri çerçevesi olarak verileri döndüren R çalışma zamanına geçilen sorgu tarafından döndürülen veriler |
|SONUÇ KÜMELERIYLE | yan tümcesi döndürülen veri tablosunun şemasını tanımlar, veri türü olarak "Merhaba Dünya" sütun adı, **int** olarak ekler |
...
1. `SELECT`Tabloyu sorgulamak için ifadesini kullanın.
    ...
    **Sonucunun**
    ...
    ![RTestData tablosunun içeriği](./media/r-script-create-quickstart/select-rtestdata.png)

1. Aşağıdaki R betiğini çalıştırın. Deyimini kullanarak tablodaki verileri alır `SELECT` , R çalışma zamanına geçirir ve verileri veri çerçevesi olarak döndürür. `WITH RESULT SETS`Yan TÜMCE SQL veritabanı için döndürülen veri tablosunun şemasını tanımlar, *newcolname*sütun adını ekler.
    ...
    **Sonucunun**
    ...
    ![Tablodan veri döndüren R betiğinin çıktısı](./media/r-script-create-quickstart/r-output-rtestdata.png)
...
1. Şimdi giriş ve çıkış değişkenlerinin adlarını değiştirelim. Varsayılan giriş ve çıkış değişkeni adları **ınputdataset** ve **outputdataset**, bu betik adları **SQL_in** ve **SQL_out**olarak değiştirir:
    ...
    R 'nin büyük/küçük harfe duyarlı olduğunu unutmayın. R betikte (**SQL_out**, **SQL_in**) kullanılan giriş ve çıkış değişkenlerinin, ve ile tanımlanan değerlerle eşleşmesi gerekir `@input_data_1_name` `@output_data_1_name` .
...
1. Ayrıca, giriş verisi olmayan ( `@input_data_1` boş olarak ayarlanır) R betiği kullanarak yalnızca değerler oluşturabilirsiniz.
    ...
    **Sonucunun**
    ...
    ![Giriş olarak @script kullanarak sonuçları sorgulama](./media/r-script-create-quickstart/r-data-generated-output.png)
...
Çıktı `installed.packages()` R 'den ve sonuç kümesi olarak döndürülür.

**Sonucunun**

![R içindeki yüklü paketler](./media/r-script-create-quickstart/r-installed-packages.png)
...
> [Azure SQL veritabanı Machine Learning Services (Önizleme) ile R 'de tahmine dayalı bir model oluşturma ve eğitme](r-train-score-model-create-quickstart.md)

R (Önizleme) ile Azure SQL veritabanı Machine Learning Services hakkında daha fazla bilgi için aşağıdaki makalelere bakın.

- [R ile Azure SQL veritabanı Machine Learning Services (Önizleme)](machine-learning-services-overview.md)
...
- [Azure SQL veritabanı 'nda R ve SQL verileriyle çalışma Machine Learning Services (Önizleme)](machine-learning-services-data-issues.md)
