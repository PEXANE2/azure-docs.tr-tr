...
> [!TIP]
> Birçok veri kümesi R çalışma zamanına dahildir, yüklü veri kümelerinin listesini almak için `library(help="datasets")` r komut isteminden yazın.
...
1. Kullanarak regresyon modeli oluşturun `rxLinMod` . 
...
![Ek çıktıya sahip eğitilen model](./media/r-train-score-model-create-quickstart/r-train-model-with-additional-output.png)
...
2. Bu yeni hız değerlerinden uzaklığı durdurmayı tahmin edin.
   ...
   Modeliniz **, geri alma paketinin bir** parçası olarak sunulan **Rxlinmod** algoritmasını temel aldığı için, genel R işlevi yerine [rxınpredıal](https://docs.microsoft.com/machine-learning-server/r-reference/revoscaler/rxpredict) işlevini çağırın `predict` .
   ...
   - `unserialize`Modeldeki işlevi çağırır
   - `rxPredict`Uygun bağımsız değişkenlerle işlevi modele uygular
   ...
   **Sonucunun**
   ...
   ![Durdurma mesafesini tahmin etmek için sonuç kümesi](./media/r-train-score-model-create-quickstart/r-predict-stopping-distance-resultset.png)
