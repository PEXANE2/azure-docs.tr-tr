...
Yanıt genellikle R komutu kullanılarak bulunabilir `str()` . `str(object_name)`Belirtilen r nesnesinin veri şemasına bir bilgi iletisi olarak döndürüldüğünden, bu Işlevi R betiğinizde herhangi bir yere ekleyin. İletileri SSMS 'deki **iletiler** sekmesinde görüntüleyebilirsiniz.

Örnek 1 ve örnek 2 ' nin bu tür farklı sonuçlara neden olduğunu anlamak için, aşağıdaki `str(OutputDataSet)` `@script` gibi her bir ifadede değişken tanımının sonuna çizgiyi ekleyin:
...
> [!TIP]
> `is.matrix` `is.vector` İç veri yapısı hakkında bilgi döndürmek için, gibi R Identity işlevlerini de kullanabilirsiniz.
...
Kapsıyorsa, üç değerin sütunu tek sütunlu bir matrise dönüştürülür. Matris yalnızca R 'deki bir dizinin özel bir durumu olduğundan, `y` iki bağımsız değişkeni uyumlu hale getirmek için dizi tek sütunlu bir matriste örtük olarak zorlanır.
...
Ancak, dizinin boyutunu değiştirdiğinizde ne olacağını aklınızda görürsünüz `y` .
...
Örneğin, aşağıdaki betik 6 uzunluğunda bir sayısal diziyi tanımlar ve R değişkeninde depolar `df1` . Sayısal dizi daha sonra, yeni bir veri çerçevesini oluşturmak için üç (3) değer içeren RTestData tablosunun (yukarıda oluşturulan) tamsayılarla birleştirilir `df2` .
...
R, veri çerçevesini doldurmak için, RTestData 'dan alınan öğeleri dizideki öğelerin sayısıyla eşleştirmek için gereken sayıda yineler `df1` .
...
Sorguyu çalıştıktan sonra, `str` R 'nin giriş verilerini nasıl değerlendirmiş olduğunu görmek için işlevin sonuçlarını gözden geçirin.
...
- Giriş veri kaynağınızdaki sütunları kullanmak yerine ayrı ayrı belirtin `SELECT *` ve her sütunun nasıl işleneceğini öğrenin.
