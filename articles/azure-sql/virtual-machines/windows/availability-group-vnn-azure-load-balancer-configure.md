---
title: AG VNN dinleyicisi için yük dengeleyiciyi yapılandırma
description: Yüksek kullanılabilirlik ve olağanüstü durum kurtarma (HADR) için Azure VM 'lerinde SQL Server, kullanılabilirlik grubunuz için sanal ağ adı (VNN) dinleyicisine trafik yönlendirmek üzere bir Azure Load Balancer yapılandırmayı öğrenin.
...
Bu makale, Azure Load Balancer hizmetini kullanarak bir yük dengeleyici yapılandırmayı öğretir. Yük dengeleyici, yüksek kullanılabilirlik ve olağanüstü durum kurtarma (HADR) için Azure VM 'lerinde SQL Server, trafiği [kullanılabilirlik grubunuz (AG) dinleyicinize](availability-group-overview.md) yönlendirir. 

SQL Server 2019 CU8 ve sonraki sürümlerde bulunan müşterilere alternatif bir bağlantı seçeneği için, Basitleştirilmiş yapılandırma ve gelişmiş yük devretme için bir [DNN dinleyicisi](availability-group-vnn-azure-load-balancer-configure.md) düşünün.  
...
- [Kullanılabilirlik grubu dinleyiciniz](availability-group-overview.md)yapılandırıldı.
...
1. Yük Dengeleme kuralı parametrelerini ayarlayın:
   ...
   - **Ön uç IP adresi**: FCSQL Server IP adresı veya AG dinleyicisinin kümelenmiş ağ kaynağı.
