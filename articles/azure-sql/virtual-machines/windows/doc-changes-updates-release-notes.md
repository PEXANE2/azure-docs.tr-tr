...
| **Toplu kaynak sağlayıcısı kaydı** | Artık SQL sanal makinelerini kaynak sağlayıcıyla [toplu olarak kaydedebilirsiniz](sql-vm-resource-provider-bulk-register.md) . | 
...
| **Azure adanmış ana bilgisayar** | SQL Server VM [Azure adanmış bir konakta](dedicated-host.md)çalıştırabilirsiniz. | 
| **SQL VM 'yi farklı bir bölgeye taşıma** | [SQL Server VM bir bölgeden diğerine geçirmek](move-sql-vm-different-region.md)için Azure Site Recovery kullanın. |
...
| **Özel görüntü desteklenebilirliği** | Artık [SQL Server IaaS uzantısını](sql-server-iaas-agent-extension-automate-management.md#installation) özel işletim SISTEMI ve SQL yansımalarına yükleyebilirsiniz ve bu da [Esnek lisansın](licensing-model-azure-hybrid-benefit-ahb-change.md)sınırlı işlevlerini sunar. Özel görüntünüzü SQL kaynak sağlayıcısı ile kaydederken, lisans türünü "AHUB" olarak belirtin. Aksi takdirde, kayıt başarısız olur. | 
...
* [SQL Server Linux sanal makinesi sağlama](../linux/sql-vm-create-portal-quickstart.md)
