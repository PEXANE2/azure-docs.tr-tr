...
Premium dosya paylaşımları, birçok iş yükünün ihtiyaçlarını karşılayacak ıOPS ve aktarım hızı kapasitesi sağlar. GÇ yoğun iş yükleri için, yönetilen Premium disklere veya ultra disklere göre [SQL Server yük devretme kümesi örneklerini depolama alanları doğrudan ile](failover-cluster-instance-storage-spaces-direct-manually-configure.md)düşünün.  
...
Azure VM 'de SQL Server Kullandıkça öde ve KLG lisanslamayı karşılaştırmak için bkz. [SQL Server VM 'leri kullanmaya başlama](sql-server-on-azure-vm-iaas-what-is-overview.md#get-started-with-sql-server-vms).
...
- SQL Server hizmetini çalıştırmak için bir etki alanı kullanıcı hesabı ve dosya paylaşımından bağlama sırasında sanal makinede oturum açmak için kullanabilirsiniz. 
...
1. [Azure kullanılabilirlik kümesi oluşturun](../../../virtual-machines/linux/tutorial-availability-sets.md).
...
1. Kullanılabilirlik kümesinde sanal makineleri oluşturun.
   ...
   Azure kullanılabilirlik kümesinde iki SQL Server sanal makine sağlayın. Yönergeler için bkz. [Azure portal SQL Server sanal makinesi sağlama](create-sql-vm-portal.md).
   ...
   Azure Marketi 'nden bir görüntü seçin. Windows Server ve SQL Server içeren bir Azure Market görüntüsü kullanabilir veya yalnızca Windows Server içeren bir tane kullanabilirsiniz. Ayrıntılar için bkz. [Azure sanal makinelerinde SQL Server genel bakış](sql-server-on-azure-vm-iaas-what-is-overview.md).
...
1. Azure sanal makinelerinizi oluşturduktan sonra, Uzak Masaüstü Protokolü (RDP) kullanarak her birine bağlanın.

   RDP ile bir sanal makineye ilk kez bağlandığınızda, bir istem, BILGISAYARıN ağda keşfedilmesini sağlamak isteyip istemediğinizi sorar. **Evet**' i seçin.
...
1. SQL Server tabanlı sanal makine görüntülerinden birini kullanıyorsanız, SQL Server örneğini kaldırın.
   ...
   1. **Veritabanı motoru Hizmetleri**altındaki tüm özellikleri kaldırın. **Paylaşılan özellikleri**kaldırmayın. Aşağıdaki ekran görüntüsüne benzer bir şey göreceksiniz:
        ...
        ![Özellik seç](./media/failover-cluster-instance-premium-file-share-manually-configure/03-remove-features.png)
...
1. [Sanal makineleri önceden var olan etki alanına ekleyin](availability-group-manually-configure-prerequisites-tutorial.md#joinDomain).
...
1. Açılan listeden kullanmak istediğiniz sürücü harfini seçin ve ardından her iki kod bloğunu Not defteri 'ne kopyalayın.
   ...
   :::image type="content" source="media/manually-configure-failover-cluster-instance-premium-file-share/premium-file-storage-commands.png" alt-text="Her iki PowerShell komutunu da dosya paylaşımının Connect portalından Kopyala":::
...
1. Dosya Gezgini 'ni veya **Çalıştır** iletişim kutusunu (Windows logosu tuşu + r) kullanarak paylaşıma gidin. Ağ yolunu kullanın `\\storageaccountname.file.core.windows.net\filesharename` . Örneğin, `\\sqlvmstorageaccount.file.core.windows.net\sqlpremiumfileshare`
...
Şimdi yük devretme kümesini yapılandırırsınız. Bu bölümde, aşağıdaki adımları tamamlayadınız:
...
1. **Test seçimi**altında, **depolama** ve **depolama alanları doğrudan**dışındaki tüm testleri aşağıda gösterildiği gibi seçin:
   ...
   :::image type="content" source="media/manually-configure-failover-cluster-instance-premium-file-share/cluster-validation.png" alt-text="Küme doğrulama testlerini seçin":::
...
### <a name="create-a-cloud-witness-win-2016-and-later"></a>Bulut tanığı oluşturma (Win 2016 ve üzeri)

Windows Server 2016 ve sonraki bir sürümü kullanıyorsanız, bir bulut tanığı oluşturmanız gerekecektir. Bulut tanığı, bir Azure Depolama Blobu içinde depolanan yeni bir küme çekirdeği tanığı türüdür. Bu, bir tanık paylaşımının barındırıp veya ayrı bir dosya paylaşımının kullanıldığı ayrı bir VM gereksinimini ortadan kaldırır.
...
Kümenizin yük devretmesini test edin. **Yük devretme kümesi Yöneticisi**, kümenize sağ tıklayıp **daha fazla eylem**  >  **çekirdek küme kaynağı**  >  **seçin düğüm**Seç ' i seçin ve ardından kümenin diğer düğümünü seçin. Çekirdek küme kaynağını kümenin her düğümüne taşıyın ve ardından birincil düğüme geri taşıyın. Kümeyi her düğüme başarıyla taşıyabiliyorsanız SQL Server yüklemeye hazırsınız demektir.  

:::image type="content" source="media/manually-configure-failover-cluster-instance-premium-file-share/test-cluster-failover.png" alt-text="Çekirdek kaynağı diğer düğümlere taşıyarak küme yük devretmesini test etme":::
...
1. **Yeni SQL Server yük devretme kümesi yüklemesi ' ni**seçin. SQL Server FCı 'yi yüklemek için sihirbazdaki yönergeleri izleyin.
   ...
   :::image type="content" source="media/manually-configure-failover-cluster-instance-premium-file-share/use-file-share-as-data-directories.png" alt-text="Dosya paylaşımının SQL veri dizinleri olarak kullanılması":::
...
Daha fazla bilgi için bkz. [Azure yük dengeleyici oluşturma ve yapılandırma](availability-group-manually-configure-tutorial.md#configure-internal-load-balancer).
...
1. **Sistem durumu araştırması Ekle** dikey penceresinde aşağıdaki <span id="probe"> </span> sistem durumu araştırma parametrelerini ayarlayın.
   ...
   - **Protokol**: TCP.
   - **Bağlantı noktası**: [Bu adımdaki](#ports)sistem durumu araştırması için güvenlik duvarında oluşturduğunuz bağlantı noktası. Bu makalede, örnek TCP bağlantı noktasını kullanır `59999` .
   - **Aralık**: 5 saniye.
...
Aşağıdaki listede, güncelleştirmeniz gereken değerler açıklanmaktadır:
   ...
   - `<SQL Server FCI IP Address Resource Name>`: SQL Server FCı IP adresi kaynak adı. **Yük devretme kümesi Yöneticisi**  >  **Roller**' de, SQL Server FCI rolü altında, **sunucu adı**altında, IP adresi kaynağına sağ tıklayın ve **Özellikler**' i seçin. Doğru değer **genel** sekmesinde **ad** ' ın altında bulunur.

   - `<ILBIP>`: ILB IP adresi. Bu adres, Azure portal ıLB ön uç adresi olarak yapılandırılır. Bu ayrıca SQL Server FCı IP adresidir. **Yük devretme kümesi Yöneticisi** içinde bulduğunuz aynı Özellikler sayfasında bulabilirsiniz `<SQL Server FCI IP Address Resource Name>` .  
