---
title: Azure Information Protection verilerini Azure Sentinel önizlemesine bağlama | Microsoft Docs
...
- Kiracınız için aşağıdaki Azure AD yönetici rollerinden biri: Yönetici, güvenlik yöneticisi veya genel yönetici Azure Information Protection.
    
    > [!NOTE]
    > Kiracınız [Birleşik etiketleme platformundan](https://docs.microsoft.com/azure/information-protection/faqs#how-can-i-determine-if-my-tenant-is-on-the-unified-labeling-platform)ise Azure Information Protection yönetici rolünü kullanamazsınız.

- Sentinel ve Azure Information Protection için kullanmakta olduğunuz Log Analytics çalışma alanına okuma ve yazma izinleri.
...
1. Azure Sentinel 'de **veri bağlayıcıları**' nı seçin ve ardından **Azure Information Protection**.

2. **Azure Information Protection** dikey penceresinde **bağlayıcı sayfasını aç**' ı seçin.

3. Sonraki dikey pencerede, **yapılandırma** bölümünde **Azure Information Protection** ' yi seçerek **Azure Information Protection analiz**' e gidin.

4. Kullanılabilir çalışma alanları listesinden, şu anda Azure Sentinel için kullanmakta olduğunuz çalışma alanını seçin. Farklı bir çalışma alanı seçerseniz Azure Information Protection raporlama verileri Azure Sentinel tarafından kullanılamaz.

5. Bir çalışma alanı seçtiğinizde **Tamam** ' ı seçin ve bağlayıcı **durumu** şimdi **bağlı**olarak değişir.
