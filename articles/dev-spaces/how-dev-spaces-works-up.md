...
Kodunuzu bir geliştirme alanında çalıştırmak için, `up` `azds.yaml` dosyanız ile aynı dizinde komutunu verin:
...
1. Kullanarak http://localhostuygulama uç noktanıza erişim sağlamak için *bağlantı noktası iletme* kullanır.
...
Daha ayrıntılı bir düzeyde, şunu çalıştırdığınızda `azds up`ne olur:
...
Azure Dev Spaces, `azds.yaml` hizmetinizi yüklemek ve yapılandırmak için dosyasını kullanır. Denetleyici, helk `install` grafiğini yüklemek ve `azds.yaml` Kubernetes nesnelerini oluşturmak için dosyadaki özelliğini kullanır:
...
Hele grafiklerini yüklerken Azure Dev Spaces Helu grafiğindeki değerleri geçersiz kılmak için bir yol sağlar. Held grafiğinin varsayılan değerleri içinde `charts/APP_NAME/values.yaml`bulunur.
...
Oluşturulan HELI grafiğinde kapsayıcı görüntüsü *{{olarak ayarlanır. Values. Image. Repository}}: {{. Values. Image. Tag}}*. Dosya, *install. set. Image. Tag* özelliğini varsayılan olarak *$ (Tag)* olarak tanımlar; bu, {{için değer olarak kullanılır. `azds.yaml` * Values. Image. Tag}}*. *Install. set. Image. Tag* özelliğini bu şekilde ayarlayarak, uygulamanızın kapsayıcı görüntüsünün Azure dev Spaces çalıştırılırken farklı bir şekilde etiketlenmesine izin verir. Bu özel durumda görüntü, * \<Image. Repository>: $ (Tag) öğesinden değer*olarak etiketlenir. Geliştirme alanları tanıması ve AKS kümesindeki kapsayıcıyı bulmak için, *$ (Tag)* değişkenini *Install. set. image. Tag* değeri olarak kullanmanız gerekir.
...
Geliştirme sırasında hızlıca yinelemenize yardımcı olmak için Azure Dev Spaces değişiklikleri yerel projenizden eşitler ve uygulamanızı artımlı olarak güncelleştirir. Eşitleme ve güncelleştirmeyi yapılandırmak için `azds.yaml` yapılandırma dosyasının aşağıdaki bölümü kullanılır:
...
`azds prep` Komutunu kullanarak kodunuzu hazırlarken `--enable-ingress` bayrağını ekleme seçeneğiniz vardır. Bayrağı eklemek `--enable-ingress` , uygulamanız için herkese açık olarak ERIŞILEBILEN bir URL oluşturur. Bu bayrağı atlarsanız, uygulamaya yalnızca küme içinden veya localhost tüneli kullanılarak erişilebilir. Komutu çalıştırdıktan sonra bu ayarı değiştirmek için içindeki giriş `charts/APPNAME/values.yaml` *. Enabled* özelliğini değiştirme: `azds prep`
...
Hızlı yineleme ve geliştirme için Azure Dev Spaces kullanma hakkında daha fazla bilgi edinmek için, bkz. [geliştirme bilgisayarınızı geliştirme alanınıza bağlama][how-it-works-connect] ve [Azure dev Spaces ile kodunuzda uzaktan hata ayıklama][how-it-works-remote-debugging]nasıl yapılır?.
