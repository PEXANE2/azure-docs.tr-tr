...
| Subscription | Azure Data Lake Storage hesabınızı içeren abonelik. |
...
> [!NOTE]
> Azure SQL veritabanı teklifi, Stream Analytics bir iş çıkışı için desteklenir, ancak SQL Server çalıştıran bir veritabanı ile veya SQL Azure yönetilen bir örneğe eklenmiş bir Azure sanal makinesi henüz desteklenmiyor. Gelecekteki sürümlerde değişebilir budur.
...
| Olay hub'ı ilke adı | Olay Hub 'ının **Yapılandır** sekmesinde oluşturabileceğiniz paylaşılan erişim ilkesi. Her paylaşılan erişim ilkesinin bir adı ayarlayın ve erişim anahtarları izinleri vardır. |
...
Satırlarda dinamik şema güncelleştirmesini önlemek için sorgukullanmaktankaçının.`SELECT *` Olası performans etkilerine ek olarak, sonuçlar için geçen sürenin açıklanmasına neden olabilir. Power BI panosunda gösterilmesi gereken tam alanları seçin. Ayrıca, veri değerleri, seçilen veri türüyle uyumlu olmalıdır.
...
| Kuyruk ilkesi adı |Bir kuyruk oluşturduğunuzda, sıranın **Yapılandır** sekmesinde paylaşılan erişim ilkeleri de oluşturabilirsiniz. Her paylaşılan erişim ilkesinin bir adı ayarlayın ve erişim anahtarları izinleri vardır. |
...
| Konu ilkesi adı |Bir Service Bus konu oluşturduğunuzda, konunun **Yapılandır** sekmesinde paylaşılan erişim ilkeleri de oluşturabilirsiniz. Her paylaşılan erişim ilkesinin bir adı ayarlayın ve erişim anahtarları izinleri vardır. |
...
| Azure SQL Database | En fazla toplu iş sayısı kullanılarak yapılandırılabilir. Varsayılan olarak, tek toplu ekleme başına en fazla 10.000 ve 100 en az satır.<br />Bkz. [Azure SQL sınırları](../sql-database/sql-database-resource-limits.md). |  Her toplu iş ilk olarak en fazla toplu iş sayısı ile toplu olarak eklenir. Batch, SQL 'deki yeniden denenebilir hatalara göre yarı (en az toplu işlem sayısına kadar) yarıya bölünür. |
