...
Hizmet tanımı dosyasının `LoadBalancerProbes` öğesi aşağıdaki öğeleri içerir:
...
| `protocol`          | `string` | Gereklidir. Bitiş noktasının protokolünü belirtir. Olası değerler: `http` veya `tcp`. `tcp` belirtilirse, araştırmanın başarılı olması için alınan bir ACK gereklidir. `http` belirtilirse, araştırmanın başarılı olması için belirtilen URI 'den 200 bir Tamam yanıtı gerekir.|
| `path`              | `string` | VM 'den sistem durumu istemek için kullanılan URI. `protocol` `http`olarak ayarlandıysa `path` gerekir. Aksi takdirde, buna izin verilmez.<br /><br /> Varsayılan değer yoktur.|
