...
Bu düzen, hata etki alanı kısıtlamasının "en yüksek fark" garantisi tanımımızı ihlal ediyor. FD0 'in iki çoğaltması vardır, ancak FD1 sıfır içerir. FD0 ve FD1 arasındaki fark, en yüksek fark değerinden büyük olan bir toplam ikdir. Kısıtlama ihlal edildiğinden, Küme Kaynak Yöneticisi bu düzenlemeye izin vermez.

Benzer şekilde, N2 ve N6 (N1 ve N2 yerine) aldıysanız şunları alacaksınız:
...
## <a name="node-buffer-and-overbooking-capacity"></a>Düğüm arabelleği ve fazla kayıt kapasitesi
...
Örneğin, bir düğüm, ölçüm *Cpukullanımı* için belirtilen kapasiteye sahipse ve bu ölçüm için düğüm arabelleği yüzdesi %20 olarak ayarlanırsa, toplam ve olmayan kapasite sırasıyla 100 ve 80 olur ve küme kaynak yöneticisi, normal durumlar sırasında düğüm üzerine 100 ' 80 den fazla yük birimi yerleştirmez.

![Toplam kapasite eşit düğüm kapasitesi (düğüm arabelleği + arabelleğe alınmamış)](./media/service-fabric-cluster-resource-manager-cluster-description/node-capacity.png)

Düğüm kapasitesinin yalnızca yukarıda bahsedilen hizmet kullanılabilirliğini artıran eylemler için kullanılacak bir bölümünü ayırmak istediğinizde, düğüm arabelleği kullanılmalıdır.

Öte yandan, düğüm fazla kayıt yüzdesi kullanılıyorsa ve %20 olarak ayarlandıysa toplam ve arabelleğe alınmamış kapasiteler sırasıyla 120 ve 100 olacaktır.

![Toplam kapasite fazla kayıt kapasitesi ve düğüm kapasitesi (fazla kayıt + arabelleğe alınmamış)](./media/service-fabric-cluster-resource-manager-cluster-description/node-capacity-with-overbooking.png)

Küme Kaynak Yöneticisi, Toplam kaynak kullanımı kapasiteyi aşacağından bile, bir düğüme çoğaltmalar yerleştirmelerini sağlamak istediğinizde, aşırı kayıt kapasitesi kullanılmalıdır. Bu, performans masrafına hizmetler için ek kullanılabilirlik sağlamak üzere kullanılabilir. Fazla kayıt kullanılırsa, Kullanıcı uygulaması mantığının gerektirdiğinden daha az sayıda fiziksel kaynakla çalışabilmesi gerekir.

Düğüm arabelleği veya fazla kayıt kapasitesi belirtilmişse, hedef düğüm üzerindeki toplam yük toplam kapasiteden (düğüm arabelleği ve düğüm kapasitesi ve fazla kayıt durumunda fazla kayıt kapasitesi olması durumunda düğüm kapasitesi), Küme Kaynak Yöneticisi çoğaltmaları taşımaz veya yerleştirmeyecektir.

Fazla kayıt kapasitesi, sınırsız olacak şekilde de belirlenebilir. Bu durumda, Küme Kaynak Yöneticisi, belirtilen düğüm kapasitesinin altındaki düğüm üzerindeki toplam yükü tutmaya çalışır, ancak önemli performans düşüşüne yol açabilecek şekilde düğüme çok daha fazla yük yerleştirebilir.

Bir metriğin aynı anda hem düğüm arabelleği hem de fazla kayıt kapasitesi belirtilemez.

*ClusterManifest.xml*içinde düğüm arabelleğinin veya fazla kayıt kapasitelerinin nasıl belirtilme örneği aşağıda verilmiştir:
