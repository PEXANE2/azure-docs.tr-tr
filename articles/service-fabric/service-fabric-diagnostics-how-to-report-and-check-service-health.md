...
* [Bölüm](https://docs.microsoft.com/dotnet/api/system.fabric.istatefulservicepartition) veya [Codepackageactivationcontext](https://docs.microsoft.com/dotnet/api/system.fabric.codepackageactivationcontext) nesneleri kullanın.  
  Geçerli bağlamın parçası olan öğelerin sistem durumunu raporlamak için `Partition` ve `CodePackageActivationContext` nesnelerini kullanabilirsiniz. Örneğin, bir çoğaltmanın parçası olarak çalışan kod yalnızca o çoğaltma üzerinde sistem durumunu, ait olduğu bölümü ve bir parçası olan uygulamayı rapor edebilir.
* `FabricClient`kullanın.   
  Küme [güvenli](service-fabric-cluster-security.md) değilse veya hizmet yönetici ayrıcalıklarıyla çalışıyorsa, hizmet kodundan sistem durumunu raporlamak için `FabricClient` kullanabilirsiniz. En gerçek dünyada senaryolar güvenli olmayan kümeler kullanmaz veya yönetici ayrıcalıkları sağlamaz. `FabricClient`ile, kümenin bir parçası olan herhangi bir varlıkta sistem durumunu rapor edebilirsiniz. Bununla birlikte, hizmet kodu yalnızca kendi sistem durumuyla ilgili raporları göndermelidir.
...
1. Ayrıca, PowerShell kullanarak sistem durumunu kontrol edebilirsiniz. Bir uygulamanın sistem durumunu denetlemek için ```Get-ServiceFabricApplicationHealth``` kullanabilir ve bir hizmetin sistem durumunu denetlemek için ```Get-ServiceFabricServiceHealth``` kullanabilirsiniz. Bu görüntüde, PowerShell 'deki aynı uygulama için sistem durumu raporu bulunur.
...
1. Stateful1.cs dosyasını açın ve `RunAsync` yönteminde `myDictionary.TryGetValueAsync` çağrısını bulun. Bu yöntemin, bu uygulamadaki anahtar mantığı bir sayıyı çalışır durumda tutacağından, bu yöntemin sayacın geçerli değerini tutan bir `result` döndürdüğünü görebilirsiniz. Bu uygulama gerçek bir uygulamadır ve sonucun bulunmaması bir hatayı gösteriyorsa, bu olayı işaretlemek istersiniz.
1. Sonucun olmaması bir başarısızlığı temsil ettiğinde bir sistem durumu olayı raporlamak için aşağıdaki adımları ekleyin.
   
    a. `System.Fabric.Health` ad alanını Stateful1.cs dosyasına ekleyin.
   
    ```csharp
    using System.Fabric.Health;
    ```
   
    b. `myDictionary.TryGetValueAsync` çağrısından sonra aşağıdaki kodu ekleyin
    ...
    ```
1. Hizmetiniz yönetici ayrıcalıklarıyla çalışıyorsa veya küme [güvenli](service-fabric-cluster-security.md)değilse, durumu aşağıdaki adımlarda gösterildiği gibi raporlamak için `FabricClient` de kullanabilirsiniz.  
   
    a. `var myDictionary` bildiriminden sonra `FabricClient` örneğini oluşturun.
...
1. Bu hatanın benzetimini yapmanızı sağlar ve sistem durumu izleme araçlarında göster bölümüne bakın. Hatanın benzetimini yapmak için, daha önce eklediğiniz sistem durumu raporlama kodundaki ilk satırı açıklama olarak değerlendirin. İlk satırı yorumladıktan sonra, kod aşağıdaki örneğe benzer şekilde görünür.
    ...
    ```
   Bu kod, `RunAsync` her yürütüldüğünde sistem durumu raporunu tetikler. Değişikliği yaptıktan sonra **F5** tuşuna basarak uygulamayı çalıştırın.
...
1. Service Fabric Explorer ağaç görünümünde birincil çoğaltmayı seçerseniz, **sistem durumunun** bir hata olduğunu görürsünüz. Service Fabric Explorer ayrıca, koddaki `HealthInformation` parametresine eklenen sistem durumu raporu ayrıntılarını görüntüler. Aynı sistem durumu raporlarını PowerShell ve Azure portal görebilirsiniz.
...
Bu rapor, başka bir rapor tarafından değiştirilene veya bu çoğaltma silinene kadar sistem durumu yöneticisinde kalır. `HealthInformation` nesnesinde bu sistem durumu raporu için `TimeToLive` ayarlanmadığından, raporun süresi dolmaz.

Bu durumda çoğaltma olan en ayrıntılı düzeyde sistem durumunun bildirilmesi önerilir. Ayrıca `Partition`sistem durumunu da bildirebilirsiniz.
...
`Application`, `DeployedApplication`ve `DeployedServicePackage`sistem durumunu raporlamak için `CodePackageActivationContext`kullanın.
