...
`msbuild.exe`kullanarak uygulamanızı programlı bir şekilde paketlemek da mümkündür. Aynı şekilde, Visual Studio bunu çalıştırırken çıktının aynı olması gerekir.
...
[Copy-ServiceFabricApplicationPackage](/powershell/module/servicefabric/copy-servicefabricapplicationpackage?view=azureservicefabricps) Powershell komutunu `CompressPackage` anahtarıyla çalıştırarak bir paketi sıkıştırabilirsiniz. `UncompressPackage` anahtarı kullanarak paketin sıkıştırmasını aynı komutla açabilirsiniz.

Aşağıdaki komut, paketi görüntü deposuna kopyalamadan sıkıştırır. Bir veya daha fazla Service Fabric kümesine, `SkipCopy` bayrağı olmadan [Copy-ServiceFabricApplicationPackage](/powershell/module/servicefabric/copy-servicefabricapplicationpackage?view=azureservicefabricps) kullanarak, bir veya daha fazla kümeye kopyalayabilirsiniz.
Paket artık `code`, `config`ve `data` paketlerine ait daraltılmış dosyaları içerir. Birçok iç işlem için gerekli olduklarından, uygulama bildirimi ve hizmet bildirimleri sıkıştırıldı. Örneğin, belirli doğrulamaları için paket paylaşımı, uygulama türü adı ve sürüm ayıklama tüm bildirimlere erişmesi gerekir. Bildirimleri sıkıştırma işlemi bu işlemleri verimsiz hale getirir.
...
Yayımlama profilinize `CopyPackageParameters` öğesini ekleyerek ve `CompressPackage` özniteliğini `true`olarak ayarlayıp, Visual Studio 'Yu dağıtımdaki paketleri sıkıştırmak üzere söyleyebilirsiniz.
...
Bu seçenekle, uygulama paketinin görüntü deposuna kopyalanması gerekmez. Bunun yerine, bir `sfpkg` oluşturup bir dış depoya yükleyebilir ve sonra sağlama sırasında Service Fabric indirme URI 'sini sağlayabilirsiniz. Aynı paket birden çok kümeye sağlanabilir. Dış depodan sağlama, paketi her kümeye kopyalamak için gereken süreyi kaydeder.

`sfpkg` dosyası, ilk uygulama paketini içeren ve ". sfpkg" uzantısına sahip bir zip dosyasıdır.
...
Bir `sfpkg`oluşturmak için, sıkıştırılmış veya olmayan özgün uygulama paketini içeren bir klasör ile başlayın. Sonra, ". sfpkg" uzantısına sahip klasörü ZIP için herhangi bir yardımcı programı kullanın. Örneğin, [ZipFile. CreateFromDirectory](https://msdn.microsoft.com/library/hh485721(v=vs.110).aspx)kullanın.
...
>[!NOTE]
> Görüntü deposu göreli yolunu temel alan sağlama Şu anda `sfpkg` dosyalarını desteklemiyor. Bu nedenle, `sfpkg` görüntü deposuna kopyalanmamalıdır.
