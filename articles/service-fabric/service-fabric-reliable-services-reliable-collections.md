...
* Out parametresi yok: Out `ConditionalValue<T>` parametreleri yerine bir `bool` ve değeri döndürmek için kullanılır. `ConditionalValue<T>`gibidir `Nullable<T>` , ancak T 'nin bir struct olması gerekmez.
