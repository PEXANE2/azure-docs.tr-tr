...
* [Visual Studio Code için (omnisharp tarafından desteklenen) vs Code uzantısı C# ](https://marketplace.visualstudio.com/items?itemName=ms-dotnettools.csharp) Hata ayıklama C# için gerekli. VS Code ve uzantıyı arayarak ya da VS Code marketinden, **etkinlik çubuğundaki** uzantılar simgesine tıklayarak yükleyebilirsiniz.
...
VS Code için Service Fabric Reliable Services uzantısı, geliştiricilerin Service Fabric projeleri oluşturup dağıtmalarına yardımcı olmak için birçok komut sağlar. **Komut paletindeki** komutları, `(Ctrl + Shift + p)`tuşuna basarak komut adını giriş çubuğuna yazarak ve istem listesinden istediğiniz komutu seçerek çağırabilirsiniz. 
...
**Service Fabric: Build Application** komutu Java ya C# da Service Fabric uygulamaları oluşturabilir. 
