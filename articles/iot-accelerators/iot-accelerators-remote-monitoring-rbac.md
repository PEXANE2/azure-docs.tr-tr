...
description: Bu makalede, uzaktan Izleme çözüm hızlandırıcısında rol tabanlı erişim denetimi 'ni (RBAC) nasıl yapılandırabileceğiniz hakkında bilgi sağlanır
author: dominicbetts
manager: timlt
ms.author: dobett
...
Bu makalede, uzaktan Izleme çözüm hızlandırıcısında rol tabanlı erişim denetiminin nasıl yapılandırılacağı hakkında bilgi sağlanır. Rol tabanlı erişim denetimi, bireysel kullanıcılar için erişimi çözümdeki belirli özelliklerle kısıtlamanızı sağlar.
...
1. Kullanıcıyı bir role atamak için, **Rol Seç, seçili** değil ' e tıklayın ve Kullanıcı için **yönetici** ya da **salt okuma** rolü ' nü seçin. **Seç**' e ve ardından **ata**' ya tıklayın.
    ...
    ![Rol seçin](media/iot-accelerators-remote-monitoring-rbac/selectrole.png)
...
1. Uygulamanızı seçin ve ardından **bildirim**' e tıklayın. Uygulama için tanımlanmış olan iki [uygulama rolünü](/azure/architecture/multitenant-identity/app-roles) görebilirsiniz:
...
Rolü Azure portal uygulamaya ekledikten sonra, cihazları yönetmek için gereken izinleri atayan rol için [roles.jsüzerinde](https://github.com/Azure/remote-monitoring-services-dotnet/blob/master/auth/Services/data/policies/roles.json) bir ilke tanımlamanız gerekir.
...
1. Dosya **üzerinde hizmetleri/verileri/ilkeleri/roles.js** düzenlemesini bitirdiğinizde, kimlik doğrulama ve yetkilendirme mikro hizmetini çözüm hızlandırıcısına yeniden derleyin ve yeniden dağıtın.
...
[Web Kullanıcı arabirimindeki](https://github.com/Azure/pcs-remote-monitoring-webui/) [deviceDelete.js](https://github.com/Azure/pcs-remote-monitoring-webui/blob/master/src/components/pages/devices/flyouts/deviceDelete/deviceDelete.js) aşağıdaki kod parçacığı, izinlerin bildirimli olarak nasıl uygulanacağını gösterir:
...
Bu makalede, rol tabanlı erişim denetiminin uzaktan Izleme çözüm hızlandırıcısında nasıl uygulandığını öğrendiniz.
