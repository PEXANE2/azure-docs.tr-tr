...
![Üst düzey mantıksal uygulama iş akışına genel bakış](./media/quickstart-create-logic-apps-with-visual-studio/high-level-workflow-overview.png)
...
1. **Dosya** menüsünde **Yeni** > **Proje**’yi seçin. (Klavye: Ctrl+Shift+N)
   ...
   ![Yeni Visual Studio projesi oluştur](./media/quickstart-create-logic-apps-with-visual-studio/create-new-visual-studio-project.png)
...
1. Şablon listesinden **mantıksal uygulama** şablonu ' nu seçin. **Tamam**’ı seçin.
   ...
   ![Proje oluşturmak için "Logic App" şablonunu seçin](./media/quickstart-create-logic-apps-with-visual-studio/select-logic-app-template.png)
...
Arama kutusuna "RSS" yazın. Tetikleyiciler listesinden şu tetikleyiciyi seçin: **bir akış öğesi yayımlandığında-RSS**
...
1. Tetikleyici tasarımcıda görüntülendikten sonra, [Azure Portal Hızlı](../logic-apps/quickstart-create-first-logic-app-workflow.md#add-rss-trigger)başlangıcı ' nda iş akışı adımlarını izleyerek mantıksal uygulamayı oluşturmayı tamamladıktan sonra bu makaleye geri dönün. İşiniz bittiğinde mantıksal uygulamanız şu örnekteki gibi görünür:
   ...
   ![Tamamlanmış örnek mantıksal uygulama iş akışı](./media/quickstart-create-logic-apps-with-visual-studio/finished-logic-app-workflow.png)
...
1. Çözüm Gezgini'nde projenizin kısayol menüsünden **Dağıt** > **Yeni**'yi seçin. Sorulursa Azure hesabınızla oturum açın.
   ...
   ![Yeni mantıksal uygulama dağıtımı oluştur](./media/quickstart-create-logic-apps-with-visual-studio/create-logic-app-deployment.png)
...
1. **Parametreleri Düzenle** kutusu görüntülenirse, mantıksal uygulamanız için bir kaynak adı girin. Ayarlarınızı kaydedin.
   ...
   ![Visual Studio çıkış penceresinde dağıtım durumu](./media/quickstart-create-logic-apps-with-visual-studio/logic-app-output-window.png)
   ...
   ![Parolalar veya gizli anahtarlar için PowerShell komut istemi](./media/quickstart-create-logic-apps-with-visual-studio/logic-apps-powershell-window.png)
   ...
   ![Outlook her yeni RSS öğesi için e-posta gönderir](./media/quickstart-create-logic-apps-with-visual-studio/example-outlook-email.png)
...
1. Çözüm Gezgini ' de, `<logic-app-name>.json` dosyasını açın.

1. **Görünüm** menüsünde **diğer Windows** > **JSON ana hattını**seçin.

1. Şablon dosyasına bir kaynak eklemek için JSON ana hattı penceresinin üst kısmında **Kaynak Ekle** ' yi seçin. Ya da JSON ana hattı penceresinde **kaynaklar**' a sağ tıklayıp **Yeni Kaynak Ekle**' yi seçin.

   ![JSON ana hattı penceresinde yeni kaynak ekle](./media/quickstart-create-logic-apps-with-visual-studio/json-outline-window-add-resource.png)

1. **Kaynak Ekle** Iletişim kutusunda **mantıksal uygulama**bulun ve seçin. Mantıksal uygulamanızı adlandırın ve **Ekle**' yi seçin.

   ![Projeye yeni mantıksal uygulama kaynağı Ekle](./media/quickstart-create-logic-apps-with-visual-studio/add-logic-app-resource.png)
...
1. **Genel Bakış** sayfasında **Kaynak grubunu sil**’i seçin. Onay olarak kaynak grubunun adını girip **Sil**’i seçin.
   ...
   ![Mantıksal uygulama kaynak grubunu silme](./media/quickstart-create-logic-apps-with-visual-studio/delete-resource-group.png)
