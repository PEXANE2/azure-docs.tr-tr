...
| <*ayarlar*> | No | Dize | Tarafından desteklendiği şekilde kullanılacak yerel ayar `number.ToString(<format>, <locale>)` . Belirtilmemişse, varsayılan değer `en-us` . |
|||||
...
| Parametre | Gerekli | Tür | Description |
| --------- | -------- | ---- | ----------- |
| <*formatını*> | No | Dize | Döndürülen GUID için tek bir [Biçim belirleyicisi](/dotnet/api/system.guid.tostring?view=netcore-3.1#system_guid_tostring_system_string_) . Varsayılan olarak, "D" biçimindedir, ancak "N", "D", "B", "P" veya "X" kullanabilirsiniz. |
|||||
