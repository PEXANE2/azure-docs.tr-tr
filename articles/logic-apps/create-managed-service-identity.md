...
|-----------------|-------|-------------|
| `principalId` | <*sorumlu KIMLIĞI*> | Azure AD kiracısında mantıksal uygulamanızı temsil eden yönetilen kimliğin hizmet sorumlusu nesnesinin genel benzersiz tanıtıcısı (GUID). Bu GUID bazen "nesne KIMLIĞI" veya `objectID`olarak görünür. |
...
* Özelliği olarak ayarlanmış bir `identity` `type``UserAssigned`

* Ve özelliklerine sahip başka bir alt nesne olan kimliğin kaynak KIMLIĞINI belirten bir alt `userAssignedIdentities` `principalId` `clientId`

Bu örnek, bir HTTP PUT isteği için mantıksal uygulama kaynak tanımını gösterir ve parametreli `identity` olmayan bir nesne içerir. PUT isteğinin ve sonraki GET işleminin yanıtı da bu `identity` nesneye sahiptir:
...
| Özellik (JSON) | Değer | Açıklama |
|-----------------|-------|-------------|
| `principalId` | <*sorumlu KIMLIĞI*> | Azure AD kiracısında Kullanıcı tarafından atanan yönetilen kimliğin genel benzersiz tanıtıcısı (GUID) |
...
Şablonunuz yönetilen kimliğin kaynak tanımını da içeriyorsa, `identity` nesneyi parametreleştirebilirsiniz. Bu örnek, alt `userAssignedIdentities` nesnenin, şablonunuzun `userAssignedIdentity` `variables` bölümünde tanımladığınız bir değişkene nasıl başvurmuş olduğunu gösterir. Bu değişken, Kullanıcı tarafından atanan kimliğinizin kaynak KIMLIĞINE başvurur.
...
1. Henüz yapmadıysanız, [yönetilen kimlikleri destekleyen tetikleyiciyi veya eylemi](logic-apps-securing-a-logic-app.md#managed-identity-authentication)ekleyin.
   ...
   | **Üst bilgiler** | Evet, Azure depolama için | `x-ms-blob-type` = `BlockBlob` <p>`x-ms-version` = `2019-02-02` | Azure `x-ms-blob-type` depolama `x-ms-version` işlemleri için gerekli olan ve üst bilgi değerleri. <p><p>**Önemli**: giden http tetikleyicisinde ve Azure depolama için eylem isteklerinde, üst bilgi, çalıştırmak `x-ms-version` istediğiniz Işlem için özelliği ve API sürümünü gerektirir. <p>Daha fazla bilgi için şu konulara bakın: <p><p>- [İstek üstbilgileri-anlık görüntü blobu](https://docs.microsoft.com/rest/api/storageservices/snapshot-blob#request) <br>- [Azure depolama hizmetleri için sürüm oluşturma](https://docs.microsoft.com/rest/api/storageservices/versioning-for-the-azure-storage-services#specifying-service-versions-in-requests) |
...
1. Bazı Tetikleyiciler ve eylemlerde, hedef kaynak KIMLIĞINI ayarlamanız için **izleyici** özelliği de görüntülenir. [Hedef kaynak veya hizmetin kaynak kimliği için hedef](../active-directory/managed-identities-azure-resources/services-support-managed-identities.md#azure-services-that-support-azure-ad-authentication) **kitle** özelliğini ayarlayın. Aksi takdirde, varsayılan olarak, **hedef kitle** özelliği Azure Resource Manager `https://management.azure.com/` kaynak kimliği olan kaynak kimliğini kullanır.
   ...
   Bu örnek, kimlik doğrulaması için kullanılan `https://storage.azure.com/` erişim belirteçlerinin tüm depolama hesapları için geçerli olması Için **hedef kitle** özelliğini olarak ayarlar. Bununla birlikte, belirli bir depolama hesabı için kök hizmeti URL `https://fabrikamstorageaccount.blob.core.windows.net`'sini de belirtebilirsiniz.
...
Mantıksal uygulamanın yönetilen kimliğini bir Azure Resource Manager şablonu kullanarak oluşturduysanız, `identity` nesnenin `type` alt özelliğini olarak `None`ayarlayın. Sistem tarafından yönetilen kimlik için bu eylem ayrıca Azure AD 'den asıl KIMLIĞI de siler.
