---
title: Azure Red Hat OpenShift’in sorunlarını çözün
...
`az` CLı komutunu kullanarak bir Azure Red Hat OpenShift kümesi oluşturulamazsa, oluşturma işleminin yeniden denenmeye devam edilecek.
Başarısız kümeyi silmek için `az openshift delete` kullanın, ardından tamamen yeni bir küme oluşturun.

## <a name="hidden-azure-red-hat-openshift-cluster-resource-group"></a>Gizli Azure Red Hat OpenShift kümesi kaynak grubu

Şu anda, Azure CLı (`az openshift create` komutu) tarafından otomatik olarak oluşturulan `Microsoft.ContainerService/openShiftManagedClusters` kaynak Azure portal gizlenir. Kaynak grubunu görüntülemek için **kaynak grubu** görünümünde **gizli türleri göster** ' i işaretleyin.
