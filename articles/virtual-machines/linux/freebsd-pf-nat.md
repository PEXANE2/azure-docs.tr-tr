...
Sonra, [az Group Deployment Create](/cli/azure/group/deployment)ile PF-FreeBSD-Setup şablonunu dağıtın. Azuredeploy. Parameters. JSON öğesini aynı yol altında indirin ve, ve gibi kendi kaynak değerlerinizi tanımlayın `adminPassword` `networkPrefix` `domainNamePrefix` . 
