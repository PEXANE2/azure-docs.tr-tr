---
title: Azure 'da bir güvenlik duvarı oluşturmak için FreeBSD 'nin paket filtresini kullanma | Microsoft Docs
...
Yaklaşık beş dakika sonra, bilgileri `"provisioningState": "Succeeded"`alacaksınız. Ardından, ön uç VM 'ye (NAT) SSH ekleyebilir veya genel IP adresini veya ön uç VM 'nin (NAT) FQDN 'sini kullanarak bir tarayıcıda NGINX web sunucusuna erişebilirsiniz. Aşağıdaki örnek, `myResourceGroup` kaynak grubunda ön uç VM 'sine (NAT) atanan FQDN ve genel IP adresini listeler. 
