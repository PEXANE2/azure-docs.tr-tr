...
1. Birincil düğümde, SQLCMD veya SSMS 'de aşağıdaki komutu çalıştırın:
    ...
    - Aşağıda kullanılan IP adresini `virtualip` IP adresi ile değiştirin.
...
1. Her VM düğümünde oturum açın. Konaklar dosyasını açmak ve her makinede `ag1-listener` için konak adı çözümlemesini ayarlamak için aşağıdaki komutu kullanın.

    ```
    sudo vi /etc/hosts
    ```

    **VI** düzenleyicisinde metin eklemek için `i` girin ve boş bir satıra `ag1-listener`IP 'sini ekleyin. Ardından, IP 'nin yanındaki bir alandan sonra `ag1-listener` ekleyin.

    ```output
    <IP of ag1-listener> ag1-listener
    ```

    **VI** düzenleyiciden çıkmak için, önce **ESC** tuşuna basın ve ardından dosyayı yazıp çıkmak üzere komut `:wq` girin. Bunu her düğümde yapın.
...
1. Kullanılabilirlik grubu dinleyicisi adını kullanarak SQL Server birincil düğümünde oturum açmak için SQLCMD kullanın:
    ...
    - Daha önce oluşturulmuş bir oturum açma kullanın ve `<YourPassword>` doğru parolayla değiştirin. Aşağıdaki örnek, SQL Server oluşturulan `sa` oturum açma bilgilerini kullanır.
...
1. Bağlı olduğunuz sunucunun adını denetleyin. SQLCMD 'de şu komutu çalıştırın:
    ...
    Çıktın geçerli birincil düğümü göstermesi gerekir. Bu, bir yük devretmeyi hiç test etmadıysanız `VM1` olmalıdır.

    `exit` komutunu yazarak SQL oturumundan çıkın.

### <a name="test-a-failover"></a>Yük devretmeyi test etme

1. Birincil çoğaltmayı `<VM2>` veya başka bir çoğaltmaya el ile yük devretmek için aşağıdaki komutu çalıştırın. `<VM2>` değerini sunucu adınızın değeriyle değiştirin.
