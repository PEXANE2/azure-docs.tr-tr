...
> [!NOTE]
> Aşağıdaki kılavuz, Azure Müşteri Danışma ekibi ve bulut çözümü mimarları tarafından büyük ortamlar üzerinde müşterilerle birlikte çalışarak büyük ölçüde katkıda bulunmuştur. Bu nedenle, yeni başarı kalıpları ortaya çıktıkça bu belge güncelleştirilemeye devam edecektir, bu nedenle yeni öneriler olup olmadığını görmek için zaman zaman tekrar kontrol edin.

Göç yolculuğunun dört genel aşaması vardır:
...
4. Geçiş API'si ile hangi senaryolar desteklenir?  [Desteklenmeyen özellikleri ve yapılandırmaları gözden geçirin.](migration-classic-resource-manager-overview.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json#unsupported-features-and-configurations)
...
Aşağıdaki sorunlar büyük göçlerin çoğunda keşfedilen edildi. Bu kapsamlı bir liste değildir ve daha fazla ayrıntı için [desteklenmeyen özelliklere ve yapılandırmalara](migration-classic-resource-manager-overview.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json#unsupported-features-and-configurations) başvurmalısınız. Bu teknik sorunlarla karşılaşabilirsiniz veya karşılaşmayabilirsiniz, ancak geçiş girişiminden önce bunları çözerseniz daha sorunsuz bir deneyim sağlarsınız.
...
- **VM Uzantıları** - Sanal Makine uzantıları, çalışan VM'lerin geçişinin en büyük engellerinden biridir. VM Uzantılarının düzeltilmesi 1-2 günden fazla sürebilir, bu nedenle buna göre plan layın.  Çalışan VM'lerin VM Uzantısı durumunu geri bildirmek için çalışan bir Azure aracısı gerekir. Durum çalışan bir VM için kötü olarak geri gelirse, bu geçişi durdurur. Geçişi etkinleştirmek için aracının kendisinin çalışıyor olması gerekmez, ancak VM'de uzantılar varsa, geçişin ilerlemesi için hem çalışan bir aracı hem de giden internet bağlantısı (DNS ile) gerekir.
  ...
  - BGInfo uzantısı iki sürümü vardır: v1 ve v2.  VM, Azure portalı veya PowerShell kullanılarak oluşturulduysa, VM'de büyük olasılıkla v1 uzantısı olacaktır. Bu uzantın kaldırılması gerekmez ve geçiş API'sı tarafından atlanır (geçirilmez). Ancak, Klasik VM yeni Azure portalı ile oluşturulduysa, büyük olasılıkla BGInfo'nun JSON tabanlı v2 sürümüne sahip olacaktır ve bu sürümün temsilcisi çalışıyor ve giden internet erişimi (ve DNS) olması koşuluyla Azure Kaynak Yöneticisi'ne geçirilebilir.
...
- **Web/İşçi Rol Dağıtımları** - Web ve çalışan rolleri içeren Bulut Hizmetleri Azure Kaynak Yöneticisi'ne geçirilemez. Geçişin başlayaabilmesi için önce web/işçi rolleri sanal ağdan kaldırılmalıdır.  Tipik bir çözüm, web/işçi rol örneklerini expressroute devresine de bağlı ayrı bir Klasik sanal ağa taşımak veya kodu yeni PaaS App Services'e geçirmektir (bu tartışma bu belgenin kapsamı dışındadır). Eski yeniden dağıtma durumunda, yeni bir Klasik sanal ağ oluşturun, web/işçi rollerini bu yeni sanal ağa taşıyın/yeniden dağıtın, ardından taşınan sanal ağdaki dağıtımları silin. Kod değişikliği gerekmez. Yeni [Sanal Ağ Eşleme](../../virtual-network/virtual-network-peering-overview.md) özelliği, aynı Azure bölgesindeki web/işçi rollerini ve diğer sanal ağları içeren klasik sanal ağı birlikte eşlemek için kullanılabilir **(öteden sanal ağ geçirilemediği için sanal ağ geçişi tamamlandıktan sonra)** performans kaybı ve gecikme/bant genişliği cezası olmadan aynı yetenekleri sağlar. Sanal Ağ [Eşlemesi](../../virtual-network/virtual-network-peering-overview.md)eklenmesi göz önüne alındığında, web/işçi rol dağıtımları artık kolayca azaltılabilir ve Azure Kaynak Yöneticisi'ne geçişi engellemeyebilir.
...
- **Azure Kaynak Yöneticisi Kotaları** - Azure bölgeleri, hem Klasik hem de Azure Kaynak Yöneticisi için ayrı kotalara/sınırlara sahiptir. Geçiş senaryosunda yeni donanım tüketilmese de *(mevcut VM'leri Klasik'ten Azure Kaynak Yöneticisi'ne değiştiriyoruz)* rağmen, geçiş başlamadan önce Azure Kaynak Yöneticisi kotalarının yeterli kapasiteyle yerinde olması gerekir. Aşağıda sorunlara neden gördüğümüz ana sınırları vardır.  Sınırları yükseltmek için kota destek bileti açın.
  ...
  - Yönlendirme Tabloları
    ...
    Geçerli Azure Kaynak Yöneticisi kotalarınızı, Azure CLI'nin en son sürümüyle aşağıdaki komutları kullanarak denetleyebilirsiniz.
...
- **Azure Kaynak Yöneticisi API azaltma sınırları** - Yeterince büyük bir ortamınız varsa (örneğin. VNET'te 400 VM'>, Azure Kaynak Yöneticisi'nde yazmalar için varsayılan API azaltma sınırlarını (şu anda **1200 yazma/saat)** vurabilirsiniz. Geçişe başlamadan önce, aboneliğiniz için bu sınırı artırmak için bir destek bileti yükseltmeniz gerekir.

- **Zamanlanmış VM Durumunu Sağlama** - Herhangi bir VM **zaman sızma**sağlama durumuna sahipse, bu geçiş öncesi çözülmesi gerekir. Bunu yapmanın tek yolu, VM'yi devre-kapalı tutma/yeniden sağlama (silme, diski saklama ve VM'yi yeniden oluşturma) ile birlikte kesintidir.

- **RoleStateUnknown VM Durumu** - Bir **rol durumu bilinmeyen** hata iletisi nedeniyle geçiş durursa, portalı kullanarak VM'yi inceleyin ve çalıştığını sağlayın. Bu hata genellikle birkaç dakika sonra kendi başına gider (hiçbir düzeltme gerekli) ve genellikle geçici bir tür genellikle Sanal Makine **başlatmak**sırasında görülen , **durdurmak**, **işlemleri yeniden başlatın.** **Önerilen uygulama:** birkaç dakika sonra geçişi yeniden deneyin.

- **Kumaş Kümesi yok** - Bazı durumlarda, bazı VM'ler çeşitli garip nedenlerle geçirilemez. Bu bilinen durumlardan biri, VM'nin yakın zamanda (son bir hafta içinde) oluşturulduğu ve Azure Kaynak Yöneticisi iş yükleri için henüz donanıma sahip olmayan bir Azure kümesi nin inmesi durumunda olduğudur.  **Kumaş kümesi nin var olmadığını** ve VM'nin geçirilemeyeceğini söyleyen bir hata alırsınız. Küme yakında Azure Kaynak Yöneticisi'ni etkinleştireceği için birkaç gün beklemek genellikle bu sorunu çözer. Ancak, hemen bir geçici `stop-deallocate` çözüm VM, sonra geçiş ile devam ve geçiş ten sonra Azure Kaynak Yöneticisi VM geri başlatın.
...
Yukarıdaki Laboratuvar Testi bölümünden gelen teknik kılavuz, gerçek bir geçişten önce dikkate alınmalı ve azaltılmalıdır.  Yeterli sınama ile, geçiş aslında olmayan bir olaydır.  Üretim ortamları için, güvenilir bir Microsoft iş ortağı veya Microsoft Premier hizmetleri gibi ek desteğe sahip olmak yararlı olabilir.
