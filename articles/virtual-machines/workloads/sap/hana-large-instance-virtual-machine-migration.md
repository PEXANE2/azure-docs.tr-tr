...
- Yeni Azure VM yeni bir Microsoft Azure Sanal Ağ yerleştirilmişse, başka bir bölgede olabilir ve var olan sanal ağla eşlenirse, bu yeni sanal ağa yönelik erişime izin vermek için özgün HLI sağlamadan ExpressRoute hizmet anahtarı ve kaynak KIMLIĞI kullanılabilir ağ IP aralığı.  Sanal ağın HLI bağlantı kurmasını sağlamak için Microsoft hizmet yönetimiyle koordine edin.  Note: uygulama ve veritabanı katmanları arasındaki ağ gecikmesini en aza indirmek Için hem uygulama hem de veritabanı katmanları aynı sanal ağda olmalıdır.  
