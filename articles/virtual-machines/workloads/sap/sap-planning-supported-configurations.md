...
- Windows Yük devretme kümesi sunucusu, SIOS `Datakeeper`'yi temel alır. Microsoft tarafından belgelense de, bu çözüm kullanılırken SIOS desteğiyle birlikte çalışmak için, SIOS ile bir destek ilişkisine ihtiyacınız vardır. Ayrıntılar makalesinde açıklanmaktadır:
...
- SUSE işletim sisteminde, iki SUSE VM kullanarak ve `drdb` dosya çoğaltma için yüksek düzeyde KULLANILABILIR bir NFS paylaşımıyla oluşturma ile Pacemaker. Ayrıntılar makalede belgelenmiştir
...
- Bir `glusterfs` kümede barındırılan NFS paylaşımıyla Red Hat işletim sisteminde pacemaker. Makalelerde Ayrıntılar bulunabilir
...
- SIOS `Datakeeper` tabanlı Windows paylaşılan diski olan Windows Yük devretme kümesi sunucusu, Azure NetApp Files hariç tüm yerel Azure depolama türlerine dağıtılabilir. Bununla birlikte, verimlilik ve ıOPS 'nin üstün hizmet düzeyi anlaşmaları nedeniyle Premium depolamanın yararlanması önerilir.
- Azure NetApp Files üzerindeki NFS paylaşımlarını kullanan SUSE veya Red Hat Paceyapıcısı Azure NetApp Files desteklenir. 
- İki VM arasındaki `drdb` YAPıLANDıRMAYı kullanan SUSE pacemaker, Azure NetApp Files hariç yerel Azure Depolama türleri kullanılarak desteklenir. Bununla birlikte, verimlilik ve ıOPS 'nin üstün hizmet düzeyi anlaşmaları nedeniyle Premium depolamanın yararlanması önerilir.
- NFS paylaşımının sağlanması için kullanılan `glusterfs` Red Hat pacemaker, Azure NetApp Files hariç yerel Azure Depolama türleri kullanılarak desteklenir. Bununla birlikte, verimlilik ve ıOPS 'nin üstün hizmet düzeyi anlaşmaları nedeniyle Premium depolamanın yararlanması önerilir.
...
- SAP uygulama katmanı ve SAP DBMS katmanı arasında NetWeaver, S/4HANA 'da gösterildiği gibi daha büyük bir ağ gecikmesi sunan dağıtım senaryoları ve örn `Hybris`. Buna aşağıdakiler dahildir:
