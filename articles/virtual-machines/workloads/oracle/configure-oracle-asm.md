...
3. Aşağıdaki komutu çalıştırarak ve istemlerle yanıt vererek disk */dev/SDC* biçimlendirin:
   ...
   - Varsayılan ilk silindir için `enter` tuşuna basın
   - Varsayılan son silindir için `enter` tuşuna basın
...
4. Önceki `fdisk` komutunu `/dev/sdd`, `/dev/sde`ve `/dev/sdf`için tekrarlayın.
...
6. Yapılandırılmış değiştirme alanını güncelleştir. Oracle Grid bileşenlerinde, Grid 'i yüklemek için en az 6,8 GB takas alanı gerekir. Azure 'daki Oracle Linux görüntüleri için varsayılan takas dosyası boyutu yalnızca 20 48MB 'tır. Güncelleştirilmiş ayarların etkili olabilmesi için `/etc/waagent.conf` dosyasındaki `ResourceDisk.SwapSizeMB` artırmanız ve Walınuxagent hizmetini yeniden başlatmanız gerekir. Salt okunurdur bir dosya olduğundan, yazma erişimini etkinleştirmek için dosya izinlerini değiştirmeniz gerekir.
   ...
   `ResourceDisk.SwapSizeMB` arayın ve değeri **8192**olarak değiştirin. Ekleme moduna girmek için `insert` tuşuna basmanız gerekir, **8192** değerini yazın ve ardından komut moduna geri dönmek için `esc` ' a basın. Değişiklikleri yazmak ve dosyadan çıkmak için `:wq` yazın ve `enter`' a basın.
   
   > [!NOTE]
   > En iyi performansı elde etmek için her zaman yerel geçici diskte (geçici disk) oluşturulacak şekilde değiştirme alanını yapılandırmak üzere `WALinuxAgent` kullanmanızı kesinlikle öneririz. Hakkında daha fazla bilgi için bkz. [Linux Azure sanal makinelerinde takas dosyası ekleme](https://support.microsoft.com/en-us/help/4010058/how-to-add-a-swap-file-in-linux-azure-virtual-machines).
...
2. PuTTY 'yı yükledikten sonra bir komut istemi açın, PuTTY klasörünü (örneğin, C:\Program Files\PuTTY) değiştirin ve bir anahtar oluşturmak için `puttygen.exe` çalıştırın.

3. PuTTY anahtar Oluşturucu:
   
   1. `Generate` düğmesini seçerek bir anahtar oluşturun.
   2. Anahtarın içeriğini kopyalayın (CTRL + C).
   3. `Save private key` düğmesini seçin.
   4. Anahtarı bir parola ile güvenli hale getirme uyarısını yoksayın ve sonra `OK`' yi seçin.
...
5. `authorized_keys` adlı bir dosya oluşturun. Anahtarın içeriğini bu dosyaya yapıştırın ve dosyayı kaydedin.
   ...
   > [!NOTE]
   > Anahtarın `ssh-rsa`dize içermesi gerekir. Ayrıca, anahtarın içeriği tek satırlık bir metin olmalıdır.
   >  
...
8. **Kategori** bölmesinde **oturum**' a gidin. Konak adı iletişim kutusuna Oracle ASM VM 'nizi girin `<publicIPaddress>` yeni bir `Saved Session` adı doldurup `Save`' ye tıklayın.  Kaydedildikten sonra, Oracle ASM sanal makinenize bağlanmak için `open` ' ye tıklayın.  İlk bağlandığınız zaman, uzak sistem kayıt defterinizde önbelleğe alınmamış uyarılardır. Eklemek için `yes` tıklayın ve devam edin.
...
6. **Yönetim seçeneklerini belirtin** sayfasında em bulut denetimini yapılandırma seçeneğiniz vardır. Bu seçeneği atlıyoruz ve devam etmek için `next` ' a tıklayın. 

7. **Ayrıcalıklı Işletim sistemi grupları** sayfasında, varsayılan ayarları kullanın. Devam etmek için `next` ' ye tıklayın.

8. **Yükleme konumunu belirtin** sayfasında, varsayılan ayarları kullanın. Devam etmek için `next` ' ye tıklayın.

9. **Envanter oluştur** sayfasında, envanter dizinini `/u01/app/grid/oraInventory`olarak değiştirin. Devam etmek için `next` ' ye tıklayın.
...
12. **Düzeltme betiği** iletişim kutusunda `OK`' ye tıklayın.

13. **Özet** sayfasında, seçtiğiniz ayarları gözden geçirin ve ardından `Install`' ye tıklayın.

    ![Yükleyicinin özet sayfasının ekran görüntüsü](./media/oracle-asm/install12.png)

14. Yapılandırma komut dosyalarının ayrıcalıklı kullanıcı olarak çalıştırılması gerektiğini bildiren bir uyarı iletişim kutusu görüntülenir. Devam etmek için `Yes` ' ye tıklayın.

15. **Son** sayfasında `Close` ' ye tıklayarak yüklemeyi sona erdirin.
...
1. X11 oturumunuzda hala **kılavuz**olarak oturum açtığınızdan emin olun. Terminalden önce `enter` vurması gerekebilir. Ardından Oracle otomatik depolama yönetimi yapılandırma yardımcısını başlatın:
...
2. **ASM: disk grupları Yapılandır** iletişim kutusunda `Create` düğmesine tıklayın ve ardından `Show Advanced Options`' e tıklayın.
...
3. **Disk grubu oluştur** iletişim kutusunda:
   ...
   - **Ayırma birimi boyutu**altında **4**' ü seçin.
   - Disk grubunu oluşturmak için `ok` ' ye tıklayın.
   - Onay penceresini kapatmak için `ok` ' a tıklayın.

   ![Disk grubu oluştur iletişim kutusunun ekran görüntüsü](./media/oracle-asm/asm02.png)

4. **ASM: disk grupları Yapılandır** iletişim kutusunda `Create` düğmesine tıklayın ve ardından `Show Advanced Options`' e tıklayın.
...
2. **Veritabanı işlemi** sayfasında, `Create Database`' ye tıklayın.
...
3. **Oluşturma modu** sayfasında:
   ...
   - `create as container database` seçili olduğundan emin olun.
   - `pluggable database name` bir değer yazın.

4. **Özet** sayfasında, seçtiğiniz ayarları gözden geçirin ve sonra veritabanını oluşturmak için `Finish` ' ye tıklayın.

   ![Özet sayfasının ekran görüntüsü](./media/oracle-asm/createdb03.png)

5. Veritabanı oluşturuldu. **Son** sayfasında, bu veritabanını kullanmak ve parolaları değiştirmek için ek hesapların kilidini açma seçeneğiniz vardır. Bunu yapmak isterseniz, **parola yönetimi** ' ni seçin, yoksa `close`' ye tıklayın.
