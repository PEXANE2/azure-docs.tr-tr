...
| Ağ arabirimi oluşturma |$nic 1 = [New-Aznetworkınterface](/powershell/module/az.network/new-aznetworkinterface) -resourcegroupname $MyResourceGroup-Name "MYNIC"-Location $Location-PRIVATEıPADDRESS xx. X. X. x-subnet $subnet 2-Loadbalancerbackendadddresspool $LoadBalancer. Backendavddresspoles [0]-Loadbalancerınboundnatrule $LoadBalancer. ınboundnatrules [0]<BR><BR>Daha önce oluşturduğunuz genel IP adresini ve sanal ağ alt ağını kullanarak bir ağ arabirimi oluşturun. |
...
| Görev | Komut |
| ---- | ------- |
| Sanal ağa alt ağ ekleme |[Add-AzVirtualNetworkSubnetConfig](/powershell/module/az.network/add-azvirtualnetworksubnetconfig) -addresspredüzeltmesini xx. X. X. x/xx-Name "mySubnet1"-VirtualNetwork $VNET<BR><BR>Mevcut bir sanal ağa alt ağ ekler. $Vnet değeri, Get-AzVirtualNetwork tarafından döndürülen nesneyi temsil eder. |
