...
Aynı bulut hizmetinde birden çok sanal makine oluşturulabilir. Bu sanal makine senaryosunun anlaşılması için sanal [makinelerin sanal ağ veya bulut hizmetiyle nasıl bağlanılabildiğini](/previous-versions/azure/virtual-machines/windows/classic/connect-vms-classic#connect-vms-in-a-standalone-cloud-service)öğrenin. Bu senaryo, bir sanal makinedeki bir istemcinin aynı bulut hizmetinde başka bir sanal makinede çalışan SQL Server'a bağlanmaya çalışmasıdır.
...
> [!NOTE]
> Bu tekniği SQL Server ile iletişim kurmak için kullandığınızda, Azure veri merkezinden gelen tüm giden verilerin [giden veri aktarımlarında](https://azure.microsoft.com/pricing/details/data-transfers/)normal fiyatlandırmaya tabi olduğunu unutmayın.
> 
> 
...
Bir etki alanı ortamı nı ve Windows Kimlik Doğrulamasını yapılandırmak için bu makaledeki adımları kullanarak ortak bitiş noktasını veya SQL Kimlik Doğrulamasını ve oturum açma işlemlerini yapılandırmanız gerekmez. Bu senaryoda, bağlantı dizesinde SQL Server VM adını belirterek SQL Server örneğinize bağlanabilirsiniz. Aşağıdaki örnek, Windows Kimlik Doğrulama'nın da yapılandırıldığını ve kullanıcıya SQL Server örneğine erişim izni verildiğini varsayar.
...
SQL Server örneğine başka bir VM veya internetten bağlanabilmek için, aşağıdaki bölümlerde açıklandığı gibi aşağıdaki görevleri tamamlamanız gerekir:
...
Azure sanal makinede çalışan SQL Server için en iyi güvenlik uygulamalarının tümlerini gözden geçirmek önemlidir. Daha fazla bilgi için bkz. [Azure Sanal Makineler'de SQL Server için Güvenlikle İlgili Dikkat Edilmesi Gerekenler](../sql/virtual-machines-windows-sql-security.md).
