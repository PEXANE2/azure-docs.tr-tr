...
1. Dosya Gezgini 'ni veya **Çalıştır** iletişim kutusunu (Windows logosu tuşu + r) kullanarak paylaşıma gidin. Ağ yolunu `\\storageaccountname.file.core.windows.net\filesharename`kullanın. Örneğin, `\\sqlvmstorageaccount.file.core.windows.net\sqlpremiumfileshare`
...
1. Yükleme medyasını bulun. Sanal makine Azure Marketi görüntülerinden birini kullanıyorsa medya konumunda `C:\SQLServer_<version number>_Full`bulunur. **Kurulum 'u**seçin.
...
1. **Sistem durumu araştırması Ekle** dikey penceresinde aşağıdaki <span id="probe"> </span> sistem durumu araştırma parametrelerini ayarlayın.
   ...
   - **Protokol**: TCP.
   - **Bağlantı noktası**: [Bu adımdaki](#ports)sistem durumu araştırması için güvenlik duvarında oluşturduğunuz bağlantı noktası. Bu makalede, örnek TCP bağlantı noktasını `59999`kullanır.
   - **Aralık**: 5 saniye.
