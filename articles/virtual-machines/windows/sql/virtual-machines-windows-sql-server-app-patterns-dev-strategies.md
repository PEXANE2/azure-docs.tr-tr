...
* Azure ortamının özelliklerinden yararlanmak istiyorsunuz, ancak Azure SQL veritabanı, uygulamanızın gerektirdiği tüm özellikleri desteklemiyor. Bu, aşağıdaki alanlara dahil olabilir:
  ...
  * **HIPAA uyumluluğu**: bir Azure sanal MAKINESINDEKI SQL Server HIPAA Iş ilişkisi Sözleşmesi (BAA) [kapsamında olduğundan,](../../../sql-database/sql-database-technical-overview.md) sağlık müşterileri ve bağımsız yazılım satıcıları (ISV 'Ler) Azure [sanal makinelerinde SQL Server](virtual-machines-windows-sql-server-iaas-overview.md) tercih edebilir. Uyumluluk hakkında bilgi için bkz. [Güven Merkezi: uyumluluk Microsoft Azure](https://azure.microsoft.com/support/trust-center/compliance/).
...
Bu, ile başlamak için çok yaygın bir model olduğundan, verilerinizi SQL Server VM taşımak için yararlı olan geçiş hakkında aşağıdaki makaleyi bulabilirsiniz: bir [veritabanını bir Azure VM 'de SQL Server geçirme](virtual-machines-windows-migrate-sql.md).
