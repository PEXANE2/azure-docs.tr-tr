...
| --- | --- |
| **Kullanılabilirlik grupları** |Aynı bölgedeki Azure VM 'lerinde çalışan kullanılabilirlik çoğaltmaları, yüksek kullanılabilirlik sağlar. Daha yüksek artıklık ve kullanılabilirlik için Azure VM 'Leri [burada](https://docs.microsoft.com/en-us/azure/virtual-machines/windows/sql/virtual-machines-windows-portal-sql-availability-group-overview)belgelenen farklı [kullanılabilirlik bölgelerinde](https://docs.microsoft.com/en-us/azure/availability-zones/az-overview) dağıtılabilir. Bir kullanılabilirlik grubundaki SQL Server VM 'Ler kullanılabilirlik alanlarında dağıtılmışsa, bu makalelerde belgelendiği gibi dinleyici için [Standart yük dengeleyici](https://docs.microsoft.com/en-us/azure/load-balancer/load-balancer-standard-overview) kullanın- [Azure SQL VM CLI](https://docs.microsoft.com/en-us/azure/virtual-machines/windows/sql/virtual-machines-windows-sql-availability-group-cli) & [Azure hızlı başlangıç şablonları](https://docs.microsoft.com/en-us/azure/virtual-machines/windows/sql/virtual-machines-windows-sql-availability-group-quickstart-template). Windows Yük Devretme Kümelemesi bir Active Directory etki alanı gerektirdiğinden bir etki alanı denetleyicisi VM 'si yapılandırmanız gerekir.<br/> ![Kullanılabilirlik grupları @ no__t-1<br/>Daha fazla bilgi için bkz. [Azure 'Da kullanılabilirlik grupları yapılandırma (GUI)](virtual-machines-windows-portal-sql-alwayson-availability-groups.md). |
| **Yük devretme kümesi örnekleri** |Paylaşılan depolama gerektiren yük devretme kümesi örnekleri (FCı) 3 farklı şekilde oluşturulabilir.<br/><br/>1. [Windows Server 2016 Depolama Alanları Doğrudan \(S2D @ no__t-2](virtual-machines-windows-portal-sql-create-failover-cluster.md) kullanarak Azure VM 'lerinde çalışan ve yazılım tabanlı BIR sanal San sağlamak için iki düğümlü bir yük devretme kümesi.<br/><br/>2. Azure VM 'lerde çalışan, bir üçüncü taraf kümeleme çözümü tarafından desteklenen depolama ile iki düğümlü bir yük devretme kümesi. SIOS veri Man kullanan belirli bir örnek için bkz. [Yük Devretme Kümelemesi ve 3. taraf yazılım SIOS veri kullanımı kullanılarak bir dosya paylaşımının yüksek kullanılabilirliği](https://azure.microsoft.com/blog/high-availability-for-a-file-share-using-wsfc-ilb-and-3rd-party-software-sios-datakeeper/).<br/><br/>3. ExpressRoute aracılığıyla uzak Iscsı hedefi paylaşılan blok depolama ile Azure VM 'lerinde çalışan iki düğümlü bir yük devretme kümesi. Örneğin, NetApp özel depolama (NPS), Azure VM 'lerine Equinx ile ExpressRoute aracılığıyla bir Iscsı hedefi sunar.<br/><br/>Üçüncü taraf paylaşılan depolama ve veri çoğaltma çözümleri için, yük devretmede veriye erişme ile ilgili herhangi bir sorun için satıcıyla iletişim kurmanız gerekir.<br/><br/>Bu çözüm Premium Storage 'ı kullanmadığından, [Azure dosya depolaması](https://azure.microsoft.com/services/storage/files/) üzerinde FCI kullanmanın henüz desteklenmediğini unutmayın. Yakında bu desteği desteklemeye çalışıyoruz. |
...
| Teknoloji | Örnek mimarilerin |
| --- | --- |
| **Kullanılabilirlik grupları** |Olağanüstü durum kurtarma için Azure VM 'lerinde birden çok veri merkezinde çalışan kullanılabilirlik çoğaltmaları. Bu çapraz bölge çözümü, tam site kesintiine karşı koruma sağlar. <br/> ![Kullanılabilirlik grupları @ no__t-1<br/>Bir bölge içinde tüm çoğaltmaların aynı bulut hizmeti ve aynı VNet içinde olması gerekir. Her bölge ayrı bir sanal ağa sahip olacağı için bu çözümler VNET 'in VNet bağlantısı olmasını gerektirir. Daha fazla bilgi için, [Azure Portal kullanarak VNET 'Ten VNET 'e bağlantı yapılandırma](../../../vpn-gateway/vpn-gateway-howto-vnet-vnet-resource-manager-portal.md)konusuna bakın. Ayrıntılı yönergeler için bkz. [farklı bölgelerdeki Azure sanal makinelerinde SQL Server kullanılabilirlik grubu yapılandırma](virtual-machines-windows-portal-sql-availability-group-dr.md).|
...
| **Azure Site Recovery ile Azure 'a çoğaltma ve yük devretme SQL Server** |Tek bir Azure veri merkezinin üretim SQL Server olağanüstü durum kurtarma için farklı Azure veri merkezi 'nin Azure depolama 'ya doğrudan çoğaltılır.<br/>![ Azure Site Recovery @ no__t-1 kullanarak<br/>Daha fazla bilgi için bkz. [SQL Server olağanüstü durum kurtarma ve Azure Site Recovery kullanarak SQL Server koruma](../../../site-recovery/site-recovery-sql.md). |
...
| **Azure Site Recovery ile Azure 'a çoğaltma ve yük devretme SQL Server** |Şirket içi üretim SQL Server olağanüstü durum kurtarma için doğrudan Azure Storage 'a çoğaltılır.<br/>![ Azure Site Recovery @ no__t-1 kullanarak<br/>Daha fazla bilgi için bkz. [SQL Server olağanüstü durum kurtarma ve Azure Site Recovery kullanarak SQL Server koruma](../../../site-recovery/site-recovery-sql.md). |
