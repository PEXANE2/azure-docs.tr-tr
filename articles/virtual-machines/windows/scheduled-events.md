---
title: Azure 'da Windows VM 'Leri için Zamanlanan Olaylar | Microsoft Docs
...
SANAL ağ etkin VM 'Ler için, meta veri hizmeti statik bir yönlendirilemeyen IP `169.254.169.254`'den kullanılabilir. Zamanlanan Olaylar en son sürümü için tam uç nokta şunlardır: 
...
Zamanlanan Olaylar hizmeti sürümlenmiş. Sürümler zorunludur ve geçerli sürümdür `2017-11-01`.
...
| Olay durumu | Bu olayın durumu. <br><br> Deðerler <ul><li>`Scheduled`: Bu olay, `NotBefore` özellikte belirtilen süreden sonra başlaması için zamanlanır.<li>`Started`: Bu olay başlatıldı.</ul> Hiç `Completed` veya benzer bir durum sağlanmamıştır; olay tamamlandığında olay artık döndürülmeyecektir.
...
Yaklaşan bir olayı öğrendikten ve düzgün kapanma için mantığınızı tamamladıktan sonra, `POST` `EventId`ile meta veri hizmetine bir çağrı yaparak bekleyen olayı onaylayabilirsiniz. Bu, Azure 'a, en düşük bildirim süresini (mümkün olduğunda) kısaltıp kısaltamayacağını gösterir. 

`POST` İstek gövdesinde beklenen JSON aşağıda verilmiştir. İstek bir listesi `StartRequests`içermelidir. Her `StartRequest` biri, `EventId` hızlandırmak istediğiniz olay için şunu içerir:
```
{
    "StartRequests" : [
        {
