...
[Oluşturma işlecini](/azure/kusto/query/renderoperator) kullanarak verileri sorgulama ve Ploy.ly kitaplığını kullanarak verileri görselleştirme. Bu sorgu ve görselleştirme, yerel KQL kullanan tümleşik bir deneyim sağlar. Ksqlmagic `timepivot`, `pivotchart`ve `ladderchart`dışındaki birçok grafiği destekler. Render, `kind`, `ysplit`ve `accumulate`dışındaki tüm özniteliklerle desteklenir. 
...
Varsayılan renk paletini beğenmezseniz, palet seçeneklerini kullanarak grafikleri özelleştirin. Kullanılabilir paletler burada bulunabilir: [KQL sihirli sorgu grafik sonucu için renk paleti seçin](https://mybinder.org/v2/gh/Microsoft/jupyter-Kqlmagic/master?filepath=notebooks%2FColorYourCharts.ipynb)
...
KQL Magic, kusto sorgu dili ve Python arasında basit değişim yapılmasına izin verir. Daha fazla bilgi edinmek için: [Python Ile KQL Magic sorgunuzu parametreleştirin](https://mybinder.org/v2/gh/Microsoft/jupyter-Kqlmagic/master?filepath=notebooks%2FParametrizeYourQuery.ipynb)
...
Bir KQL sorgusunun sonuçlarına Pandas DataFrame içinde erişebilirsiniz. En son yürütülen sorgu sonuçlarına değişken `_kql_raw_result_` göre erişin ve sonuçları Pandas DataFrame 'e kolayca aşağıdaki gibi dönüştürün:
...
Birçok analiz senaryosunda, çok sayıda sorgu içeren yeniden kullanılabilir Not defterleri oluşturmak ve sonuçları bir sorgudan sonraki sorgulara akışa almak isteyebilirsiniz. Aşağıdaki örnek, verileri filtrelemek için `statefilter` Python değişkenini kullanır.

1. En fazla `DamageProperty`en fazla 10 durumu görüntülemek için bir sorgu çalıştırın:
...
1. `let` ifadesini ve Python değişkenini kullanarak bir sorgu çalıştırın:
...
> [!TIP]
> Kullanılabilir tüm yapılandırmalara ilişkin bilgi almak için `%config Kqlmagic`kullanın. Bağlantı sorunları ve hatalı sorgular gibi kusto hatalarını gidermek ve yakalamak için `%config Kqlmagic.short_errors=False` kullanın
