...
Bu makalede öncelikle Azure Dosya Eşitleme dağıtmaya yönelik dağıtım konuları ele alınmaktadır. Azure dosya paylaşımlarının dağıtımını şirket içi veya bulut istemcisi tarafından doğrudan takılacak şekilde planlamak için bkz. [Azure dosyaları dağıtımı Için planlama](storage-files-planning.md).
...
Windows Server 'da eşitleme özelliğini etkinleştirmek için, indirilebilir Azure Dosya Eşitleme aracısını yüklemelisiniz. Azure Dosya Eşitleme Aracısı iki ana bileşen sağlar: `FileSyncSvc.exe`, sunucu uç noktalarında yapılan değişiklikleri izlemekten sorumlu olan ve eşitleme oturumlarını başlatan ve `StorageSync.sys`, bulut katmanlaması ve hızlı olağanüstü durum kurtarmayı sağlayan bir dosya sistemi filtresi.  
...
\*100.000.000 'den fazla dosya eşitleme & Dizin şu anda önerilmez. Bu, test edilen eşiklerimize göre bir geçici limit. Daha fazla bilgi için bkz. [Azure dosyaları ölçeklenebilirlik ve performans hedefleri](storage-files-scale-targets.md#azure-file-sync-scale-targets).
...
- [Azure Dosya Eşitleme ara sunucu ve güvenli duvarı ayarları](storage-sync-files-firewall-and-proxy.md)
...
BitLocker 'da benzer şekilde çalışan üçüncü taraf ürünleri, NTFS biriminin altına oturduklarında benzer şekilde tamamen saydam şekilde Azure Dosya Eşitleme. 
...
| Azure bulutu | Coğrafi bölge | Azure bölgesi | Bölge kodu |
|-------------|-------------------|--------------|-------------|
...
- Yalnızca yeni dosya sunucusunda bir sunucu uç noktası oluşturun ve `robocopy`kullanarak eski dosya paylaşımından verileri kopyalayın. Yeni sunucunuzdaki dosya paylaşımlarının topolojisine (her birimde sahip olduğunuz paylaşım sayısı, her birimin ne kadar boş olduğu vb.) bağlı olarak, eski sunucunuzdaki şirket içi veri merkezinizdeki yeni sunucunuza `robocopy`, verileri Azure 'a taşıyaAzure dosya eşitleme bileceğinden, geçici olarak ek depolama sağlamanız gerekebilir.
