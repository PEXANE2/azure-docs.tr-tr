...
Özel bir uç nokta oluşturduğunuzda, varsayılan olarak `privatelink` alt etki alanına karşılık gelen bir özel DNS bölgesi de oluşturur (veya var olan). Tam anlamda, depolama hesabınız için özel bir uç nokta kullanmak üzere özel bir DNS bölgesi oluşturmak gerekmez, ancak Azure dosya paylaşımınızı Active Directory bir Kullanıcı sorumlusu veya erişim için bağlama sırasında kesinlikle önerilir ve açıkça gereklidir .
...
Özel DNS bölgeinizde, `storageaccount.privatelink.file.core.windows.net` için bir kayıt ve bir CNAME kaydı oluşturuyoruz ve bu, `storageaccount.file.core.windows.net`. Azure özel DNS bölgeniz özel uç noktasını içeren sanal ağa bağlı olduğundan, Azure VM 'de PowerShell 'den `Resolve-DnsName` cmdlet 'ini çağırarak DNS yapılandırmasını gözlemleyebilirsiniz (alternatif olarak Windows ve Linux 'ta `nslookup`):

```powershell
Resolve-DnsName -Name "storageaccount.file.core.windows.net"
```

Bu örnekte, depolama hesabı `storageaccount.file.core.windows.net` özel uç noktanın özel IP adresi olarak çözümlenir ve bu `192.168.0.4`olur.
...
Aynı komutu şirket içinde çalıştırırsanız, aynı depolama hesabı adının bunun yerine depolama hesabının genel IP adresine çözümlendiğini görürsünüz; `storageaccount.file.core.windows.net`, `storageaccount.privatelink.file.core.windows.net`için bir CNAME kaydıdır, bu da depolama hesabını barındıran Azure Storage kümesi için bir CNAME kaydıdır:
...
- İstemcilerinizdeki ana bilgisayar dosyasını değiştirerek istenen özel uç noktanın özel IP adresine `storageaccount.file.core.windows.net` çözün. Azure dosya paylaşımlarınızı bağlamak isteyen her istemcide bu değişiklikleri yapmanız gerektiğinden, depolama hesabında veya özel uç noktada yapılan değişiklikler otomatik olarak işlenmediğinden, bu, üretim ortamları için kesinlikle önerilmez.
- Şirket içi DNS sunucularınızda `storageaccount.file.core.windows.net` için bir kayıt oluşturma. Bu, şirket içi ortamınızdaki istemcilerin her bir istemciyi yapılandırmaya gerek kalmadan depolama hesabını otomatik olarak çözümleyebilmesini sağlar, ancak bu çözüm, değişiklikler olmadığından, ana bilgisayar dosyasını değiştirmek için benzer şekilde Brittle labilmesi. Bu çözüm Brittle olsa da, bazı ortamlar için en iyi seçenek olabilir.
- Şirket içi DNS sunucularınızdan `core.windows.net` bölgesini Azure özel DNS bölgenize iletin. Azure özel DNS ana bilgisayarına yalnızca Azure özel DNS bölgesine bağlı sanal ağlarda erişilebilen özel bir IP adresi (`168.63.129.16`) üzerinden erişilebilir. Bu sınırlamaya geçici bir çözüm olarak, Azure özel DNS bölgesine `core.windows.net` iletecek sanal ağınız içinde ek DNS sunucuları çalıştırabilirsiniz. Bu ayarı basitleştirmek için, Azure sanal ağınızda DNS sunucularını otomatik olarak dağıtan ve istediğiniz gibi yapılandırabilecek PowerShell cmdlet 'leri sağladık.
