...
Her depolama hesabının tam etki alanı adı (FQDN) vardır. Genel bulut bölgeleri için, bu FQDN, depolama hesabının adı `storageaccount.file.core.windows.net` `storageaccount` olan kalıbı izler. SMB kullanarak iş istasyonunuza bağlama gibi bu ada karşı istek yaptığınızda işletim sisteminiz, tam etki alanı adını SMB isteklerini göndermek için kullanabileceği bir IP adresine çözümlemek için bir DNS araması gerçekleştirir.

Varsayılan olarak, `storageaccount.file.core.windows.net` genel BITIŞ noktasının IP adresini çözümler. Bir depolama hesabının genel uç noktası, diğer birçok depolama hesabının genel uç noktalarını barındıran bir Azure Depolama kümesinde barındırılır. Özel bir uç nokta oluşturduğunuzda, özel bir DNS bölgesi öğesine eklendiği sanal ağa bağlanır ve bu, depolama hesabınızın özel uç noktasının özel IP adresi için `storageaccount.file.core.windows.net` bir kayıt GIRIŞINE bir CNAME kaydı eşlemesi olur. Bu, sanal ağ içinde `storageaccount.file.core.windows.net` FQDN kullanmanıza ve özel uç noktanın IP adresine çözümlenmenize olanak sağlar.

En son hedefimiz, bir VPN veya ExpressRoute bağlantısı gibi bir ağ tüneli kullanarak, depolama hesabı dahilinde barındırılan Azure dosya paylaşımlarına erişmek için, şirket içi DNS sunucularınızı Azure dosyaları hizmetine yapılan istekleri Azure özel DNS hizmetine iletecek şekilde yapılandırmanız gerekir. Bunu gerçekleştirmek için, Azure sanal ağınızda barındırılan bir DNS sunucusuna `*.core.windows.net` (veya ABD Kamu, Almanya veya Çin Ulusal bulutları için uygun depolama uç noktası sonekini) *koşullu iletmeyi* ayarlamanız gerekir. Daha sonra bu DNS sunucusu, depolama hesabının tam etki alanı adını uygun özel IP adresine çözümleyecek şekilde Azure 'un özel DNS hizmetine yapılan isteği yinelemeli olarak iletir.
...
Şirket içi DNS sunucularınızda, kullanarak `Add-DnsServerConditionalForwarderZone`koşullu bir iletici oluşturun. Trafiği Azure 'a doğru şekilde iletmek için bu koşullu ileticinin tüm şirket içi DNS sunucularınızda dağıtılması gerekir. Ortamınız için uygun `<azure-dns-server-ip>` IP adresleriyle değiştirmeyi unutmayın.
...
Azure sanal ağınızdaki DNS sunucularında, depolama hesabı DNS bölgesi isteklerinin, ayrılmış IP adresi tarafından ele `168.63.129.16`alındığı Azure özel DNS hizmetine yönlendirilmesi gibi bir iletici de yerleştirmeniz gerekir. (Komutları farklı bir `$storageAccountEndpoint` PowerShell oturumunda çalıştırıyorsanız, bu komutları doldurmanız gerektiğini unutmayın.)
...
Aşağıdaki örnek, istekleri depolama hesabına, Azure dosyaları, Azure Blob depolama, Azure Tablo depolama ve Azure kuyruk depolama 'ya dahil edilen istekleri iletir. İsterseniz, `-AzureEndpoints` `New-AzDnsForwardingRuleSet` cmdlet 'in parametresi aracılığıyla kurala ek Azure hizmeti için iletme ekleyebilirsiniz. , Ve `<subnet-name>` değerlerini `<virtual-network-resource-group>`, `<virtual-network-name>`ortamınız için uygun değerlerle değiştirmeyi unutmayın.
...
| `DnsForwarderRootName` | `string` | Varsayılan olarak, Azure 'da dağıtılan DNS sunucuları, yıldız işareti bir yineleyici tarafından `DnsFwder-*`doldurulduğu adlara sahiptir. Bu parametre, bu adın kökünü değiştirir (ör. `DnsFwder`). |
...
| `DnsForwarderRedundancyCount` | `int` | Sanal ağınız için dağıtılacak DNS sanal makine sayısı. Varsayılan olarak, `New-AzDnsForwarder` artıklık sağlamak için Azure sanal AĞıNıZDA iki DNS sunucusunu bir kullanılabilirlik kümesinde dağıtır. Bu sayı istenen şekilde değiştirilebilir. |
...
| `SkipParentDomain` | `SwitchParameter` | Varsayılan olarak, DNS ileticileri ortamınızda bulunan en üst düzey etki alanına uygulanır. Örneğin, `northamerica.corp.contoso.com` bir alt etki alanı ise `corp.contoso.com`, iletici ile `corp.contoso.com`ilişkili DNS sunucuları için oluşturulur. Bu parametre, içinde `northamerica.corp.contoso.com`ileticilerin oluşturulmasına neden olur. |

## <a name="confirm-dns-forwarders"></a>DNS ileticilerini Onayla
DNS ileticilerinin başarıyla uygulanmış olup olmadığını görmek için test etmeden önce, kullanarak `Clear-DnsClientCache`yerel iş istasyonunuzda DNS önbelleğini temizlemeniz önerilir. Depolama hesabınızın tam etki alanı adını başarıyla çözümleyip çözebildiğini sınamak için, veya `Resolve-DnsName` `nslookup`kullanın.
...
Zaten bir VPN veya ExpressRoute bağlantısı ayarladıysanız, depolama hesabınıza bir TCP bağlantısının başarıyla yapıldığını görmek `Test-NetConnection` için ' yi de kullanabilirsiniz.
