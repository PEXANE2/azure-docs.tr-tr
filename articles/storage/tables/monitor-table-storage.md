---
title: Azure Tablo depolamayı izleme | Microsoft Docs
description: Azure Tablo depolamanın performansını ve kullanılabilirliğini izlemeyi öğrenin. Azure Tablo depolama verilerini izleyin, yapılandırma hakkında bilgi edinin ve ölçüm ve günlük verilerini çözümleyin.
author: normesta
services: storage
ms.service: storage
ms.topic: conceptual
...
Her tablo depolama kaynağı için Azure portal **genel bakış** sayfası, istekler ve saatlik faturalandırma gibi kaynak kullanımının kısa bir görünümünü içerir. Bu bilgiler faydalıdır, ancak yalnızca küçük miktarda izleme verisi kullanılabilir. Bu verilerden bazıları otomatik olarak toplanır ve kaynağı oluşturur almaz analiz için kullanılabilir. Bazı yapılandırma ile ek veri koleksiyonu türlerini etkinleştirebilirsiniz.

## <a name="what-is-azure-monitor"></a>Azure İzleyici nedir?
Azure Tablo depolama, Azure 'da tam yığın izleme hizmeti olan [Azure izleyici](../../azure-monitor/overview.md)'yi kullanarak izleme verileri oluşturur. Azure Izleyici, diğer bulutlardaki ve Şirket içindeki Azure kaynaklarınızı ve kaynaklarınızı izlemek için tam bir özellik kümesi sağlar. 
...
Azure Tablo depolama, [Azure kaynaklarından gelen verileri izleme](../../azure-monitor/essentials/monitor-azure-resource.md#monitoring-data)bölümünde açıklanan diğer Azure kaynaklarıyla aynı türde izleme verilerini toplar. 

Azure Tablo Depolaması tarafından oluşturulan ölçümler ve günlük ölçümleri hakkında ayrıntılı bilgi için bkz. [Azure Tablo depolama izleme verileri başvurusu](monitor-table-storage-reference.md) .
...
Kaynak günlüklerini Azure depolama 'ya arşivleme hakkında daha fazla bilgi için bkz. [Azure Kaynak günlükleri](../../azure-monitor/essentials/resource-logs.md#send-to-azure-storage).
...
Kaynak günlüklerini Olay Hub 'larına gönderme hakkında daha fazla bilgi için bkz. [Azure Kaynak günlükleri](../../azure-monitor/essentials/resource-logs.md#send-to-azure-event-hubs).
...
![Azure portal ölçümlere erişme ekran görüntüsü](./media/monitor-table-storage/access-metrics-portal.png)
...
![Azure portal Dimension ile ölçümlere erişme ekran görüntüsü](./media/monitor-table-storage/access-metrics-portal-with-dimension.png)

Azure depolamanın desteklediği boyutların tüm listesi için bkz. [ölçüm boyutları](monitor-table-storage-reference.md#metrics-dimensions).

Azure Tablo Depolaması ölçümleri şu ad alanlarında: 

- Microsoft. Storage/storageAccounts
- Microsoft. Storage/storageAccounts/tableServices

Azure Tablo depolama da dahil olmak üzere tüm Azure Izleyici destek ölçümlerinin bir listesi için bkz. [Azure izleyici desteklenen ölçümler](../../azure-monitor/essentials/metrics-supported.md).
...
Depolama hesabınızın veya tablo depolama hizmetinin ölçüm tanımını listeleyebilirsiniz. [Get-AzMetricDefinition](/powershell/module/az.monitor/get-azmetricdefinition) cmdlet 'ini kullanın.

Bu örnekte, `<resource-ID>` yer tutucuyu tüm depolama hesabının kaynak kimliğiyle veya tablo depolama hizmetinin kaynak kimliği ile değiştirin.  Bu kaynak kimliklerini, Azure portal depolama hesabınızın **Özellikler** sayfalarında bulabilirsiniz.
...
Depolama hesabınızın veya tablo depolama hizmetinin hesap düzeyi ölçüm değerlerini okuyabilirsiniz. [Get-AzMetric](/powershell/module/Az.Monitor/Get-AzMetric) cmdlet 'ini kullanın.
...
Depolama hesabınızın veya tablo depolama hizmetinin ölçüm tanımını listeleyebilirsiniz. [Az Monitor ölçümleri List-Definitions](/cli/azure/monitor/metrics#az-monitor-metrics-list-definitions) komutunu kullanın.
...
Depolama hesabınızın veya tablo depolama hizmetinin ölçüm değerlerini okuyabilirsiniz. [Az Monitor ölçümleri List](/cli/azure/monitor/metrics#az-monitor-metrics-list) komutunu kullanın.
...
Bu örneklerde, `<resource-ID>` yer tutucuyu tüm depolama hesabının kaynak kimliğiyle veya tablo depolama hizmeti ile değiştirin. Bu kaynak kimliklerini, Azure portal depolama hesabınızın **Özellikler** sayfalarında bulabilirsiniz.
...
Bu günlüklerde görünen alanların ayrıntılı bir başvurusu için bkz. [Azure Tablo depolama izleme verileri başvurusu](monitor-table-storage-reference.md).
...
Günlük girişleri yalnızca hizmet uç noktasında yapılan istekler varsa oluşturulur. Örneğin, bir depolama hesabının kendi tablo uç noktasında etkinliği varsa ancak blob veya sıra uç noktalarında etkinlik varsa, yalnızca tablo hizmetine ait günlükler oluşturulur. Azure depolama günlükleri, başarılı ve başarısız istekler hakkında bir depolama hizmetine ilişkin ayrıntılı bilgiler içerir. Bu bilgileri kullanarak istekleri ayrı ayrı izleyebilir ve depolama hizmetiyle ilgili sorunları tanılayabilirsiniz. İstekler en iyi çaba temelinde günlüğe kaydedilir.
...
Günlük oluşturma veya silme gibi, tablo depolama hizmeti tarafından yapılan istekler günlüğe kaydedilmez. Günlüğe kaydedilen verilerin tam listesi için bkz. [depolama oturumu açma işlemleri ve durum iletileri](/rest/api/storageservices/storage-analytics-logged-operations-and-status-messages) ve [depolama günlüğü biçimi](monitor-table-storage-reference.md).
...
Tüm başarısız anonim istekler günlüğe kaydedilmez. Günlüğe kaydedilen verilerin tam listesi için bkz. [depolama oturumu açma işlemleri ve durum iletileri](/rest/api/storageservices/storage-analytics-logged-operations-and-status-messages) ve [depolama günlüğü biçimi](monitor-table-storage-reference.md).
...
![Denetim günlükleri](media/monitor-table-storage/event-hub-log.png)

Güvenlik bilgileri ve olay yönetimi ve izleme araçlarını kullanarak, Olay Hub 'ınıza gönderilen günlük verilerine erişebilir ve bu verileri okuyabilirsiniz. Daha fazla bilgi için bkz. [Azure Kaynak günlükleri](../../azure-monitor/essentials/resource-logs.md#send-to-azure-event-hubs).
...
Daha fazla bilgi için bkz. [Azure izleme verilerini Olay Hub 'ına ve dış iş ortaklarına akış](../../azure-monitor/essentials/stream-monitoring-data-event-hubs.md).

Veriler **Storagetablelogs** tablosunda depolanır. 

#### <a name="sample-kusto-queries"></a>Örnek kusto sorguları

Tablo Depolamayı izlemenize yardımcı olması için **günlük araması** çubuğuna girebileceğiniz bazı sorgular aşağıda verilmiştir. Bu sorgular [Yeni dille](../../azure-monitor/logs/log-query-overview.md)çalışır.
...
- Azure Tablo Depolaması tarafından oluşturulan günlüklere ve ölçümlere yönelik bir başvuru için bkz. [Azure Tablo depolama izleme verileri başvurusu](monitor-table-storage-reference.md).
