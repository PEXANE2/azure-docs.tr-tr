...
:heavy_check_mark: Yükseltme planı

:heavy_check_mark: Yükseltme gerçekleştir
...
| Veri işlemleri – günlükleri  | Evet | Destek bileti Azure izleme tümleştirmesini kullanarak belirli bir süre için günlükleri için bir kerelik istekleri | Destek bileti kullanılarak belirli bir süre günlük günlükler için tek kapalı istekler: *Şimdi kullanılabilir*<br><br> Azure Izleme tümleştirmesi – *henüz kullanılamıyor* |
...
Bu tablo API'si için özel uygulamalarınızın kullanılabilir olan ayarlayan açıklar. Şeyleri biraz daha net hale getirmek için bu API kümelerini 2 tür olarak ayırdık: Yönetim API 'Leri ve dosya sistemi API 'Leri.
...
| REST API dosya sistemi                  | [Bağlantı](https://docs.microsoft.com/rest/api/datalakestore/webhdfs-filesystem-apis)                                                                                                                                                                                                                                       | *Kullanıma sunuldu*                                                    | *Artık kullanılabilir -* [bağlantı](https://docs.microsoft.com/rest/api/storageservices/data-lake-storage-gen2)                                                      |
...
Yukarıdaki veri/meta veri kopyalama araçlarını eklemeden Data Lake Storage 2. yükseltmek için Data Lake Storage 1. işleyebilen üçüncü taraflar olduğunu unutmayın (örneğin: [Cloudera](https://blog.cloudera.com/blog/2017/08/use-amazon-s3-with-cloudera-bdr/)). Bunlar, veri taşıma ve bunun yanı sıra iş yükü geçişi gerçekleştiren bir "tek" deneyimi sağlar. Kendi ekosistemi dışında olan araçlar için bir bant dışı yükseltme gerçekleştirmek zorunda kalabilirsiniz.
...
Buradaki ana görev, öneki olan URI `adl://` `abfss://` 'nin öneki olan URI 'yi çevirecek.
...
Daha sonra, yukarıdaki gibi adlandırılan öğeler (örneğin: URI ve kimlik bilgileri), değiştirilmesi gerekecektir. Data Lake depolama Gen1 ile çalışan var olan bir örneğini değiştirebilir veya Data Lake depolama 2. nesil ile işe yarar yeni bir örneğini oluşturabilirsiniz.
