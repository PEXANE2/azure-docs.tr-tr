...
Aşağıdaki komutta `<web_app>` benzersiz bir ada sahip. Geçerli karakterler: `a-z`, `0-9`, ve `-`. Benzersiz değilse, şu hata iletisini alırsınız: `<web_app>` _Verilen ada `<web_app>` sahip Web sitesi zaten var._ Web uygulamasının varsayılan URL'si `https://<web_app>.azurewebsites.net` şeklindedir.  
