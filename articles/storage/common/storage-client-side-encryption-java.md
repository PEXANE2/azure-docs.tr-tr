---
title: Microsoft Azure Depolama için Java ile İstemci Tarafı Şifreleme | Microsoft Dokümanlar
description: Java için Azure Depolama İstemci Kitaplığı, Azure Depolama uygulamalarınız için maksimum güvenlik için istemci tarafı şifrelemeve Azure Anahtar Kasası ile tümleştirmeyi destekler.
services: storage
author: tamram
ms.service: storage
ms.devlang: java
...
ms.locfileid: "75460438"
---
# <a name="client-side-encryption-and-azure-key-vault-with-java-for-microsoft-azure-storage"></a>Microsoft Azure Depolama için Java ile İstemci Tarafı Şifreleme ve Azure Anahtar Kasası
[!INCLUDE [storage-selector-client-side-encryption-include](../../../includes/storage-selector-client-side-encryption-include.md)]

## <a name="overview"></a>Genel Bakış
[Java için Azure Depolama İstemci Kitaplığı,](https://mvnrepository.com/artifact/com.microsoft.azure/azure-storage) Azure Depolama'ya yüklemeden önce istemci uygulamaları içindeki verileri şifrelemeyi ve istemciye indirirken verilerin şifresini çözmeyi destekler. Kitaplık, depolama hesabı anahtar yönetimi için [Azure Key Vault](https://azure.microsoft.com/services/key-vault/) ile tümleştirmeyi de destekler.
...
İstemci kitaplığı şu anda yalnızca tüm lekelerin şifrelemesini destekler. Özellikle, şifreleme kullanıcılar **yükleme*** yöntemleri veya **openOutputStream** yöntemini kullandığınızda desteklenir. İndirmeler için hem tam hem de aralıklı indirmeler desteklenir.  
...
Şifreli bir blob **indirme indirme**/**openInputStream** kolaylık yöntemlerini kullanarak tüm blob içeriğini alma içerir. Sarılmış CEK unwrapped ve iv ile birlikte kullanılır (bu durumda blob meta veri olarak saklanır) kullanıcılara şifresi çözülmüş verileri döndürmek için.
...
> 
> 
> Sorgu işlemlerini gerçekleştirmek için, sonuç kümesindeki tüm anahtarları çözebilecek bir anahtar çözümleyicisi belirtmeniz gerekir. Sorgu sonucu bulunan bir varlık bir sağlayıcıya çözülemez, istemci kitaplık bir hata atar. Sunucu yan projeksiyonları gerçekleştiren herhangi bir sorgu için, istemci kitaplığı seçilen sütunlara varsayılan olarak özel şifreleme meta veri özelliklerini (_ClientEncryptionMetadata1 ve _ClientEncryptionMetadata2) ekler.
...
3. Şifreleme ilkesini oluştururken önbelleğe alma çözümleyicisini giriş olarak kullanın.
   Anahtar Kasa kullanımı yla ilgili daha fazla bilgiyi şifreleme kodu örneklerinde bulabilirsiniz.

## <a name="best-practices"></a>En iyi uygulamalar
Şifreleme desteği yalnızca Java için depolama istemcisi kitaplığında kullanılabilir.
...
* Şifre çözme için:  
  ...
  * Çözümleyici belirtilmemişse, ancak bir anahtar belirtilmişse, tanımlayıcısı gerekli anahtar tanımlayıcısıile eşleşiyorsa anahtar kullanılır. Tanımlayıcı eşleşmiyorsa, bir hata atılır.  
    ...
    [Şifreleme örnekleri,](https://github.com/Azure/azure-storage-net/tree/master/Samples/GettingStarted/EncryptionSamples) Key Vault tümleştirmesiyle birlikte lekeler, kuyruklar ve tablolar için daha ayrıntılı bir uçtan uca senaryo gösterir.

### <a name="requireencryption-mode"></a>Şifreleme modu gerektirir
Kullanıcılar isteğe bağlı olarak tüm yüklemelerin ve indirmelerin şifrelenmesi gereken bir çalışma moduna olanak sağlayabilir. Bu modda, şifreleme ilkesi olmadan veri yükleme veya hizmette şifrelenmemiş verileri indirme girişimleri istemcide başarısız olur. İstek seçenekleri nesnesinin şifreleme bayrağı **bu** davranışı denetler. Uygulamanız Azure Depolama'da depolanan tüm nesneleri şifreleyecekse, hizmet istemcisi nesnesi için varsayılan istek seçeneklerinde **gereksinimŞifreleme** özelliğini ayarlayabilirsiniz.   

Örneğin, **cloudblobClient.getDefaultRequestOptions().setRequireEncryption(true)** kullanarak bu istemci nesnesi üzerinden gerçekleştirilen tüm blob işlemleri için şifreleme gerektirir.
...
Bir şifreleme ilkesi oluşturmanın ve istek seçeneklerine ayarlanın yanı sıra, **TableRequestOptions'da**bir **Şifreleme Çözümleyicisi** belirtmeniz veya varlığın alıcı ve ayarlayıcısında [Şifreleme] özniteliğini ayarlamanız gerekir.
...
Yukarıda belirtildiği gibi, varlık TableEntity uygularsa, o zaman özellikleri alıcı ve ayarlayıcı **Şifreleme Çözer**belirterek yerine [Encrypt] özniteliği ile dekore edilebilir.
...
* Java [Maven paketi için Azure Depolama İstemci Kitaplığını](https://mvnrepository.com/artifact/com.microsoft.azure/azure-storage) indirin  
* [GitHub'dan Java Kaynak Kodu için Azure Depolama İstemci Kitaplığını İndirin](https://github.com/Azure/azure-storage-java)
* Java Maven paketleri için Azure Key Vault Maven Kitaplığını indirin:
  * [Çekirdek](https://mvnrepository.com/artifact/com.microsoft.azure/azure-keyvault-core) paketi
