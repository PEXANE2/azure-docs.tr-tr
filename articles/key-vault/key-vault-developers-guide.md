...
- Anahtar Kasa sertifikalarını yönetin. Daha fazla bilgi için [anahtarlar, sırlar ve sertifikalar hakkında](about-keys-secrets-and-certificates.md)bilgi alabiliyorum.

Azure Anahtar Kasası hakkında daha genel bilgi için [Key Vault nedir'e](key-vault-overview.md)bakın.
...
Azure kaynakları için yönetilen kimlikler hakkında daha fazla bilgi için [yönetilen kimliklere genel bakış'a](../active-directory/managed-identities-azure-resources/overview.md)bakın. AAD ile çalışma hakkında daha fazla bilgi [için](../active-directory/develop/active-directory-integrating-applications.md)bkz.
...
- [CLI ile Anahtar Kasaları oluşturma ve yönetme](quick-create-cli.md)
- [PowerShell ile Anahtar Kasaları oluşturun ve yönetin](quick-create-powershell.md)
- [Azure bağlantı noktası yla Anahtar Kasaları oluşturma ve yönetme](quick-create-portal.md)
- [Python ile Anahtar Kasaları oluşturma ve yönetme](quick-create-python.md)
- [Java ile Anahtar Kasaları oluşturma ve yönetme](quick-create-java.md)
- [Node.js ile Anahtar Kasaları oluşturma ve yönetme](quick-create-node.md)
- [.NET (v4 SDK) ile Anahtar Kasaları oluşturma ve yönetme](quick-create-net.md)
- [Azure Kaynak Yöneticisi şablonu aracılığıyla önemli bir kasa oluşturun ve bir sır ekleyin](quick-create-template.md)
...
.NET SDK'nın 2.x sürümü hakkında daha fazla bilgi için [Sürüm notlarına](key-vault-dotnet2api-release-notes.md)bakın.
...
- [Azure'daki bir web uygulamasından](quick-create-net.md) Azure Key Vault'u kullanmayı öğrenmenize yardımcı olmak için bir Web Uygulamasından Azure Key Vault 'u kullanın . 
...
- [Abonelik taşındıktan sonra anahtar kasa kiracı kimliğini değiştirin](key-vault-subscription-move-fix.md) - Azure aboneliğinizi kiracı A'dan kiracı B'ye taşıdığınızda, varolan anahtar kasalarınıza kiracı B'deki asıllar (kullanıcılar ve uygulamalar) erişemez.
- [Güvenlik duvarının arkasındaki Key Vault'a erişim](key-vault-access-behind-firewall.md) - Anahtar kasa istemciuygulamasına erişmek için çeşitli işlevler için birden çok uç noktaya erişebilmesi gerekir.
- [Azure Anahtar Kasası için HSM Korumalı Anahtarları Oluşturma ve Aktarma](key-vault-hsm-protected-keys.md) - Bu, Azure Key Vault ile kullanmak üzere kendi HSM korumalı anahtarlarınızı planlamanıza, oluşturmanıza ve aktarmanıza yardımcı olur.
...
- [Key Vault'u uçtan uca anahtar döndürme ve denetleme yle nasıl ayarlaylayın](key-vault-key-rotation-log-monitoring.md) - Bu, Azure Key Vault ile anahtar döndürme ve denetlemenin nasıl ayarlanıştu.
...
- [CLI ile Key Vault yumuşak silme nasıl kullanılacağı,](key-vault-soft-delete-cli.md) bir anahtar kasasının ve yumuşak silme etkinleştirilmiş çeşitli anahtar tonoz nesnelerinin kullanım ve yaşam döngüsü boyunca size yol gösterin.
- [PowerShell ile Key Vault yumuşak silme nasıl kullanılır,](key-vault-soft-delete-powershell.md) bir anahtar kasasının ve yumuşak silme etkinleştirilmiş çeşitli anahtar kasa nesnelerinin kullanım ve yaşam döngüsü boyunca size yol gösterin.
...
- [Key Vault yumuşak silme davranışı](key-vault-ovw-soft-delete.md) silinen nesnelerin kurtarılmasına izin veren bir özelliği açıklar, silme yanlışlıkla veya kasıtlı olsun.
- [Key Vault istemcisi azaltma](key-vault-ovw-throttling.md) sizi temel azaltma kavramlarına yönlendirir ve uygulamanız için bir yaklaşım sunar.
- [Key Vault depolama hesabı anahtarlarına genel bakış,](key-vault-ovw-storage-keys.md) Key Vault tümleştirmeAzure Depolama Hesapları anahtarlarını açıklar.
- [Key Vault güvenlik dünyaları](key-vault-ovw-security-worlds.md) bölgeler ve güvenlik alanları arasındaki ilişkileri açıklar.
