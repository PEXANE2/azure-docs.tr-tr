...
ms.locfileid: "71003888"
---
# <a name="tutorial-use-a-linux-vm-and-a-python-app-to-store-secrets-in-azure-key-vault"></a>Öğretici: Azure Key Vault gizli dizileri depolamak için bir Linux sanal makinesi ve Python uygulaması kullanma
...
Aşağıdaki örnek, **myVM** adlı bir VM oluşturur ve **azureuser** adlı bir kullanıcı hesabı ekler. Parametresi otomatik olarak bir SSH anahtarı oluşturur ve varsayılan anahtar konumuna ( **~/vb SSH**) koyar. `--generate-ssh-keys` Bunun yerine belirli bir anahtar kümesi oluşturmak için `--ssh-key-value` seçeneğini kullanın.
