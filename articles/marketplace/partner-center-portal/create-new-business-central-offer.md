---
title: Microsoft ticari marketi 'nde yeni bir Dynamics 365 Iş Merkezi teklifi oluşturun
description: Microsoft Iş Ortağı Merkezi 'nde ticari Market portalı 'nı kullanarak Azure Marketi 'nde, Microsoft AppSource veya bulut çözümü sağlayıcısı (CSP) aracılığıyla listeleme veya satma için yeni bir Dynamics 365 Iş Merkezi teklifi oluşturma.
author: dsindona
ms.author: dsindona
...
Teklif listesi sayfası teklifiniz için ayrıntıları tanımladığınız yerdir (ad, açıklama, görüntüler vb.).
...
Bu bölümde, müşteri teklifinizi gösterirken kullanılacak logo ve görüntüler sağlayabilirsiniz. Tüm görüntülerin. png biçiminde olması gerekir. Teklif logonuzu iki boyutta karşıya yükleyin: **küçük (48 x 48)** ve **büyük (216 x 216)**.
...
İsteğe bağlı olarak, teklifinizi gösteren en fazla beş video ekleyebilirsiniz. Bu videoların YouTube ve/veya Vimeo 'da barındırılması gerekir. Her biri için videonun adını, URL 'sini ve videonun küçük bir görüntüsünü girin (1280 x 720 piksel)
...
Yukarıdaki **Bağlan** ' ı seçtiyseniz uygulama yüklemenizin URL 'sini burada belirtin. Yüklemesi gerektirmeyen bağlı hizmetler için, hizmet giriş sayfanızın veya kaydolma sayfanızın URL 'sini sağlayın.
...
- **Sınama sürücüsü süresi** (zorunlu): test sürücüsünün etkin kalacağı sürenin uzunluğunu saat cinsinden belirtin. Bu süre dolduktan sonra test sürücüsü otomatik olarak sona erer. Bu süre yalnızca birkaç saat kadar ayarlanabilir (örneğin, "2" saat, "1,5" geçerli değildir).
...
**Test sürücüsü** sekmesinin altında bulunan **Market listeleme** seçeneği, test sürücüsü deneyiminin ayrıntılarını tanımladığınız yerdir.
...
Teklifinizin bir belge ( `.pdf` . PDF biçiminde) içinde listelenen anahtar kullanımı senaryolarını listeleyen bir dosyayı karşıya yüklemeniz gerekir. Burada listelenen tüm senaryolar, Market için teklifinizi onaylamadan önce doğrulama takımımız tarafından doğrulanabilir.
