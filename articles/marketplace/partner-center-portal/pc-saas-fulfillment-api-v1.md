...
Azure Marketi API 'SI için uç nokta `https://marketplaceapi.microsoft.com`.
...
| yetkilendirme      | Yes          | JSON Web belirteci (JWT) taşıyıcı belirteci.                    |
...
| içerik türü           |   Yes        |    `application/json`                                                                   |
|  yetkilendirme         |   Yes        |    JSON Web belirteci (JWT) taşıyıcı belirteci.                                               |
...
| resourceLocation   | Dize        | Oluşturulan veya değiştirilen aboneliğin bağlantısı. Bu, istemcinin güncelleştirilmiş durum post işlemini almasını sağlar. Bu değer işlemler için `Unsubscribe` ayarlanmadı. |
