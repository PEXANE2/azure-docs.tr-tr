...
ms.locfileid: "74666078"
---
# <a name="azure-iot-edge-security-manager"></a>Azure IoT Edge Güvenlik Yöneticisi

Azure IoT Edge Güvenlik Yöneticisi, güvenli Silicon Hardware 'i soyutlayarak IoT Edge cihazı ve tüm bileşenlerini korumak için iyi bir güvenlik çekirdedir. Bu, güvenlik sağlamlaştırma için odak noktasıdır ve özgün ekipman üreticilerine (OEM) teknoloji tümleştirme noktası sağlar.
...
Yönetim API 'si, yalnızca IoT Edge aracılarından çağrılabilir olan ayrıcalıklı bir API 'dir.  IoT Edge güvenlik arka plan programı önyükleme ve IoT Edge Aracısı başladığından, IoT Edge aracısının kurcalanmadığını test ettikten sonra IoT Edge Aracısı için örtülü bir kayıt oluşturabilir. İş yükü API 'sinin kullandığı kanıtlama işlemi aynı zamanda yönetim API 'sine erişimi yalnızca IoT Edge aracısına kısıtlar.
