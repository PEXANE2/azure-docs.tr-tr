...
1. CLı kullanarak `myUserAssignedIdentity` adlı Kullanıcı tarafından atanan bir kimlik oluşturun.

    ```azurecli-interactive
    az identity create -resource-group myResourceGroup --name myUserAssignedIdentity
    ```

    Bu komutun çıkışında `id` özelliğinin değerini aklınızda edin.

1. Bu yapılandırma deposuna Kullanıcı tarafından atanan yeni kimliği atamak için [az appconfig Identity Assign] komutunu çalıştırın. Önceki adımda not ettiğiniz `id` özelliğinin değerini kullanın.
