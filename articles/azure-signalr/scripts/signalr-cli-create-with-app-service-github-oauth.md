...
| [az webapp config appsettings set](/cli/azure/webapp/config/appsettings#az_webapp_config_appsettings_set) | Web uygulaması için yeni uygulama ayarları ekler. Bu uygulama ayarları, SignalR bağlantı dizesini ve GitHub OAuth uygulama gizli dizilerini depolamak için kullanılır. |
| [az webapp deployment user set](/cli/azure/webapp/deployment/user#az_webapp_deployment_user_set) | Dağıtım kimlik bilgilerini güncelleştirir. |
| [az webapp deployment source config-local-git](/cli/azure/webapp/deployment/source#az_webapp_deployment_source_config_local_git) | Web uygulaması dağıtımı için kopyalanıp gönderilecek bir git deposu uç noktası için URL alın. |
