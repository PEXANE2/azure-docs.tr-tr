...
description: Azure SignalR hizmeti için tanılama günlüklerini ayarlamayı ve bunu kendi kendine sorun gidermeye nasıl kullanacağınızı öğrenin.
author: wanl
ms.service: signalr
ms.topic: conceptual
ms.date: 12/17/2019
...
ms.locfileid: "75750273"
---
# <a name="diagnostic-logs-for-azure-signalr-service"></a>Azure SignalR hizmeti için tanılama günlükleri

Bu öğreticide, Azure SignalR hizmeti için tanılama günlüklerini ve tanılama günlüklerinin nasıl ayarlanacağı ve tanılama günlükleri ile nasıl sorun giderebileceğiniz ele alınmaktadır.

## <a name="prerequisites"></a>Ön koşullar
Tanılama günlüklerini etkinleştirmek için, günlük verilerinizi depolamak üzere bir yere ihtiyacınız vardır. Bu öğretici, Azure depolama ve Log Analytics kullanır.

* [Azure depolama](../azure-monitor/platform/resource-logs-collect-storage.md) -ilke denetimi, statik analiz veya yedekleme için tanılama günlüklerini saklar.
...
## <a name="set-up-diagnostic-logs-for-an-azure-signalr-service"></a>Azure SignalR hizmeti için tanılama günlüklerini ayarlama

Azure SignalR hizmeti için tanılama günlüklerini görüntüleyebilirsiniz. Bu Günlükler, Azure SignalR hizmet örneğiniz için daha zengin bir bağlantı görünümü sağlar. Tanılama günlükleri, her bağlantıyla ilişkin ayrıntılı bilgi sağlar. Örneğin, temel bilgiler (Kullanıcı KIMLIĞI, bağlantı KIMLIĞI ve aktarım türü vb.) ve bağlantının olay bilgileri (Connect, bağlantıyı kes ve iptal olayı vb.). Tanılama günlükleri, sorun tanımlama, bağlantı izleme ve analiz için kullanılabilir.
...
1.  Yeni tanılama ayarları kaydedin.
...
Azure SignalR hizmeti, tanılama günlüklerini tek bir kategoride yakalar:
...
Günlükler, **tanılama günlükleri** bölmesinde yapılandırılan depolama hesabında depolanır. Tanılama günlüklerini depolamak için `insights-logs-alllogs` adlı bir kapsayıcı otomatik olarak oluşturulur. Kapsayıcının içinde Günlükler dosya `resourceId=/SUBSCRIPTIONS/XXXXXXXX-XXXX-XXXX-XXXX-XXXXXXXXXXXX/RESOURCEGROUPS/XXXX/PROVIDERS/MICROSOFT.SIGNALRSERVICE/SIGNALR/XXX/y=YYYY/m=MM/d=DD/h=HH/m=00/PT1H.json`depolanır. Temel olarak, yol `resource ID` ve `Date Time`tarafından birleştirilir. Günlük dosyaları `hour`göre bölünür. Bu nedenle, dakikalar her zaman `m=00`.
...
Ad | Açıklama
------- | -------
type | Günlük olayının türü. Şu anda Azure SignalR hizmetine bağlantı hakkında bilgi sağlıyoruz. Yalnızca `ConnectivityLogs` türü kullanılabilir
koleksiyon | Günlük olayı koleksiyonu. İzin verilen değerler şunlardır: `Connection`, `Authorization` ve `Throttling`
...
1. Hedef Log Analytics `Logs` tıklayın.

    ![Log Analytics menü öğesi](./media/signalr-tutorial-diagnostic-logs/log-analytics-menu-item.png)

2. Tanılama günlüklerini sorgulamak için `SignalRServiceDiagnosticLogs` girin ve zaman aralığını seçin. Gelişmiş sorgu için bkz. [Azure izleyici 'de Log Analytics kullanmaya başlama](../azure-monitor/log-query/get-started-portal.md)
...
### <a name="troubleshooting-with-diagnostic-logs"></a>Tanılama günlükleri ile sorun giderme

Azure SignalR hizmeti sorunlarını gidermek için, hataları yakalamak üzere sunucu/istemci tarafı günlüklerini etkinleştirebilirsiniz. Azure SignalR hizmeti, şu anda tanılama günlüklerini kullanıma sunar. Ayrıca, hizmet tarafı için günlükleri de etkinleştirebilirsiniz.

Bağlantının beklenmedik şekilde büyümesi veya düşürülme durumunda sorun gidermek için tanılama günlüklerinden yararlanabilirsiniz.
...
Bağlantının bağlantısı kesilirse, tanılama günlükleri bu bağlantı kesme olayını kaydeder; `operationName``ConnectionAborted` veya `ConnectionEnded` görürsünüz.

`ConnectionAborted` ve `ConnectionEnded` arasındaki fark `ConnectionEnded`, istemci veya sunucu tarafı tarafından tetiklenen beklenen bir bağlantı kesilmesi olur. `ConnectionAborted` genellikle beklenmedik bir bağlantı bırakma olayıdır ve bu nedenle iptal nedeni `message`olarak sağlanır.
...
Büyüyen beklenmeyen bağlantıyla ilgili sorunları gidermek için yapmanız gereken ilk şey, fazladan bağlantıları filtrelemeniz. Test istemci bağlantınıza benzersiz test Kullanıcı KIMLIĞI ekleyebilirsiniz. Ardından tanılama günlükleri ile birlikte doğrulayın, birden fazla istemci bağlantısı aynı test Kullanıcı KIMLIĞINE veya IP 'ye sahip olursa, bu durum büyük olasılıkla istemci tarafında beklentiden daha fazla bağlantı oluşturabilir ve bu bağlantıyı kurabilir. İstemci tarafını denetleyin.

#### <a name="authorization-failure"></a>Yetkilendirme hatası

İstemci istekleri için 401 Yetkisiz döndürülürse, tanılama günlüklerinizi denetleyin. `Failed to validate audience. Expected Audiences: <valid audience>. Actual Audiences: <actual audience>`karşılaşırsanız, erişim belirteçinizdeki tüm seyircilerin geçersiz olduğu anlamına gelir. Günlükte önerilen geçerli izleyicileri kullanmayı deneyin.


#### <a name="throttling"></a>Azaltma

Azure SignalR hizmeti ile SignalR istemci bağlantıları kuramadık, tanılama günlüklerinizi kontrol edin. Tanılama günlüğünde `Connection count reaches limit` yaşarsanız, SignalR hizmetine bağlantı sayısı sınırına ulaşan çok fazla bağlantı kurarsınız. SignalR hizmetinizi ölçeklendirin. Tanılama günlüğünde `Message count reaches limit` yaşarsanız, bu, ücretsiz katmanı kullandığınızda ve ileti kotasını kullandığınız anlamına gelir. Daha fazla ileti göndermek istiyorsanız, SignalR hizmetinizi Standart katmana değiştirerek ek iletiler gönderilmesini düşünün. Daha fazla bilgi için bkz. [Azure SignalR hizmeti fiyatlandırması](https://azure.microsoft.com/pricing/details/signalr-service/).

### <a name="get-help"></a>Yardım alın

İlk olarak sorun gidermeyi öneririz. Çoğu soruna uygulama sunucusu veya ağ sorunları neden olmuştur. Temel nedeni bulmak için tanılama günlüğü ve [temel sorun giderme kılavuzu](https://github.com/Azure/azure-signalr/blob/dev/docs/tsg.md) [ile sorun giderme kılavuzunu](#troubleshooting-with-diagnostic-logs) izleyin.
...
> Note: sorun GitHub 'da açarsanız, hassas bilgilerinizi (örneğin, kaynak KIMLIĞI, sunucu/istemci günlükleri) özel olarak saklayın, yalnızca Microsoft kuruluşundaki üyelere özel olarak gönderin.  
