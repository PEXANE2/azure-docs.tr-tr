...
|-|-|-|-|
|\[Preview\] Audit PCI v3.2.1:2018 controls and deploy specific VM Extensions to support audit requirements|İlke Ataması|List of Resource Types | Audit diagnostic setting for selected resource types. Default value is all resources are selected| 
|İzin verilen konumlar|İlke Ataması|List Of Allowed Locations|List of data center locations allowed for any resource to be deployed into. This list is customizable to the desired Azure locations globally. Select locations you wish to allow.| 
|Allowed Locations for resource groups|İlke Ataması |Allowed Location |This policy enables you to restrict the locations your organization can create resource groups in. Coğrafi uyumluluk gereksinimlerinizi zorunlu kılmak için kullanabilirsiniz.| 
|Deploy Auditing on SQL servers|İlke Ataması|Retention days|Data retention in number of days. Default value is 180 but PCI requires 365.| 
|Deploy Auditing on SQL servers|İlke Ataması|Resource group name for storage account|Auditing writes database events to an audit log in your Azure Storage account (a storage account will be created in each region where a SQL Server is created that will be shared by all servers in that region).| 
