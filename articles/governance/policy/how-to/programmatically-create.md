...
1. İlke tanımınız oluşturduktan sonra aşağıdaki komutları çalıştırarak ilke ataması oluşturabilirsiniz:
   ...
   ' Deki `New-AzPolicyAssignment` kapsam parametresi, yönetim grubu, abonelik, kaynak grubu veya tek bir kaynakla birlikte geçerlidir. Parametresi bir tam kaynak yolu kullanır, **ResourceId** özelliği `Get-AzResourceGroup` döndürür. Desenini **kapsam** her kapsayıcı aşağıdaki gibidir. ,,, Ve `{rName}` 'ıkaynakadı,kaynakgrubuadı,abonelikkimliğiveyönetimgrubuadıyladeğiştirin.`{mgName}` `{rgName}` `{subId}`
