---
title: Azure Güvenlik ve Uyumluluk Şeması-PCI DSS için veri ambarı
description: Azure Güvenlik ve Uyumluluk Şeması-PCI DSS için veri ambarı
services: security
author: meladie
...
ms.locfileid: "68780714"
---
# <a name="azure-security-and-compliance-blueprint-data-warehouse-for-pci-dss"></a>Azure Güvenliği ve Uyumluluğu Şeması: PCI DSS için veri ambarı

## <a name="overview"></a>Genel Bakış

Bu Azure Güvenlik ve Uyumluluk Şeması, Azure 'da ödeme kartı sektör verileri güvenlik standartları (PCI DSS 3,2) gereksinimlerine yardımcı olan bir veri ambarı mimarisinin dağıtımına yönelik rehberlik sağlar. Ortak bir başvuru mimarisine sahiptir ve kredi kartı verilerinin (kart numarası, sona erme ve doğrulama verileri dahil) güvenli, uyumlu, çok katmanlı bir ortamda uygun şekilde işlenmesini gösterir. Bu şema, müşterilerin belirli güvenlik ve uyumluluk gereksinimlerini karşılayabileceği ve müşterilerin kendi veri ambarı çözümlerini Azure 'da oluşturup yapılandırabileceği bir temel görevi gören yolları gösterir.
...
![PCI DSS başvuru mimarisi diyagramı Için veri ambarı](images/pcidss-dw-architecture.png "PCI DSS başvuru mimarisi diyagramı Için veri ambarı")
...
**SQL veri ambarı**: [SQL veri ambarı](https://docs.microsoft.com/azure/sql-data-warehouse/sql-data-warehouse-overview-what-is) , yüksek düzeyde paralel Işleme (MPP) kullanan ve kullanıcıların finansal verileri verimli bir şekilde belirlemesine izin veren bir kurumsal veri ambarı (EDW) kullanır. Kullanıcılar, büyük verileri SQL veri ambarı 'na aktarmak ve yüksek performanslı analizleri çalıştırmak için MPP gücünden yararlanmak üzere basit PolyBase T-SQL sorguları kullanabilir.
...
**Veri Kataloğu**: Veri [kataloğu](https://docs.microsoft.com/azure/data-catalog/data-catalog-what-is-data-catalog) , veri kaynaklarını verileri yöneten kullanıcılar tarafından kolayca bulunabilir ve anlaşılabilir hale getirir. Ortak veri kaynakları, finansal veriler için kaydedilebilir, etiketlenebilir ve aranabilir. Veriler mevcut konumunda kalır, ancak veri kaynağı konumuna yönelik bir başvuru ile birlikte meta verilerinin bir kopyası veri kataloğuna eklenir. Bu meta veriler ayrıca her bir veri kaynağının arama ile kolayca bulunabilmesini ve bunları bulan kullanıcılar tarafından anlaşılabilmesini sağlamak üzere dizine alınır.
...
**Ağ güvenlik grupları**: [Ağ güvenlik grupları](https://docs.microsoft.com/azure/virtual-network/virtual-networks-nsg) , bir sanal ağ içindeki trafiğe izin veren veya bu trafiği reddeden erişim denetimi listelerini içerir. Ağ güvenlik grupları, trafiği bir alt ağda veya tek bir sanal makine düzeyinde güvenli hale getirmek için kullanılabilir. Aşağıdaki ağ güvenlik grupları var:
  ...
  - Veri katmanı (SQL Server kümeleri, SQL Server tanığı ve SQL Load Balancer) için bir ağ güvenlik grubu
...
Bu başvuru mimarisi için veri akışı diyagramı [indirilebilir](https://aka.ms/pcidss-dw-tm) veya aşağıda bulunabilir. Bu model, müşterilerin değişiklik yaparken sistem altyapısında olası risk noktalarını anlamasına yardımcı olabilir.

![PCI DSS başvuru mimarisi diyagramı Için veri ambarı](images/pcidss-dw-threat-model.png "PCI DSS tehdit modeli Için PaaS Web uygulaması")
...
[Azure Güvenlik ve Uyumluluk Şeması PCI DSS veri ambarı uygulama matrisi](https://aka.ms/pcidss-dw-cim) , veri ambarı mimarisi tarafından hangi PCI DSS 3,2 gereksinimlerinin giderildiği hakkında ayrıntılı açıklamalar dahil bilgi sağlar. uygulama, kapsanan her denetimin gereksinimlerini karşılar.
...
[Azure Active Directory](https://docs.microsoft.com/azure/active-directory/active-directory-whatis) , dağıtımı yönetmek ve ortamla etkileşime geçen personele erişimi sağlamak için gereklidir. Mevcut bir Windows Server Active Directory, [dört tıklamayla](https://docs.microsoft.com/azure/active-directory/connect/active-directory-aadconnect-get-started-express)Azure Active Directory tümleştirilebilir. Müşteriler Dağıtılmış Active Directory altyapısını (etki alanı denetleyicileri), dağıtılan Active Directory altyapısını Azure Active Directory ormanının bir alt etki alanı yaparak mevcut bir Azure Active Directory da bağlayabilir.
...
-   [Azure Data Factory](https://docs.microsoft.com/azure/data-factory/introduction) , karmaşık karma Ayıkla-Dönüştür-yükle ve veri tümleştirme projeleri için oluşturulmuş, yönetilen bir bulut hizmetidir. Azure Data Factory, verilerin ne zaman geldiğini ve nereden geldiğini belirlemek için görselleştirme ve izleme araçları dahil, kart sahibi verilerini izlemeye ve bulmaya yardımcı olacak yetenekler içerir. Müşteriler, Azure Data Factory kullanarak, farklı veri depolarından veri alan işlem hatları adlı veri odaklı iş akışları oluşturabilir ve zamanlayabilir. Bu işlem hatları, müşterilerin hem iç hem de dış kaynaklardan veri almasına olanak tanır. Müşteriler daha sonra çıktıyı verileri Azure SQL veri ambarı gibi veri depolarına işleyebilir ve dönüştürebilir.
