---
title: Azure Güvenlik ve Uyumluluk Şeması-PCI DSS için veri analizi
description: Azure Güvenlik ve Uyumluluk Şeması-PCI DSS için veri analizi
services: security
author: meladie
...
ms.locfileid: "73496397"
---
# <a name="azure-security-and-compliance-blueprint-analytics-for-pci-dss"></a>Azure Güvenlik ve Uyumluluk Şeması: PCI DSS Analytics

## <a name="overview"></a>Genel Bakış

Bu Azure Güvenlik ve Uyumluluk Şeması, Azure 'da ödeme kartı sektör verileri güvenlik standartları (PCI DSS 3,2) gereksinimlerine yardımcı olan bir veri analizi mimarisinin dağıtımına yönelik rehberlik sağlar. Ortak bir başvuru mimarisine sahiptir ve kredi kartı verilerinin (kart numarası, sona erme ve doğrulama verileri dahil) güvenli, uyumlu, çok katmanlı bir ortamda uygun şekilde işlenmesini gösterir. Bu şema, müşterilerin belirli güvenlik ve uyumluluk gereksinimlerini karşılayabilme yollarını gösterir ve müşterilerin Azure 'da kendi veri analizi çözümlerini oluşturup yapılandırmasına yönelik bir temel işlevi görür.
...
![PCI DSS başvuru mimarisi diyagramı için analiz](images/pcidss-analytics-architecture.png "PCI DSS başvuru mimarisi diyagramı için analiz")
...
**Azure Güvenlik Merkezi**: müşteriler [Azure Güvenlik Merkezi](https://docs.microsoft.com/azure/security-center/security-center-intro)ile iş yükleri genelinde güvenlik ilkelerini merkezi olarak uygulayabilir ve yönetebilir, tehditlere maruz kalma olasılığını sınırlayabilir, saldırıları algılayıp yanıtlayabilir. Ayrıca, Azure Güvenlik Merkezi, güvenlik duruşunu artırmaya ve verileri korumaya yardımcı olmak için yapılandırma ve hizmet önerileri sağlamak üzere Azure hizmetleri 'nin mevcut yapılandırmalarına erişir.
...
-   [Active Directory değerlendirmesi](../../azure-monitor/insights/ad-assessment.md): Active Directory durum denetimi çözümü, düzenli aralıklarla sunucu ortamlarının riskini ve sistem durumunu değerlendirir ve dağıtılan Sunucu altyapısına özgü önerilerin öncelikli bir listesini sağlar.
...
**Azure**İzleyici: [Azure Izleyici](https://docs.microsoft.com/azure/monitoring-and-diagnostics/) , kuruluşların Azure kaynaklarında API çağrılarını izleme de dahil olmak üzere performansı izlemelerine, uyarıları oluşturmalarına ve verileri arşivlemesini sağlayarak eğilimleri belirlemesine yardımcı olur.
...
Bu başvuru mimarisi için veri akışı diyagramı [indirilebilir](https://aka.ms/PCIAnalyticsTM) veya aşağıda bulunabilir. Bu model, müşterilerin değişiklik yaparken sistem altyapısında olası risk noktalarını anlamasına yardımcı olabilir.

![PCI DSS başvuru mimarisi diyagramı için analiz](images/pcidss-analytics-threat-model.png "PCI DSS tehdit modeli için analiz")

## <a name="compliance-documentation"></a>Uyumluluk belgeleri
[Azure Güvenlik ve Uyumluluk Şeması PCI DSS müşteri sorumluluğu matrisi](https://aka.ms/pcidss-analytics-tm) , tüm PCI DSS 3,2 gereksinimlerinin sorumluluklarını listeler.

[Azure Güvenlik ve uyumluluk şeması-PCI DSS Data Analytics Uygulama matrisi](https://aka.ms/pcidss-analytics-cim) , veri analizi mimarisi tarafından hangi PCI DSS 3,2 gereksinimlerinin giderildiği hakkında ayrıntılı açıklamalar dahil bilgi sağlar. uygulama, kapsanan her denetimin gereksinimlerini karşılar.
