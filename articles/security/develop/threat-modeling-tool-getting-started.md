...
description: Threat Modeling Tool kullanmaya nasıl başlaleyeceğinizi öğrenin. Bir diyagram oluşturun, tehditleri ve tehditleri azaltın ve her bir hafifletme doğrulaması yapın.
services: security
...
> Ricardo: Hi Cristina, tehdit modeli diyagramı üzerinde çalıştım ve ayrıntılarımız olduğundan emin olmak istiyor. Üzerinde görünmeme yardımcı olabilir mi?
...
> Cristina: hemen bana görünür. Tehditler hakkında ne var?
> Ricardo: sizi gösterelim.

## <a name="analyzing-threats"></a>Tehditler çözümleniyor

Simge menü seçiminden (büyüteç ile dosya) analiz görünümü ' ne tıkladıktan sonra, bir oluşturulan tehditler listesine alınır. Bu, bir dizi Threat Modeling Tool tehdit **[(sahtekarlık, müdahale etme, bilgi açığa çıkması, ret, hizmet reddi ve ayrıcalık yükselmesi)](https://en.wikipedia.org/wiki/STRIDE_(security))** adlı SDL yaklaşımını kullanır. Bu, yazılımın bu 6 kategori kullanılarak bulunan öngörülebilir bir tehdit kümesi altına geldiği fikrdir.
...
Mevcut azaltma işlemleri veya güvenlik garantisi nedeniyle, durum açılan listesinden "uygulanamaz" olarak değiştirilebiliyorsa tehditler tasarıma uygulanmaz. Üç farklı seçenek vardır: başlatılmamış – varsayılan seçim, üzerinde tam olarak çalıştıktan sonra öğeleri izlemek için kullanılan ve hafifletilmeye yönelik olarak kullanılır.
