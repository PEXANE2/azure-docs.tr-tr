...
| **Veritabanınızı** | <ul><li>[Saklı yordamlarda dinamik sorgular kullanmayın](#stored-proc)</li></ul> |
...
| **İlgili teknolojiler** | Genel |
| **Öznitelikler**              | Yok  |
| **Başvurular**              | [IE8 güvenlik bölümü V-kapsamlı koruma](https://blogs.msdn.com/ie/archive/2008/07/02/ie8-security-part-v-comprehensive-protection.aspx)  |
...
XmlReader reader = XmlReader.Create(stream, settings);
```
Öğesinin `ProhibitDtd` `XmlReaderSettings` varsayılan değeri true, ancak içinde `XmlTextReader` false olduğunu unutmayın. XmlReaderSettings kullanıyorsanız, Probitdtd 'yi açıkça true olarak ayarlamanız gerekmez, ancak bunu yaptığınız güvenliği sake için tavsiye edilir. Ayrıca, XmlDocument sınıfının varsayılan olarak varlık çözümlemesine izin verdiğini unutmayın. 
...
## <a name="applications-utilizing-httpsys-perform-url-canonicalization-verification"></a><a id="app-verification"></a>Http. sys kullanan uygulamalar URL Kurallaştırma doğrulaması gerçekleştirir
...
| **İlgili teknolojiler** | Genel |
| **Öznitelikler**              | Yok  |
| **Başvurular**              | [Kısıtlanmamış dosya yükleme](https://www.owasp.org/index.php/Unrestricted_File_Upload), [Dosya imza tablosu](https://www.garykessler.net/library/file_sigs.html) |
...
$('body').append(resHTML);   
```
Kullanmayın `innerHtml`; Bunun yerine `innerText`kullanın. Benzer şekilde, yerine `$("#elm").html()`kullanın`$("#elm").text()` 
