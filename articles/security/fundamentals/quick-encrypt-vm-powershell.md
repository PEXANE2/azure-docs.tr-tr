...
ms.locfileid: "68610579"
---
# <a name="quickstart-encrypt-a-windows-iaas-vm-with-azure-powershell"></a>Hızlı Başlangıç: Windows IaaS VM 'sini Azure PowerShell ile şifreleme
...
- [Azure PowerShell modülünün en son sürümünü](/powershell/azure/install-az-ps) yüklemek veya güncelleştirmek
    - Az Module sürümünün 1.0.0 veya üzeri olması gerekir. Sürümü `Get-Module Az -ListAvailable | Select-Object -Property Name,Version,Path` denetlemek için kullanın.
...
1. **Yönetici: Windows PowerShell ISE** pencere, **Görünüm** ' ü ve ardından **komut dosyası bölmesini göster**' i tıklatın.
...
![PowerShell ISE'de çalışan Azure Disk Şifrelemesi önkoşulları betiği](./media/quick-encrypt-vm-powershell/ade-prereq-script.PNG)
...
1. Betiği **yöneticiye kopyalayın: Betik** bölmesini Windows PowerShell ISE ve en üstteki üç değişkeni değiştirin. Kaynak grubu ve konumun, [önkoşullar betiği](#bkmk_PrereqScript) için kullandıklarınızla aynı olması gerekir.  
...
Artık bir anahtar kasası ve VM oluşturup yapılandırdığınıza göre, diski **set-AzVmDiskEncryptionExtension** cmdlet 'i ile şifreleyebilirsiniz. 
...
1. Şifreleme tamamlandığında şu cmdlet komutunu kullanarak, diskin şifrelendiğini doğrulayabilirsiniz: 
     ...
     ```
    ![Get-AzVmDiskEncryptionStatus çıkışı](./media/quick-encrypt-vm-powershell/ade-get-encryption-status.PNG)
