...
description: Microsoft Azure için Müşteri Kasası Azure Güvenlik temeli
author: msmbaldwin
ms.service: security
ms.topic: conceptual
ms.date: 06/05/2020
...
Microsoft Azure için Müşteri Kasası Azure Güvenlik temeli, dağıtımınızın güvenlik duruşunu artırmanıza yardımcı olacak öneriler içerir.

Bu hizmetin taban çizgisi, Azure [güvenlik kıyaslama sürümü 1,0](../benchmarks/overview.md)' dan çizilir ve bu, en iyi yöntemler kılavuzumuzdan Azure 'da bulut çözümlerinizi nasıl güvence altına almak için öneriler sağlar.

Daha fazla bilgi için bkz. [Azure güvenlik temelleri 'ne genel bakış](../benchmarks/security-baselines-overview.md).

## <a name="network-security"></a>Ağ güvenliği

*Daha fazla bilgi için bkz. [güvenlik denetimi: ağ güvenliği](../benchmarks/security-control-network-security.md).*
...
**Rehberlik**: uygulanamaz; bir sanal ağ, alt ağ veya ağ güvenlik grubunu Müşteri Kasası ile ilişkilendiremezsiniz.
...
**Rehberlik**: uygulanamaz; bir sanal ağ, alt ağ veya ağ güvenlik grubunu Azure Müşteri Kasası ile ilişkilendiremezsiniz.
...
**Rehberlik**: uygulanamaz; bir sanal ağ, alt ağ veya ağ güvenlik grubunu Müşteri Kasası ile ilişkilendiremezsiniz. Yapılacak veya İzlenecek ağ yapılandırması yok.
...
**Rehberlik**: uygulanamaz; Microsoft, Müşteri Kasası gibi kaynaklar için kullanılan zaman kaynağını korur.
...
* [Müşteri Kasası için denetim günlükleri](./customer-lockbox-overview.md#auditing-logs)
...
**Rehberlik**: uygulanamaz; Müşteri Kasası kötü amaçlı yazılımdan koruma ile ilgili günlükleri işlemez veya oluşturmaz.
...
**Rehberlik**: uygulanamaz; Müşteri Kasası, DNS ile ilgili günlükleri işlemez veya oluşturmaz.
...
**Rehberlik**: müşteri kasası istekleriniz için yönetici erişimine sahip kullanıcı hesaplarının envanterini saklayın. Azure rol tabanlı erişim denetimi 'ni (Azure RBAC) yapılandırmak için, aboneliğiniz için Azure portal kimlik ve erişim denetimi (ıAM) bölmesini kullanabilirsiniz. Roller, kullanıcılar, gruplar, hizmet sorumluları ve Azure Active Directory içindeki yönetilen kimliklere uygulanır.
...
**Rehberlik**: müşteri kasası isteklerinizi etkinleştirmek ve yapılandırmak IÇIN Azure AD MULTI-Factor AUTHENTICATION (MFA) özellikli bir ayrıcalıklı erişim iş istasyonu (Paw) kullanın.
...
**Rehberlik**: ortamda şüpheli veya güvenli olmayan bir etkinlik oluştuğunda günlükler ve uyarılar oluşturmak için Azure ACTIVE DIRECTORY PRIVILEGED IDENTITY Management (PIM) kullanın.

Ayrıca, riskli Kullanıcı davranışında uyarıları ve raporları görüntülemek için risk algılamalarını Azure Active Directory kullanın.
...
**Rehberlik**: uygun yerlerde merkezi kimlik doğrulama ve yetkilendirme sistemi olarak Azure Active Directory kullanın. Azure Active Directory, bekleyen ve aktarım sırasında veriler için güçlü şifrelemeyi kullanarak verileri korur. Ayrıca, Azure Active Directory sallar, karma ve Kullanıcı kimlik bilgilerini güvenli bir şekilde depolar.

* [Azure Active Directory örneği oluşturma ve yapılandırma](../../active-directory/fundamentals/active-directory-access-create-new-tenant.md)
...
**Rehberlik**: Azure Active Directory eski hesapları keşfetmenize yardımcı olacak Günlükler sağlar. Ayrıca, grup üyeliklerini verimli bir şekilde yönetmek, kurumsal uygulamalara erişmek ve rol atamaları için Azure Active Directory erişim gözden geçirmeleri kullanın. Yalnızca doğru kullanıcıların erişmeye devam ettiğinden emin olmak için, Kullanıcı erişimi düzenli olarak incelenebilir.

* [Azure Active Directory raporlamayı anlayın](../../active-directory/reports-monitoring/index.yml)

* [Azure Active Directory erişim incelemelerini kullanma](../../active-directory/governance/access-reviews-overview.md)
...
Azure Sentinel veya bir üçüncü taraf SıEM ile tümleştirmenize olanak tanıyan, oturum açma etkinliğine Azure Active Directory, denetim ve risk olay günlüğü kaynaklarına erişebilirsiniz.

Azure Active Directory Kullanıcı hesapları için Tanılama ayarları oluşturarak ve denetim günlüklerini ve oturum açma günlüklerini bir Log Analytics çalışma alanına göndererek bu işlemi kolaylaştırabilirsiniz. Log Analytics içinde, istenen günlük uyarılarını yapılandırabilirsiniz.
...
**Rehberlik**: bu öneri müşteri kasası için geçerli değildir.
...
**Rehberlik**: Bu öneri geçerli değildir; Müşteri Kasası için Etiketler desteklenmez.
...
**Rehberlik**: uygulanamaz; Müşteri Kasası, erişim verdiğiniz kaynaklarla aynı abonelikte sağlanacak. Korunacak veya yalıtılacağı ortak uç nokta yok. Müşteri Kasası istek erişimi, kiracı düzeyinde sahip rolünü tutan kullanıcıya verilir.
...
**Rehberlik**: Microsoft, müşteri kasası için temel altyapıyı yönetir ve müşteri verilerinin kaybını veya açıklanmasını engellemek için katı denetimler uygulamıştır.

* [Azure’da müşteri verilerinin korunmasını anlama](./protection-customer-data.md)
...
**Kılavuz**: varsayılan olarak, Microsoft, bulut hizmetleri ve müşteriler arasında seyahat edildiğinde verileri korumak Için aktarım katmanı GÜVENLIĞI (TLS) protokolünü kullanır. Microsoft veri merkezleri, Azure hizmetlerine bağlanan istemci sistemleriyle bir TLS bağlantısı anlaşmasına sahiptir. TLS, güçlü kimlik doğrulaması, ileti gizliliği ve bütünlük (ileti izinsiz değişikliği, yakalama ve forgery), birlikte çalışabilirlik, algoritma esnekliği ve dağıtım ve kullanım kolaylığı sağlar.

* [Azure ile iletim sırasında şifrelemeyi anlama](./encryption-overview.md#encryption-of-data-in-transit)
...
**Rehberlik**: uygulanamaz; Müşteri Kasası kendisi hiçbir müşteri verisi içermez.
...
**Rehberlik**: uygulanamaz; Bu öneri, işlem kaynaklarına yöneliktir. Microsoft, Müşteri Kasası için temel altyapıyı yönetir ve müşteri verilerinin kaybını veya açıklanmasını engellemek için katı denetimler uygulamıştır.
...
**Rehberlik**: uygulanamaz; Müşteri Kasası kendisi müşteri verilerini tutamaz.
...
*Daha fazla bilgi için bkz. [güvenlik denetimi: güvenlik açığı yönetimi](../benchmarks/security-control-vulnerability-management.md).*

### <a name="51-run-automated-vulnerability-scanning-tools"></a>5,1: otomatikleştirilmiş güvenlik açığı tarama araçlarını çalıştırma

**Rehberlik**: uygulanamaz; Microsoft, Müşteri Kasası destekleyen temel sistemler üzerinde güvenlik açığı yönetimi gerçekleştirir.
...
**Rehberlik**: müşteri kasası için Etiketler desteklenmez.
...
**Rehberlik**: uygulanabilir değil, müşteri kasası yapılandırılabilir güvenlik ayarlarına sahip değil.
...
**Rehberlik**: uygulanamaz; Müşteri Kasası, yapılandırılabilir güvenlik ayarlarına sahip değil.
...
**Rehberlik**: uygulanamaz; Müşteri Kasası isteklerine erişim, kaynağın barındırıldığı Azure aboneliğinin sahibiyle sınırlıdır. Kiracı sahibi olarak oturum açma dışında Müşteri Kasası erişmek için parola, gizli dizi veya anahtar yoktur.
...
**Rehberlik**: uygulanamaz; Müşteri Kasası Yönetilen kimlikler kullanmaz.
...
**Rehberlik**: uygulanamaz; Bu kılavuz işlem kaynaklarına yöneliktir. Microsoft kötü amaçlı yazılımdan koruma, Müşteri Kasası çözümünü destekleyen temel ana bilgisayar üzerinde etkinleştirilmiştir.
...
*Daha fazla bilgi için bkz. [güvenlik denetimi: veri kurtarma](../benchmarks/security-control-data-recovery.md).*

### <a name="91-ensure-regular-automated-back-ups"></a>9,1: düzenli Otomatik yedeklemeli UPS sağlayın

**Rehberlik**: uygulanamaz; Müşteri Kasası kendisi müşteri verilerini depolamaz.
...
**Rehberlik**: uygulanamaz; Müşteri Kasası kendisi müşteri verilerini depolamaz, erişim için anahtar veya parola kullanmaz.
