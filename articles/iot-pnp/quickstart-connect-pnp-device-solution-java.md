...
1. _Cihazı_ terminalde bağladığınızda, çıkış iletilerinden biri çevrimiçi durumunu göstermek için aşağıdaki mesajdır. Cihazın çevrimiçi olup olmadığını göstermek için kullanılan `state` özelliği, _doğru_olsun:
...
1. _Hizmet_ terminali çıkışında `environmentalSensor` bileşenine gidin. `state` özelliğinin _doğru_olarak raporlanmışsa görürsünüz:
    ```JSON
    "environmentalSensor" : {
