...
1. Cihaz SDK 'Sı `SampleDevice.capabilitymodel.json` kök klasöründeki `digitaltwin_client/samples` klasörde bulunan dosyayı seçin. **Aç** ' ı seçin ve ardından **Kaydet** ' i seçerek model dosyasını deponuza yükleyin.

1. **Şirket deposu** ' nu ve ardından **arabirimler**' i seçin.

1. **Yeni** ' yi seçip **karşıya yükleyin**.

1. Cihaz SDK 'Sı `EnvironmentalSensor.interface.json` kök klasöründeki `digitaltwin_client/samples/digitaltwin_sample_environmental_sensor` klasörde bulunan dosyayı seçin. **Aç** ' ı seçin ve ardından **Kaydet** ' i seçerek arabirim dosyasını deponuza yükleyin.
