---
title: Azure Stack Edge Pro R cihazını dağıtmaya yönelik ön dağıtım denetim listesi | Microsoft Docs
description: Bu makalede, Azure Stack Edge Pro R cihazınızı dağıtmadan önce toplanacak bilgiler açıklanmaktadır.
services: databox
author: alkohli
ms.service: databox
ms.subservice: edge
...
ms.locfileid: "96531747"
---
# <a name="deployment-checklist-for-your-azure-stack-edge-pro-r-device"></a>Azure Stack Edge Pro R cihazınız için dağıtım denetim listesi  

Bu makalede, Azure Stack Edge Pro R cihazınızın gerçek dağıtımının önüne toplanacak bilgiler açıklanmaktadır. 

Azure Stack Edge Pro R cihazına sipariş verdikten sonra ve cihazı almadan önce bu bilgilere sahip olduğunuzdan emin olmak için aşağıdaki denetim listesini kullanın. 
...
|                                   | <li>Bağlantı noktası 1 için en az 1 X 1-GbE RJ-45 ağ kablosu  </li><li> Bağlantı noktası 3 için en az 1 X 25-GbE SFP + bakır kablo, bağlantı noktası 4</li>| Müşterinin bu kabloları sağlaması gerekir.<br>Cihaz ağ kartları için desteklenen ağ kabloları, anahtarlar ve alıcı vericiler tam listesi için bkz. [Cavium Fastlınq 41000 serisi birlikte çalışabilirlik matrisi](https://www.marvell.com/documents/xalflardzafh32cfvi0z/) ve [Mellanox Dual Port 25G ConnectX-4 kanal ağ bağdaştırıcısı uyumlu ürünler](https://docs.mellanox.com/display/ConnectX4LxFirmwarev14271016/Firmware+Compatible+Products).| 
| Cihaz bağlantısı ilk kez      | <li>IPv4 ayarlarını değiştirilebilen dizüstü bilgisayar. Bu dizüstü bilgisayar bir anahtar veya USB-Ethernet bağdaştırıcısı aracılığıyla bağlantı noktası 1 ' e bağlanır.  </li><!--<li> A minimum of 1 GbE switch must be used for the device once the initial setup is complete. The local web UI will not be accessible if the connected switch is not at least 1 Gbe.</li>-->|   |
| Cihaz oturum açma                      | 8 ila 16 karakter arasında cihaz yöneticisi parolası ve aşağıdakilerden üçünü içerir: büyük harf, küçük harf, sayısal ve özel karakterler.                                            | Varsayılan parola ilk oturum açma sırasında süresi dolan *Parola1* .                                                     |
...
| Seçim Web proxy ayarları     | <li>Web proxy sunucusu IP/FQDN, bağlantı noktası </li><li>Web Proxy Kullanıcı adı, parola</li> | Web proxy, işlem kurulumu ile desteklenmiyor. |
...
| Seçim Sertifika  | İmza Zinciri de dahil olmak üzere kendi sertifikalarınızı getiruyorsanız, sertifikaları uygun biçimde [ekleyin](azure-stack-edge-pro-r-deploy-configure-certificates-vpn-encryption.md#bring-your-own-certificates) .| Yalnızca cihaz adını ve/veya DNS etki alanını değiştirirseniz sertifikaları yapılandırın. |
| VPN  | <!--Need VPN certificate, VPN gateway, firewall setup in Azure,  passphrase and region info VPN scripts. -->   | |
| Bekleyen şifreleme  | Otomatik olarak oluşturulan şifreleme anahtarını kullanmanızı öneririz.   |Kendi anahtarınızı kullanıyorsanız, 32 karakterli bir Base-64 kodlu anahtar gerekir.  |
...
[Azure Stack Edge Pro cihazınızı](azure-stack-edge-pro-r-deploy-prep.md)dağıtmaya hazırlanın.
