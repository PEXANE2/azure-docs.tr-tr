---
title: İşlem yönetimini Azure Data Box Edge | Microsoft Docs
description: Tetikleyici, modüller, işlem yapılandırmasını görüntüleme, Azure Data Box Edge Azure portal aracılığıyla yapılandırmayı kaldırma gibi kenar işlem ayarlarının nasıl yönetileceğini açıklar.
services: databox
author: alkohli
ms.service: databox
ms.subservice: edge
...
---
# <a name="manage-compute-on-your-azure-data-box-edge"></a>Azure Data Box Edge işlem yönetme

Bu makalede Azure Data Box Edge işlem yönetimi açıklanmaktadır. Azure portal veya yerel Web Kullanıcı arabirimi aracılığıyla işlem yönetebilirsiniz. Modül, Tetikleyiciler ve işlem yapılandırmasını yönetmek için Azure portal kullanın ve işlem ayarlarını yönetmek için yerel Web Kullanıcı arabirimini kullanın.
...
Olaylar, bulut ortamınızda veya cihazınızda işlem yapmak isteyebileceğiniz bir işlemdir. Örneğin, bir paylaşımda bir dosya oluşturulduğunda bu bir olaydır. Tetikler olayları yükseltir. Data Box Edge, Tetikleyiciler dosya olaylarına veya bir zamanlamaya yanıt olarak olabilir.
...
1. Azure portal Data Box Edge kaynağına gidip **kenar işlem > tetikleyicisi**' ne gidin. Komut çubuğunda **+ tetikleyici Ekle** ' yi seçin.
...
İşlem yapılandırmasını görüntülemek, mevcut bir işlem yapılandırmasını kaldırmak veya Data Box Edge IoT cihazının ve IoT Edge cihaz için erişim anahtarlarını eşitlemek üzere işlem yapılandırmasını yenilemek için Azure portal kullanın.
...
1. Azure portal, Data Box Edge kaynağınız ' ne gidin ve ardından **uç işlem > modüller**' e gidin. Komut çubuğunda **Işlem görüntüle** ' yi seçin.

    ![İşlem görüntüleme seçin](media/data-box-edge-manage-compute/view-compute-1.png)
...
1. Azure portal, Data Box Edge kaynağınız ' ne gidin ve ardından **uç işlem >** başlayın ' a gidin. Komut çubuğunda işlemi **Kaldır** ' ı seçin.

    ![İşlem kaldırma seçeneğini belirleyin](media/data-box-edge-manage-compute/remove-compute-1.png)
...
2. İşlem yapılandırmasını kaldırırsanız, işlem yeniden kullanmanız gerektiğinde cihazınızı yeniden yapılandırmanız gerekir. Onay istendiğinde **Evet**' i seçin.
    ...
    ![İşlem kaldırma seçeneğini belirleyin](media/data-box-edge-manage-compute/remove-compute-2.png)
...
Data Box Edge işlem yapılandırdığınızda, bir IoT cihazı ve bir IoT Edge cihazı oluşturulur. Bu cihazlara otomatik olarak simetrik erişim anahtarları atanır. En iyi güvenlik uygulaması olarak, bu anahtarlar IoT Hub hizmeti aracılığıyla düzenli olarak döndürülür.
...
IoT cihazınız ve IoT Edge cihaz anahtarlarınız döndürülürse, en son erişim anahtarlarını almak için Data Box Edge yapılandırmayı yenilemeniz gerekir. Eşitleme, cihazın IoT cihazınız ve IoT Edge cihazınız için en son anahtarları almasını sağlar. Data Box Edge yalnızca birincil erişim anahtarlarını kullanır.

Cihazınızın erişim anahtarlarını eşitlemek için Azure portal aşağıdaki adımları uygulayın.

1. Azure portal, Data Box Edge kaynağınız ' ne gidin ve ardından **uç işlem >** başlayın ' a gidin. Komut çubuğunda **yapılandırmayı Yenile** ' yi seçin.

    ![Yapılandırmayı Yenile seçeneğini belirleyin](media/data-box-edge-manage-compute/refresh-configuration-1.png)
...
- [Uç işlem ağını Azure Portal aracılığıyla yönetmeyi](data-box-edge-extend-compute-access-modules.md)öğrenin.
