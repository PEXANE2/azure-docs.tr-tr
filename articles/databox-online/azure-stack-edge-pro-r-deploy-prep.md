---
title: Azure portal, Datacenter ortamını Azure Stack Edge Pro R 'yi dağıtmaya hazırlama öğreticisi
description: Azure Stack Edge Pro R 'yi dağıtmaya yönelik ilk öğretici, Azure portal hazırlamayı içerir.
services: databox
author: alkohli
ms.service: databox
ms.subservice: edge
...
ms.locfileid: "104613133"
---
# <a name="tutorial-prepare-to-deploy-azure-stack-edge-pro-r"></a>Öğretici: Azure Stack Edge Pro R 'yi dağıtmaya hazırlanma

Bu öğretici, Azure Stack Edge Pro R 'yi tamamen dağıtmak için gereken dağıtım öğreticilerinde ilkidir. Bu öğreticide, Azure portal Azure Stack Edge kaynağını dağıtmaya yönelik nasıl hazırlanılacağı açıklanmaktadır. Öğretici, kesintisiz güç kaynağı (UPS) ile birlikte gelen 1 düğümlü Azure Stack Edge Pro R cihazını kullanır.
...
| Bu adımı yapmak için | Bu belgeleri kullanın |
| --- | --- |
...
|**[6. güvenlik ayarlarını yapılandırma](azure-stack-edge-pro-r-deploy-configure-certificates-vpn-encryption.md)** |Cihazınız için sertifikaları, VPN, bekleyen şifrelemeyi yapılandırın. Cihaz tarafından oluşturulan sertifikaları kullanın veya kendi sertifikalarınızı getirin.   |
|**[7. cihazı etkinleştirin](azure-stack-edge-pro-r-deploy-activate.md)** |Cihazı etkinleştirmek için hizmetten etkinleştirme anahtarını kullanın. Cihaz, SMB veya NFS paylaşımlarını ayarlamaya veya REST aracılığıyla bağlamaya hazırlanıyor. |
...
- Bu cihaz için güvenlik bilgilerini şu adreste gözden geçirdiniz: [Azure Stack Edge cihazınız Için güvenlik yönergeleri](azure-stack-edge-pro-r-safety.md).
...
- Veri merkezinizdeki ağ, Azure Stack Edge cihazınız için ağ gereksinimlerine göre yapılandırılır. Daha fazla bilgi için bkz. [Edge Pro R sistem gereksinimleri Azure Stack](azure-stack-edge-pro-r-system-requirements.md).

- Cihazınızın normal çalışma koşullarında şunları yapabilirsiniz:
...
3. Azure Stack Edge Pro cihazı için kullanmak istediğiniz aboneliği seçin. Bu fiziksel cihazı göndermek istediğiniz ülkeyi seçin. **Cihazları göster**' i seçin.
    ...
    ![Kaynak oluşturma 1](media/azure-stack-edge-pro-r-deploy-prep/create-resource-1.png)

4. Cihaz türü seçin. **Azure Stack Edge** altında **Azure Stack Edge Pro R** ' yi ve ardından **Seç**' i seçin. Herhangi bir sorun görürseniz veya cihaz türünü seçmezseniz, [sorun giderme sorunları gider](azure-stack-edge-troubleshoot-ordering.md)bölümüne gidin.

    ![Kaynak oluştur 2](media/azure-stack-edge-pro-r-deploy-prep/create-resource-2.png)

5. İş ihtiyacınıza göre **Azure Stack Edge Pro r tek düğüm** veya **Azure Stack Edge Pro r tek düğümünü kesintisiz** olarak seçebilirsiniz.  

    ![Kaynak oluşturma 3](media/azure-stack-edge-pro-r-deploy-prep/create-resource-3.png)
...
7. Aşağıdaki **örnek ayrıntılarını** girin veya seçin.
    ...
    ![Kaynak oluşturma 4](media/azure-stack-edge-pro-r-deploy-prep/create-resource-4.png)
...
8. **İleri ' yi seçin: sevkiyat adresi**.
    ...
    - Zaten bir cihazınız varsa, **Azure Stack Edge Pro R cihazım** için Birleşik giriş kutusunu seçin.
        ...
        ![Kaynak oluştur 5](media/azure-stack-edge-pro-r-deploy-prep/create-resource-5.png)

    - Bu cihaz, sipariş ettiğiniz yeni bir cihaz ise, ilgili kişi adını, şirketi, cihazı göndermek için adresi ve iletişim bilgilerini girin.

        ![Kaynak oluştur 6](media/azure-stack-edge-pro-r-deploy-prep/create-resource-6.png)
...
10. **Gözden geçir + oluştur** sekmesinde, **fiyatlandırma ayrıntılarını**, **kullanım koşulları** ve kaynağınızın ayrıntılarını gözden geçirin. **Gizlilik koşullarını Incelediğim** Birleşik giriş kutusunu seçin.
    ...
    ![Kaynak oluşturma 7](media/azure-stack-edge-pro-r-deploy-prep/create-resource-7.png) 
...
11. **Oluştur**’u seçin.
    ...
    ![Azure Stack Edge Pro kaynağına git](media/azure-stack-edge-pro-r-deploy-prep/azure-stack-edge-resource-1.png)
...
2. Sağ bölmede, Azure Key Vault için bir ad girin veya varsayılan adı kabul edin. Anahtar Kasası adı 3 ila 24 karakter arasında olabilir.
   ...
   ![Etkinleştirme anahtarını alma](media/azure-stack-edge-pro-r-deploy-prep/azure-stack-edge-resource-3.png)
...
> [Azure Stack Edge 'i yükler](./azure-stack-edge-pro-r-deploy-install.md)
