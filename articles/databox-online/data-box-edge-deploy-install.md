---
title: Install-Unpack, rafı, Cable Azure Data Box Edge fiziksel cihaz | Microsoft Docs
description: Azure Data Box Edge yüklemeyle ilgili ikinci öğretici fiziksel cihazın paketini açmak, rafa eklemek ve kablosunu içerir.
services: databox
author: alkohli
ms.service: databox
ms.subservice: edge
...
---
# <a name="tutorial-install-azure-data-box-edge"></a>Öğretici: Azure Data Box Edge yüklemesi

Bu öğreticide Data Box Edge fiziksel cihazı kurulum adımları anlatılmaktadır. Yükleme yordamı, cihazı açmak, rafa monte etmek ve kablolama işlemini içerir. 
...
* [Azure Data Box Edge dağıtmaya hazırlanma](data-box-edge-deploy-prep.md)bölümündeki tüm adımları tamamladınız.
    * Cihazınızı dağıtmak için bir Data Box Edge kaynağı oluşturdunuz.
...
- Data Box Edge dağıtmaya yönelik ağ gereksinimlerini gözden geçirin ve veri merkezi ağını gereksinimlere göre yapılandırın. Daha fazla bilgi için bkz. [Data Box Edge ağ gereksinimleri](data-box-edge-system-requirements.md#networking-port-requirements).
...
3. Kutuyu açın. Kutuyu açtıktan sonra aşağıdakilerin bulunduğundan emin olun:
    - Tek bir kutu Data Box Edge cihazı
    - İki güç kablosu
    - Bir demiryolu seti derlemesi
    - Güvenlik, ortam ve mevzuat bilgileri kitapçık

Burada listelenen öğelerin tümünü almadıysanız Data Box Edge desteğe başvurun. Sonraki adım, cihazınızı takmaya yönelik bir adımdır.
...
2. İki kanca ve döngü Straps
    ...
    ![Demiryolu Kit içeriğini tanımla](./media/data-box-edge-deploy-install/identify-rail-kit-contents.png)
...
4. Rayları kaldırmak için, son nokta orta noktasına mandal yayını düğmesini çekin ve her bir demiryolu 'yi serbest bırakın.
    ...
    ![Araç-daha az rayın ve Kaldır](./media/data-box-edge-deploy-install/installing-removing-tool-less-rails.png)
...
4. Sol dikey dolapta, sol ve sağ köşeli ayraçlara doğru kaydırın ve iki adet screws çifti kullanarak bunları ekleyin.
    ...
    ![Toulan rayları yükleyip kaldırın](./media/data-box-edge-deploy-install/installing-removing-tooled-rails.png)
...
4. Her iki rayda da slayt yayın kilidi düğmelerine basın ve sistemi rafa kaydırın.
    ...
    ![Sistemi bir rafa yükler](./media/data-box-edge-deploy-install/installing-system-rack.png)
...
3. Sistemin yüzlerini korkutın ve RAIL stanon, J yuvalarının önünde oluncaya kadar ileriye doğru çekin. Sistemi rafa kaldırın ve bir düzey yüzeyine yerleştirin.
    ...
    ![Sistemi rafa kaldır](./media/data-box-edge-deploy-install/removing-system-rack.png)
...
3. Raf veya diğer kararsız ortamlar için sistemin sevk irsaliyesine karşı korumak için her mandal altında sabit takel vidalı konumunu bulun ve her bir vida #2 Phillips screwdriver ile güçlendir.
    ...
    ![SLA ve yayın ile katılın](./media/data-box-edge-deploy-install/engaging-releasing-slam-latch.png)


## <a name="cable-the-device"></a>Cihazın kablolarını bağlama

Kabloları yönlendirin ve sonra cihazınızı bağlayın. Aşağıdaki yordamlarda Data Box Edge cihazınızın güç ve ağ için nasıl kablo yapılacağı açıklanmaktadır.

Cihazınızı kablolamayı başlatmaya başlamadan önce şunlar gerekir:

- Data Box Edge fiziksel cihazınız, paketi açılmış ve raf takılmış.
- İki güç kablosu.
...
> - Data Box Edge cihazın veri kaynağı sunucularından veri alabilmesi için veri merkezi ağına bağlı olması gerekir.

Data Box Edge cihazınızda:
...
1. Cihazınızın arka düzlemine ait çeşitli bağlantı noktalarını belirler.
    ...
    ![Kablolu cihazın arka düzlemi](./media/data-box-edge-deploy-install/backplane-cabled.png)
...
Bu öğreticide, şu konularda nasıl yapılacağını Data Box Edge öğrendiniz:
...
> [Bağlanma ve kurma Data Box Edge](./data-box-edge-deploy-connect-setup-activate.md)
