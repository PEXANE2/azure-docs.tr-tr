...
*ConnectionString* 'in değeri, appSettings içinde bu tür bir dize varsa, appSettings 'ten gerçek bağlantı dizesini getirmek için anahtar olarak kullanılır. AppSettings içinde bulunmazsa, *ConnectionString* değeri web.config **ConnectionString** bölümünden gerçek bağlantı dizesini getirmek için anahtar olarak kullanılır, bu bölüm varsa. Bağlantı dizesi AppSettings veya web.config **ConnectionString** bölümünde yoksa, StackExchange. redsıs. connectionçoğullayıcı oluşturulurken bağlantı dizesi olarak *ConnectionString* öğesinin sabit değeri kullanılacaktır.
