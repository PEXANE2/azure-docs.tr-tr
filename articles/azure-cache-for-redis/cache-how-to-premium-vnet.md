...
<sup>1</sup> ' AzureKeyVault ' ve ' AzureMonitor ' hizmet etiketlerini Kaynak Yöneticisi ağ güvenlik grupları ile birlikte kullanabilirsiniz.
...
<sup>4</sup> daha fazla bilgi için bkz. [ek VNET ağ bağlantısı gereksinimleri](#additional-vnet-network-connectivity-requirements).
...
| 15000-15999 |Gelen |TCP |Redsıs kümelerine istemci iletişimi, Azure Yük Dengelemesi ve Geo-Replication | (Redsıs alt ağı) | (Redsıs alt ağı), (Istemci alt ağı), AzureLoadBalancer, (coğrafi çoğaltma eş alt ağı) |
...
Redsıs için bir Azure önbelleğini VNet 'e dağıtıyorsanız, önbelleğin başka kaynak türü içermeyen bir ayrılmış alt ağda olması gerekir. Redsıs için bir Azure önbelleğini diğer kaynakları (örneğin, uygulama ağ geçitleri, giden NAT vb.) içeren bir Kaynak Yöneticisi VNet alt ağına dağıtmaya yönelik bir girişim yapılırsa, dağıtım genellikle başarısız olur. Redsıs için yeni bir Azure önbelleği oluşturabilmeniz için, diğer türdeki mevcut kaynakları silmeniz gerekir.
...
Azure VNET altyapısı tarafından kullanılan IP adreslerine ek olarak, alt ağdaki her reddo örneği, küme parça başına iki IP adresi (ek çoğaltmalar için ek IP adresleri, varsa) ve yük dengeleyici için bir ek IP adresi kullanır. Kümelenmemiş bir önbelleğin bir parça olduğu kabul edilir.
...
* Redsıs konsolu-Redsıs konsolu yerel tarayıcınızda çalıştığı için genellikle VNET 'e bağlı olmayan bir geliştirici makinesinde çalışır, daha sonra önbelleğinize bağlanamaz.
...
Bununla birlikte, genel bir müşteri yapılandırması, giden Internet trafiğini şirket içi akışa yönelmeye zorlayan Zorlamalı tünel oluşturma (varsayılan rota tanıtma) kullanmaktır. Bu trafik akışı Reda için Azure Cache ile bağlantıyı keser ve ardından, redin örneği için Azure önbelleğinin bağımlılıklarıyla iletişim kuramaması gibi şirket içi trafik engellenir.
