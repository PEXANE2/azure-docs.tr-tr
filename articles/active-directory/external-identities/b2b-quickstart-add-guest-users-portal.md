...
Herhangi bir kişiyi dizininize konuk kullanıcı olarak ekleyerek onu kuruluşunuzla işbirliği yapması için davet edebilirsiniz. Daha sonra paylaşmak istediğiniz bir uygulamanın doğrudan bağlantısını gönderebilir veya kullanım bağlantısını içeren bir davet e-postası gönderebilirsiniz. Konuk kullanıcılar, kendi iş, okul veya sosyal kimlikleriyle oturum açabilir. Bu hızlı başlangıç ile birlikte [Azure Portal](add-users-administrator.md), [PowerShell](b2b-quickstart-invite-powershell.md)aracılığıyla veya [toplu olarak](tutorial-bulk-invite.md)Konuk kullanıcıları ekleme hakkında daha fazla bilgi edinebilirsiniz.

Bu hızlı başlangıçta, Azure AD dizininize Azure portal, davet gönderecek ve Konuk kullanıcının davet kullanım sürecinin nasıl göründüğünü görmeniz için yeni bir Konuk Kullanıcı ekleyeceksiniz.
