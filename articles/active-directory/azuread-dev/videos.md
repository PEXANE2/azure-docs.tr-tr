---
title: Azure ADAL MSAL geçiş videoları | Mavisi
description: Azure Active Directory geliştirici platformundan Microsoft Identity platformuna geçiş yapmanıza yardımcı olan videolar
services: active-directory
author: mmacy
manager: CelesteDG
ms.service: active-directory
ms.subservice: develop
ms.topic: conceptual
ms.workload: identity
ms.date: 02/12/2020
ms.author: marsma
ms.custom: aaddev
ms.reviewer: celested
ROBOTS: NOINDEX
ms.openlocfilehash: a4bb9151e9d4b76507b0c7f8ab5aaa7f5f1820b8
ms.sourcegitcommit: f28ebb95ae9aaaff3f87d8388a09b41e0b3445b5
ms.translationtype: MT
ms.contentlocale: tr-TR
ms.lasthandoff: 03/29/2021
ms.locfileid: "96608206"
---
# <a name="azure-active-directory-developer-platform-videos"></a>Azure Active Directory geliştirici platformu videoları

Yeni Microsoft Identity platformu hakkında bilgi edinin ve bu uygulamayı Azure Active Directory (Azure AD) geliştirici platformundan nasıl geçirebileceğinizi öğrenin. Videolar genellikle 1-2 dakika uzunluğundadır.

## <a name="migrate-from-v10-to-v20"></a>V 1.0 'dan v 2.0'a geçiş

**Microsoft Identity platform 'un en son sürümüne geçme hakkında bilgi edinin**

:::row:::
    :::column:::
        Yeni Microsoft Identity platformuna genel bakış
    :::column-end:::
    :::column:::
        > [!VIDEO https://www.youtube.com/embed/bNlcFuIo3r8]
    :::column-end:::
    :::column:::
        MSAL kitaplıklarına giriş
    :::column-end:::
    :::column:::
        > [!VIDEO https://www.youtube.com/embed/apbbx2n4tnU]
    :::column-end:::
:::row-end:::
:::row:::
    :::column:::
        Uç noktalar ve v 2.0 'a geçme avantajları
    :::column-end:::
    :::column:::
        > [!VIDEO https://www.youtube.com/embed/qpdC45tZYDg]
    :::column-end:::
     :::column:::
        ADAL kod tabanınızı MSAL 'e geçirme
    :::column-end:::
    :::column:::
        > [!VIDEO https://www.youtube.com/embed/xgL_z9yCnrE]
    :::column-end:::
:::row-end:::
:::row:::
    :::column:::
        ADAL 'dan MSAL 'e geçiş yapma
    :::column-end:::
    :::column:::
        > [!VIDEO https://www.youtube.com/embed/qpdC45tZYDg]
    :::column-end:::
    :::column:::
        ADAL üzerinden MSAL avantajları
    :::column-end:::
    :::column:::
        > [!VIDEO https://www.youtube.com/embed/q-TDszj2O-4]
    :::column-end:::
:::row-end:::

## <a name="next-steps"></a>Sonraki adımlar

Yeni [Microsoft Identity platformu](../develop/index.yml) hakkında bilgi edinin
