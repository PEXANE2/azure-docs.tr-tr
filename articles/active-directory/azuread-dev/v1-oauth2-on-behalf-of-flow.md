...
Aşağıdaki HTTP POST, https://graph.microsoft.com Web API 'SI için bir erişim belirteci ister. `client_id`, erişim belirtecini isteyen hizmeti tanımlar.
...
| client_id |gerekli | Azure AD 'ye kayıt sırasında çağıran hizmete atanan uygulama KIMLIĞI. Azure portal uygulama KIMLIĞINI bulmak için **Active Directory**' i seçin, dizini seçin ve ardından uygulama adını seçin. |
| client_assertion_type |gerekli |Değer `urn:ietf:params:oauth:client-assertion-type:jwt-bearer` olmalıdır |
...
Aşağıdaki HTTP POST, bir sertifikayla https://graph.microsoft.com Web API 'SI için bir erişim belirteci ister. `client_id`, erişim belirtecini isteyen hizmeti tanımlar.
