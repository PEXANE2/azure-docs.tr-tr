...
Tek [ve çok kiracılı](../develop/howto-convert-app-to-be-multi-tenant.md?toc=/azure/active-directory/azuread-dev/toc.json&bc=/azure/active-directory/azuread-dev/breadcrumb/toc.json) uygulamalar ve [ortak kimlik doğrulama desenleri](v1-authentication-scenarios.md) hakkında bilgi edinilir.
...
Azure AD Koşullu Erişim, [Azure AD Premium'da](https://docs.microsoft.com/azure/active-directory/active-directory-whatis)yer alan bir özelliktir. [Lisanssız kullanım raporunda](../active-directory-conditional-access-unlicensed-usage-report.md)lisans gereksinimleri hakkında daha fazla bilgi edinebilirsiniz. Geliştiriciler, Azure AD Premium'u içeren Enterprise Mobility Suite'e ücretsiz abonelik içeren [Microsoft Geliştirici Ağı'na](https://msdn.microsoft.com/dn308572.aspx)katılabilir.
...
![Akış diyagramını adına gerçekleştiren uygulama](./media/conditional-access-dev-guide/app-performing-on-behalf-of-scenario.png)
...
Bu senaryoyu denemek için [.NET kod örneğimize](https://github.com/Azure-Samples/active-directory-dotnet-webapi-onbehalfof-ca)bakın. Web API 1'den yerel uygulamaya talep meydan okumasının nasıl geçirilebildiğini ve istemci uygulaması içinde yeni bir istek oluşturmanın nasıl yapılacağını gösterir.
...
![Birden çok hizmet akış diyagramına erişen uygulama](./media/conditional-access-dev-guide/app-accessing-multiple-services-scenario.png)
...
Uygulama ADAL kitaplığını kullanıyorsa, belirteci elde edilememesi her zaman etkileşimli olarak yeniden denenir. Bu etkileşimli istek oluştuğunda, son kullanıcı Koşullu Erişim'e uyma fırsatına sahiptir. Bu, isteğ inatla `AcquireTokenSilentAsync` `PromptBehavior.Never` kullanıcıya politikaya uyma fırsatı ```AcquireToken``` vermek için etkileşimli bir istek tesbit etmesi gerekmediği sürece geçerlidir.

## <a name="scenario-single-page-app-spa-using-adaljs"></a>Senaryo: ADAL.js kullanarak tek sayfalık uygulama (SPA)

Bu senaryoda, Koşullu Erişim korumalı web API'sını aramak için ADAL.js kullanarak tek sayfalık bir uygulamamız (SPA) olduğunda servis talebiyle yürüyoruz. Bu basit bir mimari dir, ancak Koşullu Erişim etrafında geliştirirken dikkate alınması gereken bazı nüansları vardır.

ADAL.js'de belirteçleri elde eden birkaç `login()` `acquireToken(...)`işlev `acquireTokenPopup(…)`vardır: , , , ve `acquireTokenRedirect(…)`.

* `login()`Etkileşimli oturum açma isteği aracılığıyla bir kimlik belirteci alır, ancak herhangi bir hizmet için erişim belirteçleri almaz (Koşullu Erişim korumalı web API'si dahil).
* `acquireToken(…)`daha sonra sessizce hiçbir koşulda UI göstermez anlamı bir erişim belirteci elde etmek için kullanılabilir.
...
Bir uygulamanın Web API'sini çağırmak için bir `acquireToken(…)`erişim jetonuna ihtiyacı olduğunda, bir . Belirteç oturumunun süresi dolmuşsa veya Koşullu Erişim ilkesine uymamız gerekiyorsa, `acquireTokenPopup()` elde `acquireTokenRedirect()` *edilen Token* işlevi başarısız olur ve uygulama kullanır veya .

![ADAL akış diyagramı kullanarak tek sayfalık uygulama](./media/conditional-access-dev-guide/spa-using-adal-scenario.png)

Koşullu Erişim senaryomuzla bir örnek üzerinden bakalım. Son kullanıcı sadece siteye indi ve bir oturum yok. Bir `login()` arama gerçekleştiriyoruz, çok faktörlü kimlik doğrulaması olmadan bir kimlik belirteci alıyoruz. Daha sonra kullanıcı, uygulamanın bir web API'sinden veri istemesini gerektiren bir düğmeye basar. Uygulama bir `acquireToken()` arama yapmaya çalışır, ancak kullanıcı henüz çok faktörlü kimlik doğrulaması gerçekleştirmediğinden ve Koşullu Erişim ilkesine uyması gerektiğinden başarısız olur.
...
Bu senaryoyu denemek [için, kod örneği adına JS SPA'mıza](https://github.com/Azure-Samples/active-directory-dotnet-webapi-onbehalfof-ca)bakın. Bu kod örneği, bu senaryoyu göstermek için daha önce bir JS SPA'ya kaydettiğiniz Koşullu Erişim ilkesini ve web API'sını kullanır. Talepler meydan okumasını nasıl düzgün bir şekilde ele alacağınızı ve Web API'nız için kullanılabilecek bir erişim belirteci elde etmeyi gösterir. Alternatif olarak, bir Açısal SPA rehberlik için genel [Açısal.js kod örneği](https://github.com/Azure-Samples/active-directory-angularjs-singlepageapp) çıkış

## <a name="see-also"></a>Ayrıca bkz.

* Özellikler hakkında daha fazla bilgi edinmek için [Azure Etkin Dizini'nde Koşullu Erişim'e](../active-directory-conditional-access-azure-portal.md)bakın.
* Daha fazla Azure AD kodu örneği [için, kod örneklerinin GitHub repo'su](https://github.com/azure-samples?utf8=%E2%9C%93&q=active-directory)bölümüne bakın.
* ADAL SDK hakkında daha fazla bilgi ve başvuru belgelerine erişmek için [kitaplık kılavuzuna](active-directory-authentication-libraries.md)bakın.
* Çok kiracılı senaryolar hakkında daha fazla bilgi edinmek [için, çok kiracılı deseni kullanarak kullanıcıları nasıl oturum alabildiğini](../develop/howto-convert-app-to-be-multi-tenant.md?toc=/azure/active-directory/azuread-dev/toc.json&bc=/azure/active-directory/azuread-dev/breadcrumb/toc.json)öğrenin.
