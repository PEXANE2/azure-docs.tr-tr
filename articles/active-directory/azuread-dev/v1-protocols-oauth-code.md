...
Yetkilendirme kodu akışı, kullanıcıyı uç noktaya yönlendiren istemciyle başlar `/authorize` . Bu istekte istemci, kullanıcıdan almaları gereken izinleri gösterir. Azure portal **Uygulama kayıtları > uç noktaları** ' nı seçerek kiracınızın OAuth 2,0 yetkilendirme uç noktasını alabilirsiniz.
...
| code_challenge_method | Önerilen    | Parametresi için öğesini kodlamak için kullanılan yöntem `code_verifier` `code_challenge` . Veya bunlardan biri olabilir `plain` `S256` . Dışlanmazsa, varsa `code_challenge` düz metin olarak kabul edilir `code_challenge` . Azure AAD v 1.0 hem hem de destekler `plain` `S256` . Daha fazla bilgi için bkz. [Pkce RFC](https://tools.ietf.org/html/rfc7636). |
| code_challenge        | Önerilen    | Yerel veya genel bir istemciden kod değişimi (PKCE) için kanıt anahtarı aracılığıyla yetkilendirme kodu yetkisini güvenli hale getirmek için kullanılır. Dahil ise gereklidir `code_challenge_method` . Daha fazla bilgi için bkz. [Pkce RFC](https://tools.ietf.org/html/rfc7636). |
...
Bu noktada, kullanıcıdan Azure portalında uygulama tarafından istenen izinlerin kimlik bilgilerini ve onayını girmesi istenir. Kullanıcı kimlik doğrulaması yapar ve izin verdiğinde, Azure AD, `redirect_uri` kod ile talebinizdeki adreste uygulamanıza bir yanıt gönderir.
...
Artık bir yetkilendirme kodu edindiniz ve Kullanıcı tarafından izin verildiğinden, uç noktaya bir POST isteği göndererek istenen kaynağa erişim belirtecinin kodunu kullanabilirsiniz `/token` :
...
| redirect_uri |gerekli | `redirect_uri`İstemci uygulamasına kayıtlı. |
| client_secret |Web uygulamaları için gerekli, ortak istemcilerde izin verilmiyor |**Anahtarlar**altındaki uygulamanız Için Azure portalında oluşturduğunuz uygulama gizli anahtarı. Client_secrets, cihazlarda güvenilir bir şekilde depolanamadığı için yerel bir uygulamada (ortak istemci) kullanılamaz. Sunucu tarafında güvenli bir şekilde depolama yeteneği olan Web uygulamaları ve Web API 'Leri (tüm gizli istemciler) için gereklidir `client_secret` . Client_secret gönderilmeden önce URL kodlamalı olmalıdır. |
...
JSON Web belirteçleri hakkında daha fazla bilgi için bkz. [JWT IETF taslak belirtimi](https://go.microsoft.com/fwlink/?LinkId=392344).   Hakkında daha fazla bilgi edinmek için `id_tokens` , bkz. [v 1.0 OpenID Connect Flow](v1-protocols-openid-connect-code.md).
...
| Parametre | Açıklama |
| --- | --- |
| authorization_uri |Yetkilendirme sunucusunun URI 'SI (fiziksel uç noktası). Bu değer, bir bulma uç noktasından sunucu hakkında daha fazla bilgi almak için bir arama anahtarı olarak da kullanılır. <p><p> İstemcinin, yetkilendirme sunucusunun güvenilir olduğunu doğrulaması gerekir. Kaynak Azure AD tarafından korunduğunda, URL 'nin ile başladığını `https://login.microsoftonline.com` veya Azure AD 'nin desteklediği başka bir ana bilgisayar adını doğrulamak yeterlidir. Kiracıya özgü bir kaynak her zaman kiracıya özgü bir yetkilendirme URI 'SI döndürmelidir. |
...
| error_description |Hatanın daha ayrıntılı bir açıklaması. Bu ileti, son kullanıcı kullanımı için tasarlanmamıştır. |
| resource_id |Kaynağın benzersiz tanımlayıcısını döndürür. İstemci uygulaması bu tanımlayıcıyı, `resource` kaynak için bir belirteç istediğinde parametresinin değeri olarak kullanabilir. <p><p> İstemci uygulamasının bu değeri doğrulaması önemlidir, aksi takdirde kötü amaçlı bir hizmet ayrıcalık **yükselmesi** saldırısı yapabilir <p><p> Bir saldırının önlenmesi için önerilen strateji, öğesinin `resource_id` erişilmekte olan Web API URL 'siyle aynı olduğunu doğrulamadır. Örneğin, erişilmesi durumunda olabilir `https://service.contoso.com/data` `resource_id` `https://service.contoso.com/` . `resource_id`Kimliği doğrulamak için güvenilir bir alternatif yol olmadıkça, istemci uygulamanın temel URL ile başlamayan bir öğesini reddetmesi gerekir. |
...
Erişim belirteçleri kısa süreli olduğundan, kaynaklara erişmeye devam etmek için süreleri dolduktan sonra yenilenmelidir. `access_token`Bunu, uç noktaya başka bir istek göndererek yenileyebilirsiniz `POST` `/token` , ancak bu kez, yerine öğesini sağlar `refresh_token` `code` .  Yenileme belirteçleri, istemcinize erişim izni verilen tüm kaynaklar için geçerlidir. bu nedenle, için bir istekte verilen yenileme belirteci, `resource=https://graph.microsoft.com` için yeni bir erişim belirteci istemek üzere kullanılabilir `resource=https://contoso.com/api` . 
