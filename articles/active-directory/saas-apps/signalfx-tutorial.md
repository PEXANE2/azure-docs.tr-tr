...
1. **SAML ile çoklu oturum açmayı ayarlama** sayfasında, aşağıdaki alanları doldurun: 
    ...
    a. **Tanımlayıcı**ALANıNA aşağıdaki URL `https://api.<realm>.signalfx.com/v1/saml/metadata` girin ve `<realm>` signalfx bölgesi ile değiştirin. 

    b. **Yanıt URL 'si**' ne `https://api.<realm>.signalfx.com/v1/saml/acs/<integration ID>` aşağıdaki URL 'yi girin ve `<realm>` signalfx Realm ile değiştirin ve daha önce SIGNALFX kullanıcı arabiriminden KOPYALADıĞıNıZ **tümleştirme kimliğiyle** `<integration ID>`.
...
1. **Kullanıcı** özelliklerinde:
   1. **Kullanıcı adı**' nda, `b.simon@contoso.com`gibi `username@companydomain.extension`girin.
