...
* Azure AD 'de, Confluence için Easysa erişimi olan denetim.
* Kullanıcılarınızın Azure AD hesaplarıyla uyumlu hale girmesi için kuruluşunuzun otomatik olarak oturum açabilmesi için bu ayarı etkinleştirin.
...
## <a name="configure-and-test-azure-ad-single-sign-on-for-easysso-for-confluence"></a>Yapılandırma için Azure AD çoklu oturum açmayı yapılandırın ve test edin
...
Bu nedenle, Azure AD SSO 'yu yapılandırmak ve test etmek için aşağıdaki yapı taşlarını doldurun:
...
1. [Azure Portal](https://portal.azure.com/), **Confluence Için easysso** uygulama tümleştirmesi sayfasında **Yönet** bölümünü bulun ve **Çoklu oturum açma**' yı seçin.
...
1. Uygulamayı **SP** tarafından başlatılan modda yapılandırmak Istiyorsanız **ek URL 'ler ayarla** ' ya tıklayın ve aşağıdaki adımı gerçekleştirin:
    ...
    **Oturum açma URL 'si** metin kutusunda, aşağıdaki kalıbı kullanarak bir URL yazın:`https://<server-base-url>/`
    ...
    > [!NOTE]
    > Bu değerler gerçek değildir. Bu değerleri gerçek tanımlayıcı, yanıt URL 'SI ve oturum açma URL 'SI ile güncelleştirin. Bu değerleri almak için, [Confluence istemci destek ekibi Için Easysso](mailto:support@techtime.co.nz) ile iletişim kurun. Ayrıca, Azure portal **temel SAML yapılandırması** bölümünde gösterilen desenlere de başvurabilirsiniz.
...
1. Yönetici ayrıcalıklarıyla Confluence örneği için Easyssize 'da oturum açın ve **Uygulamaları Yönet** bölümüne gidin.

    ![Uygulamaları Yönetme](./media/easysso-for-confluence-tutorial/jira-admin-1.png)

1. **Easysso**'a tıklayın.

    ![Kolay SSO](./media/easysso-for-confluence-tutorial/jira-admin-2.png)
...
1. Üstteki **Sertifikalar** sekmesini seçin ve aşağıdaki ekran görüntülenir ve **Azure AD SSO** yapılandırmasının önceki adımlarında kaydettiğiniz **sertifika (base64)** veya **meta veri dosyasını** bulun. İlerlemeniz için aşağıdaki seçenekleri kullanabilirsiniz:
    ...
    ![Meta veri URL 'SI](./media/easysso-for-confluence-tutorial/jira-admin-4.png)
...
1. Sayfanın alt kısmındaki **Kaydet** düğmesine tıklayın. Meta verilerin içeriğini veya sertifika dosyalarını yapılandırma alanlarına ayrıştırmış olursunuz. Confluence yapılandırması için EasySSO işlemi tamamlanmıştır.
...
1. En iyi test deneyimi için, **&** Me sekmesine gidin ve üzerinde **SAML oturum açma düğmesi** seçeneğini işaretleyin. Bu, özellikle Azure AD SAML tümleştirme uçtan uca test etmek için Jira oturum açma ekranında ayrı bir düğmeye olanak sağlar. Bu düğmeyi açık bırakabilir ve üretim modu için yerleşimini, rengini ve çevirisini de yapılandırabilirsiniz.
    ...
    ![& göz atın](./media/easysso-for-confluence-tutorial/jira-admin-5.png)
...
Bu bölümde, Confluence için Easysbu 'da B. Simon adlı bir Kullanıcı oluşturulur. Confluence için EasySSO, varsayılan olarak **devre dışı** olan tam zamanında Kullanıcı sağlamayı destekler. Kullanıcı sağlamayı etkinleştirmek için EasySSO eklentisi yapılandırmasının Genel bölümünde oturum **açma başarılı olduğunda Kullanıcı oluşturma** seçeneğini açıkça denetlemeniz gerekir. Bu nedenle, bir Kullanıcı, Conflubilmesi için zaten mevcut değilse, kimlik doğrulamasından sonra yeni bir tane oluşturulur.

Ancak, Kullanıcı ilk oturum açmada otomatik Kullanıcı sağlamayı etkinleştirmek istemiyorsanız, kullanıcıların, LDAP veya Atlasbir Crowd gibi, Confluence örneği için EasySSO 'nın, bir arka uç Kullanıcı dizinlerinde mevcut olmaları gerekir.

![Kullanıcı sağlama](./media/easysso-for-confluence-tutorial/jira-admin-6.png)

## <a name="test-sso"></a>Test SSO 'SU

Bu bölümde, erişim panelini kullanarak Azure AD çoklu oturum açma yapılandırmanızı test edersiniz.

Erişim panelinde Confluence için EasySSO kutucuğuna tıkladığınızda, SSO 'yu ayarladığınız Confluence için EasySSO 'da otomatik olarak oturum açmış olmanız gerekir. Erişim paneli hakkında daha fazla bilgi için bkz. [erişim paneline giriş](https://docs.microsoft.com/azure/active-directory/active-directory-saas-access-panel-introduction).
...
- [Azure AD ile Confluence için EasySSO 'yi deneyin](https://aad.portal.azure.com/)

- [Microsoft Cloud App Security oturum denetimi nedir?](https://docs.microsoft.com/cloud-app-security/proxy-intro-aad)

- [Gelişmiş görünürlük ve denetimlerle ilgili bir sorun olması için Easysto koruma](https://docs.microsoft.com/cloud-app-security/proxy-intro-aad)
