...
1. Sol menü çubuğunda **Çoklu oturum açma (SSO)** seçeneğine tıklayın ve **SAML tabanlı çoklu oturum açma (SSO)** sayfasında aşağıdaki adımları uygulayın:
    ...
    e. İndirmiş olduğunuz sertifikayı açıklama olmadan `--BEGIN CERTIFICATE and END CERTIFICATE--` Azure Portal kopyalayın ve **sertifikayı sertifikaya (kimlik sağlayıcısı tarafından sunulur)** metin kutusuna yapıştırın.
