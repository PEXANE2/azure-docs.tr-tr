...
- https @ no__t-0//Login. microsoftonline. com/\<tenant @ no__t-2/, burada &lt;tenant @ no__t-4, Azure Active Directory (Azure AD) kiracının kiracı KIMLIĞI veya bu Azure AD kiracısıyla ilişkili bir etki alanıdır. Yalnızca belirli bir kuruluşun kullanıcılarına oturum açmak için kullanılır.
- https @ no__t-0//Login. microsoftonline. com/Common/. Kullanıcıları iş ve okul hesaplarıyla veya kişisel Microsoft hesaplarıyla oturum açmak için kullanılır.
- https @ no__t-0//Login. microsoftonline. com/kuruluşlar/. Kullanıcılara iş ve okul hesaplarıyla oturum açmak için kullanılır.
- https @ no__t-0//Login. microsoftonline. com/tüketicileri/. Kullanıcılara yalnızca kişisel Microsoft hesaplarıyla (eski adıyla Windows Live ID hesapları olarak biliniyordu) oturum açmak için kullanılır.
...
*Örnek* , uygulamanızın Azure genel bulutundaki veya ulusal bulutlardan Kullanıcı imzalanmasını belirtmek için kullanılır. Kodunuzda MSAL kullanarak, bir numaralandırma kullanarak veya URL 'YI `Instance` üyesi (biliyorsanız) olarak [National Cloud Instance](authentication-national-cloud.md#azure-ad-authentication-endpoints) 'A geçirerek Azure Cloud Instance 'ı ayarlayabilirsiniz.

@No__t-0 ve `AzureCloudInstance` belirtilirse, MSAL.NET açık bir özel durum oluşturur.

Bir örnek belirtmezseniz, uygulamanız Azure genel bulut örneğini (URL `https://login.onmicrosoftonline.com`) hedefleyecek.
...
- Azure AD yetkilisi hedef kitlesi numaralandırması yerine bir kiracı KIMLIĞI olarak bu yer tutuculardan biri:
    ...
    - yalnızca kişisel hesaplarıyla kullanıcıları oturum açmak için `consumers`
    - kullanıcıları iş ve okul hesaplarıyla veya kendi kişisel Microsoft hesaplarıyla oturum açmak için `common`

Hem Azure AD yetkilisi kitleyi hem de kiracı KIMLIĞINI belirtirseniz MSAL anlamlı bir özel durum oluşturur.

Hedef kitle belirtmezseniz, uygulamanız Azure AD ve kişisel Microsoft hesaplarını bir hedef kitle olarak hedefleyecek. (Diğer bir deyişle, `common` belirtilmediği halde davranacaktır.)
...
- @No__t-0 ' yı masaüstü veya UWP uygulamalarında (MSAL.NET 4.1 +) kullanmak istersiniz. Bu yöntem, ortak istemci uygulamasının yeniden yönlendirme URI özelliğini genel istemci uygulamaları için önerilen varsayılan yeniden yönlendirme URI 'si olarak ayarlar. 
  ...
  UWP | @no__t değeri-0. Bu, kaydolmanız gereken WebAuthenticationBroker. GetCurrentApplicationCallbackUri () sonucuna değer ayarlayarak, SSO 'yu tarayıcıda sağlar
...
- Aracıyı desteklemeyen bir Xamarin Android ve iOS uygulaması oluşturuyorsanız (yeniden yönlendirme URI 'si, Xamarin Android ve iOS için otomatik olarak `msal{ClientId}://auth` olarak ayarlanır) bir yeniden yönlendirme URI 'SI eklemeniz gerekmez.
...
@No__t-0 özelliğini kullanarak yeniden yönlendirme URI 'sini geçersiz kılabilirsiniz (örneğin, aracılar kullanıyorsanız). Bu senaryo için yeniden yönlendirme URI 'Leri örnekleri aşağıda verilmiştir:
