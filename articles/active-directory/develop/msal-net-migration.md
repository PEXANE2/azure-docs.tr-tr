...
- `AcquireTokenInteractive` Oluşturucuya uygulanabilecek bir `.WithClaim(claims)` yöntem vardır.
...
V 1.0 belirteçlerini kabul eden bir uygulama için belirteçler almak istiyorsanız Microsoft Graph (diğer bir deyişle https://graph.microsoft.com), bu kaynak için Istenen bir OAuth2 izniyle istenen kaynak tanımlayıcısını birleştirerek `scopes` oluşturmanız gerekir.
...
ADAL.NET v2. X, yenileme belirteçleri kullanıma sunulmuştur ve ADAL 2. x tarafından sunulan `AcquireTokenByRefreshToken` yöntemleri kullanarak bu belirteçlerin kullanımı etrafında çözümler geliştirmenize olanak tanır.
...
Bu yöntem, tipik olmayan senaryolar için düşünülbildiği için, bu, ilk olarak kendisine atama `IConfidentialClientApplication` yapılmadan ile erişilebilir değildir. `IByRefreshToken`
