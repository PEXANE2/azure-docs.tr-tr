...
* Azure AD için eski portalı kullanarak uygulamanızı sağlayın. Daha fazla bilgi için bkz. [uygulamayı kaydetme](quickstart-register-app.md)
...
Yetkilendirmeleri doğru şekilde ayarladığınızda, proje dizininizde aşağıdaki gibi görünen bir öğe içeren `entitlements.plist` sahip bir dosya görmeniz gerekir:
...
Her bir uygulamanızda Anahtarlık yetkilendirme yetkilerini etkinleştirdikten sonra, SSO kullanmaya hazırsanız, aşağıdaki ayarla `ADAuthenticationSettings` aşağıdaki ayarı kullanarak kimlik anahtarlığınıza ilişkin kimlik SDK 'sını söyleyin:
...
```
`AD_CREDENTIALS_AUTO` ayarı SDK 'nın aracıya çağrı yapmasına çalışmasına izin verir, `AD_CREDENTIALS_EMBEDDED` SDK 'nın aracıya çağrılmasını engeller.
...
Bu yeniden yönlendirme URI 'sinin [Azure Portal](https://portal.azure.com/)kullanarak uygulama kaydlarınızın belirtilmesi gerekir. Azure AD uygulama kaydı hakkında daha fazla bilgi için bkz. [Azure Active Directory tümleştirme](active-directory-how-to-integrate.md).
...
ADAL, aracının cihaza yüklenip yüklenmediğini denetlemek için – canOpenURL: kullanır. Üzerinde iOS 9 ' da, Apple bir uygulamanın sorgulayabilecekleri düzenleri kilitlemiş. `info.plist file`Lsapplicationqueriesdüzenleri bölümüne "msauth" eklemeniz gerekir.
