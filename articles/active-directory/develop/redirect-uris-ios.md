---
title: Microsoft kimlik doğrulama kitaplığı ile yeniden yönlendirme URI 'Leri kullanma | Microsoft Identity platformu
...
* `<scheme>`Uygulamanızın Info. plist `CFBundleURLTypes > CFBundleURLSchemes`dosyasında kayıtlı olmalıdır.  Bu örnekte, Info. plist kaynak kodu olarak açıldı:
...
Uygulamanız, URL şemaları veya evrensel bağlantılar aracılığıyla herhangi bir yanıt aldığında MSAL öğesini çağırmalıdır. Uygulamanızın açıldığı `MSALPublicClientApplication` zaman yönteminiçağırın.`handleMSALResponse:sourceApplication:` Özel düzenler için bir örnek aşağıda verilmiştir:
