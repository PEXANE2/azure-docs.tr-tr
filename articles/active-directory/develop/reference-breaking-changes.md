...
1. İstemci başvurunuz bir `response_type=id_token`id_token talep ettiğinde,`response_type=token`yukarıda oluşturulan web API'sı için bir erişim jetonu () isteğinde de bulunulabilir. Böylece, v2.0 uç noktası `scope` kullanırken parametre `api://GUID/SCOPE`benzer görünmelidir . v1.0 bitiş noktasında, `resource` parametre web API uygulaması URI olmalıdır.
