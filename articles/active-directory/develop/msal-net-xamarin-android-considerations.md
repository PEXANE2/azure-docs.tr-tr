...
Kimlik doğrulama akışının etkileşimli kısmı sona erdiğinde, denetimin MSAL 'e geri gitmesini sağlayın. Android 'de, `OnActivityResult` yöntemini geçersiz kılın `Activity`. Ardından `AuthenticationContinuationHelper` msal sınıfının `SetAuthenticationContinuationEventArgs` yöntemini çağırın. 
...
`android:host=` Değer için Azure Portal kaydettiğiniz paket adını değiştirin. `android:path=` Değer için Azure Portal kaydettiğiniz anahtar karmasını değiştirin. İmza karması URL kodlamalı *olmamalıdır.* İmza karmalarınızın başlangıcında önde gelen eğik`/`çizgi () göründüğünden emin olun.
...
Xamarin. Forms 4.3. x, `package` `com.companyname.{appName}` *AndroidManifest. xml*içinde özniteliğini ayarlayan kodu oluşturur. Olarak `DataScheme` `msal{client_id}`kullanıyorsanız, değeri `MainActivity.cs` ad alanı değeriyle eşleşecek şekilde değiştirmek isteyebilirsiniz.
...
Bir hata geçerli bağlamda mevcut `AuthenticationContinuationHelper` olmadığını gösteriyorsa, Visual Studio Android. csproj * dosyasını yanlış bir şekilde güncelleştirmiş olabilir. Bazı durumlarda, * \<HintPath>* dosya yolu, *monoandroid90*yerine *netstandard13* içerir.
