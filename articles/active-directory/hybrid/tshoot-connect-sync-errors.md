...
Azure AD Connect \(Ağustos 2016 veya üzeri\)en son sürümü ile eşitleme hatalarının bir raporu, eşitleme için Azure AD Connect Health bir parçası olarak [Azure Portal](https://aka.ms/aadconnecthealth) mevcuttur.
...
Azure AD 'ye dışarı aktarma sırasında oluşan hatalar işlemin \(ekleme, güncelleştirme, silme vb.\) Azure AD Connect \(eşitleme altyapısı tarafından denendiği\) başarısız olduğunu gösterir.
...
11. Daha sonra Azure AD, emre Taylor ile aynı şekilde yararlanmaya çalışacaktır. Yani, SMTP: bob@contoso.com dahil olmak üzere üç değere eşit proxyAddresses sahip herhangi bir nesne varsa arama yapılır.
...
InvalidSoftMatch hatasının en yaygın nedeni, ProxyAddresses ve/veya UserPrincipalName öznitelikleri için Azure AD 'deki geçici eşleştirme sürecinde kullanılan aynı değere sahiptir \(ImmutableID\) aynı değere sahiptir. Geçersiz yumuşak eşleşmeyi onarmak için

1. Hataya neden olan yinelenen proxyAddresses, userPrincipalName veya diğer öznitelik değerini belirler. Ayrıca, çakışmaya hangi iki \(veya daha fazla\) nesnesinin dahil olduğunu da belirler. [Eşitleme için Azure AD Connect Health](https://aka.ms/aadchsyncerrors) tarafından oluşturulan rapor, iki nesneyi belirlemenize yardımcı olabilir.
...
#### <a name="example-case"></a>Örnek durum
1. Yönetici, Office 365 ' de vergi departmanı için yeni bir posta etkin güvenlik grubu oluşturur ve tax@contoso.comolarak bir e-posta adresi sağlar. Bu gruba **SMTP: tax\@contoso.com** proxyAddresses öznitelik değeri atandı
...
AttributeValueMustBeUnique hatasının en yaygın nedeni, farklı Sourcetutturucu \(ımutableıd\) aynı değere sahiptir ve ProxyAddresses ve/veya UserPrincipalName öznitelikleri için aynı değere sahiptir. AttributeValueMustBeUnique hatasını onarmak için
...
1. Contoso.com için bir hesap olan Bob Smith, UserPrincipalName ile Active Directory yeni bir kullanıcı olarak eklenmiştir bob@contoso.com
2. Bob, Fabrikam.com adlı farklı bir Contoso.com bölümüne gider ve UserPrincipalName bob@fabrikam.com olarak değiştirilir
...
2. Eşitlemeyi denemek için bir sonraki eşitleme döngüsüne izin verin. Bu zaman eşitlemesi başarılı olur ve bob@fabrikam.com için Bob 'un UserPrincipalName değerini güncelleştirir.
