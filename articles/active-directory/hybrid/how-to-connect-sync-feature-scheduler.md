...
Bu ayarlardan bazılarını `Set-ADSyncScheduler`değiştirebilirsiniz. Aşağıdaki parametreler değiştirilebilir:
...
Zamanlayıcıyı devre dışı bırakmak için `Set-ADSyncScheduler -SyncCycleEnabled $false`çalıştırın.

![Zamanlayıcıyı devre dışı bırak](./media/how-to-connect-sync-feature-scheduler/schedulerdisable.png)

Değişikliklerinizi yaptıktan sonra, `Set-ADSyncScheduler -SyncCycleEnabled $true`Zamanlayıcı 'yı yeniden etkinleştirmeyi unutmayın.
...
Bir eşitleme döngüsünü el ile çalıştırmanız gerekiyorsa, PowerShell Run `Start-ADSyncSyncCycle -PolicyType Delta`'den.
...
1. Zamanlayıcı, `Stop-ADSyncSyncCycle`PowerShell cmdlet 'i ile geçerli döngüsünü durdurmasına söyleyerek başlatın.
...
`Invoke-ADSyncRunProfile` cmdlet 'i zaman uyumludur, diğer bir deyişle bağlayıcı, başarıyla veya bir hata ile işlemi tamamlayana kadar denetimi döndürmez.
