...
- Değiştirilen varsayılan eşitleme kuralı Işleme – daha fazla [](how-to-connect-fix-default-rules.md)bilgi edinin.
...
12/18/2018: İndirilmek üzere yayınlandı
...
### <a name="release-status"></a>Yayın durumu
12/11/2018: İndirilmek üzere yayınlandı
...
11/30/2018:  İndirilmek üzere yayınlandı
...
11/19/2018:  İndirilmek üzere yayınlandı
...
8/21/2018: İndirme ve otomatik yükseltme için yayınlandı. 
...
- Yükseltme sırasında, yükleyici varsayılan eşitleme kurallarında değişiklik algılarsa, değiştirilen kuralların üzerine yazmadan önce yöneticiye bir uyarı gönderilir. Bu, kullanıcının düzeltici eylemler yapmasına ve daha sonra sürdürmesine imkan sağlar. Eski davranış: Değişiklik dışı bir kural varsa, el ile yükseltme, kullanıcıya hiçbir uyarı vermeden bu kuralların üzerine yazılmıştı ve Kullanıcı bilgilendirmeden eşitleme zamanlayıcı devre dışı bırakıldı. Yeni davranış: Değiştirilen kullanıma hazır eşitleme kurallarının üzerine yazmadan önce kullanıcıdan uyarı istenir. Kullanıcı, yükseltme işlemini durdurmak ve düzeltici eylem gerçekleştirdikten sonra daha sonra sürdürülmesi için seçim yapmış olacaktır.
...
- Artık bağlı posta kutuları ve AD dinamik grupları gibi daha fazla hata senaryosunu çözümleyen Azure AD Connect Sihirbazı sorun giderme yardımcı programı güncelleştirildi. Sorun giderme yardımcı programı hakkında daha [](tshoot-connect-objectsync.md)fazla bilgi edinin.
...
- Yeni "cihaz seçeneklerini yapılandır" ek görevi eklendi. Aşağıdaki iki işlemi yapılandırmak için görevini kullanabilirsiniz: 
  - **Karma Azure AD katılımı**: Ortamınızda şirket içi AD ayak izi varsa ve Azure Active Directory ile sağlanan özelliklerden yararlanmak istiyorsanız hibrit Azure AD’ye katılmış cihazları uygulayabilirsiniz. Bunlar hem şirket içi Active Directory alanınıza hem de Azure Active Directory alanınıza katılmış cihazlardır.
...
- Azure AD Connect Sihirbazı: Azure AD Connect bir çalışma grubunda olduğunda AD bağlayıcı hesabı oluşturulurken hata oluştu
...
- Azure AD Connect Sihirbazı: AD FS 'den geçişli kimlik doğrulamaya geçiş yapmak için **Kullanıcı oturum açma görevini Değiştir '** i kullandığınızda aşağıdaki değişiklikler yapılmıştır:
...
- Azure AD Connect Sihirbazı: Kullanıcı UPN 'nin özel karakterleri desteklemesi için özel karakter Regex güncelleştirmesi olduğunda AD FS çok etki alanı Regex doğru değil
- Azure AD Connect Sihirbazı: Değişiklik olmadığında spurmerak "kaynak bağlayıcı özniteliğini yapılandırma" iletisini kaldır 
- Azure AD Connect Sihirbazı: Çift Federasyon senaryosu için AD FS desteği
- Azure AD Connect Sihirbazı: Yönetilen bir etki alanı federe 'a dönüştürülürken AD FS talepler eklenen etki alanı için güncelleştirilmedi
- Azure AD Connect Sihirbazı: Yüklü paketlerin algılanması sırasında, eski DirSync/Azure AD Eşitleme/Azure AD Connect ilgili ürünleri bulacaksınız. Şimdi eski ürünleri kaldırmayı deneyeceğiz.
- Azure AD Connect Sihirbazı: Geçiş kimlik doğrulama aracısının yüklemesi başarısız olduğunda doğru hata Iletisi eşleme
- Azure AD Connect Sihirbazı: "Yapılandırma" kapsayıcısı etki alanı OU filtreleme sayfasından kaldırıldı
...
- Azure AD Connect Sihirbazı: Parola karması eşitleme için Isteğe bağlı özellikler sayfasında açılan menü yardım metni
- Eşitleme altyapısı çalışma zamanı: Bir CS nesnesinde içeri aktarılan silme ve eşitleme kuralları, nesneyi yeniden sağlamayı deneyeceği senaryoyu düzeltir.
- Eşitleme altyapısı çalışma zamanı: Çevrimiçi bağlantı sorun giderme kılavuzu için yardım bağlantısı ekleme hata bir Içeri aktarma hatası için olay günlüğü
- Eşitleme altyapısı çalışma zamanı: Bağlayıcılar numaralandırılırken eşitleme Scheduler 'ın azaltılmış bellek kullanımı
- Azure AD Connect Sihirbazı: AD okuma ayrıcalıklarına sahip olmayan bir özel eşitleme hizmeti hesabını çözmede sorunu çözme
- Azure AD Connect Sihirbazı: Etki alanı ve OU filtreleme seçimlerinin günlüğe kaydedilmesini geliştirme
- Azure AD Connect Sihirbazı: MFA senaryosu için oluşturulan Federasyon güvenine varsayılan talepler eklemek AD FS
- Azure AD Connect Sihirbazı: WAP dağıtımı AD FS: Sunucu ekleme yeni sertifika kullanamaz
- Azure AD Connect Sihirbazı: Bir etki alanı için onPremCredentials başlatılmadığında DSSO özel durumu 
...
>[!NOTE]
>"1.1.524.0 'dan daha sonra derleme dağıtan bazı kiracılar için, oto yükseltme işlevselliği yanlış şekilde devre dışı bırakıldı. Azure AD Connect örneğinizin hala tekrar yükseltme için uygun olduğundan emin olmak için aşağıdaki PowerShell cmdlet 'ini çalıştırın: "Set-ADSyncAutoUpgrade-oto Upgradestate etkin"
...
Durum: Müşteriler seçmek için yayınlandı
...
Yardımcı programı, parola değişikliği gerektirmez. Azure AD Connect sihirbazın "parola karma eşitlemesi sorunlarını giderme" seçeneğinde ek görevde sorun giderin.
...
#### <a name="lock"></a>AD DS hesabına erişimi kilitle
...
* Aşağıdaki yapılandırmalara sahip müşterileri desteklemek için [otomatik yükseltme özelliği](how-to-connect-install-automatic-upgrade.md) genişletildi:
  ...
  * Kullanıcı geri yazma özelliğini etkinleştirdiniz.
  ...
  >[!NOTE]
  >Otomatik yükseltme özelliğinin kapsam genişletmesi, müşterileri Azure AD Connect Build 1.1.105.0 ve After ile etkiler. Azure AD Connect sunucunuzun otomatik olarak yükseltilmesini istemiyorsanız, Azure AD Connect sunucunuzda aşağıdaki cmdlet 'i çalıştırmanız gerekir: `Set-ADSyncAutoUpgrade -AutoUpgradeState disabled`. Otomatik yükseltmeyi etkinleştirme/devre dışı bırakma hakkında daha fazla bilgi için Azure AD Connect [makalesine başvurun: Otomatik yükseltme](how-to-connect-install-automatic-upgrade.md) makalesine bakın.

## <a name="115580"></a>1.1.558.0
Durum: Yayımlanmayacak. Bu derlemedeki değişiklikler 1.1.561.0 sürümüne dahildir.
...
#### <a name="new-features-and-improvements"></a>Yeni özellikler ve geliştirmeler
* Parola geri yazma özelliği artık Microsoft Azure Kamu bulut ve Microsoft Bulut Almanya ile önizlemede kullanılabilir. Farklı hizmet örnekleri için Azure AD Connect desteği hakkında daha fazla bilgi için, Azure AD Connect makalesine [başvurun: Örneklerle](reference-connect-instances.md)ilgili özel konular.
...
* Azure AD Connect, Azure AD 'den LargeObject hatası aldığında Azure AD Connect EventID 6941 ve "sağlanan nesne çok büyük *" iletisini içeren bir olay oluşturur. Bu nesnedeki öznitelik değerlerinin sayısını kırpın. "* Aynı zamanda, Azure AD Connect EventID 6900 ve "Microsoft. online. Ile. *ProvisionRetryException:" iletisiyle bir yanılur olayı da oluşturur. Windows Azure Active Directory hizmetiyle iletişim kurulamıyor. "* Karışıklığı en aza indirmek için, LargeObject hatası alındığında Azure AD Connect artık ikinci olayı oluşturmaz.
...
* Azure AD Connect artık, şirket içi AD nesneleri için kaynak bağlayıcı özniteliği olarak artık '% nGUID ' özelliğinin kullanılmasına otomatik olarak izin verebilir. Ayrıca, Azure AD Connect, boş ise, ' de bir Objectguıd özniteliği değeri ile birlikte,. Bu özellik yalnızca yeni dağıtım için geçerlidir. Bu özellik hakkında daha fazla bilgi edinmek için Azure AD Connect makale bölümüne [bakın: Tasarım kavramları-ms-DS-ımıbu GUID 'i Sourcetutturucu](plan-connect-design-concepts.md#using-ms-ds-consistencyguid-as-sourceanchor)olarak kullanma.
...
AD FS Yönetimi
* AD FS grubu SSL sertifikasını güncelleştirmek için destek eklendi.
...
* Azure Multi-Factor Authentication ve Yükleme sihirbazında Privileged Identity Management kullanarak genel yönetici için destek.
  * Multi-Factor Authentication kullanıyorsanız https://secure.aadcdn.microsoftonline-p.com , proxy 'nizin trafiğe de izin vermek için izin vermeniz gerekir.
  * Uygun şekilde çalışması için https://secure.aadcdn.microsoftonline-p.com , çok faktörlü kimlik doğrulaması için güvenilen siteler listenize eklemeniz gerekir.
...
* Birden çok şirket içi Exchange kuruluşu destekler. Daha fazla bilgi için bkz. [birden çok Active Directory ormanlı karma dağıtımlar](https://technet.microsoft.com/library/jj873754.aspx).
