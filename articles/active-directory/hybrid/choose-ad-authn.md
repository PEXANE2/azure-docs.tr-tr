...
> [!NOTE]
> Kimlik doğrulama yönteminizi değiştirmek için planlama, test ve olası kapalı kalma süresi gerekir. [Hazırlanan dağıtım](./how-to-connect-staged-rollout.md) , kullanıcıların Federasyondan bulut kimlik doğrulamasına geçişini test etmenin harika bir yoludur.
...
* **Gelişmiş senaryolar**. Kuruluşlar tercih ederseniz, Azure AD Premium P2 ile Azure AD Kimlik Koruması raporları olan kimliklerden Öngörüler kullanmak mümkündür. Sızdırılan kimlik bilgileri raporu bir örnektir. Windows Hello for Business, [Parola karması eşitlemesini kullandığınızda belirli gereksinimlere](/windows/access-protection/hello-for-business/hello-identity-verification)sahiptir. [Azure AD Domain Services](../../active-directory-domain-services/tutorial-create-instance.md) , kullanıcılara yönetilen etki alanında kurumsal kimlik bilgilerini sağlamak için Parola karması eşitlemesi gerektirir.

    Parola karması eşitlemesi ile Multi-Factor Authentication gerektiren kuruluşların Azure AD Multi-Factor Authentication veya [koşullu erişim özel denetimlerini](../../active-directory/conditional-access/controls.md#custom-controls-preview)kullanması gerekir. Bu kuruluşlar, Federasyonu kullanan üçüncü taraf veya şirket içi çok faktörlü kimlik doğrulama yöntemlerini kullanamaz.
...
* **Gelişmiş senaryolar**. Doğrudan kimlik doğrulaması, oturum açma sırasında şirket içi hesap ilkesini uygular. Örneğin, şirket içi bir kullanıcının hesap durumu devre dışı bırakıldığında, kilitlendiğinde veya [parolanın süresi dolarsa](../../active-directory/hybrid/how-to-connect-pta-faq.md#what-happens-if-my-users-password-has-expired-and-they-try-to-sign-in-by-using-pass-through-authentication) veya Kullanıcı oturum açmaya izin verildiğinde oturum açma denemesi saatlerin dışında kalır.
    ...
    Geçişli kimlik doğrulaması ile Multi-Factor Authentication gerektiren kuruluşların Azure AD Multi-Factor Authentication (MFA) veya [koşullu erişim özel denetimlerini](../../active-directory/conditional-access/controls.md#custom-controls-preview)kullanması gerekir. Bu kuruluşlar, Federasyona dayanan bir üçüncü taraf veya şirket içi çok faktörlü kimlik doğrulama yöntemi kullanamaz. Gelişmiş özellikler, Parola karması eşitlemesinin doğrudan geçiş kimlik doğrulaması seçmenize bakılmaksızın dağıtılmasını gerektirir. Kimlik koruması kimlik bilgilerinin sızdırılan kimlik bilgileri rapor örneğidir.
...
Azure AD 'de doğrulanamayan yönlendirilebilir olmayan bir etki alanı için Kullanıcı KIMLIĞI oturum açma 'yı uygulamak üzere ek yapılandırmaya ihtiyacınız vardır. Bu gereksinim alternatif oturum açma KIMLIĞI desteği olarak bilinir. Sınırlamalar ve gereksinimler için [Alternatif oturum açma kimliğini yapılandırma](/windows-server/identity/ad-fs/operations/configuring-alternate-login-id) konusuna bakın. Federasyon ile bir üçüncü taraf Multi-Factor Authentication sağlayıcısı kullanmayı tercih ederseniz, sağlayıcının cihazların Azure AD 'ye katılmasına izin vermek için WS-Trust desteklediğinden emin olun.

Dağıtım adımları için [Federasyon Sunucularını Dağıtma](/windows-server/identity/ad-fs/deployment/deploying-federation-servers) konusuna bakın.
...
|Sağlama sisteminin ötesinde şirket içi Internet ve ağ gereksinimleri nelerdir?|Yok|Kimlik doğrulama aracılarını çalıştıran sunuculardan [giden Internet erişimi](../../active-directory/hybrid/how-to-connect-pta-quick-start.md)|Çevre 'te WAP sunucularına [gelen Internet erişimi](/windows-server/identity/ad-fs/overview/ad-fs-requirements)<br><br>Çevre 'teki WAP sunucularından sunucuya AD FS sunuculara gelen ağ erişimi<br><br>Ağ yük dengeleme|
...
|Hangi oturum açma türleri desteklenir?|UserPrincipalName + parola<br><br>[Sorunsuz SSO](../../active-directory/hybrid/how-to-connect-sso.md) kullanarak kimlik doğrulaması Windows-Integrated<br><br>[Alternatif oturum açma KIMLIĞI](../../active-directory/hybrid/how-to-connect-install-custom.md)|UserPrincipalName + parola<br><br>[Sorunsuz SSO](../../active-directory/hybrid/how-to-connect-sso.md) kullanarak kimlik doğrulaması Windows-Integrated<br><br>[Alternatif oturum açma KIMLIĞI](../../active-directory/hybrid/how-to-connect-pta-faq.md)|UserPrincipalName + parola<br><br>sAMAccountName + Password<br><br>Windows-Integrated kimlik doğrulaması<br><br>[Sertifika ve akıllı kart kimlik doğrulaması](/windows-server/identity/ad-fs/operations/configure-user-certificate-authentication)<br><br>[Alternatif oturum açma KIMLIĞI](/windows-server/identity/ad-fs/operations/configuring-alternate-login-id)|
|Iş için Windows Hello destekleniyor mu?|[Anahtar güven modeli](/windows/security/identity-protection/hello-for-business/hello-identity-verification)|[Anahtar güven modeli](/windows/security/identity-protection/hello-for-business/hello-identity-verification)<br>*Windows Server 2016 etki alanı işlev düzeyi gerektirir*|[Anahtar güven modeli](/windows/security/identity-protection/hello-for-business/hello-identity-verification)<br><br>[Sertifika güven modeli](/windows/security/identity-protection/hello-for-business/hello-key-trust-adfs)|
|Çok faktörlü kimlik doğrulama seçenekleri nelerdir?|[Azure AD MFA](/azure/multi-factor-authentication/)<br><br>[Koşullu erişimle özel denetimler *](../../active-directory/conditional-access/controls.md)|[Azure AD MFA](/azure/multi-factor-authentication/)<br><br>[Koşullu erişimle özel denetimler *](../../active-directory/conditional-access/controls.md)|[Azure AD MFA](/azure/multi-factor-authentication/)<br><br>[Azure MFA sunucusu](../../active-directory/authentication/howto-mfaserver-deploy.md)<br><br>[Üçüncü taraf MFA](/windows-server/identity/ad-fs/operations/configure-additional-authentication-methods-for-ad-fs)<br><br>[Koşullu erişimle özel denetimler *](../../active-directory/conditional-access/controls.md)|
...
|Eski protokollerin engellenmesi destekleniyor mu?|[Evet](../../active-directory/conditional-access/overview.md)|[Evet](../../active-directory/conditional-access/overview.md)|[Evet](/windows-server/identity/ad-fs/operations/access-control-policies-w2k12)|
...
|Hangi gelişmiş senaryolar destekleniyor?|[Akıllı parola kilitleme](../../active-directory/authentication/howto-password-smart-lockout.md)<br><br>[Azure AD Premium P2 ile sızdırılan kimlik bilgileri raporları](../identity-protection/overview-identity-protection.md)|[Akıllı parola kilitleme](../../active-directory/authentication/howto-password-smart-lockout.md)|Çok siteli düşük Gecikmeli kimlik doğrulama sistemi<br><br>[Extranet kilitlemeyi AD FS](/windows-server/identity/ad-fs/operations/configure-ad-fs-extranet-soft-lockout-protection)<br><br>[Üçüncü taraf kimlik sistemleriyle tümleştirme](../../active-directory/hybrid/how-to-connect-fed-compatibility.md)|
