...
Uç nokta **https://secure.aadcdn.microsoftonline-p.com** ulaşılamadığından ve genel yöneticinizin MFA 'nın etkin olması durumunda bu hata görüntülenir.  
...
* Parolanın geçici bir parola olması ve değiştirilmesi gerekiyor mu? Gerçekten doğru parola mi? https://login.microsoftonline.com (Azure AD Connect sunucusundan başka bir bilgisayarda) oturum açmayı deneyin ve hesabın kullanılabilir olduğunu doğrulayın.

### <a name="verify-proxy-connectivity"></a>Proxy bağlantısını doğrulama
Azure AD Connect sunucusunun proxy ve Internet ile gerçek bağlantı olup olmadığını doğrulamak için, proxy 'nin Web isteklerine izin vermeyi öğrenmek için bazı PowerShell kullanın. Bir PowerShell isteminde `Invoke-WebRequest -Uri https://adminwebservice.microsoftonline.com/ProvisioningService.svc`çalıştırın. (Teknik olarak ilk çağrı https://login.microsoftonline.com ve bu URI de çalışır, ancak diğer URI yanıt vermek daha hızlıdır.)
...
* https://dc.services.visualstudio.comçağrılar vardır. Yüklemenin başarılı olması için bu URL 'nin ara sunucuda açılması gerekmez ve bu çağrılar yoksayılabilir.
...
![etsh](./media/tshoot-connect-connectivity/nonetsh.png)
