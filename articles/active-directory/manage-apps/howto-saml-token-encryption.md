...
Belirteç şifrelemesini yapılandırmak için, uygulamayı temsil eden Azure AD Uygulama nesnesine ortak anahtarı içeren bir X. 509.440 sertifika dosyası yüklemeniz gerekir. X. 509.440 sertifikasını almak için, uygulamayı uygulamanın kendisinden indirebilir veya uygulama satıcısının şifreleme anahtarları sağladığı veya uygulamanın özel bir anahtar sağlamanızı beklediği durumlarda bu uygulamayı, şifreleme araçları kullanılarak oluşturulabilir, uygulamanın anahtar deposuna yüklenen özel anahtar bölümü ve Azure AD 'ye yüklenen eşleşen ortak anahtar sertifikası gibi durumlar da buna erişebilirsiniz.
...
1. Şifreleme için, uygulamayı `keyCredentials` bir X. 509.440 sertifikasıyla güncelleştirin. Aşağıdaki örnek bunun nasıl yapılacağını göstermektedir.
...
1. **[Set-AzureApplication](/powershell/module/azuread/set-azureadapplication?view=azureadps-2.0-preview&preserve-view=true)** komutunu kullanarak belirteç şifreleme ayarlarını ayarlayın.
