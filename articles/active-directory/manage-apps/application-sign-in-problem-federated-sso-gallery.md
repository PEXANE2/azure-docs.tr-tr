...
Uygulamanın `Issuer` SAML ISTEĞINDEKI Azure AD 'ye gönderdiği öznitelik, Azure AD 'de uygulama Için yapılandırılmış tanımlayıcı değeriyle eşleşmiyor.
...
SAML isteğindeki `AssertionConsumerServiceURL` DEĞERIN Azure AD 'de yapılandırılan yanıt URL 'si değeri ile eşleştiğinden emin olun. Uygulamalarım güvenli tarayıcı uzantısı ile Azure portal [Test deneyimini](../azuread-dev/howto-v1-debug-saml-sso-issues.md) kullanıyorsanız, bu adımları el ile izlemeniz gerekmez.
