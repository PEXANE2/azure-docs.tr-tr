...
- Yalnızca SAML çoklu oturum açma için yapılandırılmış bulut uygulamalarına sahip işlevler, bir kullanıcı veya yönetici tarafından bir kullanıcı veya yönetici tarafından derlendikten sonra OAuth 2,0/OpenID Connect kimlik doğrulaması kullanan Azure AD uygulama platformunda doğrudan oluşturulmuş Azure Active Directory ön kimlik doğrulama veya uygulamaları kullanan uygulama proxy uygulamaları. Bkz. [uygulamalar Için çoklu oturum açma](what-is-single-sign-on.md). Bkz. [bir uygulamaya son kullanıcıların onayını yapılandırma](configure-user-consent.md).
