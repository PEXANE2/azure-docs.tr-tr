...
description: Tüm Azure Active Directory (Azure AD) uygulama proxy uygulamalarını, uygulama KIMLIĞI (AppID), adı (DisplayName), dış URL (ExternalUrl), iç URL (InternalUrl) ve kimlik doğrulama türü ile birlikte listeleyen PowerShell örneği ( ExternalAuthenticationType).
...
Bu PowerShell betiği örneği, uygulama KIMLIĞI (AppID), adı (DisplayName), dış URL (ExternalUrl), iç URL (InternalUrl) ve dahil olmak üzere tüm Azure Active Directory (Azure AD) uygulama proxy uygulamaları hakkındaki bilgileri listeler. kimlik doğrulama türü (ExternalAuthenticationType).
