...
Sonraki adımınız, [uygulamaya Kullanıcı veya grup atamak](methods-for-assigning-users-and-groups.md)olur. Kullanıcıları ve grupları atadıktan sonra, uygulamada oturum açtıklarında Kullanıcı için kullanılacak kimlik bilgilerini sağlayabilirsiniz. **Kullanıcılar ve gruplar**' ı seçin, kullanıcının veya grubun satırı için onay kutusunu işaretleyin ve ardından **kimlik bilgilerini güncelleştir**' i seçin. Son olarak, Kullanıcı veya grup için kullanılacak kullanıcı adını ve parolayı girin. Bunu yapmazsanız, kullanıcılardan başlatma sırasında kimlik bilgilerini girmesi istenir.
