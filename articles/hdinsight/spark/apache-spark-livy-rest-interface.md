...
1. Kullanım kolaylığı için ortam değişkenlerini ayarlayın. Bu örnek, bir Windows ortamını temel alır, ortamınız için gerektiğinde değişkenleri gözden geçirin. `CLUSTERNAME`ve `PASSWORD` uygun değerlerle değiştirin.
...
HDInsight 3,5 kümeleri ve üzeri, varsayılan olarak, örnek veri dosyalarına veya jars 'e erişmek için yerel dosya yollarının kullanımını devre dışı bırakır. Kümeden jar dosyaları dışındaki veya örnek veri dosyalarına erişmek için `wasbs://` yolunu kullanmanızı öneririz.
