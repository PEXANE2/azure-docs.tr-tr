...
description: Verileri okumak, verileri yazmak ve verileri BIR SQL veritabanına aktarmak için HDInsight Spark kümesi ile Azure SQL Veritabanı arasında nasıl bağlantı kurabilirsiniz öğrenin
...
Azure HDInsight'taki Bir Apache Spark kümesini Azure SQL Veritabanı'na nasıl bağlayıp ardından verileri SQL veritabanına nasıl okuyacağını, yazacağını ve aktaracağını öğrenin. Bu makaledeki yönergeler, Scala kod parçacıklarını çalıştırmak için bir [Jupyter Notebook](https://jupyter.org/) kullanır. Ancak, Scala veya Python'da bağımsız bir uygulama oluşturabilir ve aynı görevleri gerçekleştirebilirsiniz.
...
Kıvılcım kümesiyle ilişkili bir [Jupyter Notebook](https://jupyter.org/) oluşturarak başlayın. Bu not defterini, bu makalede kullanılan kod parçacıklarını çalıştırmak için kullanırsınız.
...
1. Bu, varsayılan adı olan **Adsız**yeni bir not defteri açar. Not defteri adını tıklatın ve seçtiğiniz bir ad girin.
...
1. Artık veri şemasını almak gibi veri çerçevesi üzerinde işlemler gerçekleştirebilirsiniz:

       sqlTableDF.printSchema

    Aşağıdakilere benzer bir çıktı görürsünüz:

    ![şema çıktısı](./media/apache-spark-connect-to-sql-database/read-from-sql-schema-output.png "şema çıktısı")

1. Ayrıca, en iyi 10 satır almak gibi işlemleri gerçekleştirebilirsiniz.
