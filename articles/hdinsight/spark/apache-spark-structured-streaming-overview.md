...
Bu JSON dosyaları, HDInsight kümesinin kapsayıcısının altındaki `temps` alt klasöründe depolanır.
...
Sonra, her tetikleyici aralığı içindeki sonuçlar tablosuna eklenen satırlar için hedefi tanımlayın. Bu örnek, daha sonra daha sonra `temps` bir bellek içi tabloya yalnızca bir mini Çıkış modunu tamamen doldurun tüm pencereler için tüm satırların her seferinde çıkış olmasını sağlar.
