...
1. Bir Web tarayıcısından, `CLUSTERNAME` Spark Kümenizin adı olan `https://CLUSTERNAME.azurehdinsight.net/#/main/services/SPARK2/configs`' a gidin.

1. **Gelişmiş spark2-Log4J-Özellikler**' e gidin.

    1. `log4j.appender.console.Threshold=INFO` `log4j.appender.console.Threshold=DEBUG`olarak değiştirin.

    1. `log4j.logger.org.apache.hadoop.fs.azure.NativeAzureFileSystem=DEBUG`ekleyin.

1. **Gelişmiş livy2-Log4J-Özellikler**' e gidin.

    `log4j.logger.org.apache.hadoop.fs.azure.NativeAzureFileSystem=DEBUG`ekleyin.

1. Değişiklikleri kaydedin.

## <a name="additional-logging"></a>Ek günlüğe kaydetme

Yukarıdaki Günlükler dosya sistemi işlemlerinin üst düzey olarak anlaşılmasına sahip olmalıdır. Yukarıdaki Günlükler hala yararlı bilgiler sağlamaıyorsa veya blob depolama API çağrılarını araştırmak istiyorsanız `core-site``fs.azure.storage.client.logging=true` ekleyin. Bu ayar, tek bir depolama sürücüsüne yönelik Java SDK günlüklerini etkinleştirir ve her bir blob Storage Server çağrısını yazdırır. Diski hızla doldurabileceğinden ve işlemi yavaşlatabileceğinden, araştırmalar sonrasında ayarı kaldırın.
...
Günlüklerde Günlükler için `/var/log/adl/adl.log` bakın.
