...
1. Bir Web tarayıcısından CLUSTERNAME öğesinin Kümenizin adı olduğu `https://CLUSTERNAME.azurehdinsight.net/jupyter` ' a gidin.
...
3. Sayfada ilk alanda ( **hücre**olarak adlandırılır) aşağıdaki metni girin:
   ...
   ```
    ...
    Bu kod Spark 'ı, giriş verileri için dizin yapısına yinelemeli olarak erişecek şekilde yapılandırır. Application Insights telemetri, `/{telemetry type}/YYYY-MM-DD/{##}/`benzer bir dizin yapısına kaydedilir.

4. Kodu çalıştırmak için **SHIFT + enter** tuşlarını kullanın. Hücrenin sol tarafında, bu hücredeki kodun yürütüldüğü belirten köşeli ayraçlar arasında bir '\*' görüntülenir. İşlem tamamlandıktan sonra, '\*' bir sayıya değişir ve aşağıdaki metne benzer bir çıkış hücrenin altında görüntülenir:
...
5. Birinci bir hücre aşağıda oluşturulur. Yeni hücreye aşağıdaki metni girin. `CONTAINER` ve `STORAGEACCOUNT`, Application Insights verileri içeren Azure depolama hesabı adı ve BLOB kapsayıcısı adıyla değiştirin.
   ...
   ```
    ...
    Döndürülen IBS yolu, Application Insights Telemetri verilerinin konumudur. Döndürülen IBS yolunu kullanmak için hücredeki `hdfs dfs -ls` satırını değiştirin ve sonra hücreyi yeniden çalıştırmak için **SHIFT + enter** tuşlarını kullanın. Bu kez, sonuçlar telemetri verilerini içeren dizinleri görüntülemelidir.

   > [!NOTE]  
   > Bu bölümdeki adımların geri kalanı için `wasbs://appinsights@contosostore.blob.core.windows.net/contosoappinsights_{ID}/Requests` dizini kullanılmıştır. Dizin yapınız farklı olabilir.

6. Sonraki hücrede aşağıdaki kodu girin: `WASB_PATH` önceki adımdaki yol ile değiştirin.
...
7. Bir sonraki hücrede, Spark 'ın JSON dosyaları için oluşturduğu şemayı görüntülemek için aşağıdakileri girin ve aşağıdakini çalıştırın:
   ...
   ```
    ...
    Her telemetri türünün şeması farklıdır. Aşağıdaki örnek, Web istekleri için oluşturulan şemadır (`Requests` alt dizininde depolanan veriler):
...
5. Birinci bir hücre aşağıda oluşturulur. Yeni hücreye aşağıdaki metni girin. `CONTAINER` ve `STORAGEACCOUNT`, Azure depolama hesabı adı ve Application Insights günlükleri içeren blob kapsayıcısı adıyla değiştirin.
   ...
   > [!NOTE]  
   > Bu bölümdeki adımların geri kalanı için `wasbs://appinsights@contosostore.blob.core.windows.net/contosoappinsights_{ID}/Requests` dizini kullanılmıştır. Telemetri verileriniz bir Web uygulaması için değilse, bu dizin mevcut olmayabilir.

6. Sonraki hücrede aşağıdaki kodu girin: `WASB\_PATH` önceki adımdaki yol ile değiştirin.
