...
Kurumsal Güvenlik Paketi (ESP) kümeleri Azure HDInsight kümelerinde çok kullanıcılı erişim sağlar. ESP'li HDInsight kümeleri, etki alanı kullanıcılarının kümeleri doğrulamak ve büyük veri işleri çalıştırmak için etki alanı bilgilerini kullanabilmeleri için bir etki alanına bağlanır.
...
ESP kümeleri kurmak için, zaten bir tane yoksa, kullanıcı tarafından atanmış yönetilen bir kimlik oluşturun. Talimatlar için, [Azure portalını kullanarak kullanıcı tarafından atanan bir kimliğe rol oluşturun, listeleyin, silin veya atayın.](../../active-directory/managed-identities-azure-resources/how-to-manage-ua-identity-portal.md) 
...
**HDInsight Etki Alanı Hizmetleri Katılımcısı** rolünün atanması, bu kimliğin Azure AD DS etki alanında etki alanı hizmetleri işlemleri gerçekleştirmek için uygun (adına) erişime sahip olmasını sağlar. Bu işlemler, OSB oluşturma ve silme içerir.

Yönetilen kimlik oluşturulduktan ve doğru rol verildikten sonra, Azure AD DS yöneticisi bu yönetilen kimliği kimlerin kullanabileceğini ayarlayabilir. İlk olarak, yönetici portalda yönetilen kimliği seçer ve ardından **Genel Bakış**altında Erişim **Denetimi 'ni (IAM)** seçer. Daha sonra, sağda yönetici Yönetilen **Kimlik İşlemi** rolünü HDInsight ESP kümeleri oluşturmak isteyen kullanıcılara veya gruplara atar. 

Örneğin, Azure AD DS yöneticisi, aşağıdaki resimde gösterildiği gibi, **sjmsi** yönetilen kimlik için bu rolü **MarketingTeam** grubuna atayabilir. Bu atama, kuruluştaki doğru kişilerin ESP kümeleri oluşturmak için yönetilen kimliği kullanabilmesini sağlar.
...
**Ayarlar** kategorisindeDNS sunucularını seçerek bu özel IP'leri kullanmak için Azure AD DS sanal ağındaki **DNS sunucularının** yapılandırmasını değiştirin. Ardından **Özel** seçeneğini seçin, metin kutusuna ilk IP adresini girin ve **Kaydet'i**seçin. Aynı adımları kullanarak daha fazla IP adresi ekleyin.
...
Ağınızın doğru ayarlanıp ayarlanıp ayarlamamasını test etmek için, HDInsight sanal ağına/alt ağına bir Windows VM'ye katılın ve etki alanı adını pingleyin. (Bir IP'ye çözülmelidir.) Azure AD DS etki alanına erişmek için **ldp.exe'yi** çalıştırın. Ardından, istemci ve sunucu arasında gerekli tüm RPC çağrılarının başarılı olduğunu doğrulamak için bu Windows VM'yi etki alanına katılın. 

**NSLookup'ı,** depolama hesabınıza veya kullanabileceğiniz herhangi bir harici veritabanına (örneğin, harici Hive metastore veya Ranger DB) ağ erişimini onaylamak için de kullanabilirsiniz. Bir NSG Azure AD DS'nin güvenliğini sağlamaya yardımcı oluyorsa, gerekli tüm [bağlantı noktalarına](/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/dd772723(v=ws.10)#communication-to-domain-controllers) Azure AD DS alt netinin NSG kurallarında izin verildiğinden emin olun. Bu Windows VM'nin etki alanına katılması başarılı olursa, bir sonraki adıma devam edebilir ve ESP kümeleri oluşturabilirsiniz.

## <a name="create-an-hdinsight-cluster-with-esp"></a>ESP ile HDInsight kümesi oluşturma

Önceki adımları doğru ayarladıktan sonra, bir sonraki adım ESP etkin leştirilmiş HDInsight kümesini oluşturmaktır. Bir HDInsight kümesi oluşturduğunuzda, Güvenlik + **ağ** sekmesinde Kurumsal Güvenlik Paketi'ni etkinleştirebilirsiniz. Dağıtım için bir Azure Kaynak Yöneticisi şablonu kullanmayı tercih ediyorsanız, portal deneyimini bir kez kullanın ve ileride yeniden kullanmak için **Gözden Geçirme + oluştur** sayfasında önceden doldurulmuş şablonu indirin. 
