...
SCP gerçek zamanlı, güvenilir, tutarlı ve yüksek performanslı veri işleme uygulaması oluşturmaya yönelik bir platformdur. Bu, OSS toplulukları tarafından tasarlanan bir akış işleme sistemi olan [Apache Storm](https://storm.incubator.apache.org/) üzerine kurulmuştur. Fırtınası, Nathan Marz tarafından tasarlanmıştır ve Twitter tarafından açık kaynaklanmıştır. Son derece güvenilir dağıtılmış koordinasyon ve durum yönetimini etkinleştirmek için [Apache ZooKeeper](https://zookeeper.apache.org/), başka bir Apache projesi kullanır.
...
Fırtınası ' da, bir uygulama topolojisi hesaplama grafiğini tanımlar. Bir topolojideki her düğüm, işlem mantığını içerir ve düğümler arasındaki bağlantılar veri akışını gösterir. Topolojiye giriş verisi ekleme düğümlerine, verileri sıralamak için kullanılabilecek _Spout_'lar adı verilir. Giriş verileri dosya günlüklerinde, işlem veritabanında, sistem performans sayacında ve bu şekilde bulunabilir. Hem giriş hem de çıkış verisi akışına sahip düğümler, gerçek veri filtreleme ve seçim ve toplama işlemlerini sağlayan _cıvatalar_olarak adlandırılır.

SCP, en az bir kez ve tam olarak bir kez veri işleme olmak üzere en iyi çabaları destekler. Dağıtılmış bir akış işleme uygulamasında, veri işleme sırasında ağ kesintisi, makine arızası veya Kullanıcı kodu hatası gibi çeşitli hatalar oluşabilir. En az bir kez işleme, hata oluştuğunda otomatik olarak aynı veriler yeniden çalıştırılarak tüm verilerin en az bir kez işlenmesini sağlar. En az bir kez işleme basit ve güvenilirdir ve birçok uygulama için uygun olur. Ancak, bir uygulama tam sayım gerektirdiğinde, aynı veriler uygulama topolojisinde yürütülebildiğinden en az bir kez işleme yeterli değildir. Bu durumda, veriler yeniden çalındığında ve birden çok kez işlenebilse bile sonucun doğru olduğundan emin olmak için tam olarak bir kez işleme tasarlanmıştır.

SCP, .NET geliştiricilerinin Java Sanal Makinesi (JVM) üzerinde, kapsamaları altında geçen bir süre içinde yaşam sırasında gerçek zamanlı veri işlem uygulamaları geliştirmesini sağlar. .NET ve JVM, TCP yerel Yuvaları üzerinden iletişim kurar. Temel olarak her Spout/Cıvam, Kullanıcı mantığının .NET işleminde bir eklenti olarak çalıştırıldığı .NET/Java işlem çiftidir.
...
`NextTuple()` çağrıldığında, C# Kullanıcı kodu bir veya daha fazla tanımlama grubu yayabilir. Yayma için bir şey yoksa, bu yöntem hiçbir şeyi yaymadan döndürmelidir. `NextTuple()`, `Ack()`ve `Fail()` tümünün C# işlemdeki tek bir iş parçacığında sıkı bir döngüde çağrıldığından emin olun. Görüntülenecek bir tanımlama grubu olmadığında, çok fazla CPU harcanmaması için kısa bir süre (10 milisaniyelik gibi) için NextTuple uykuya geçme imkanı vardır.

`Ack()` ve `Fail()` yalnızca belirtim dosyasında ACK mekanizması etkinleştirildiğinde çağrılır. `seqId`, kabul edilen veya başarısız olan tanımlama grubunu belirlemek için kullanılır. Bu nedenle, ACK işlem dışı topolojide etkinleştirilirse, Spout 'de aşağıdaki yayma işlevi kullanılmalıdır:

    public abstract void Emit(string streamId, List<object> values, long seqId); 

ACK, işlemsel olmayan topolojide desteklenmiyorsa, `Ack()` ve `Fail()` boş işlev olarak bırakılabilir.

Bu işlevlerdeki `parms` giriş parametresi boş bir sözlüktür ve gelecekte kullanılmak üzere ayrılmıştır.
...
Yeni kayıt düzeni kullanılabilir olduğunda, `Execute()` işlevi bunu işlemek için çağırılır.
...
İşlemsel olmayan sayaç, `NextTx()`, `Ack()`ve `Fail()` gibi C# işlemler, işlemdeki tek bir iş parçacığında sıkı bir döngüde çağırılır. Görüntülenecek veri olmadığında bu, kısa bir süre (10 milisaniye) için `NextTx` uykuya geçecek ve böylece çok fazla CPU harcanmayacak.

`NextTx()` yeni bir işlem başlatmak için çağrılır, `Ack()` ve `Fail()`de kullanılan işlemi tanımlamak için out parametresi `seqId` kullanılır. `NextTx()`, Kullanıcı Java tarafında veri yayabilir. Veriler, yeniden yürütmeyi desteklemek için ZooKeeper içinde depolanır. ZooKeeper kapasitesi sınırlı olduğundan, Kullanıcı işlem biriktirme listesinden toplu verileri değil yalnızca meta verileri yaymalıdır.

Fırtınası başarısız olursa, bir işlemi otomatik olarak yeniden çalıştırır, bu nedenle `Fail()` normal durumda çağrılmamalıdır. Ancak SCP, işlemsel Spout tarafından oluşturulan meta verileri denet, meta veriler geçersiz olduğunda `Fail()` çağırabilir.
...
`Execute()`, cıvata yeni bir kayıt düzeni olduğunda çağrılır. `FinishBatch()`, bu işlem bittiğinde çağrılır. `parms` giriş parametresi gelecekte kullanılmak üzere ayrılmıştır.

İşlemsel topoloji için önemli bir kavram vardır: `StormTxAttempt`. `TxId` ve `AttemptId`iki alanı vardır. `TxId` belirli bir işlemi tanımlamak için kullanılır ve belirli bir işlem için işlem başarısız olursa ve yeniden yürütülemezse birden çok girişim olabilir. SCP.NET, her bir `StormTxAttempt`işlemek için, Java 'daki hangi fırtınası gibi yeni bir ıscpbatchcıvam nesnesi oluşturur. Bu tasarımın amacı paralel işlem işlemeyi destekliyoruz. Kullanıcı, işlem denemesinin tamamlanıyorsa, karşılık gelen ıscpbatchcıvam nesnesinin yok edildiğini ve atık olarak toplandığını göz önünde bulundurmanız gerekir.

## <a name="object-model"></a>Nesne modeli

SCP.NET ayrıca geliştiricilerin ile programlama için basit bir temel nesne kümesi sağlar. Bunlar **bağlam**, **Statestore**ve **SCPRuntime**. Bunlar, bu bölümün Rest bölümünde ele alınmıştır.

### <a name="context"></a>Bağlam

Bağlam, uygulamaya çalışan bir ortam sağlar. Her bir SCC (ıscpspout/Iscpsürgüsü/ıscptxspout/Iscpbatchcıvatsu) her SCC, karşılık gelen bir bağlam örneğine sahiptir. Bağlam tarafından sağlanan işlevsellik iki parçaya ayrılabilir: (1) tüm C# işlemde kullanılabilir olan statik bölüm, (2) yalnızca belirli bağlam örneği için kullanılabilen dinamik bölüm.
...
günlük amacı için `Logger` sağlanır.

`pluginType`, C# işlemin eklenti türünü belirtmek için kullanılır. C# İşlem yerel test modunda (Java olmadan) çalışıyorsa, eklenti türü `SCP_NET_LOCAL`.
...
Java tarafında yapılandırma parametrelerini almak için `Config` sağlanır. C# Eklenti başlatıldığında parametreler Java tarafında geçirilir. `Config` parametreleri iki parçaya ayrılmıştır: `stormConf` ve `pluginConf`.

    public Dictionary<string, Object> stormConf { get; set; }  
    public Dictionary<string, Object> pluginConf { get; set; }  

`stormConf`, fırtınası tarafından tanımlanan parametrelerdir ve `pluginConf` SCP tarafından tanımlanan parametrelerdir. Örneğin:
...
topoloji bağlamını almak için `TopologyContext` sağlanır, bu en çok paralellik olan bileşenler için kullanışlıdır. Örnek aşağıda verilmiştir:
...
ACK 'i destekleyen işlem olmayan bir cıvata, açıkça `Ack()` veya aldığı tanımlama grubunu `Fail()` gerekir. Ayrıca, yeni kayıt düzeni yayırken yeni demet 'in tutturucularını da belirtmelidir. Aşağıdaki yöntemler verilmiştir.
...
`StateStore` meta veri Hizmetleri, monoton sıra oluşturma ve ücretsiz düzenleme koordinasyonu sağlar. Daha yüksek düzeyde dağıtılmış eşzamanlılık soyutlama, dağıtılmış kilitler, dağıtılmış kuyruklar, engelleri ve işlem hizmetleri dahil `StateStore`derlenebilir.

SCP uygulamaları, özellikle de işlemsel topoloji için bazı bilgileri [Apache ZooKeeper](https://zookeeper.apache.org/)içinde kalıcı hale getirmek için `State` nesnesini kullanabilir. Böylece, işlem biriktirme listesinden Kilitlenmeler ve yeniden başlatırsanız, ZooKeeper adresinden gerekli bilgileri alabilir ve ardışık düzeni yeniden başlatabilir.
...
`Commit()` yöntemi için, simpleMode true olarak ayarlandığında, ZooKeeper içinde karşılık gelen ZNode öğesini siler. Aksi takdirde, geçerli ZNode öğesini siler ve yürütülen\_yoluna yeni bir düğüm ekler.
...
`Initialize()` SCP çalışma zamanı ortamını başlatmak için kullanılır. Bu yöntemde, C# işlem Java tarafına bağlanır ve yapılandırma parametrelerini ve topoloji bağlamını alır.

`LaunchPlugin()` ileti işleme döngüsünü devre dışı bırakmak için kullanılır. Bu döngüde, C# eklenti iletileri Java tarafında alır (diziler ve denetim sinyalleri dahil) ve ardından, Kullanıcı kodu tarafından sağlanacak arabirim yöntemini çağıran iletileri işler. `LaunchPlugin()` yöntemi için giriş parametresi, ıscpspout/Iscpcıvata/ıscptxspout/Iscpbatchcıvatu arabirimini uygulayan bir nesne döndürebileceğinizi temsil eden bir temsilcisidir.

    public delegate ISCPPlugin newSCPPlugin(Context ctx, Dictionary\<string, Object\> parms); 

Iscpbatchcıvata, `parms``StormTxAttempt` alabilir ve yeniden yürütülmüş bir girişimin olup olmadığını jtik hale getirmek için kullanabilirsiniz. Yeniden yürütme denemesine yönelik denetim genellikle yürütme sürgüsü üzerinde yapılır ve `HelloWorldTx` örnekte gösterilmiştir.

Genellikle, SCP eklentileri burada iki modda çalışabilir:

1. Yerel test modu: Bu modda, SCP eklentileri ( C# Kullanıcı kodu) geliştirme aşamasında Visual Studio içinde çalıştırılır. `LocalContext`, bu modda kullanılabilir tanımlama gruplarını yerel dosyalara seri hale getirmek ve bunları belleğe geri okumak için yöntemler sağlar.
...
| SCP-TX-Spout |exec-adı<br />args<br />alanlar |İşlem biriktirme listesini tanımlayın. Uygulamayı, ***bağımsız değişkenleri***kullanarak ***Exec-Name*** ile çalıştırır.<br /><br />***Alanlar*** , Spout Için çıkış alanlarıdır |
| SCP-TX-Batch-sürgüsü |exec-adı<br />args<br />alanlar |İşlem Batch sürgüsü tanımlayın. Uygulamayı, ***bağımsız değişkenleri*** kullanarak ***Exec-Name*** ile çalıştırır.<br /><br />Alanlar, cıvata çıkış alanlarıdır. |
| SCP-TX-COMMIT-cıvata |exec-adı<br />args<br />alanlar |İşlemsel bir işleme sürgüsü tanımlayın. Uygulamayı, ***bağımsız değişkenleri***kullanarak ***Exec-Name*** ile çalıştırır.<br /><br />***Alanlar*** , cıvata çıkış alanlarıdır |
...
| SCP-Spout |exec-adı<br />args<br />alanlar<br />parametreler |İşlem dışı bir Spout tanımlayın. Uygulamayı, ***bağımsız değişkenleri***kullanarak ***Exec-Name*** ile çalıştırır.<br /><br />***Alanlar*** , Spout Için çıkış alanlarıdır<br /><br />***Parametreler*** , "işlem dışı. ACK. Enabled" gibi bazı parametreleri belirtmek için kullanılarak isteğe bağlıdır. |
| SCP-sürgüsü |exec-adı<br />args<br />alanlar<br />parametreler |İşlem dışı bir sürgüsü tanımlayın. Uygulamayı, ***bağımsız değişkenleri***kullanarak ***Exec-Name*** ile çalıştırır.<br /><br />***Alanlar*** , cıvata çıkış alanlarıdır<br /><br />***Parametreler*** , "işlem dışı. ACK. Enabled" gibi bazı parametreleri belirtmek için kullanılarak isteğe bağlıdır. |
...
***Resource-dir*** parametresi isteğe bağlıdır, C# uygulamayı bağlamak istediğinizde belirtmeniz gerekir ve bu dizin uygulamayı, bağımlılıkları ve konfigürasyonları içerir.
...
Kullanıcılar, C# işlemlere tanımlama gruplarını yayabilir, platformun tanımlama grubunu Byte [], Java tarafında aktarma ve fırtınası bu kayıt grubunu hedeflere aktarabilmesi gerekir. Aşağı akış bileşenlerinde, C# işlemler Java tarafından geri tanımlama gruplarını alır ve platforma göre orijinal türlere dönüştürür, tüm bu işlemler platform tarafından gizlenir.
...
SCP.NET Context nesnesindeki iki yöntem eklenmiştir. Bunlar, Streamıd belirtmek için tanımlama grubu veya tanımlama grupları oluşturmak için kullanılır. Streamıd bir dizedir ve hem C# hem de topoloji tanımı belirtiminde tutarlı olması gerekir.
...
Fırtınası içinde yerleşik alanlar gruplandırmada SCP.NET içinde düzgün çalışmıyor. Java proxy tarafında, tüm alanlar veri türleri gerçekte Byte [] ve alanlar gruplandırmada, gruplandırmayı gerçekleştirmek için Byte [] nesne karma kodu kullanılır. Byte [] nesnesi karma kodu, bu nesnenin bellekteki adresidir. Bu nedenle, aynı içeriği paylaşan ancak aynı adresi içermeyen iki baytlık nesneler için gruplama yanlış olur.
...
Yerel fırtınası Java 'da yazılmıştır. Ve SCP.NET, geliştiricilerin iş mantığını işlemek C# üzere kod yazmasını C# sağlamak için geliştirmiştir. Ancak aynı zamanda yalnızca C# Spoler/cıvatları değil, Java Spout/cıvatları içeren karma topolojileri da destekler.
...
Burada **\\Hybridtopoloji\\java\\target\\** , Java Spout/cıvata jar dosyasını içeren klasördür.

### <a name="serialization-and-deserialization-between-java-and-c"></a>Java ve arasında serileştirme ve seri durumundan çıkarmaC#
...
1. Java tarafında serileştirme ve C# yan yana seri hale getirme

   İlk olarak Java tarafında serileştirme ve C# yan yana seri hale getirme için varsayılan uygulama sağlıyoruz. Java tarafında serileştirme yöntemi SPEC dosyasında belirtilebilir:
   ...
   C# Yan taraftaki seri kaldırma yöntemi C# Kullanıcı kodunda belirtilmelidir:
   ...
   Bu varsayılan uygulama, veri türü çok karmaşık olmadığı için çoğu durumu işlemelidir. Bazı durumlarda, Kullanıcı veri türü çok karmaşık olduğundan ya da varsayılan uygulamamız performansı kullanıcının gereksinimini karşılamadığından, kullanıcılar kendi uygulamasını ekleyebilir.
   ...
   C# Yan yana Serisi kaldırma arabirimi şöyle tanımlanır:
...
2. Java tarafında C# seri hale getirme yan ve seri durumundan çıkarma

   C# Yan taraftaki serileştirme yöntemi C# Kullanıcı kodunda belirtilmelidir:
   ...
   Kullanıcı ayrıca C# serileştirici ve Java seri hale getirici uygulamasını da ekleyebilir. Bu kod seri hale getirici için C# arabirimidir:
...
Burada `plugin.name` SCP SDK tarafından sağlanmış `SCPHost.exe` olarak belirtilir. SCPHost. exe üç parametre kabul eder:

1. Birincisi, bu örnekte `"HelloWorld.dll"` DLL adıdır.
2. İkinci tane, bu örnekte `"Scp.App.HelloWorld.Generator"` sınıf adıdır.
...
**HelloWorld** , SCP.net 'in gösterilmesi için basit bir örnektir. **Oluşturucu**adlı bir Spout ve **Splitter** ve **sayaç**adlı iki cıvatiyle birlikte işlemsel olmayan bir topoloji kullanır. Spout **Oluşturucu** rastgele cümleler oluşturur ve bu cümleleri **bölümlendiriciye**yayar. Sürgüsü * * Splitter, cümleleri bir sözcük olarak böler ve bu sözcükleri **sayaç** sürgüsü için yayar. "Counter" sürgüsü, her sözcüğün oluşum numarasını kaydetmek için bir sözlük kullanır.

Bu örnekte, **Merhaba. spec** ve **HelloWorld\_enableack. spec** olmak üzere iki teknik dosya vardır. C# Kodda, Java tarafında pluginconf dosyasını alarak ACK 'in etkinleştirilip etkinleştirilmediğini öğrenebilirsiniz.
...
Spout, ACK etkinleştirilirse, onaylanan tanımlama gruplarını önbelleğe almak için bir sözlük kullanılır. Başarısız olursa, başarısız kayıt düzeni yeniden yürütülür:
...
**Merhaba Worldtx** örneği, işlem topolojisinin nasıl uygulanacağını gösterir. **Oluşturucu**adlı bir Spout, **kısmi sayı**olarak adlandırılan bir yığın işareti ve **Count-Sum**adlı bir işleme işareti vardır. Ayrıca, önceden oluşturulmuş üç txt dosyası vardır: **DataSource0. txt**, **DataSource1. txt**ve **DataSource2. txt**.
...
Bir ıscpbatchcıvam örneği oluşturulduğunda, Giriş parametrelerinden `txAttempt` alır:
...
`FinishBatch()` çağrıldığında, yeniden yürütülmüş bir işlem değilse `lastCommittedTxId` güncelleştirilir.
...
Bu topoloji bir Java Spout ve bir C# sürgüsü içerir. SCP platformu tarafından sunulan varsayılan serileştirme ve seri kaldırma uygulamasını kullanır. Örnek dosya ayrıntıları için **\\hybridtopology** **. spec** ve Java sınıfyolunu belirtme için **submittopology. bat** ' a bakın.
