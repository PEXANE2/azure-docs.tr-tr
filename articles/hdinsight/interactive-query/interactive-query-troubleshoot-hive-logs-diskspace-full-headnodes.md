...
`ambari-agent` Günlükler, sorun oluştuğunda şunları gösterir.
```
...
1. Ambarı portalındaki Hive bileşen Özeti ' ne gidin ve `Configs` sekmesine tıklayın.

2. Gelişmiş ayarlar içindeki `Advanced hive-log4j` bölümüne gidin.

3. `log4j.appender.RFA` parametresini RollingFileAppender olarak ayarlayın. 

4. `log4j.appender.RFA.MaxFileSize` ve `log4j.appender.RFA.MaxBackupIndex` aşağıdaki şekilde ayarlayın.
...
```
5. `hive.root.logger` aşağıdaki gibi `INFO,RFA` olarak ayarlayın. Varsayılan ayar, günlükleri çok büyük hale getiren hata AYıKLA.
