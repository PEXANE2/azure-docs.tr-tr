...
* `azure hdinsight cluster show`-var olan bir küme hakkındaki bilgileri görüntüleme
* `azure hdinsight cluster list`-Azure aboneliğiniz için HDInsight kümelerini listeler

Her komut için kullanılabilen parametreleri ve anahtarları incelemek için `-h` anahtarını kullanın.
...
> [!NOTE]  
> Liste komutu için `-g` kullanarak kaynak grubunu belirtmek yalnızca belirtilen kaynak grubundaki kümeleri döndürür.
...
| --- | --- |
| .NET için Azure HDInsight SDK|Bkz. [.net Için Azure HDıNSIGHT SDK](https://docs.microsoft.com/dotnet/api/overview/azure/hdinsight?view=azure-dotnet) |
