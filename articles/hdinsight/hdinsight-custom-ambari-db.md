...
Yeni kümeniz ve ambarı barındıracak `azuredeploy.parameters.json` veritabanı hakkındaki bilgileri belirtmek için içindeki parametreleri düzenleyin.

Azure CLı kullanarak dağıtımı başlatabilirsiniz. Kümenizi `<RESOURCEGROUPNAME>` dağıtmak istediğiniz kaynak grubuyla değiştirin.
