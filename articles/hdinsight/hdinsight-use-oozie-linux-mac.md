...
* **Bir Azure SQL veritabanı**.  Bkz. [Azure Portal Azure SQL veritabanı oluşturma](../sql-database/sql-database-get-started.md).  Bu makale adlı `oozietest`bir veritabanını kullanır.
...
2. Dizini oluşturmak için aşağıdaki komutu kullanın:
    ...
    > [!NOTE]  
    > `-p` Parametresi, yoldaki tüm dizinlerin oluşturulmasına neden olur. Dizin, `useooziewf.hql` komut dosyası tarafından kullanılan verileri tutmak için kullanılır. `data`

3. Aşağıdaki kodu, SSH kullanıcı adınızla `username` değiştirmek için düzenleyin.  Oozie 'nin Kullanıcı hesabınızın kimliğine bürünebileceği emin olmak için aşağıdaki komutu kullanın:
...
3. GNU nano Düzenleyicisi açıldıktan sonra dosyanın içeriği olarak aşağıdaki sorguyu kullanın:
...
4. Dosyayı kaydetmek için CTRL + X ' `Y`i seçin ve ENTER ' u seçin.  

5. ' A kopyalamak `useooziewf.hql` `wasbs:///tutorials/useoozie/useooziewf.hql`için aşağıdaki komutu kullanın:
...
2. Nano düzenleyici açıldıktan sonra, dosya içeriği olarak aşağıdaki XML 'i girin:
   ...
   * `RunSqoopExport`: Bu eylem, Hive betikten oluşturulan verileri Sqoop kullanarak bir SQL veritabanına aktarır. Bu eylem yalnızca `RunHiveScript` eylem başarılı olursa çalışır.
     ...
     İş akışında gibi birkaç giriş `${jobTracker}`vardır. Bu girişleri, iş tanımında kullandığınız değerlerle değiştirirsiniz. Bu belgede daha sonra iş tanımını oluşturacaksınız.
...
3. Dosyayı kaydetmek için CTRL + X ' `Y`i seçin ve ENTER ' u seçin.  

4. `workflow.xml`Dosyayıkopyalamak için aşağıdaki komutu kullanın: `/tutorials/useoozie/workflow.xml`
...
4. İstemine`1>` girerek `exit` TSQL yardımcı programından çıkın.

## <a name="create-the-job-definition"></a>İş tanımını oluşturma

İş tanımı iş akışı. xml ' nin nerede bulunacağını açıklar. Ayrıca, iş akışı tarafından kullanılan diğer dosyaları (örneğin `useooziewf.hql`,) nerede bulabileceğinizi açıklar. Ayrıca, iş akışı ve ilişkili dosyalar içinde kullanılan özellikler için değerleri tanımlar.
...
2. Aşağıdaki XML 'i aşağıda gösterildiği gibi düzenleyin:
    ...
    Bu dosyadaki bilgilerin çoğu, iş akışı. xml veya oozıewf. HQL dosyalarında `${nameNode}`kullanılan değerleri (gibi) doldurmak için kullanılır.  Yol bir `wasbs` yol ise, tam yolu kullanmanız gerekir. Bunu yalnızca `wasbs:///`olarak kısalmayın. `oozie.wf.application.path` Giriş, Workflow. xml dosyasının nerede bulunacağını tanımlar. Bu dosya, bu iş tarafından çalıştırılan iş akışını içerir.
...
5. Dosyayı kaydetmek için CTRL + X ' `Y`i seçin ve ENTER ' u seçin.
...
1. Oozie hizmetinin URL 'sini almak için aşağıdaki komutu kullanın:
    ...
    `http://hn0-CLUSTERNAME.randomcharacters.cx.internal.cloudapp.net:11000/oozie` Bölüm, Oozie komutuyla kullanılacak URL 'dir.
...
4. Önceki adımda döndürülen kimlikle değiştirmek `<JOBID>` için aşağıdaki kodu düzenleyin.  İşin durumunu görüntülemek için aşağıdaki komutu kullanın:
    ...
    Bu işin durumu `PREP`vardır. Bu durum işin oluşturulduğunu, ancak başlatılmadığını gösterir.
...
* **URI**: REST API ' de kümenin `https://CLUSTERNAME.azurehdinsight.net/oozie`dışından erişebilirsiniz.

* **Kimlik doğrulama**: Kimlik doğrulamak için, küme HTTP hesabı (yönetici) ve parola API 'sini kullanın. Örneğin:
...
3. Sayfanın sol tarafındaki **Oozie** > **hızlı bağlantılar** > **Oozie Web Kullanıcı arabirimi**' ni seçin.
    ...
    ![Menülerin görüntüsü](./media/hdinsight-use-oozie-linux-mac/hdi-oozie-web-ui-steps.png)
...
4. Oozie Web Kullanıcı arabirimi, çalışan iş akışı işlerinin varsayılan değerlerini görüntüler. Tüm iş akışı işlerini görmek için **tüm işler**' i seçin.
    ...
    ![Tüm işler görüntülendi](./media/hdinsight-use-oozie-linux-mac/hdinsight-oozie-jobs.png)

5. Bir iş hakkında daha fazla bilgi görüntülemek için işi seçin.

    ![İş bilgileri](./media/hdinsight-use-oozie-linux-mac/hdinsight-oozie-job-info.png)
...
6. **Iş bilgileri** sekmesinden, iş içindeki temel iş bilgilerini ve bireysel eylemleri görebilirsiniz. En üstteki sekmeleri kullanarak **Iş tanımı**, **Iş yapılandırması**, **Iş günlüğüne**erişin veya iş **dag**altında işin yönlendirilmiş bir döngüsel bir grafiğini (DAG) görüntüleyebilirsiniz.
   ...
   * **Iş günlüğü**: İş için tüm günlükleri almak üzere **günlükleri al** düğmesini seçin veya günlükleri filtrelemek Için **arama filtresi gir** alanını kullanın.

       ![İş Günlüğü](./media/hdinsight-use-oozie-linux-mac/hdinsight-oozie-job-log.png)

   * **Iş dag**: DAG, iş akışı aracılığıyla alınan veri yollarına ilişkin grafiksel bir genel bakıştır.

       ![İş DAG](./media/hdinsight-use-oozie-linux-mac/hdinsight-oozie-job-dag.png)
...
7. **Iş bilgileri** sekmesindeki eylemlerden birini seçerseniz eyleme ilişkin bilgileri getirir. Örneğin **Runsqoopexport** eylemini seçin.
    ...
    ![Eylem bilgileri](./media/hdinsight-use-oozie-linux-mac/oozie-job-action-info.png)
...
1. **Coordinator. xml**adlı bir dosya oluşturmak için aşağıdaki komutu kullanın:
    ...
    >
    > * `${coordFrequency}`: İşin çalıştırma örnekleri arasındaki süre.
...
2. Dosyayı kaydetmek için CTRL + X ' `Y`i seçin ve ENTER ' u seçin.
...
7. Oozie Web Kullanıcı arabirimine giderseniz ve **Düzenleyici işleri** sekmesini seçerseniz, aşağıdaki görüntüde olduğu gibi bilgileri görürsünüz:
    ...
    ![Düzenleyici Işleri sekmesi](./media/hdinsight-use-oozie-linux-mac/coordinator-jobs-tab.png)
...
8. Önceki iş akışı işi gibi, Web Kullanıcı arabiriminde iş girişini seçerseniz iş üzerinde bilgi görüntülenir:
    ...
    ![Düzenleyici Iş bilgileri](./media/hdinsight-use-oozie-linux-mac/coordinator-job-info.png)
    ...
    ![Düzenleyici eylem bilgileri](./media/hdinsight-use-oozie-linux-mac/coordinator-action-job.png)
...
**Belirtiler**: İş durumu **askıya alındı**olarak değişir. İşin ayrıntıları `RunHiveScript` durumu **START_MANUAL**olarak gösterir. Eylemi seçmek şu hata iletisini görüntüler:

    JA009: Cannot initialize Cluster. Please check your configuration for map

**Neden**: **İş. xml** dosyasında kullanılan Azure Blob depolama adresleri depolama kapsayıcısı veya depolama hesabı adı içermiyor. BLOB depolama adresi biçimi `wasbs://containername@storageaccountname.blob.core.windows.net`olmalıdır.

**Çözüm**: İşin kullandığı BLOB depolama adreslerini değiştirin.

### <a name="ja002-oozie-is-not-allowed-to-impersonate-ltusergt"></a>JA002: Oozie 'nin kullanıcının kimliğine bürünmesine &lt;izin verilmiyor&gt;

**Belirtiler**: İş durumu **askıya alındı**olarak değişir. İşin ayrıntıları `RunHiveScript` durumu **START_MANUAL**olarak gösterir. Eylemi seçerseniz, aşağıdaki hata iletisini gösterir:
...
**Belirtiler**: İş durumu **sonlandırıldı**olarak değişir. İşin ayrıntıları `RunSqoopExport` durumu **hata**olarak gösterir. Eylemi seçerseniz, aşağıdaki hata iletisini gösterir:
...
1. Dosyayı/Tutorials/useoozie dizinine kopyalayın: `mssql-jdbc-7.0.0.jre8.jar`

    ```bash
    hdfs dfs -put /usr/share/java/sqljdbc_7.0/enu/mssql-jdbc-7.0.0.jre8.jar /tutorials/useoozie/mssql-jdbc-7.0.0.jre8.jar
    ```

2. Aşağıdaki XML 'i Yukarıdaki `</sqoop>`yeni bir satıra eklemek içinöğesinideğiştirin:`workflow.xml`
...
* [HDInsight üzerinde Apache Hadoop Apache Pig kullanma][hdinsight-use-pig]
