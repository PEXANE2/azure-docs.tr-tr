...
description: Bir SFTP sunucusundan havuz olarak desteklenen bir veri deposuna veri kopyalamanızı sağlayan Azure Data Factory MySQL Bağlayıcısı hakkında bilgi edinin.
...
Bu makalede, SFTP sunucusundan verilerin nasıl kopyalanacağı özetlenmektedir. Azure Data Factory hakkında bilgi edinmek için [giriş makalesi](introduction.md).
...
- **Temel** veya **sshpublickey** kimlik doğrulamasını kullanarak dosyaları kopyalama.
...
|:--- |:--- |:--- |
| type | Type özelliği şu şekilde ayarlanmalıdır: **SFTP**. |Evet |
...
| privateKeyContent | Base64 kodlamalı SSH özel anahtar içeriği. SSH özel anahtarı OpenSSH biçiminde olmalıdır. Data Factory'de güvenle depolamak için bir SecureString olarak bu alanı işaretleyin veya [Azure Key Vault'ta depolanan bir gizli dizi başvuru](store-credentials-in-key-vault.md). | `privateKeyPath` Ya`privateKeyContent`da belirtin. |
| passPhrase | Anahtar dosyası bir pass ifadesi tarafından korunuyorsa, özel anahtarın şifresini çözmek için geçiş tümceciğini/parolayı belirtin. Data Factory'de güvenle depolamak için bir SecureString olarak bu alanı işaretleyin veya [Azure Key Vault'ta depolanan bir gizli dizi başvuru](store-credentials-in-key-vault.md). | Özel anahtar dosyası bir pass ifadesi tarafından korunuyorsa Evet. |
...
**Orc/avro/JSON biçiminde**SFTP 'den veri kopyalamak için aşağıdaki özellikler desteklenir:
...
Bölümleri ve etkinlikleri tanımlamak için mevcut özelliklerin tam listesi için bkz: [işlem hatları](concepts-pipelines-activities.md) makalesi. Bu bölüm, SFTP kaynağı tarafından desteklenen özelliklerin bir listesini sağlar.
...
**Orc/avro/JSON biçimindeki**SFTP verilerini kopyalamak için, etkinlik **kaynağını** kopyalama bölümünde aşağıdaki özellikler desteklenir:
