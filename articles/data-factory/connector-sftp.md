...
|:--- |:--- |:--- |
| tür | Type özelliği *SFTP* olarak ayarlanmalıdır. |Evet |
...
| authenticationType | Kimlik doğrulama türünü belirtin.<br/>İzin verilen değerler *temel* ve *sshpublickey*. Daha fazla özellik için, [temel kimlik doğrulaması kullanma](#use-basic-authentication) bölümüne bakın. JSON örnekleri için [SSH ortak anahtar kimlik doğrulamasını kullanma](#use-ssh-public-key-authentication) bölümüne bakın. |Evet |
...
| password | Kullanıcının parolası (Kullanıcı adı). Veri fabrikanıza güvenli bir şekilde depolamak için bu alanı SecureString olarak işaretleyin veya bir [Azure Anahtar Kasası 'nda depolanan bir gizli dizi başvurusu](store-credentials-in-key-vault.md)yapın. | Evet |
...
| Özellik   | Açıklama                                                  | Gerekli |
| ---------- | ------------------------------------------------------------ | -------- |
| tür       | DataSet içinde *Type* özelliği `location` *sftplocation* olarak ayarlanmalıdır. | Evet      |
...
| Özellik                 | Açıklama                                                  | Gerekli                                      |
| ------------------------ | ------------------------------------------------------------ | --------------------------------------------- |
| tür                     | Altındaki *Type* özelliği `storeSettings` *Sftpreadsettings* olarak ayarlanmalıdır. | Evet                                           |
| ***Kopyalanacak dosyaları bulun** _ |  |  |
| SEÇENEK 1: statik yol<br> | Veri kümesinde belirtilen klasör/dosya yolundan kopyalama. Tüm dosyaları bir klasörden kopyalamak istiyorsanız, ayrıca olarak öğesini belirtin `wildcardFileName` `_` . |  |
...
| Özellik                 | Açıklama                                                  | Gerekli |
| ------------------------ | ------------------------------------------------------------ | -------- |
| tür                     | Altındaki *Type* özelliği `storeSettings` *sftpwritesettings* olarak ayarlanmalıdır. | Evet      |
...
| Özellik | Açıklama | Gerekli |
|:--- |:--- |:--- |
| tür | Veri kümesinin *Type* özelliği *FileShare* olarak ayarlanmalıdır. |Evet |
