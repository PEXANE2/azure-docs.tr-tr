...
Müşteriler modern veri ambarı ve analiz projelerini önemli ölçüde daha fazla veri olmasa da, veri temelinde verileri üzerinde daha fazla görünürlük gerektirir. Bu atölye, Azure Data Factory ve Azure veri paylaşımındaki geliştirmelerin Azure 'da veri tümleştirmeyi ve yönetimini nasıl basitleştireceğinizi ele alır. Veri üzerinde kapsamlı bir görünüm oluşturmak için Code-Free ETL/ELT etkinleştirildikten sonra, Azure Data Factory iyileştirmeler veri mühendislerinizi daha fazla veri almak ve bu nedenle kuruluşunuza daha fazla değer getirmek için güçlendirilecek. Azure veri paylaşımı, iş paylaşımını yönetilen bir şekilde yapmanıza olanak sağlar.
...
1. ' Üçlü Veri kümenizi ' çağırın. Bağlı hizmetiniz olarak ' SQLDB 'yi seçin. ' Dbo ' tablo adını seçin. Tablo adı açılır listesinden gezinme verileri. Şemayı **bağlantı/depodan**içeri aktarın. Bitirdikten sonra Tamam’a tıklayın.
...
### <a name="configure-adls-gen-2-sink-dataset"></a>ADLS Gen 2 havuz veri kümesini yapılandırma
...
1. Havuz veri kümenizi ' Üçlü CSV ' olarak adlandırın. Bağlı hizmetiniz olarak ' ADLSGen2 ' seçeneğini belirleyin. CSV dosyanızı yazmak istediğiniz konumu girin. Örneğin, verilerinizi kapsayıcı `staging-container`dosya `trip-data.csv` yazabilirsiniz. Çıktı verilerinizin üst bilgi olmasını istiyorsanız **ilk satırı üst bilgi olarak** ayarlayın. Hedefte henüz dosya olmadığından, **Içeri aktarma şemasını** **none**olarak ayarlayın. Bitirdikten sonra Tamam’a tıklayın.
...
Bu adımda oluşturulan veri akışı, önceki bölümde oluşturulan ' "Üçlü CSV" veri kümesini ' dbo ' tablosu ile birleştirir. Üçlü dört sütuna göre ' SQLDB ' içinde depolanan Üçlü Fares. Ardından veriler, belirli alanların ortalamasını hesaplamak ve bir Azure SYNAPSE analiz tablosunda yazılmak üzere sütun `payment_type` temel alınarak toplanır.
...
1. ' Üçlü Veri kümenizi çağırın. Bağlı hizmetiniz olarak ' SQLDB 'yi seçin. ' Dbo ' tablo adını seçin. Üçlü aşağı açılan tablo adı. Şemayı **bağlantı/depodan**içeri aktarın. Bitirdikten sonra Tamam’a tıklayın.
...
1. ' AggregateByPaymentType ' toplama dönüştürmeinizi adlandırın. Grupla sütununa göre `payment_type` ' yi seçin.
...
1. Toplama ifadesi girmek için, **ENTER ifadesi**etiketli mavi kutuya tıklayın. Bu işlem, giriş şeması, yerleşik işlevler ve işlemler ve Kullanıcı tanımlı parametreleri kullanarak görsel olarak veri akışı ifadeleri oluşturmak için kullanılan bir araç olan veri akışı ifade Oluşturucusu 'nu açar. İfade oluşturucusunun özellikleri hakkında daha fazla bilgi için bkz. [Expression Builder belgeleri](https://docs.microsoft.com/azure/data-factory/concepts-data-flow-expression-builder).
    ...
    Ortalama tarifeli havayolu almak için `avg()` toplama işlevini kullanarak `total_amount` sütununu `toInteger()`bir tamsayıya atamalısınız. Veri akışı ifade dilinde, bu `avg(toInteger(total_amount))`olarak tanımlanmıştır. İşiniz bittiğinde Kaydet ' e tıklayın **ve son** ' a tıklayın.

    ![Portal](media/lab-data-flow-data-share/agg4.png)
1. Ek toplama ifadesi eklemek için, `average_fare`' nin yanındaki artı simgesine tıklayın. **Sütun Ekle**' yi seçin.
...
1. **Sütun Ekle veya Seç**etiketli metin kutusunda ' total_trip_distance ' girin. Son adımda olduğu gibi, ifadeye girmek için ifade oluşturucuyu açın.
    ...
    Toplam seyahat mesafesini almak için `sum()` toplama işlevini kullanarak `trip_distance` sütununu `toInteger()`bir tamsayıya atamalısınız. Veri akışı ifade dilinde, bu `sum(toInteger(trip_distance))`olarak tanımlanmıştır. İşiniz bittiğinde Kaydet ' e tıklayın **ve son** ' a tıklayın.
...
Bu bölümde, Azure portal kullanarak yeni bir veri paylaşımının nasıl ayarlanacağını öğreneceksiniz. Bu, Azure Data Lake Store Gen2 ve Azure SQL veri ambarı 'ndan veri kümeleri içerecek yeni bir veri paylaşımının oluşturulmasını kapsar. Böylece, veri tüketicilerine kendileriyle paylaşılan verileri otomatik olarak yenileme seçeneği veren bir anlık görüntü zamanlaması yapılandıracaksınız. Ardından, alıcıları veri paylaşımınıza davet edeceksiniz. 
...
1. **Açıklama**' nın altında, veri paylaşımının içeriğini açıklayan bir cümle koyun. Veri paylaşımında, Azure SQL veri ambarı ve Azure Data Lake Store dahil olmak üzere çeşitli depolarda depolanan dünya çapında TAXI seyahat verileri yer alacak. 
...
1. ADF dönüştürmelerinizin bulunduğu Azure SQL veri ambarı 'ndan bir tablo seçmek için **Azure SQL veri ambarı** ' nı seçin.

    ![Veri kümesi ekle](media/lab-data-flow-data-share/add-dataset-sql.png)

> [!NOTE]
> Azure SQL veri ambarı artık Azure SYNAPSE Analytics olarak biliniyor
...
1. SQL veri ambarı için **EDW** ' ı seçin ve tablo için **Aggreg, taxidata** öğesini seçin. 
...
1. Bu ekranda, veri paylaşımınıza alıcı ekleyebilirsiniz. Eklediğiniz alıcılara veri paylaşımınıza davetiye gönderilir. Bu laboratuvarın amacı için 2 e-posta adresi eklemeniz gerekir:
    ...
    1. *janedoe@fabrikam.com* adlı kurgusal veri tüketicisine ekleyin.
...
1. *janedoe@fabrikam.com* davetini seçin. Sil ' i seçin. Alıcının daveti henüz kabul etmediyse, artık bunu yapamayacaktır. 
...
1. Veri **kümeleri** sekmesini seçin. her bir veri kümesinin eşlenmediğine dikkat edin, bu da verilerin kopyalanacak hedefi olmadığı anlamına gelir. 

    ![eşlenmemiş veri kümeleri](media/lab-data-flow-data-share/unmapped.png)

1. SQL veri ambarı tablosunu seçin ve **+ hedefe eşle**' yi seçin.
...
1. SQL veritabanını seçin (yalnızca aboneliğinizde bir tane olmalıdır). SQL veri ambarını seçmemeye dikkat edin. 
...
1. Tetikleyici anlık görüntüsünü seçin > Tam kopya. 
    ...
    Beklerken, özgün veri paylaşımında (DataProvider) gidip abonelikleri ve **geçmişi** **paylaşma** sekmesinin durumunu görüntüleyin. artık etkin bir abonelik olduğunu ve veri sağlayıcısı olarak veri tüketicisinin kendileriyle paylaşılan verileri almaya ne zaman başlatıldığını da izleyebilirsiniz. 
