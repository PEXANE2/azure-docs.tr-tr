...
JSON dosyalarında bağlı hizmet ve işlem hattı tanımları oluşturdunuz. Şimdi bir veri fabrikası oluşturalım ve bağlı hizmet ve ardışık düzen JSON dosyalarını PowerShell cmdlet 'lerini kullanarak dağıtabolalım. Aşağıdaki PowerShell komutlarını tek tek çalıştırın: 
