...
* **Azure SQL veritabanı**. Veritabanını *havuz* veri deposu olarak kullanabilirsiniz. Azure SQL veritabanınız yoksa, oluşturma adımları için bkz. [SQL veritabanı oluşturma](../sql-database/sql-database-get-started-portal.md) .
...
1. **Adföğreticisi/giriş** klasörüne gidin, **bu dosyayı seçin** ve ardından **Tamam**' ı seçin.
...
1. **Yeni bağlı hizmet (Azure SQL veritabanı)** iletişim kutusunda aşağıdaki adımları uygulayın:
    ...
    g. Bağlı hizmeti dağıtmak için **Oluştur** ' u seçin.

    ![Yeni bağlı hizmeti kaydedin](./media/tutorial-copy-data-portal/new-azure-sql-linked-service-window.png)

1. Otomatik olarak **ayarlanan özellikler** iletişim kutusuna gider. **Tablo**’da **[dbo].[emp]** seçeneğini belirleyin. Sonra **Tamam**’ı seçin.
