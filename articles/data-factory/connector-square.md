...
|:--- |:--- |:--- |
| type | Type özelliği ayarlanmalıdır: **kare** | Yes |
| konak | Kare örneğinin URL'si. (yani mystore.mysquare.com)  | Yes |
| clientID | Kare uygulamanızla ilişkili istemci kimliği.  | Yes |
| clientSecret | Kare uygulamanızla ilişkili istemci gizli anahtarı. Data Factory'de güvenle depolamak için bir SecureString olarak bu alanı işaretleyin veya [Azure Key Vault'ta depolanan bir gizli dizi başvuru](store-credentials-in-key-vault.md). | Yes |
| redirectUri | Kare uygulama panosunda atanan yeniden yönlendirme URL'si. (örn. http:\//localhost: 2500)  | Yes |
...
Kareden veri kopyalamak için dataset öğesinin type özelliği ayarlamak **SquareObject**. Aşağıdaki özellikler desteklenir:

| Özellik | Açıklama | Gereklidir |
|:--- |:--- |:--- |
| type | Dataset öğesinin type özelliği ayarlanmalıdır: **SquareObject** | Yes |
...
Kareden veri kopyalamak için kopyalama etkinliği için kaynak türünü ayarlayın. **SquareSource**. Kopyalama etkinliği aşağıdaki özellikler desteklenir **source** bölümü:

| Özellik | Açıklama | Gereklidir |
|:--- |:--- |:--- |
| type | Kopyalama etkinliği kaynağı öğesinin type özelliği ayarlanmalıdır: **SquareSource** | Yes |
| sorgu | Verileri okumak için özel bir SQL sorgusu kullanın. Örneğin: `"SELECT * FROM Business"`. | Yok (veri kümesinde "tableName" değeri belirtilmişse) |
