...
Aşağıdaki özellikler, `location` Biçim tabanlı veri kümesindeki ayarlar bölümünde Amazon S3 için desteklenir:

| Özellik   | Açıklama                                                  | Gerekli |
| ---------- | ------------------------------------------------------------ | -------- |
| tür       | DataSet içinde Type özelliği `location` **AmazonS3Location**olarak ayarlanmalıdır. | Yes      |
...
Aşağıdaki özellikler, `storeSettings` Biçim tabanlı kopyalama kaynağı ayarları altında Amazon S3 için desteklenir:
...
| Seçenek 2: S3 ön eki<br>-önek | Kaynak S3 dosyalarını filtrelemek için veri kümesinde yapılandırılan belirtilen demet altındaki S3 anahtar adının öneki. Adı ile başlayan S3 anahtarları `bucket_in_dataset/this_prefix` seçilidir. Joker karakter filtresinden daha iyi performans sağlayan S3's hizmet tarafı filtresini kullanır. | Hayır |
...
| Özellik | Açıklama | Gerekli |
|:--- |:--- |:--- |
| tür | DataSet 'in Type özelliği: **AmazonS3Object** olarak ayarlanmalıdır |Yes |
