...
| konak | Azure dosya depolama uç noktası 'nı şöyle belirtir: <br/>-UI using: `\\<storage name>.file.core.windows.net\<file service name>` belirtin<br/>-JSON: `"host": "\\\\<storage name>.file.core.windows.net\\<file service name>"`kullanılıyor. | Yes |
| Kullanıcı Kimliği | Azure dosya depolama alanına erişmek için kullanıcıyı belirtin: <br/>-UI using: `AZURE\<storage name>` belirtin<br/>-JSON: `"userid": "AZURE\\<storage name>"`kullanılıyor. | Yes |
| password | Depolama erişim anahtarı belirtin. Data Factory'de güvenle depolamak için bir SecureString olarak bu alanı işaretleyin veya [Azure Key Vault'ta depolanan bir gizli dizi başvuru](store-credentials-in-key-vault.md). | Yes |
