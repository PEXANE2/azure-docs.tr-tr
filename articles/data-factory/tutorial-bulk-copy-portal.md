...
1. Azure portal menüsünün solunda **analiz** > **Data Factory** **kaynak oluştur** > ' u seçin. 
   "yeni" bölmesinde seçimi Data Factory ![](./media/doc-common-process/new-azure-data-factory-menu.png)
...
**AzureSqlDWDataset** çıkış veri kümesi **AzureSqlDWLinkedService**'e işaret eder. Bağlı hizmet, Azure SYNAPSE Analytics 'e (eski adıyla SQL DW) bağlanacak bağlantı dizesini belirtir. Veri kümesi, verilerin kopyalandığı hedef veritabanı ve tabloyu belirtir. 
...
1. **Etkinlikler** araç kutusunda **Yineleme ve Koşullar**’ı genişletin ve **ForEach** etkinliğini sürükleyerek işlem hattı tasarım yüzeyine bırakın. Ayrıca, **Etkinlikler** araç kutusunda etkinlikler için arama yapabilirsiniz. 
    ...
    ![ForEach etkinlik Oluşturucu](./media/tutorial-bulk-copy-portal/for-each-activity-builder.png)
...
1. **Gettablelistandtriggercopydata** işlem hattı ile ilişkili etkinlik çalıştırmalarını görüntülemek için işlem hattının işlem hattı adı bağlantısına tıklayın. Bu işlem hattı çalıştırması için iki etkinlik çalıştırması görüyor olmalısınız. 
    ![Izleme işlem hattı çalıştırma](./media/tutorial-bulk-copy-portal/monitor-pipeline.png)
