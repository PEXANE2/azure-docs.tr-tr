---
title: Azure SQL veritabanı 'na veya Data Factory kullanarak veri kopyalama | Microsoft Docs
description: Data Factory kullanarak desteklenen kaynak veri depolarından verileri Azure SQL veritabanı 'na veya SQL veritabanından desteklenen havuz veri depolarına nasıl kopyalayacağınızı öğrenin.
...
Bu makalede, verileri Azure SQL veritabanı 'na veya Azure veritabanına kopyalama konusu özetlenmektedir. Azure Data Factory hakkında bilgi edinmek için [giriş makalesi](introduction.md).
...
Özellikle, bu Azure SQL Veritabanı Bağlayıcısı şu işlevleri destekler:
...
Veri kümelerini tanımlamaya yönelik bölümlerin ve özelliklerin tam listesi için bkz. [veri kümeleri](https://docs.microsoft.com/azure/data-factory/concepts-datasets-linked-services). Bu bölüm, Azure SQL veritabanı veri kümesi tarafından desteklenen özelliklerin bir listesini sağlar.

Veya Azure SQL veritabanı 'ndan veri kopyalamak için aşağıdaki özellikler desteklenir:

| Özellik | Açıklama | Gerekli |
|:--- |:--- |:--- |
| türü | DataSet 'in **Type** özelliği **Azuressqltable**olarak ayarlanmalıdır. | Evet |
...
Azure SQL veritabanından veri kopyalamak için kopyalama etkinliği kaynağındaki **Type** özelliğini **SQLSource**olarak ayarlayın. Kopyalama etkinliği aşağıdaki özellikler desteklenir **kaynak** bölümü:
...
- SQLSource için **Sqlreaderquery** belirtilmişsekopyalama etkinliği verileri almak IÇIN bu sorguyu Azure SQL veritabanı kaynağına karşı çalıştırır. Saklı yordam parametreleri alırsa, **sqlReaderStoredProcedureName** ve **storedProcedureParameters** belirterek bir saklı yordam de belirtebilirsiniz.
- **Sqlreaderquery** veya **SQLREADERSTOREDPROCEDURENAME**belirtmezseniz, JSON veri kümesinin "yapı" bölümünde tanımlanan sütunlar bir sorgu oluşturmak için kullanılır. Sorgu Azure `select column1, column2 from mytable` SQL veritabanı 'nda çalışır. Veri kümesi tanımında "Structure" yoksa, tablodan tüm sütunlar seçilir.
...
Verileri Azure SQL veritabanına kopyalamak için kopyalama etkinliği havuzunda **tür** özelliğini **sqlsink**olarak ayarlayın. Kopyalama etkinliği aşağıdaki özellikler desteklenir **havuz** bölümü:
