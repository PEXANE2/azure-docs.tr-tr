...
Azure Tümleştirme Çalışma Süresi, herkese açık uç noktalarla veri depolarına bağlanmayı ve bilgileri birleştirmeyi destekler. Azure Sanal Ağ ortamı için kendiliğinden konak tümleştirme çalışma zamanı kullanın.
...
- Şirket içi veya Azure Sanal Ağı'nda bilgi işlem kaynaklarına karşı aşağıdaki dönüşüm etkinliklerinin gönderilmesi: HDInsight Hive etkinliği (BYOC-Bring Your Own Cluster), HDInsight Pig etkinliği (BYOC), HDInsight MapAzaltma etkinliği (BYOC), HDInsight Spark etkinlik (BYOC), HDInsight Streaming etkinliği (BYOC), Machine Learning Toplu Yürütme etkinliği, Machine Learning Update Kaynak etkinlikleri, Saklanan Yordam etkinliği, Data Lake Analytics U-SQL etkinliği, Özel etkinlik (Azure Toplu Çalıştırma'da çalışır), Arama ve Meta veri etkinliği alın.
...
Varsayılan olan Azure **IR'yi otomatik çözümlemeyi**seçerseniz, 
...
- Arama/GetMetadata/Delete etkinlik yürütme (Pipeline etkinlikleri olarak da bilinir), dönüşüm etkinliği gönderme (Dış etkinlikler olarak da bilinir) ve yazma işlemleri (test bağlantısı, klasör listesine ve tablo listesine göz atma, önizleme verileri) için ADF, veri fabrikası bölgesinde IR'yi kullanır.
...
- SSISDB'nin barındırıldığı mevcut Azure SQL Veritabanı veya Yönetilen Örnek sunucunuzun konumu şirket içi ağınıza bağlı bir sanal ağın konumuyla aynı değilse, önce mevcut bir Azure SQL Veritabanını kullanarak Azure-SSIS IR'nizi oluşturun veya Yönetilen Örnek sunucu ve aynı konumda başka bir sanal ağ katılma ve sonra farklı konumlar arasında sanal ağ bağlantısı için sanal ağ yapmak.
