---
title: Microsoft OPC Publisher komut satırı bağımsız değişkenleri
description: Bu makale OPC yayımcısı komut satırı bağımsız değişkenlerine genel bir bakış sağlar
author: jehona-m
ms.author: jemorina
...
ms.locfileid: "104787595"
---
# <a name="command-line-arguments"></a>Komut satırı bağımsız değişkenleri

Aşağıda, OPC yayımcısı için genel ayarları ayarlamak üzere kullanılabilecek, açıklanan birkaç komut satırı bağımsız değişkeni vardır.

## <a name="opc-publisher-command-line-arguments-for-version-25-and-below"></a>Sürüm 2,5 ve altı için OPC Publisher komut satırı bağımsız değişkenleri

* Kullanım: opcpublisher.exe \<applicationname> [ \<iothubconnectionstring> ] [ \<options> ]
    
* ApplicationName: kullanılacak OPC UA uygulama adı, uygulamanın adı IoT Hub cihaz kayıt defterinde bu adın altına kaydolmak için de kullanılır.
    
* iothubconnectionstring: IoT Hub Owner ConnectionString, isteğe bağlıdır. Genellikle, ıothub Owner ConnectionString öğesini uygulamanın ilk başında belirtirsiniz. Bağlantı dizesi şifrelenir ve platformlar sertifika deposunda depolanır.
Sonraki çağrılarda, buradan ve yeniden kullanılabilir. Her başlangıç üzerinde ConnectionString öğesini belirtirseniz, IoT Hub cihaz kayıt defterinde uygulama için oluşturulan cihaz her seferinde kaldırılır ve yeniden oluşturulur.
    
Uygulamayı denetlemek için kullanılabilecek birkaç ortam değişkeni vardır:
```
...
> [!NOTE] 
> Komut satırı bağımsız değişkenleri, ortam değişkeni ayarlarını yok kılar.
...
## <a name="opc-publisher-command-line-arguments-for-version-26-and-above"></a>Sürüm 2,6 ve üzeri için OPC Publisher komut satırı bağımsız değişkenleri
```
      --pf, --publishfile=VALUE
                                       the filename to configure the nodes  to publish.
...
GitHub depolarında daha fazla kaynak bulunabilir:
