...
1. **Geçerli OAuth yeniden yönlendirme URI 'leri** alanında, girin `https://<app-name>.azurewebsites.net/.auth/login/facebook/callback`. Azure App Service uygulamanızın adıyla `<app-name>` değiştirmeyi unutmayın.
...
1. Uygulamayı kaydetmek için kullandığınız Facebook hesabı, uygulamanın bir yöneticisidir. Bu noktada, yalnızca Yöneticiler bu uygulamada oturum açabilir.
   ...
   Diğer Facebook hesaplarının kimliğini doğrulamak için, **uygulama gözden geçirmesi** ' nı seçin ve genel genel ' i Facebook kimlik doğrulaması kullanarak uygulamaya erişmesine olanak tanımak için uygulamanızın **adını> genel yapın \<** .
