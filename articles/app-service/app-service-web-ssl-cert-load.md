...
**SSL ayarları** > **genel Sertifikalar (. cer)**  >  ' e tıklayarak uygulamanızın sol gezinti alanından**ortak sertifikayı karşıya yükleyin** .
...
> [!NOTE]
> Bu ayar, çoğu fiyatlandırma katmanı için belirtilen sertifikaları [geçerli User\store](/windows-hardware/drivers/install/local-machine-and-current-user-certificate-stores) öğesine koyar, ancak uygulamanız **yalıtılmış** katmanda çalışıyorsa (uygulama bir [App Service ortamı](environment/intro.md)çalışıyorsa), [Yerel Bunun yerine Machine\store](/windows-hardware/drivers/install/local-machine-and-current-user-certificate-stores) .
>
...
Sertifikanız erişilebilir olduğunda, sertifika parmak izine göre C# koda erişebilirsiniz. Aşağıdaki kod, `E661583E8FABEF4C0BEF694CBC41C28FB81CD870` parmak izine sahip bir sertifika yükler.
...
Bu ayar ayarlandıktan sonra aşağıdaki C# örnek, uygulamanızın deposunun `certs` dizininden `mycert.pfx` adlı bir sertifika yükler.
