...
1. Dosyayı açmak için **Aç**’ı seçin. Senaryo, [öğretici: bağımlı kaynaklarla Azure Resource Manager şablonları oluşturma konusunda](./template-tutorial-create-templates-with-dependent-resources.md)kullanılan bir ile aynıdır.
...
[Şablonu dağıtma](./template-tutorial-create-templates-with-dependent-resources.md#deploy-the-template)bölümündeki yönergeleri izleyin. Cloud Shell için hem *azuredeploy. JSON* hem de *azuredeploy. Parameters. JSON* dosyasını yükleyin ve ardından şablonu dağıtmak için aşağıdaki PowerShell betiğini kullanın:
...
> [Bağlı şablonlar oluşturma](./template-tutorial-create-linked-templates.md)
