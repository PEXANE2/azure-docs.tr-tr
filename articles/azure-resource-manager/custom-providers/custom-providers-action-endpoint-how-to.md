...
Bir **eylemi** uygulayan bir **uç noktanın** , Azure 'daki yeni API için isteği ve yanıtı işlemesi gerekir. Bir **eylem** içeren özel bir kaynak sağlayıcısı oluşturulduğunda, Azure 'da yeni bir API kümesi oluşturur. Bu durumda, eylem `POST` çağrılar için yeni bir Azure eylem API 'SI oluşturacaktır:
...
> [!NOTE]
> Eylemlerin Azure Resource Manager şablonlarda sınırlı desteği vardır. Eylemin bir şablon içinde çağrılması için, adının [`list`](../templates/template-functions-resource.md#list) önekini içermesi gerekir.
