...
9. `App.xaml.cs` proje dosyasını açın ve aşağıdaki deyimlerini ekleyin:
...
10. Projenin `App.xaml.cs` dosyasında, `App` sınıfını bulun ve aşağıdaki `InitNotificationsAsync` yöntem tanımını ekleyin. `<your hub name>`, Azure portal oluşturduğunuz Bildirim Hub 'ı adıyla değiştirin ve `<Your DefaultListenSharedAccessSignature connection string>`, Bildirim Hub 'ınızın **erişim ilkeleri** sayfasında `DefaultListenSharedAccessSignature` bağlantı dizesiyle değiştirin:
    ...
    >[!NOTE]
    > `hub name` yer tutucusunu Azure portal görüntülenen Bildirim Hub 'ı adıyla değiştirin. Ayrıca, bağlantı dizesi yer tutucusunu, önceki bölümde yer alan Notification Hub 'ınızın **erişim ilkeleri** sayfasından edindiğiniz `DefaultListenSharedAccessSignature` bağlantı dizesiyle değiştirin.

11. `App.xaml.cs``OnLaunched` olay işleyicisinin üst kısmında, yeni `InitNotificationsAsync` yöntemine aşağıdaki çağrıyı ekleyin:
