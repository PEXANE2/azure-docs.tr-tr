...
Ayrıca, `RelayConnectionStringBuilder` bir bağlantı dizesini doğrudan yöntemine geçirebilirsiniz. Bu işlem, bağlantı dizesinin geçerli bir biçimde olduğunu doğrulamanızı sağlar. Parametrelerden herhangi biri geçersiz ise, Oluşturucu bir `ArgumentException`oluşturur.
...
Bir [Hybridconnectionlistener][HCListener] nesnesi kullanarak, aşağıdaki gibi bir `HybridConnectionStream` nesne elde edebilirsiniz:
...
Bir [Hybridconnectionclient][HCClient] nesnesi kullanarak, aşağıdaki gibi bir `HybridConnectionStream` nesne elde edebilirsiniz:
