...
> * `kube-advisor` aracını kullanarak eksik Pod kaynak isteklerini ve sınırlarını denetleyin
...
NGıNX çalıştıran beş Pod içeren bir çoğaltma kümesi örneğine göz atalım. Çoğaltma kümesindeki Dizin `app: nginx-frontend`etikete atanır. Küme yükseltmesi gibi gönüllü bir kesinti olayı sırasında, en az üç Pod çalışmaya devam etmesini sağlamak istersiniz. *Pod kesintiler* için aşağıdaki YAML bildirimi, bu gereksinimleri tanımlar:
...
Birden çok geliştirme ekiplerini ve uygulamayı barındıran bir aks kümesinde, bu kaynak istekleri ve limitler kümesi olmadan Pod 'yi izlemek zor olabilir. En iyi uygulama olarak, özellikle de ad alanlarına kaynak kotaları atamadıysanız, AKS kümelerinizde `kube-advisor` düzenli olarak çalıştırın.
