...
4. Model çıkış veri kümeniz için veri kümesi izleyicileri oluşturun. 
...
Depolama hesabınızdaki verilerinize erişmek için, Azure Machine Learning veri depoları ve veri kümeleri sunar. Veri depoları soruları yanıtlayın: Azure Depolamamda bulunan verilerime nasıl güvenli bir şekilde bağlanabilirim? Veri depoları, depolama hizmetiniz üzerinde bir soyutlama katmanı sağlar. Bu, bağlantı bilgileri veri deposunda tutulduğundan ve betiklerin açığa çıkmadığından, depolama verilerinize yönelik güvenlik ve erişim kolaylığına yardımcı olur. 

Veri kümeleri soruya yanıt veriyor: veri deposundaki belirli veri dosyalarını nasıl edinebilirim? Veri kümeleri, makine öğrenimi denemeniz için kullanmak istediğiniz belirli dosya veya temel depolamadaki dosyaları işaret ediyor. Aynı şekilde, veri depoları ve veri kümeleri, makine öğrenimi görevleriniz için güvenli, ölçeklenebilir ve tekrarlanabilir bir veri teslim iş akışı sağlar.
