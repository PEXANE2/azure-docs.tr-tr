...
| Nasıl, yapılandırılmış yüklü mü?      |  GPU desteğine sahip yüklü   |
| Kullanma veya çalıştırın      | Python kitaplığı (2,7 ve 3,5), R paketi ve yol üzerine komut satırı aracı (`C:\dsvm\tools\xgboost\bin\xgboost.exe` Windows ve `/dsvm/tools/xgboost/xgboost` Linux için) olarak    |
...
| Kullanma veya çalıştırın      | Bir kullanıcı arabirimi aracı olarak. Windows üzerinde, bir komut istemi başlatın, R çalıştırın ve ardından R içinde komutunu çalıştırın `rattle()`. Linux 'ta X2Go ile bağlanın, bir Terminal başlatın, R çalıştırın ve ardından R içinde çalıştırın `rattle()`. |
...
Dsvms üzerinde, dsvms için Anaconda Python dağıtımının parçası olan popüler `scikit-learn` paket gibi diğer makine öğrenimi kitaplıkları vardır. Python, R ve Julia'nın kullanılabilir paketler listesini görmek için ilgili paket yöneticileri çalıştırın.
