...
`from_config()` işlevi çağrılırken istem gönderilir.
...
`from_config()` işlevi, çalışma alanı bağlantı bilgilerinizi içeren bir JSON dosyası arar. Ayrıca, `Workspace` oluşturucusunu kullanarak bağlantı ayrıntılarını açıkça belirtebilirsiniz ve bu da etkileşimli kimlik doğrulaması için istemde bulunabilir. Her iki çağrı de eşdeğerdir.
...
Birden çok kiracıya erişiminiz varsa, sınıfı içeri aktarmanız ve hedeflediğiniz kiracıyı açıkça tanımlamanız gerekebilir. `InteractiveLoginAuthentication` için oluşturucuyu çağırmak, yukarıdaki çağrılara benzer bir oturum açma bilgileri de ister.
...
Çıktı aşağıdakine benzer bir JSON olacaktır. `clientId`, `clientSecret`ve `tenantId` alanlarına göz atın, bu makalede bu makaledeki diğer adımlarda ihtiyacınız olacaktır.
...
Daha sonra, yeni oluşturduğunuz hizmet sorumlusu hakkındaki ayrıntıları, `--id` parametresine giriş olarak yukarıdaki `clientId` değerini kullanarak almak için aşağıdaki komutu çalıştırın.

```azurecli-interactive
az ad sp show --id your-client-id
```

Aşağıda, komuttan gelen JSON çıktısının basitleştirilmiş bir örneği verilmiştir. Bir sonraki adım için değerine ihtiyacınız olacağı için `objectId` alanı ' na göz atın.
...
Hizmet sorumlusu kimlik doğrulaması etkinleştirildikten sonra, bir kullanıcı olarak fiziksel olarak oturum açmadan SDK 'da çalışma alanınızda kimlik doğrulaması yapabilirsiniz. `ServicePrincipalAuthentication` Class oluşturucusunu kullanın ve önceki adımlardan aldığınız değerleri parametreler olarak kullanın. `tenant_id` parametresi, yukarıdaki `tenantId` eşlenir, `service_principal_id` haritalar `clientId`ve `service_principal_password` eşlenir.`clientSecret`
...
`sp` değişkeni artık doğrudan SDK 'da kullandığınız bir kimlik doğrulama nesnesi barındırır. Genel olarak, aşağıdaki kodda gösterildiği gibi, yukarıda kullanılan kimlikleri/gizli dizileri ortam değişkenlerinde depolamak iyi bir fikirdir.
...
Node. js kullanarak bir kimlik doğrulama belirteci oluşturmak için aşağıdaki adımları kullanın. Ortamınızda `npm install adal-node`çalıştırın. Ardından, yukarıdaki adımlarda oluşturduğunuz hizmet sorumlusunun `tenantId`, `clientId`ve `clientSecret` aşağıdaki betikteki eşleşen değişkenler için değerler olarak kullanın.
...
`tokenResponse` değişkeni, belirteci ve sona erme saati gibi ilişkili meta verileri içeren bir nesnedir. Belirteçler 1 saat için geçerlidir ve yeni bir belirteç almak için aynı çağrı çalıştırılarak yenilenebilir. Aşağıda örnek bir yanıt verilmiştir.
...
Kimlik doğrulama belirtecini getirmek için `accessToken` özelliğini kullanın. API çağrıları yapmak için belirteci kullanma hakkında örnekler için [REST API belgelerine](https://github.com/microsoft/MLOps/tree/master/examples/AzureML-REST-API) bakın.

### <a name="python"></a>Python 

Python kullanarak bir kimlik doğrulama belirteci oluşturmak için aşağıdaki adımları kullanın. Ortamınızda `pip install adal`çalıştırın. Ardından, yukarıdaki adımlarda oluşturduğunuz hizmet sorumlusunun `tenantId`, `clientId`ve `clientSecret` aşağıdaki betikteki uygun değişkenler için değerler olarak kullanın.
...
`token_response` değişkeni, belirteci ve sona erme saati gibi ilişkili meta verileri içeren bir sözlüktür. Belirteçler 1 saat için geçerlidir ve yeni bir belirteç almak için aynı çağrı çalıştırılarak yenilenebilir. Aşağıda örnek bir yanıt verilmiştir.
...
Kimlik doğrulama belirtecini getirmek için `token_response["accessToken"]` kullanın. API çağrıları yapmak için belirteci kullanma hakkında örnekler için [REST API belgelerine](https://github.com/microsoft/MLOps/tree/master/examples/AzureML-REST-API) bakın.
...
Kimlik doğrulama anahtarlarını getirmek için `aci_service.get_keys()`kullanın. Bir anahtarı yeniden oluşturmak için `regen_key()` işlevini kullanın ve **birincil** ya da **İkincil**geçirin.
