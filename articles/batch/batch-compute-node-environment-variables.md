...
| AZ_BATCH_NODE_IS_DEDICATED      | Varsa `true`, geçerli düğüm özel bir düğümdür. Bu `false`, [düşük öncelikli bir düğümdür](batch-low-pri-vms.md). | Tüm görevler. | `true` |
...
| AZ_BATCH_TASK_DIR               | Düğümdeki [görev dizininin][files_dirs] tam yolu. Bu dizin, görevi `stdout.txt` için `stderr.txt` ve ve AZ_BATCH_TASK_WORKING_DIR içerir. | Tüm görevler. | C:\user\tasks\workıtem\batchjob001\job-1\task001 |
