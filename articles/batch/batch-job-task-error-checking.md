...
- Görev kısıtlamalarında özelliği tarafından belirtilen görev için geçen süre `maxWallClockTime` aşıldı. [constraints](/rest/api/batchservice/task/add#taskconstraints)
...
Havuzlar ve havuz düğümleri genellikle kısa ömürlü olduğundan, düğümler sürekli olarak eklenerek ve silinirken, günlük dosyalarının kalıcı olması önerilir. [Görev çıktı dosyaları](./batch-task-output-files.md) , günlük dosyalarını Azure depolama 'ya kaydetmek için kullanışlı bir yoldur.
