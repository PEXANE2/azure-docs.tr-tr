...
### <a name="stop-ransomware-before-it-damages-iot-and-ics-networks"></a>IoT ve ICS ağlarını zarardan önce fidye yazılımı durdur
...
1. Algılayıcı veya şirket içi yönetim konsolu sol bölmesinde, **iletme**' yi seçin.
    ...
    [:::image type="content" source="media/integration-fortinet/forwarding-view.png" alt-text="İletim penceresinde iletme kurallarınızı görüntüleyin.":::](media/integration-fortinet/forwarding-view.png#lightbox)
...
Bu kötü amaçlı kaynağı engelleyen bir FortiGate güvenlik duvarı kuralı ayarlamak için:
...
## <a name="block-suspected-traffic-using-the-fortigate-firewall"></a>FortiGate güvenlik duvarını kullanarak şüpheli trafiği engelleyin

1. Sol bölmede, **iletme**' yi seçin.

    [:::image type="content" source="media/integration-fortinet/forwarding-view.png" alt-text="Bir sensörde Iletme penceresi seçeneği.":::](media/integration-fortinet/forwarding-view.png#lightbox)

1. **Iletme kuralları oluştur** ve kural parametrelerini tanımla ' yı seçin.

    :::image type="content" source="media/integration-fortinet/new-rule.png" alt-text="Iletme kuralı oluşturma penceresi ekran görüntüsü":::
    ...
    | **Protokoller** | Varsayılan olarak, tüm protokoller seçilidir.<br><br>Belirli bir protokolü seçmek için, **özel** ' i seçin ve bu kuralın uygulandığı protokolü seçin. |
    | **Yapılarının** | Varsayılan olarak, tüm güvenlik motorları dahil edilir.<br><br>Bu kuralın uygulandığı belirli bir güvenlik altyapısını seçmek için, **özel** ' i seçin ve altyapıyı seçin. |
    | **Sistem bildirimleri** | *Sensöri çevrimiçi ve çevrimdışı* durumunu ilet. Bu seçenek yalnızca şirket içi yönetim konsolunda oturum açtıysanız kullanılabilir. |

1. Defender 'ın IoT 'ye güvenlik duvarı kuralları göndermesini istemek için **eylem** ' i seçin ve ardından **Fortigate 'e gönder**' i seçin.

    :::image type="content" source="media/integration-fortinet/fortigate.png" alt-text="Iletme kuralı penceresi oluşturun ve FortiGate 'e Gönder ' i seçin":::

1. FortiGate iletme kuralını yapılandırmak için:

    :::image type="content" source="media/integration-fortinet/configure.png" alt-text="Iletme kuralı oluştur penceresini yapılandırma":::

1. **Eylemler** bölmesinde, aşağıdaki parametreleri ayarlayın:

    | Parametre | Açıklama |
    |--|--|
    | Ana bilgisayar | Fortıgate sunucu IP adresi türü. |
    | Bağlantı noktası | FortiGate sunucu bağlantı noktası türü. |
    | Kullanıcı adı | Fortıgate sunucu Kullanıcı adı türü. |
    | API Anahtarı | FortiGate içinde oluşturduğunuz API anahtarını girin. |
    | Gelen arabirim| Engellemenin nasıl yürütüleceğini tanımlayın:<br /><br />**IP adresine göre**: her zaman, IP adresine göre Panorama üzerinde engelleme ilkeleri oluşturur.<br /><br />**FQDN veya IP adresine göre**: varsa, FQDN üzerinde, aksı takdirde IP adresi olan Panorama üzerinde engelleme ilkeleri oluşturur.| 
    | Giden arabirim |İlke bildirimi e-postası için e-posta adresini ayarlayın. <br /><br /> **Note**: bir posta sunucusunu xsense ile yapılandırdığınızdan emin olun. E-posta adresi girilmezse, XSense bir bildirim e-postası göndermez.| 
    |Yapılandırma| FortiGate güvenlik duvarı tarafından şüpheli kaynakların engellenmesini sağlamak için aşağıdaki seçenekleri ayarlayın: <br /><br />**Geçersiz işlev kodlarını engelle**: protokol IHLALLERI-ICS protokol belirtimini ihlal eden alan değeri geçersiz (olası yararlanma)<br /><br />**YETKISIZ PLC programlama/bellenim güncelleştirmelerini engelle**: yetkisiz PLC değişiklikleri<br /><br />**Block YETKILENDIRILMEMIŞ PLC stop**: PLC Stop (kesinti süresi)<br /><br />**Kötü amaçlı yazılımdan ilişkili uyarıları engelleyin**: Endüstriyel kötü amaçlı yazılım girişimlerini engelleme (Triton, NotPetya vb.). **Otomatik engelleme** seçeneğini belirleyebilirsiniz. Bu durumda, engelleme otomatik olarak ve hemen yürütülür.<br /><br />*Yetkisiz taramayı engelle*: yetkisiz tarama (olası keşif)

1. **Gönder**’i seçin.

## <a name="block-the-suspicious-source"></a>Şüpheli kaynağı engelle

1. **Uyarılar** bölmesinde, Fortinet Integration ile ilgili uyarıyı seçin.
