...
2. Aşağıdaki kod parçacığını `MainActivity.java` sınıfınızın **OnCreate ()** metoduna kopyalayın.
    ...
    Yukarıdaki kod parçacığı ilk olarak, **Onready ()** geri çağırma yöntemini kullanarak bir Azure haritalar harita denetim örneği edinir. Daha sonra bir `TileLayer` nesnesi oluşturur ve `tileUrl` seçeneğine biçimlendirilen bir **xyz** kutucuğu URL 'si geçirir. Katman geçirgenliği `0.8` olarak ayarlanır ve kutucuk hizmetinden kullanılan kutucuklar 256 piksel döşeme olduğundan, bu bilgiler `tileSize` seçeneğine geçirilir. Kutucuk katmanı daha sonra Haritalar katman yöneticisine geçirilir.
