...
Bu adımda, TestVNet1 oluşturup yapılandırırsınız. Azure Sanal ağınızı ve VPN ağ geçidinizi oluşturmak ve yapılandırmak için [ağ geçidi oluşturma öğreticisinde](vpn-gateway-tutorial-create-gateway-powershell.md) bulunan adımları kullanın. Aşağıdaki ekran görüntülerinde başvuru ayarlarını kullanın.
...
Şirketler arası bağlantı kurmak için, şirket içi VPN cihazınızı temsil eden bir *yerel ağ geçidi* OLUŞTURMANıZ ve VPN ağ geçidini, [siteden siteye bağlantı oluşturma](vpn-gateway-howto-site-to-site-resource-manager-portal.md)bölümünde açıklandığı gibi yerel ağ geçidine bağlamak için bir *bağlantı* oluşturmanız gerekir. Bu makale, BGP yapılandırma parametrelerini belirtmek için gereken ek özellikleri içerir.
