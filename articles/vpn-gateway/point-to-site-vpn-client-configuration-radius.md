...
İstemci yapılandırma dosyalarını zaten oluşturduysanız, `Get-AzVpnClientConfiguration` cmdlet 'ini kullanarak bunları alabilirsiniz. Ancak P2S VPN yapılandırmanızda, VPN protokol türü veya kimlik doğrulama türü gibi herhangi bir değişiklik yaparsanız yapılandırma otomatik olarak güncellenmez. Yeni bir yapılandırma indirmesi `New-AzVpnClientConfiguration` oluşturmak için cmdlet 'ini çalıştırmanız gerekir.
...
1. Örnekteki komutu çalıştırarak **Strongswa** ve Ağ yöneticisini yüklemek için **terminali** açın. İle ilgili bir hata alırsanız `libcharon-extra-plugins`, ile `strongswan-plugin-eap-mschapv2`değiştirin.
...
5. İndirilen istemci yapılandırma dosyalarının **genel** klasöründen **vpnsettings. xml** dosyasını açın. Adlı `VpnServer` etiketi bulun ve ile başlayıp şununla biten adı `azuregateway` kopyalayın. `.cloudapp.net`
...
3. **Ağ tercihleri**altında **ağ** iletişim kutusunu açın. Azure **+** sanal ağına P2S bağlantısı için yenı bir VPN istemci bağlantı profili oluşturmayı seçin.
...
1. EapMSChapv2 için `Get-AzVpnClientConfiguration` VPN istemci yapılandırmasını oluşturmak için cmdlet 'ini kullanın.
