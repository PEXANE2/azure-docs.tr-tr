...
description: Klasik dağıtım modeli kullanılarak oluşturulan sanal ağlar için Zorlamalı tünel oluşturmayı nasıl yapılandıracağınızı öğrenin.
...
> * [Klasik](vpn-gateway-about-forced-tunneling.md)
...
* ExpressRoute Zorlamalı tünel bu mekanizma aracılığıyla yapılandırılmaz, ancak bunun yerine ExpressRoute BGP eşleme oturumları aracılığıyla varsayılan bir yol tanıtıarak etkinleştirilir. Daha fazla bilgi için bkz. [ExpressRoute nedir?](../expressroute/expressroute-introduction.md).
...
Aşağıdaki yordam, bir sanal ağ için Zorlamalı tünel belirtmenize yardımcı olur. Yapılandırma adımları VNet ağ yapılandırma dosyasına karşılık gelir.  Bu örnekte, ' MultiTier-VNet ' adlı sanal ağın üç alt ağı vardır: ' ön uç ', ' orta katman ' ve ' arka uç ' alt ağları, dört adet çapraz şirket içi bağlantıyla: ' DefaultSiteHQ ' ve üç dal.
...
Aşağıdaki adımlarda, ' DefaultSiteHQ ', Zorlamalı tünel için varsayılan site bağlantısı olarak ayarlanır ve Zorlamalı tünel kullanmak için midtier ve arka uç alt ağları yapılandırılır.

1. PowerShell konsolunuzu yükseltilmiş haklarla açın. Aşağıdaki örneği kullanarak hesabınıza bağlanın:
...
1. Yönlendirme tablosuna varsayılan bir yol ekleyin. 
...
1. Yönlendirme tablosunu alt ağlarla ilişkilendirin. 
...
1. Zorlamalı tünel için varsayılan bir site atayın. 
