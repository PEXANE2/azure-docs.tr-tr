...
description: Yeni @azure/service-bus paketi kullanarak bir Service Bus kuyruğuna ileti göndermek ve ileti almak Için bir NodeJS programı yazmayı öğrenin.
...
> - Yeni [@azure/service-bus](https://www.npmjs.com/package/@azure/service-bus) paket henüz sıra oluşturmayı desteklemiyor. Program aracılığıyla onları [@azure/arm-servicebus](https://www.npmjs.com/package/@azure/arm-servicebus) oluşturmak istiyorsanız lütfen paketi kullanın.
...
2. Adlı `send.js` bir dosya oluşturun ve içine aşağıdaki kodu yapıştırın. Bu kod, kuyruğunuza 10 ileti gönderir.
...
1. [Visual Studio Code](https://code.visualstudio.com/) gibi en sevdiğiniz düzenleyiciyi açın
2. Adlı `recieve.js` bir dosya oluşturun ve içine aşağıdaki kodu yapıştırın. Bu kod, kuyruğunuzdan 10 ileti almaya çalışır. Aldığınız gerçek sayı, sıradaki ileti sayısına ve ağ gecikmesine bağlıdır.
...
[Createreceiver](https://docs.microsoft.com/javascript/api/%40azure/service-bus/queueclient#createreceiver-receivemode-) yöntemi, `ReceiveMode` [receiveanddelete](message-transfers-locks-settlement.md#settling-receive-operations) ve [PeekLock](message-transfers-locks-settlement.md#settling-receive-operations)değerlerini içeren bir Enum olan ' ı alır. İleti üzerinde `PeekLock` `complete()` `abandon()`, `defer()`, veya `deadletter()` yöntemlerinden birini kullanarak modunu kullanırsanız, [iletilerinizi kapatmanız](message-transfers-locks-settlement.md#settling-receive-operations) gerektiğini unutmayın.
