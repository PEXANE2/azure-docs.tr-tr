---
title: PHP ile Service Bus konuları kullanma | Microsoft Docs
description: Azure 'da PHP ile Service Bus konularını nasıl kullanacağınızı öğrenin.
...
ms.locfileid: "71147228"
---
# <a name="how-to-use-service-bus-topics-and-subscriptions-with-php"></a>PHP ile Service Bus konuları ve abonelikleri kullanma
...
2. [Hızlı başlangıçtaki adımları izleyin: Service Bus](service-bus-quickstart-topics-subscriptions-portal.md) **ad alanı** oluşturmak ve **bağlantı dizesini**almak için konuya bir Service Bus konu ve abonelik oluşturmak için Azure Portal kullanın.
...
Aşağıdaki örnek `ServiceBusRestProxy` , bir `MySBNamespace` ad alanı içinde adlı `mytopic` bir konu `ServiceBusRestProxy->createTopic` oluşturmak için ve çağrısının nasıl örneklendirilecek gösterilmektedir:
...
## <a name="create-a-subscription"></a>Abonelik oluştur
...
Şimdi, `mytopic` konuya bir ileti gönderildiğinde, bu, her zaman `mysubscription` aboneliğe abone olan alıcılar için dağıtılır ve `HighMessages` ve aboneliklerine ve `LowMessages` aboneliklerine abone olan alıcılar için seçmeli olarak dağıtılır ( İleti içeriği üzerinde).
...
Service Bus konu başlıklarına gönderilen iletiler, [BrokeredMessage][BrokeredMessage] sınıfının örnekleridir. [Brokeredmessage][BrokeredMessage] nesneleri, bir dizi standart özellik ve yöntemin yanı sıra uygulamaya özgü özel özellikleri tutmak için kullanılabilen özellikleri de vardır. Aşağıdaki örnek, daha önce oluşturulan `mytopic` konuya beş test iletisinin nasıl gönderileceğini gösterir. Yöntemi `setProperty` , her iletiye özel bir`MessageNumber`özellik eklemek için kullanılır. Özellik değeri her bir ileti üzerinde değişir (Bu değeri, [abonelik oluşturma](#create-a-subscription) bölümünde gösterildiği gibi, hangi aboneliklerin alacağını öğrenmek için kullanabilirsiniz): `MessageNumber`
...
Bir konuyu veya aboneliği silmek için sırasıyla `ServiceBusRestProxy->deleteTopic` `ServiceBusRestProxy->deleteSubscripton` veya yöntemlerini kullanın. Bir konu başlığı silindiğinde bu konu başlığıyla kaydedilen tüm abonelikler de silinir.
