...
1. [Önkoşul Azure SQL veritabanı](#prerequisites) dağıtımı tamamlandıktan sonra, sol taraftaki menüden **SQL veritabanları** ' nı seçin ve ardından **SQL veritabanları** sayfasında **mysampledatabase** ' i seçin. Veritabanınızın genel bakış sayfası açılır ve tam sunucu adı (örneğin, **MyNewServer-20170824.Database.Windows.net**) görüntülenerek daha fazla yapılandırma seçeneği sunulur.
...
- İlk veritabanınızı tasarlamayı, tablolar oluşturmayı ve veri eklemeyi öğrenmek için, şu öğreticilerden birine bakın:
  ...
  - [Azure SQL veritabanı 'nda tek bir veritabanı tasarlama ve C# ve ADO.NET ile bağlanma](sql-database-design-first-database-csharp.md)
