...
1. `sysadmin` [SQL Server Management STUDIO](sql-database-managed-instance-configure-p2s.md#use-ssms-to-connect-to-the-managed-instance)kullanarak, mı Için BIR veya bir Azure AD yöneticisi olan standart bir SQL Server HESABı (Azure dışı ad) kullanarak yönetilen Örneğinizde oturum açın.
...
1. SSMS 'de oluşturulan kullanıcıyla bir bağlantı oluşturun. Daha önce oluşturulan TestTable tablosunu görmediğine dikkat edin. **TestTable** `sysadmin` Kullanıcıya veritabanından veri okuma izinleri sağlanması gerekiyor.
...
1. Aşağıdaki T-SQL söz dizimini `db_datareader` kullanarak Azure AD kullanıcısına veritabanı rolü verin:
    ...
    Aşağıdaki bob@aadsqlmi.net örnek, **Mymitestdb** veritabanı üzerinde Kullanıcı ve `db_datareader` grup _mygroup_ izinleri sağlar:
