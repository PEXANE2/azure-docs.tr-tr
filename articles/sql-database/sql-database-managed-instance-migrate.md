---
title: SQL Server yönetilen örneğe geçir
...
- Geçişlerinizi engelleyebilen engelleyici bir sorun olmadığından emin olmanız gereken [yönetilen örnek uyumluluğunu değerlendirin](#assess-managed-instance-compatibility) .
  - Bu adım, kaynak SQL Server Örneğiniz üzerinde kaynak kullanımını belirlemede [performans temeli](#create-performance-baseline) oluşturmayı da içerir. Doğru boyutlardaki yönetilen örnek dağıtmak istiyorsanız bu adım gereklidir ve geçişten sonra performansınces etkilenmeden emin olun.
