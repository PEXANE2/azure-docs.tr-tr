...
Hiper ölçekli veritabanlarında, istemci tarafından `ApplicationIntent` belirtilen bağlantı dizesindeki bağımsız değişken bağlantının yazma çoğaltmasına mı yoksa salt bir ikincil çoğaltmayla mi yönlendirildiğini belirler. Olarak `ApplicationIntent` `READONLY` ayarlanmış ve veritabanında bir ikincil çoğaltma yoksa, bağlantı birincil çoğaltmaya yönlendirilir ve varsayılan olarak `ReadWrite` davranış olur.
...
1. Henüz uygun bir sunucunuz yoksa hedef bölgede bir Azure SQL veritabanı sunucusu oluşturun.  Bu sunucunun özgün (kaynak) sunucuyla aynı aboneliğe sahip olması gerekir.
...
| Sorun | Açıklama |
| :---- | :--------- |
| Bir mantıksal sunucu için yedeklemeleri Yönet bölmesi hiper ölçekli veritabanlarını göstermez, bunlar görünümden filtrelenecektir  | Hiper ölçek, yedeklemeleri yönetmek için ayrı bir yönteme sahiptir ve bu nedenle uzun süreli bekletme ve zaman içinde yedekleme bekletme ayarları uygulanmaz/geçersiz kılınır. Buna uygun olarak, hiper ölçekli veritabanları yedekleme bölmesini yönetme bölmesinde görünmez. |
| Belirli bir noktaya geri yükleme | Hiper ölçekli bir veritabanını hiper ölçekli veritabanı saklama dönemi içinde hiper olmayan bir veritabanına geri yükleyebilirsiniz. Hiper olmayan bir veritabanını hiper ölçekli bir veritabanına geri yükleyemezsiniz.|
