...
1. Azure Marketi `private dns zone` 'nde arayın. 
...
1. DNS bölgeniz için gibi `repldns.com`rastgele bir ad sağlayın. 
...
1. Bağlantı için gibi `Pub-link`bir ad girin. 
...
Bağlantı kurulduktan sonra bir örnek veritabanınız varsa, yönetilen örneğiniz `sql-mi-distributor` üzerinde dağıtımı yapılandırabilirsiniz. Bunu yapmak için şu adımları uygulayın:
...
1. **Sihirbazı tamamladıktan** sonra yayınınızı oluşturmak için yayınınızı `ReplTest` adlandırın ve **İleri** ' yi seçin. 
...
Dosya paylaşımının dosya yolunda ters eğik çizgi yerine eğik çizgi kullanmak bu hataya neden olabilir. Bu sorun normaldir: `\\replstorage.file.core.windows.net\replshare` bu, bir OS 55 hatasına neden olabilir:`'\\replstorage.file.core.windows.net/replshare'`
