...
Varsayılan olarak, İş Aracısı döndürülen sonuçları depolamak için çıkış tablosu oluşturur. Bu `CREATE TABLE` nedenle, çıkış kimlik bilgisiyle ilişkili veritabanı sorumlusu en az şu izinlere sahip olmalıdır: veritabanı, `ALTER` `SELECT` `INSERT` `DELETE` ,,, çıkış tablosu veya şeması ve `SELECT` [sys. Indexes](https://docs.microsoft.com/sql/relational-databases/system-catalog-views/sys-indexes-transact-sql) Katalog görünümü.
...
4. Veritabanı üzerindeki `CREATE TABLE` izin dışında, yukarıda listelenen tüm izinler.
