...
7. **SQL veritabanı** menüsünde **sorgu Düzenleyicisi 'ni (Önizleme)** seçin. **Oturum açma** sayfasında, **Active Directory kimlik doğrulama** etiketi altında, bir Azure AD yöneticisiyseniz oturum açmış olduğunuzu bildiren bir ileti görüntülenir. Ardından * \<Kullanıcı veya grup kimliği* **olarak devam et**>düğmesini seçin. Sayfada başarıyla oturum açmadıysanız, sayfayı yenilemeniz gerekebilir.
...
Tabloya yeni bir ürün eklemek için aşağıdaki INSERT T-SQL ifadesini çalıştırın. [INSERT](/sql/t-sql/statements/insert-transact-sql/) `SalesLT.Product`
...
2. Tabloya yeni bir satır eklemek için Çalıştır ' ı seçin. **Run** `Product` **İletiler** bölmesi **sorgu başarılı: etkilenen satırlar: 1 ' i**görüntüler.
...
2. Tablodaki belirtilen satırı güncelleştirmek için Çalıştır ' ı seçin. **Run** `Product` **İletiler** bölmesi **sorgu başarılı: etkilenen satırlar: 1 ' i**görüntüler.
...
2. Tablodaki belirtilen satırı silmek için Çalıştır ' ı seçin. **Run** `Product` **İletiler** bölmesi **sorgu başarılı: etkilenen satırlar: 1 ' i**görüntüler.
...
* Özel bağlantı bağlantınız varsa, sorgu Düzenleyicisi, Istemci IP adresini SQL veritabanı güvenlik duvarı 'na eklemeye gerek kalmadan çalışmaktadır
