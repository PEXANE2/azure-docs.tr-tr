...
3. Araç çubuğunda **Yürüt** ' ü seçerek sorguyu çalıştırın ve `Product` ve `ProductCategory` tablolarından verileri alın.

    ![Tablo ürününden ve ProductCategory 'tan veri alma sorgusu](./media/sql-database-connect-query-ssms/query2.png)

### <a name="insert-data"></a>Veri ekleme

Tabloda yeni bir ürün oluşturmak için bu INSERT Transact-SQL kodunu çalıştırın. [INSERT](https://msdn.microsoft.com/library/ms174335.aspx) `SalesLT.Product`
...
2. Tabloya yeni bir satır eklemek için Yürüt ' ü seçin. **Execute** `Product` **İletiler** bölmesi **(1 satır etkilendi)** görüntülenir.
...
2. Tablodaki belirtilen satırı güncelleştirmek için Yürüt ' ü seçin. **Execute** `Product` **İletiler** bölmesi **(1 satır etkilendi)** görüntülenir.
...
2. Tablodaki belirtilen satırı silmek için Yürüt ' ü seçin. **Execute** `Product` **İletiler** bölmesi **(1 satır etkilendi)** görüntülenir.
...
- Azure portal kullanarak bağlanmak ve sorgulamak için bkz. [Azure Portal SQL sorgu Düzenleyicisi Ile bağlanma ve sorgulama](sql-database-connect-query-portal.md).
