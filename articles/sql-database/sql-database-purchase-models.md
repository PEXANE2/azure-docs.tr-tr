...
> [!NOTE]
> Bir veritabanının DTU sınırı, CPU, okuma, yazma ve veritabanı tarafından kullanılabilir bellek tarafından belirlenir. Ancak, SQL Server veritabanı altyapısı, performansı artırmak için genellikle veri önbelleği için kullanılabilir tüm belleği kullandığından, bu `avg_memory_usage_percent` değer genellikle geçerli veritabanı yüküne bakılmaksızın %100 ' e yakın olacaktır. Bu nedenle, bellek, DTU sınırını dolaylı olarak etkilese de, DTU kullanım formülünde kullanılmaz.
>
