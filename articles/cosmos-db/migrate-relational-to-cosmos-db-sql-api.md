...
Metin dosyamız gerçekten "sınırlandırılmış" olmadığından ve bu dosyanın virgül kullanılarak ayrı sütunlara ayrıştırılmasını ve çift tırnakları (") korumak istediğimiz için," sütun sınırlayıcısı "seçeneğini bir sekmeye (" \t ") veya veri-ve" tırnak karakteri "olmayan başka bir karakter olarak ayarlayacağız  "tırnak karakteri yok" olarak.
