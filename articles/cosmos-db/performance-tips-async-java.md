...
* **Bağlantı modu: doğrudan mod
 kullanın**<a id="direct-connection"></a>
    ...
    * [DirectSound](/java/api/com.microsoft.azure.cosmosdb.connectionmode)
...
* **Aynı Azure bölgesindeki istemcileri performans için birlikte bulun**<a id="same-region"></a>
...
* **ConnectionPolicy ayarlama**
    ...
    * ***Doğrudan mod için programlama ipuçları***
        ...
        + **Özel bir iş parçacığında yoğun işlem yoğunluğu olan iş yüklerini gerçekleştirin** ; önceki ipucunun benzer nedenlerle, karmaşık veri işleme gibi işlemler ayrı bir iş parçacığına en iyi şekilde yerleştirilir. Başka bir veri deposundan veri çeken bir istek (örneğin, iş parçacığı Azure Cosmos DB yararlanır ve Spark veri depoları aynı anda), daha fazla gecikme yaşar ve diğerinden yanıt veren ek bir iş parçacığı üretme önerilir veri deposu.
...
* **Ad tabanlı adresleme kullan**
    ...
    Bağlantıların, bağlantı oluşturmak için kullanılan tüm kaynakların RESOURCEID 'Leri almaktan kaçınmak için `dbs/<database_rid>/colls/<collection_rid>/docs/<document_rid>` biçimine sahip olan, SelfLinks (\_self) yerine `dbs/MyDatabaseId/colls/MyCollectionId/docs/MyDocumentId`biçimine sahip olan ad tabanlı adresleme kullanın. Ayrıca, bu kaynaklar yeniden oluşturulur (muhtemelen aynı ada sahip olabilir), önbelleğe alma işlemi yardımcı olmayabilir.
...
* **Netty 'ın günlüğünü devre dışı bırak**
    ...
    Netty kitaplığı günlüğü geveze ve ek CPU maliyetlerinden kaçınmak için kapalı olması gerekir (yapılandırmada oturum açmayı gizleme yeterli olmayabilir). Hata ayıklama modunda değilseniz, netty 'ın günlüğünü tamamen devre dışı bırakın. Yani ``org.apache.log4j.Category.callAppenders()`` tarafından Netty 'den kaynaklanan ek CPU maliyetlerini kaldırmak için Log4J kullanıyorsanız, kod tabanınıza aşağıdaki satırı ekleyin:
...
* **Netty için yerel SSL uygulamasını kullanma**
    ...
    Netty, daha iyi performans elde etmek için doğrudan SSL uygulama yığını için OpenSSL kullanabilir. Bu yapılandırma yokluğunda, Java 'nın varsayılan SSL uygulamasına geri dönecektir.
...
Diğer platformlar için (Red hat, Windows, Mac vb.) bu yönergelere bakın https://netty.io/wiki/forked-tomcat-native.html
