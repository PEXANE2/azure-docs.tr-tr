...
Örneğin, bir kapsayıcı öğeleri barındırır. Her öğenin `UserID` özellik için benzersiz bir değeri vardır. Kapsayıcıda `UserID` öğeler için bölüm anahtarı olarak görev yapar ve 1.000 benzersiz `UserID` değer varsa, kapsayıcı için 1.000 mantıksal bölümler oluşturulur.
...
İş yükünüzün isteklerinin çoğu sorgular ise ve sorgularınızdaki çoğu aynı özellikte bir eşitlik filtresine sahipseniz, bu özellik iyi bir bölüm anahtarı seçeneği olabilir. Örneğin, sık sık filtre uygulayan bir sorgu çalıştırırsanız `UserID`, bölüm anahtarı olarak seçilmesi `UserID` , [bölümler arası sorguların](how-to-query-container.md#avoiding-cross-partition-queries)sayısını azaltır.
