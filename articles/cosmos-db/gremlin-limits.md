...
*İşleç derinliği* | **400** |  Dolaşmadaki benzersiz adımların toplam sayısı. Örneğin, ```g.V().out()``` işleç sayısı 2: v () ve out (), ```g.V('label').repeat(out()).times(100)``` bir işleç parametresi ```.times(100)``` ```.repeat()``` olan 3: v (), Yinele () ve out () işleç derinliğine sahiptir.
