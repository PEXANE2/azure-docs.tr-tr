...
Bu makale, VMware HCX çözümüne sahip sanal makinelerinizin (VM) bir olağanüstü durum kurtarma işlemini tamamlamaya ve kurtarma veya hedef site olarak bir Azure VMware Çözüm özel bulutu kullanmaya yönelik işlemi içerir.
...
- **Resume** -seçili VM ile ilişkili geçerli çoğaltma ilkesini duraklatın.
...
1. Gerekirse, **varsayılan çoğaltma** seçeneklerini belirleyin:
   ...
   - **Hedef depolama alanı:** Korunan VM 'Ler için uzak veri deposu ve bir Azure VMware Çözüm özel bulutu, bu, vSAN veri deposu olmalıdır.

   - **Işlem kapsayıcısı:** Uzak vSphere kümesi veya kaynak havuzu.

   - **Hedef klasör:** Uzak hedef klasörü, isteğe bağlıdır ve hiçbir klasör seçilmezse VM 'Ler doğrudan seçili kümenin altına yerleştirilir.

   - **RPO:** Kaynak VM ile korunan VM arasındaki eşitleme aralığı ve 5 dakikadan 24 saat arasında bir süre olabilir.
...
1. VM korunduktan sonra, **anlık** görüntüler sekmesinden farklı anlık görüntüleri görüntüleyebilirsiniz.
   ...
   Kapalı olan ve çalıştıran bir VM arasında önemli farklar vardır. Görüntüde, sanal makine için eşitleme işlemi gösterilir. Bu, VM 'nin tam bir kopyası olan ilk anlık görüntüyü tamamlayana kadar eşitleme işlemini başlatır ve sonra yapılandırılan aralıkta bir sonraki işlemleri tamamlar. Kapalı bir sanal makine için bir kopyayı eşitler ve ardından VM etkin değil olarak görünür ve koruma işlemi tamamlandı olarak gösterilir.  VM açık olduğunda, uzak siteye eşitleme işlemini başlatır.
...
1. Azure VMware çözümünüz özel bulutunuzda **vSphere istemcisinde** oturum açın ve **HCX eklentisine**erişin.
   ...
   >[!NOTE]
   >Ters çoğaltmaya başlamadan önce kaynak sitedeki özgün VM 'Lerin gücünün kapalı olduğundan emin olun. VM 'Ler kapalıyken işlem başarısız olur.
