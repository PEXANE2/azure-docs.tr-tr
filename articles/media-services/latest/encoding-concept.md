...
* Aşamalı indirme ile teslim etmek için Azure Media Services kullanarak, bir dijital medya dosyasını (Mezzanine), [H.](https://en.wikipedia.org/wiki/H.264/MPEG-4_AVC) ve codec bileşeniyle kodlanmış video ve AAC ile kodlanmış ses Içeren bir [MP4](https://en.wikipedia.org/wiki/MPEG-4_Part_14) dosyasına dönüştürebilirsiniz. [ ](https://en.wikipedia.org/wiki/Advanced_Audio_Coding)codec bileşeni. Bu MP4 dosyası Depolama hesabınızdaki bir varlığa yazılır. Dosyayı doğrudan indirmek için Azure depolama API 'Lerini veya SDK 'Larını (örneğin, [depolama REST API](../../storage/common/storage-rest-api-auth.md) veya [.NET SDK](../../storage/blobs/storage-quickstart-blobs-dotnet.md)) kullanabilirsiniz. Depolama alanında belirli bir kapsayıcı adıyla çıkış varlığı oluşturduysanız, bu konumu kullanın. Aksi takdirde, [varlık kapsayıcısı URL 'lerini listelemek](https://docs.microsoft.com/rest/api/media/assets/listcontainersas)için Media Services kullanabilirsiniz. 
* İçeriği Uyarlamalı bit hızı akışı ile teslim etmek üzere hazırlamak için, Mezzanine dosyasının birden çok bitücret (yüksek-düşük) ile kodlanmış olması gerekir. Bit hızı düşürüldü, bu nedenle, videonun çözünürlüğü olduğundan, kalitede düzgün bir şekilde geçiş yapıldığından emin olun. Bu, bir çözüm (bkz. [otomatik olarak üretilen Uyarlamalı bit hızı el](autogen-bitrate-ladder.md)ile) adlı bir kodlama. Mezzanine dosyalarını birden çok bit hızında kodlamak için Media Services kullanabilirsiniz. bunu yapmak için, Depolama hesabınızdaki bir varlığa yazılan MP4 dosyaları ve ilişkili akış yapılandırma dosyalarını alırsınız. Daha sonra, videoyu [MPEG-Dash](https://en.wikipedia.org/wiki/Dynamic_Adaptive_Streaming_over_HTTP) ve [HLS](https://en.wikipedia.org/wiki/HTTP_Live_Streaming)gibi akış protokolleriyle teslim etmek Için Media Services ' de [dinamik paketleme](dynamic-packaging-overview.md) özelliğini kullanabilirsiniz. Bu, bir [akış Bulucu](streaming-locators-concept.md) oluşturmanız ve desteklenen protokollere karşılık gelen akış URL 'leri oluşturmanızı gerektirir. Bu, daha sonra özelliklerine göre cihazlara/uygulamalara devredilebilir.
...
Media Services v3 ile kodlamak için bir [dönüşüm](https://docs.microsoft.com/rest/api/media/transforms) ve [iş](https://docs.microsoft.com/rest/api/media/jobs)oluşturmanız gerekir. Dönüştürme, kodlama ayarlarınız ve çıktılar için bir tarif tanımlar; iş, tarif eden bir örneğidir. Daha fazla bilgi için [dönüşümler ve işler](transforms-jobs-concept.md)
...
> [!TIP]
> Videoyu yeniden kodlamaya gerek kalmadan videonuzın bir alt LIP akışını yapmak istiyorsanız, [dinamik paketlemede ön filtreleme bildirimleri](filters-dynamic-manifest-overview.md)kullanmayı göz önünde bulundurun.
...
- **Encodernamedönayar. ContentAwareEncodingExperimental** -içerik kullanan kodlama için deneysel önayar kullanıma sunar. Herhangi bir giriş içeriği verildiğinde, hizmet otomatik olarak en uygun katman sayısını, uygun bit hızını ve uyarlamalı akış gönderimi için çözüm ayarlarını otomatik olarak saptamaya çalışır. Temel algoritmalar zaman içinde gelişmeye devam edecektir. Çıktı, video ve ses Aralanmış MP4 dosyaları içerir. Daha fazla bilgi için bkz. [içerik kullanan kodlama Için deneysel önayar](cae-experimental.md).
...
- **Encodernamedönayar. H264SingleBitrate1080p** -videonun 6750 kbps ve 1080 piksellik bir resim yüksekliği ile KODLANDıĞı bir MP4 dosyası oluşturur ve stereo ses, 64 kbps 'de AAC-LC codec bileşeniyle kodlanır.
- **Encodernamedönayar. H264SingleBitrate720p** -videonun 4500 kbps ve 720 piksellik bir resim yüksekliği ile KODLANDıĞı bir MP4 dosyası oluşturur ve stereo ses, 64 kbps 'de AAC-LC codec bileşeniyle kodlanır.
- **Encodernamedönayar. H264SingleBitrateSD** -videonun 2200 kbps ve 480 piksellik bir resim yüksekliği ile KODLANDıĞı bir MP4 dosyası oluşturur ve stereo ses, 64 kbps 'de AAC-LC codec bileşeniyle kodlanır.
...
Önayarların nasıl kullanıldığını görmek için [karşıya yükleme, kodlama ve akış dosyalarını](stream-files-tutorial-with-api.md)inceleyin.
...
Media Services kodlama özgü ihtiyaçları ve gereksinimleri karşılamak için hazır olarak tüm değerleri özelleştirme tam olarak destekler. Kodlayıcı önayarlarının nasıl özelleştirileceğini gösteren örnekler için bkz.:
