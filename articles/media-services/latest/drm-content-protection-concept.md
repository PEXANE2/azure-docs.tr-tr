...
Media Services v3 'de, içerik anahtarı akış Konumlandırıcı ile ilişkilendirilir ( [Bu örneğe](drm-playready-license-template-concept.md)bakın). Media Services anahtar teslim hizmetini kullanıyorsanız, sizin için içerik anahtarını Azure Media Services oluşturabilirsiniz. Size ait anahtar teslim hizmetini kullanıyorsanız veya iki veri merkezinde aynı içerik anahtarına sahip olmanız gereken yüksek bir kullanılabilirlik senaryosunu işlemeniz gerekiyorsa, içerik anahtarı kendiniz oluşturulmalıdır.
...
1. [İçerik anahtar ilkesi](drm-content-key-policy-concept.md)oluşturun ve yapılandırın.
   ...
   * [PlayReady](drm-playready-license-template-concept.md), [Widevine](drm-widevine-license-template-concept.md)ve/veya [Fairplay](drm-fairplay-license-overview.md) lisanslarını yapılandırın. Şablonlar, her bir DRMs için hakları ve izinleri yapılandırmanızı sağlar.
...
2. Şifrelenmiş varlığı akışa almak için yapılandırılmış bir [akış Bulucu](stream-streaming-locators-concept.md) oluşturun.
  
   Akış bulucunun bir [akış ilkesiyle](stream-streaming-policy-concept.md)ilişkilendirilmesi vardır. Örnekte, `StreamingLocator.StreamingPolicyName` "Predefined_MultiDrmCencStreaming" ilkesine ayarlanıyoruz.
