...
* Java SE Development Kit 8 veya üzeri bir geliştirme makinesi. Birden çok platform için Java 8 (LTS) JDK 'yi [indirin, OpenJDK 'Nin Zulu sürümlerini indirin](https://www.azul.com/downloads/zulu-community/).
...
Java için Microsoft Azure IoT SDK 'nın kopyasında daha önce indirdiğiniz *Azure-iot-SDK-Java/Device/iot-Device-Samples/PNP-Device-Sample/Temperature-Controller-Device-Sample/src/Main/Java/Samples/com/Microsoft/Azure/SDK/IoT/Device/TemperatureController. Java* dosyasını bir metin düzenleyicisinde açın.
...
* `initializeAndProvisionDevice`Model kimliğini ayarlamak için çağrılar `dtmi:com:example:TemperatureController;2` yapın, cihaz sağlamak ve kaydetmek için DPS kullanın, bir **deviceclient** örneği oluşturun ve IoT Central uygulamanıza bağlanın. IoT Central, bu cihazın cihaz şablonunu tanımlamak veya oluşturmak için model KIMLIĞINI kullanır. Daha fazla bilgi için bkz. cihaz [şablonuyla bir cihazı ilişkilendirme](../articles/iot-central/core/concepts-get-connected.md#associate-a-device-with-a-device-template).
* Ve komutları için komut işleyicileri `getMaxMinReport` oluşturur `reboot` .
* Yazılabilir özellikler için özellik güncelleştirme işleyicileri oluşturur `targetTemperature` .
* **Cihaz bilgileri** arabirimindeki Özellikler ve **Cihaz belleği** ve **seri numarası** özellikleri için başlangıç değerlerini gönderir.
* İki termostats üzerinden sıcaklık telemetrisi göndermek ve bu özelliği beş saniyede bir güncelleştirmek için bir iş parçacığı başlatır `maxTempSinceLastReboot` .
...
`sendTemperatureTelemetry`Yöntemi, cihazın bir bileşenden IoT Central sıcaklık telemetrisini nasıl göndereceğini gösterir. Bu yöntem, şu `PnpConvention` iletiyi oluşturmak için sınıfını kullanır:
...
`updateMaxTemperatureSinceLastReboot`Yöntemi, `maxTempSinceLastReboot` bir bileşenden IoT Central bir özellik güncelleştirmesi gönderir. Bu yöntem, `PnpConvention` düzeltme ekini oluşturmak için sınıfını kullanır:
...
`TargetTemperatureUpdateCallback`Sınıfı, `TwinPropertyCallBack` IoT Central bir bileşene yazılabilir Özellik güncelleştirmelerini işlemeye yönelik yöntemi içerir. Bu yöntem, `PnpConvention` yanıtı oluşturmak için sınıfını kullanır:
...
`MethodCallback`Sınıfı, IoT Central ' `call` den çağrılan bileşen komutlarının işlenmesi yöntemini içerir:
...
Windows 'ta, indirdiğiniz Java için Azure IoT SDK deposunun kök klasörüne gidin.
...
Örnek uygulamayı çalıştırmak için bir komut satırı ortamı açın ve *Azure-IoT-SDK-Java/Device/IoT-Device-Samples/PNP-Device-Sample/sıcaklık-Controller-Device-Sample* klasörüne gidin ve *TemperatureController. Java* örnek dosyası ile *src* klasörünü içeren bir klasör.
