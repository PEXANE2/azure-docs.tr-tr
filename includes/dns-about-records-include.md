...
Azure DNS’de, kayıtlar göreli adlar kullanılarak belirtilir. *Tam* etki alanı adında (FQDN) bölge adı varken *göreli* adda bu yoktur. Örneğin, bölgedeki `www` `contoso.com` göreli kayıt adı tam nitelikli `www.contoso.com`kayıt adını verir.

*Tepe* kaydı, bir DNS bölgesinin kökündeki (veya *tepesindeki*) DNS kaydıdır. Örneğin, DNS bölgesinde, `contoso.com`bir tepe kaydı da tam `contoso.com` nitelikli adı vardır (bu bazen *çıplak* etki alanı denir).  Sözleşmeye göre, göreli adı ' '\@apeks kayıtlarını temsil etmek için kullanılır.
