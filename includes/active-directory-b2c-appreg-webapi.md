...
1. **Uygulama KIMLIĞI URI**'si IÇIN gösterilen URI 'ye bir API uç noktası tanımlayıcısı ekleyin. Bu öğreticide, tam `api`URI 'nin şuna benzer olması için girin `https://contosob2c.onmicrosoft.com/api`.
1. **Oluştur**’u seçin.
...
1. Uygulama için bir **ad** girin. Örneğin, *webapi1*.
1. **Yeniden yönlendirme URI 'si**altında **Web**' i seçin ve ardından Azure AD B2C uygulamanızın isteklerinizin belirteçleri döndürmesi gerektiği bir uç nokta girin. Bu öğreticide, örnek yerel olarak çalışır ve tarihinde `http://localhost:5000`dinler.
1. **Kaydol**’u seçin.
