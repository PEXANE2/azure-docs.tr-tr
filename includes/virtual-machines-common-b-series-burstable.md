...
ms.locfileid: "75942849"
---
B serisi VM ailesi, iş yükünüz için gereken temel düzey performansı, Intel® geniş bir E5-2673 v4 2,3 GHz veya Intel® Haswell 2,4 GHz E5-2673 v3 işlemcisi 'nin %100 ' ına kadar CPU performansı patlaması sayesinde seçmenizi sağlar vCPU.
...
| Boyut             | vCPU  | Bellek: GiB | Geçici depolama (SSD) GiB | VM 'nin temel CPU performansı | VM 'nin en yüksek CPU performansı | İlk krediler | Kazanılan krediler/saat | Maksimum kazanılan krediler | En fazla veri diski | Maksimum önbelleğe alınmış ve geçici depolama aktarım hızı: ıOPS/MBps | Maksimum önbelleğe alınmamış disk aktarım hızı: IOPS-MB/sn | En fazla NIC |          
|---------------|-------------|----------------|----------------------------|-----------------------|--------------------|--------------------|--------------------|----------------|----------------------------------------|-------------------------------------------|-------------------------------------------|----------|
...
| --- | --- | --- | --- | --- |
| B16ms dağıtımı | Kurulum | Kurulum  | 480 (ilk krediler) | 480 |
...
Y **: önümüzdeki**haftalarda 2 yeni ölçüm tanıtıyoruz, **KREDI** ölçümü, sanal makinenizin kaç kredinin kazanıldığını görüntülemenize ve **tüketilen kredi** ölçüsünün sanal makinenizin banka tarafından kaç tane kaç tane tüketireceğini gösterileceğine olanak sağlayacak.    Bu ölçümleri portalda ölçümler bölmesinden veya Azure Izleyici API 'Leri aracılığıyla programlı bir şekilde görüntüleyebileceksiniz.

Azure için ölçüm verilerine erişme hakkında daha fazla bilgi için, bkz. [Microsoft Azure ölçümlere genel bakış](../articles/monitoring-and-diagnostics/monitoring-overview-metrics.md).
...
### <a name="q-what-happens-if-i-run-out-of-credits"></a>S: krediler tükendiğinde ne olur?
**A** : VM tüm kullanılabilir kredileri kullanıyorsa, bu durumda yalnızca temel CPU kullanılmasına izin verilir ve bu taban çizgisinin üzerine veri bloğu oluşturamayacak. 
